...
public final class OreTiersAPI {
    ...
    public static void addReplacement(@Nonnull String stage, @Nonnull Block original, int originalMeta,
        ...
        addReplacement(
            stage,
            original.getStateFromMeta(originalMeta),
            replacement.getStateFromMeta(replacementMeta),
            defAllow);
    }
     ...
     */
    public static void addReplacement(@Nonnull String stage, @Nonnull IBlockState original,
                                      @Nonnull IBlockState replacement, boolean defAllow) {

        if (hasReplacement(original)) {

            // OreStages.LOG.info(String.format("Attempted to register duplicate replacement for %s on stage %s. It will
            // be replaced.", original.toString(), stage));
        }
