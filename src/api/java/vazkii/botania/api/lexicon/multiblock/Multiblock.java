...
public class Multiblock {
    ...
    private void changeAxisForNewComponent(BlockPos pos) {
        ...
        else if (pos.getX() > maxPos.getX()) maxPos = new BlockPos(pos.getX(), maxPos.getY(), maxPos.getZ());

        if (pos.getY() < minPos.getY()) minPos = new BlockPos(minPos.getX(), pos.getY(), minPos.getZ());
        else if (pos.getY() > maxPos.getY()) maxPos = new BlockPos(maxPos.getX(), pos.getY(), maxPos.getZ());

        if (pos.getZ() < minPos.getZ()) minPos = new BlockPos(minPos.getX(), minPos.getY(), pos.getZ());
        else if (pos.getZ() > maxPos.getZ()) maxPos = new BlockPos(maxPos.getX(), maxPos.getY(), pos.getZ());
    }
    ...
    public Map<EnumFacing, Multiblock> createRotations() {
        ...
        ret.put(
            EnumFacing.WEST,
            ret.get(EnumFacing.SOUTH)
                .copy());
        ret.get(EnumFacing.WEST)
            .rotate(Math.PI / 2);
    ...
    public void updateLocationCache() {
        locationCache.clear();
        for (MultiblockComponent comp : components) addComponentToLocationCache(comp);
    }
