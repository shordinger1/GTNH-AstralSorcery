...
public final class CorporeaHelper {
    ...
    public static List<InvWithLocation> getInventoriesOnNetwork(ICorporeaSpark spark) {
        ...
        if (network != null) for (ICorporeaSpark otherSpark : network)
            if (otherSpark != null) {
    ...
    public static int getCountInNetwork(ItemStack stack, Map<InvWithLocation, Integer> inventories, boolean checkNBT) {
        ...
        for (InvWithLocation inv : inventories.keySet()) count += inventories.get(inv);
     ...
     * called instead if the context for this exists to avoid having to get the value again.
     */
    public static Map<InvWithLocation, Integer> getInventoriesWithItemInNetwork(ItemStack stack, ICorporeaSpark spark,
                                                                                boolean checkNBT) {
        List<InvWithLocation> inventories = getInventoriesOnNetwork(spark);
     ...
     * called instead if the context for this exists to avoid having to get the value again.
     */
    public static Map<InvWithLocation, Integer> getInventoriesWithItemInNetwork(ItemStack stack,
                                                                                List<InvWithLocation> inventories, boolean checkNBT) {
    ...
    public static List<ItemStack> requestItem(Object matcher, int itemCount, ICorporeaSpark spark, boolean checkNBT,
        ...
        if (MinecraftForge.EVENT_BUS.post(event)) return stacks;
        ...
        for (IWrappedInventory inv : inventoriesW) {
            ...
            if (originalInventory.world.getTileEntity(originalInventory.pos) instanceof ICorporeaInterceptor) {
                ICorporeaInterceptor interceptor = (ICorporeaInterceptor) originalInventory.world
                    .getTileEntity(originalInventory.pos);
        ...
        for (ICorporeaInterceptor interceptor : interceptors.keySet())
            interceptor
                .interceptRequestLast(matcher, itemCount, interceptors.get(interceptor), spark, stacks, inventories, doit);
    ...
    public static ICorporeaSpark getSparkForBlock(World world, BlockPos pos) {
        List<Entity> sparks = world.getEntitiesWithinAABB(
            Entity.class,
            new AxisAlignedBB(pos.up(), pos.add(1, 2, 1)),
    ...
    public static boolean stacksMatch(ItemStack stack, String s) {
        ...
        String name = stripControlCodes(
            stack.getDisplayName()
                .toLowerCase()
                .trim());
        return equalOrContain(name, s, contains) || equalOrContain(name + "s", s, contains)
            || equalOrContain(name + "es", s, contains)
            || name.endsWith("y") && equalOrContain(name.substring(0, name.length() - 1) + "ies", s, contains);
    }
    ...
    public static boolean shouldAutoComplete() {
        for (ICorporeaAutoCompleteController controller : autoCompleteControllers)
            if (controller.shouldAutoComplete()) return true;
        return false;
    }

    // Copy from StringUtils
    public static String stripControlCodes(String str) {
        return patternControlCode.matcher(str)
            .replaceAll("");
    }
}
