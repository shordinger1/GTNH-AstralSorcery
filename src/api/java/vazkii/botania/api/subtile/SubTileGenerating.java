...
public class SubTileGenerating extends SubTileEntity {
    ...
    public void onUpdate() {
        ...
        if (acceptsRedstone()) {
            ...
            for (EnumFacing dir : EnumFacing.VALUES) {
                int redstoneSide = supertile.getWorld()
                    .getRedstonePower(
                        supertile.getPos()
                            .offset(dir),
                        dir);
        ...
        if (supertile.getWorld().isRemote) {
            ...
            if (Math.random() > particleChance) BotaniaAPI.internalHandler.sparkleFX(
                supertile.getWorld(),
                supertile.getPos()
                    .getX() + 0.3
                    + Math.random() * 0.5,
        ...
        if (!supertile.getWorld().isRemote) {
            ...
            if (passive && muhBalance > 0 && passiveDecayTicks > muhBalance) {
                IBlockState state = supertile.getWorld()
                    .getBlockState(supertile.getPos());
                supertile.getWorld()
                    .playEvent(2001, supertile.getPos(), Block.getStateId(state));
                if (supertile.getWorld()
                    .getBlockState(
                        supertile.getPos()
                            .down())
                    .isSideSolid(
                        ...
                        EnumFacing.UP))
                    supertile.getWorld()
                        .setBlockState(supertile.getPos(), Blocks.DEADBUSH.getDefaultState());
                else supertile.getWorld()
                    .setBlockToAir(supertile.getPos());
            }
        }

        if (passive) passiveDecayTicks++;
    }
    ...
    public void linkCollector() {
        ...
        if (linkedCollector == null) {
            ...
            if (cachedCollectorCoordinates != null) {
                needsNew = false;
                if (supertile.getWorld()
                    .isBlockLoaded(cachedCollectorCoordinates)) {
                    needsNew = true;
                    TileEntity tileAt = supertile.getWorld()
                        .getTileEntity(cachedCollectorCoordinates);
            ...
            TileEntity tileAt = supertile.getWorld()
                .getTileEntity(linkedCollector.getPos());
            if (tileAt != null && tileAt instanceof IManaCollector) linkedCollector = tileAt;
        }

        if (needsNew && ticksExisted == 1) { // New flowers only
            IManaNetwork network = BotaniaAPI.internalHandler.getManaNetworkInstance();
            int size = network.getAllCollectorsInWorld(supertile.getWorld())
                .size();
    ...
    public void writeToPacketNBT(NBTTagCompound cmp) {
        ...
        if (cachedCollectorCoordinates != null) {
            ...
            int x = linkedCollector == null ? 0
                : linkedCollector.getPos()
                .getX();
            int y = linkedCollector == null ? -1
                : linkedCollector.getPos()
                .getY();
            int z = linkedCollector == null ? 0
                : linkedCollector.getPos()
                .getZ();
    ...
    public boolean isValidBinding() {
        return linkedCollector != null && !linkedCollector.isInvalid()
            && supertile.getWorld()
            .getTileEntity(linkedCollector.getPos()) == linkedCollector;
    }
    ...
    public void renderHUD(Minecraft mc, ScaledResolution res) {
        ...
        int color = getColor();
        BotaniaAPI.internalHandler.drawComplexManaHUD(
            color,
            knownMana,
            getMaxMana(),
            name,
