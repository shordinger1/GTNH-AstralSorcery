...
public class SubTileFunctional extends SubTileEntity {
        ...
        if (linkedPool == null) {
            ...
            if (cachedPoolCoordinates != null) {
                ...
                if (supertile.getWorld()
                    .isBlockLoaded(cachedPoolCoordinates)) {
                    needsNew = true;
                    TileEntity tileAt = supertile.getWorld()
                        .getTileEntity(cachedPoolCoordinates);
            ...
            TileEntity tileAt = supertile.getWorld()
                .getTileEntity(linkedPool.getPos());
            if (tileAt != null && tileAt instanceof IManaPool) linkedPool = tileAt;
        }

        if (needsNew && ticksExisted == 1) { // Only for new flowers
            IManaNetwork network = BotaniaAPI.internalHandler.getManaNetworkInstance();
            int size = network.getAllPoolsInWorld(supertile.getWorld())
                .size();
    ...
    public void writeToPacketNBT(NBTTagCompound cmp) {
        ...
        if (cachedPoolCoordinates != null) {
            ...
            int x = linkedPool == null ? 0
                : linkedPool.getPos()
                .getX();
            int y = linkedPool == null ? -1
                : linkedPool.getPos()
                .getY();
            int z = linkedPool == null ? 0
                : linkedPool.getPos()
                .getZ();
    ...
    public boolean isValidBinding() {
        return linkedPool != null && linkedPool.hasWorld()
            && !linkedPool.isInvalid()
            && supertile.getWorld()
            .isBlockLoaded(linkedPool.getPos(), false)
            && supertile.getWorld()
            .getTileEntity(linkedPool.getPos()) == linkedPool;
    }
