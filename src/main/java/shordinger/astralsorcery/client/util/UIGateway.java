...
public class UIGateway {
    ...
    public static UIGateway initialize(World world, BlockPos gatewayPos, Vector3 source, double sphereRadius) {
        ...
        for (Map.Entry<Integer, List<GatewayCache.GatewayNode>> entries : system.getGatewayCache(Side.CLIENT)
            .entrySet()) {
    ...
    public boolean isValid(World w) {
        // true is fine for clientside here. doesn't load a new chunk, just fails into empty chunk which is fine for the
        // effect.
        return MiscUtils.getTileAt(w, this.gatewayPos, TileCelestialGateway.class, true) != null;
    }

    private static void gatherStars(UIGateway gateway, int dimId, List<GatewayCache.GatewayNode> otherPositions,
                                    boolean sameWorld, double sphereRadius) {
            ...
            if (sameWorld) {
                ...
                for (GatewayEntry otherEntry : gateway.gatewayEntries) {
                    ...
                    if (Math.abs(otherEntry.pitch - potentialEntry.pitch) < 10
                        && (Math.abs(otherEntry.yaw - potentialEntry.yaw) <= 10
                        || Math.abs(otherEntry.yaw - potentialEntry.yaw - 360F) <= 10)) {

                        if (closest == null || gatePosition.distanceSquared(otherEntry.relativePos)
                            < gatePosition.distanceSquared(closest.relativePos)) {
                            closest = otherEntry;
                        }
                    }
                }
    ...
    public void renderIntoWorld(float pticks) {
        ...
        UIGateway.GatewayEntry entry = findMatchingEntry(
        ...
        if (entry != null) {
            ...
            if (display != null && !display.isEmpty()) {
                ...
                Vector3 pos = entry.relativePos.clone()
                    .add(origin);
                RenderingUtils.removeStandartTranslationFromTESRMatrix(pticks);
                GlStateManager.translate(pos.getX(), pos.getY() + 0.25, pos.getZ());
                GlStateManager.scale(0.015, -0.015, 0.015);
                GlStateManager.glNormal3f(0.0F, 0.0F, -0.010416667F);
                float plRot = RenderingUtils.interpolateRotation(
                    MathHelper.wrapDegrees(player.prevRotationYaw),
                    MathHelper.wrapDegrees(player.rotationYaw),
                    pticks);
    ...
    public void renderGatewayTarget(float pTicks) {
        ...
        if (screenshot != null) {
            ...
            Vector3 pos = focusingEntry.relativePos.clone()
                ...
                .add(origin);
            RenderingUtils
                .renderFacingFullQuadVB(vb, pos.getX(), pos.getY(), pos.getZ(), pTicks, 0.4F, 0, 1F, 1F, 1F, perc);
            tes.draw();
            GlStateManager.enableAlpha();
            GlStateManager.popMatrix();
        }
