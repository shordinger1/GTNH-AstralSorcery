...
public class GuiProgressionClusterRenderer {
    ...
    private void renderNodeToGUI(ResearchNode node, double lowerPosX, double lowerPosY, float zLevel) {
        ...
        switch (node.getRenderType()) {
            ...
            case TEXTURE_SPRITE:
                ...
                GlStateManager.color(
                    renderLoopBrFactor * (col.getRed() / 255F),
                ...
                vb.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
                vb.pos(pxWH, zoomedWH - pxWH, zLevel)
                    .tex(uvTexture.key, uvTexture.value + res.getVLength())
                    .endVertex();
                vb.pos(zoomedWH - pxWH, zoomedWH - pxWH, zLevel)
                    .tex(uvTexture.key + res.getULength(), uvTexture.value + res.getVLength())
                    .endVertex();
                vb.pos(zoomedWH - pxWH, pxWH, zLevel)
                    .tex(uvTexture.key + res.getULength(), uvTexture.value)
                    .endVertex();
                vb.pos(pxWH, pxWH, zLevel)
                    .tex(uvTexture.key, uvTexture.value)
                    .endVertex();
                t.draw();
                GlStateManager.color(1F, 1F, 1F, 1F);
                TextureHelper.refreshTextureBindState();
    ...
    private void drawResearchItemBackground(double zoomedWH, double xAdd, double yAdd, float zLevel) {
        ...
        vb.pos(renderOffsetX + xAdd, renderOffsetY + yAdd + zoomedWH, zLevel)
            .tex(0, 1)
            .endVertex();
        vb.pos(renderOffsetX + xAdd + zoomedWH, renderOffsetY + yAdd + zoomedWH, zLevel)
            .tex(1, 1)
            .endVertex();
        vb.pos(renderOffsetX + xAdd + zoomedWH, renderOffsetY + yAdd, zLevel)
            .tex(1, 0)
            .endVertex();
        vb.pos(renderOffsetX + xAdd, renderOffsetY + yAdd, zLevel)
            .tex(0, 0)
            .endVertex();
        t.draw();
        GL11.glColor4f(1F, 1F, 1F, 1F);
