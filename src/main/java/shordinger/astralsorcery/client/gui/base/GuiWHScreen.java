...
public abstract class GuiWHScreen extends GuiScreen {
    ...
    protected void drawTexturedRect(double offsetX, double offsetY, double width, double height,
        ...
        vb.begin(7, DefaultVertexFormats.POSITION_TEX);
        vb.pos(offsetX, offsetY + height, zLevel)
            .tex(off.x, off.y + tex.getVWidth())
            .endVertex();
        vb.pos(offsetX + width, offsetY + height, zLevel)
            .tex(off.x + tex.getUWidth(), off.y + tex.getVWidth())
            .endVertex();
        vb.pos(offsetX + width, offsetY, zLevel)
            .tex(off.x + tex.getUWidth(), off.y)
            .endVertex();
        vb.pos(offsetX, offsetY, zLevel)
            .tex(off.x, off.y)
            .endVertex();
    ...
    protected void drawTexturedRectAtCurrentPos(double width, double height, float uFrom, float vFrom, float uWidth,
        ...
        vb.pos(0, 0 + height, zLevel)
            .tex(uFrom, vFrom + vWidth)
            .endVertex();
        vb.pos(0 + width, 0 + height, zLevel)
            .tex(uFrom + uWidth, vFrom + vWidth)
            .endVertex();
        vb.pos(0 + width, 0, zLevel)
            .tex(uFrom + uWidth, vFrom)
            .endVertex();
        vb.pos(0, 0, zLevel)
            .tex(uFrom, vFrom)
