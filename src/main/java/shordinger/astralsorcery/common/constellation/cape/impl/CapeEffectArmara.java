...
public class CapeEffectArmara extends CapeArmorEffect {
    ...
    public void playActiveParticleTick(EntityPlayer pl) {
        ...
        if (stacks > 0) {
            ...
            for (int i = 0; i < stacks; i++) {
                ...
                Vector3 perpEffect = axis.clone()
                    .perpendicular();

                int ticksPerCircle = 80 + r.nextInt(50);
                int tick = (pl.ticksExisted) % ticksPerCircle;

                Vector3 pos = perpEffect.normalize()
                    .multiply(r.nextFloat() * 0.4F + 0.9F)
    ...
    private int getCurrentImmunityRechargeTick() {
        return this.getData()
            .getInteger("AS_Armara_ImmunityTick");
    }

    private void setCurrentImmunityRechargeTick(int tick) {
        this.getData()
            .setInteger("AS_Armara_ImmunityTick", tick);
    }

    private int getCurrentImmunityStacks() {
        return this.getData()
            .getInteger("AS_Armara_ImmunityStacks");
    }

    private void setCurrentImmunityStacks(int stacks) {
        this.getData()
            .setInteger("AS_Armara_ImmunityStacks", stacks);
    }
    ...
    public void loadFromConfig(Configuration cfg) {
        immunityRechargeTicks = cfg.getInt(
            getKey() + "RechargeTicks",
            getConfigurationSection(),
            immunityRechargeTicks,
            1,
            10000,
            "Defines the ticks you need to wear the cape until you get a immunty-stack that prevents 1 attack/damage-hit you'd take.");
        immunityStacks = cfg.getInt(
            getKey() + "ImmunityStacks",
            getConfigurationSection(),
            immunityStacks,
            1,
            30,
            "Defines the maximum amount of immunity-stacks you can overall charge up.");
    }
