...
public class TraitUpgradeRecipe extends ConstellationRecipe
    ...
    public TraitUpgradeRecipe() {
        super(
            shapedRecipe("upgrade_tier4", new ItemStack(BlocksAS.blockAltar, 1, BlockAltar.AltarType.ALTAR_4.ordinal()))
                .addPart(ItemHandle.getCrystalVariant(false, true), ShapedRecipeSlot.CENTER)
        ...
        setAttItem(BlockMarble.MarbleBlockType.RUNED.asStack(), AttunementAltarSlot.values());
        setCstItem(
        ...
        if (state == ActiveCraftingTask.CraftingState.ACTIVE) {
            ...
            for (int i = 0; i < 1; i++) {
                Vector3 r = Vector3.random()
                    .setY(0)
                    .normalize()
                    .multiply(1.3 + rand.nextFloat() * 0.5)
                    .add(
                        thisAltar.clone()
                            .addY(2.2));
            ...
            for (int i = 0; i < 10; i++) {
                ...
                Vector3 r = altarVec.clone()
                    .add(dir);
                p = EffectHelper.genericFlareParticle(r.getX(), r.getY(), r.getZ());
                p.gravity(0.01 + 0.02 * rand.nextFloat())
                    .enableAlphaFade(EntityComplexFX.AlphaFunction.FADE_OUT)
            ...
            if (sprite == null || ((TextureSpritePlane) sprite).isRemoved()) {
                TextureSpritePlane pl = EffectHandler.getInstance()
                    .textureSpritePlane(SpriteLibrary.spriteFlare2, Vector3.RotAxis.Y_AXIS.clone());
                ...
                pl.setRefreshFunc(() -> {
                    return !altar.isInvalid() && altar.getWorld()
                        .getTileEntity(altar.getPos()) != null
                        && altar.getWorld()
                        .getTileEntity(altar.getPos())
                        .equals(altar)
