...
public class CommandAstralSorcery extends CommandBase {
    ...
    private void tryEnterSLNetworkDebugMode(ICommandSender sender) {
        ...
        StarlightNetworkDebugHandler.INSTANCE.awaitDebugInteraction(
            player,
            () -> sender.sendMessage(new TextComponentString("§cStarlight network debug-rightclick timed out.")));
        sender.sendMessage(
            new TextComponentString(
                "§aRightclick a block within 20 seconds to collect information about its starlight network activity."));
    }

    private void attuneToConstellation(MinecraftServer server, ICommandSender sender, String otherPlayerName,
                                       String majorConstellationStr) {
        ...
        if (cst == null) {
            ...
            sender.sendMessage(
                new TextComponentString("§cSee '/astralsorcery constellations' to get all constellations!"));
            return;
        }

        if (ResearchManager.setAttunedConstellation(other, cst)) {
            sender.sendMessage(
                new TextComponentString("§aSuccess! Player has been attuned to " + cst.getUnlocalizedName()));
        } else {
            sender.sendMessage(
                new TextComponentString(
    ...
    private void setExp(MinecraftServer server, ICommandSender sender, String otherPlayerName, String strCharge) {
        ...
        try {
            ...
            sender.sendMessage(
                new TextComponentString("§cFailed! Alignment charge to set should be a number! " + strCharge));
            return;
        }
        ...
        if (ResearchManager.setExp(other, chargeToSet)) {
            ...
            sender.sendMessage(
                new TextComponentString("§cFailed! Player specified doesn't seem to have a research progress!"));
        }
    }

    private void modifyResearch(MinecraftServer server, ICommandSender sender, String otherPlayerName,
                                String research) {
        Tuple<EntityPlayerMP, PlayerProgress> prTuple = tryGetProgressWithMessages(server, sender, otherPlayerName);
        if (prTuple == null) {
            return;
    ...
    private void modifyProgress(MinecraftServer server, ICommandSender sender, String otherPlayerName,
                                String argument) {
        Tuple<EntityPlayerMP, PlayerProgress> prTuple = tryGetProgressWithMessages(server, sender, otherPlayerName);
        if (prTuple == null) {
            return;
        ...
        if ("all".equalsIgnoreCase(argument)) {
            if (!ResearchManager.maximizeTier(other)) {
                sender.sendMessage(
                    new TextComponentString("§cFailed! Could not load Progress for (" + otherPlayerName + ") !"));
            } else {
                sender
                    .sendMessage(new TextComponentString("§aMaximized ProgressionTier for " + otherPlayerName + " !"));
            }
        } else if ("next".equalsIgnoreCase(argument)) {
            ProgressionTier tier = prog.getTierReached();
            if (!tier.hasNextTier()) {
                sender.sendMessage(
                    new TextComponentString("§aPlayer " + otherPlayerName + " has already reached the highest tier!"));
            } else {
                ProgressionTier next = tier.next();
                ResearchManager.giveProgressionIgnoreFail(other, next);
                sender.sendMessage(
                    new TextComponentString("§aPlayer " + otherPlayerName + " advanced to Tier " + next.name() + "!"));
            }
        }
    }
    ...
    private void addConstellations(MinecraftServer server, ICommandSender sender, String otherPlayerName,
                                   String argument) {
        Tuple<EntityPlayerMP, PlayerProgress> prTuple = tryGetProgressWithMessages(server, sender, otherPlayerName);
        if (prTuple == null) {
            return;
    ...
    private Tuple<EntityPlayerMP, PlayerProgress> tryGetProgressWithMessages(MinecraftServer server,
                                                                             ICommandSender sender, String otherPlayerName) {
    ...
    private void displayHelp(ICommandSender sender) {
        ...
        sender.sendMessage(
            new TextComponentString("§a/astralsorcery progress [playerName] <all>§7 - maximize progression"));
        sender.sendMessage(
            new TextComponentString("§a/astralsorcery research [playerName] <research;all>§7 - set/add Research"));
        sender.sendMessage(
            new TextComponentString(
        ...
        sender.sendMessage(
            new TextComponentString("§a/astralsorcery maximize [playerName]§7 - unlocks everything for that player."));
        sender.sendMessage(
            new TextComponentString("§a/astralsorcery exp [playerName] <exp>§7 - sets the perk exp for a player"));
        sender.sendMessage(
            new TextComponentString(
