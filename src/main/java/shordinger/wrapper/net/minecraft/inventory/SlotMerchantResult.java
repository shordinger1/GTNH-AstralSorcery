...
public class SlotMerchantResult extends Slot {
    ...
    public ItemStack onTake(EntityPlayer thePlayer, ItemStack stack) {
        ...
        if (merchantrecipe != null) {
            ...
            if (this.doTrade(merchantrecipe, itemstack, itemstack1)
                || this.doTrade(merchantrecipe, itemstack1, itemstack)) {
    ...
    private boolean doTrade(MerchantRecipe trade, ItemStack firstItem, ItemStack secondItem) {
        ...
        if (firstItem.getItem() == itemstack.getItem() && firstItem.getCount() >= itemstack.getCount()) {
            if (!itemstack1.isEmpty() && !secondItem.isEmpty()
                && itemstack1.getItem() == secondItem.getItem()
                && secondItem.getCount() >= itemstack1.getCount()) {
