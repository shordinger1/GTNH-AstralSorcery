...
public class ChunkRenderWorker implements Runnable {
        ...
        while (this.shouldRun) {
            try {
                ...
                Minecraft.getMinecraft()
                    .crashed(
                        Minecraft.getMinecraft()
                            .addGraphicsAndWorldToCrashReport(crashreport));
                return;
            }
        }
    }
    ...
    protected void processTask(final ChunkCompileTaskGenerator generator) throws InterruptedException {
        ...
        try {
            if (generator.getStatus() != ChunkCompileTaskGenerator.Status.PENDING) {
                if (!generator.isFinished()) {
                    LOGGER.warn(
                        "Chunk render task was {} when I expected it to be pending; ignoring task",
                        (Object) generator.getStatus());
                }
            ...
            if (blockpos1.add(8, 8, 8)
                .distanceSq(blockpos) > 576.0D) {
                World world = generator.getRenderChunk()
                    .getWorld();
                BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos(blockpos1);

                if (!this.isChunkExisting(
                    blockpos$mutableblockpos.setPos(blockpos1)
                        .move(EnumFacing.WEST, 16),
                    world)
                    || !this.isChunkExisting(
                    blockpos$mutableblockpos.setPos(blockpos1)
                        .move(EnumFacing.NORTH, 16),
                    world)
                    || !this.isChunkExisting(
                    blockpos$mutableblockpos.setPos(blockpos1)
                        .move(EnumFacing.EAST, 16),
                    world)
                    || !this.isChunkExisting(
                    blockpos$mutableblockpos.setPos(blockpos1)
                        .move(EnumFacing.SOUTH, 16),
                    world)) {
                    return;
                }
            }
        ...
        Entity entity = Minecraft.getMinecraft()
            .getRenderViewEntity();
        ...
        if (entity == null) {
                        ...
                        arraylist.add(
                            this.chunkRenderDispatcher.uploadChunk(
                                ...
                                generator.getRegionRenderCacheBuilder()
                                    .getWorldRendererByLayer(blockrenderlayer),
                                generator.getRenderChunk(),
                                compiledchunk,
            ...
            Futures.addCallback(listenablefuture, new FutureCallback<List<Object>>() {
                ...
                public void onSuccess(@Nullable List<Object> p_onSuccess_1_) {
                    ...
                    generator.getRenderChunk()
                        .setCompiledChunk(compiledchunk);
                }
    ...
    private boolean isChunkExisting(BlockPos pos, World worldIn) {
        return !worldIn.getChunkFromChunkCoords(pos.getX() >> 4, pos.getZ() >> 4)
            .isEmpty();
    }
