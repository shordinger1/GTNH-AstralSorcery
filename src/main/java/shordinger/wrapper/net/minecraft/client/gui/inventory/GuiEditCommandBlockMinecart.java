...
public class GuiEditCommandBlockMinecart extends GuiScreen implements ITabCompleter {
    ...
    public void initGui() {
        ...
        this.doneButton.enabled = !this.commandField.getText()
            .trim()
            .isEmpty();
        ...
        if (button.enabled) {
            if (button.id == 1) {
                ...
                this.mc.getConnection()
                    .sendPacket(new CPacketCustomPayload("MC|AdvCmd", packetbuffer));
    ...
    private void updateCommandOutput() {
        if (this.commandBlockLogic.shouldTrackOutput()) {
            ...
            if (this.commandBlockLogic.getLastOutput() != null) {
                this.previousEdit.setText(
                    this.commandBlockLogic.getLastOutput()
                        .getUnformattedText());
            }
        } else {
