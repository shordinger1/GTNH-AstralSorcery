...
public class DebugRendererNeighborsUpdate implements DebugRenderer.IDebugRenderer {
     ...
     */
    private final Map<Long, Map<BlockPos, Integer>> lastUpdate = Maps.newTreeMap(
        Ordering.natural()
            .reverse());
    ...
    public void render(float partialTicks, long finishTimeNano) {
        ...
        Iterator<Entry<Long, Map<BlockPos, Integer>>> iterator = this.lastUpdate.entrySet()
            .iterator();
                ...
                for (Entry<BlockPos, Integer> entry1 : map1.entrySet()) {
                    ...
                    if (set.add(blockpos)) {
                        RenderGlobal.drawSelectionBoundingBox(
                            (new AxisAlignedBB(BlockPos.ORIGIN)).grow(0.002D)
        ...
        for (Entry<BlockPos, Integer> entry2 : map.entrySet()) {
            ...
            DebugRenderer.renderDebugText(
                String.valueOf((Object) integer1),
                blockpos1.getX(),
                blockpos1.getY(),
                blockpos1.getZ(),
                partialTicks,
