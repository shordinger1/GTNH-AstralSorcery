...
public class BufferBuilder {
    ...
    // For some unknown reason Mojang changed the vanilla function to hardcode alpha as 255.... So lets re-add the
    // parameter -.-
    public void putColorRGBA(int index, int red, int green, int blue, int alpha) {
        if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN)
            this.rawIntBuffer.put(index, alpha << 24 | blue << 16 | green << 8 | red);
        else this.rawIntBuffer.put(index, red << 24 | green << 16 | blue << 8 | alpha);
    }
