...
public class RenderChunk {
    ...
    public void setPosition(int x, int y, int z) {
        if (x != this.position.getX() || y != this.position.getY() || z != this.position.getZ()) {
            ...
            for (EnumFacing enumfacing : EnumFacing.values()) {
                this.mapEnumFacing[enumfacing.ordinal()].setPos(this.position)
                    .move(enumfacing, 16);
            }
        ...
        if (!this.worldView.isEmpty()) {
            ...
            for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(blockpos, blockpos1)) {
                ...
                if (block.hasTileEntity(iblockstate)) {
                    TileEntity tileentity = this.worldView
                        .getTileEntity(blockpos$mutableblockpos, Chunk.EnumCreateEntityType.CHECK);
                ...
                for (BlockRenderLayer blockrenderlayer1 : BlockRenderLayer.values()) {
                    ...
                    if (block.getDefaultState()
                        .getRenderType() != EnumBlockRenderType.INVISIBLE) {
                        BufferBuilder bufferbuilder = generator.getRegionRenderCacheBuilder()
                        ...
                        aboolean[j] |= blockrendererdispatcher
                            .renderBlock(iblockstate, blockpos$mutableblockpos, this.worldView, bufferbuilder);
                    }
                }
    ...
    public ChunkCompileTaskGenerator makeCompileTaskChunk() {
        ...
        try {
            ...
            this.compileTask = new ChunkCompileTaskGenerator(
                this,
    ...
    private void rebuildWorldView() {
        int i = 1;
        ChunkCache cache = createRegionRenderCache(
            this.world,
            this.position.add(-1, -1, -1),
