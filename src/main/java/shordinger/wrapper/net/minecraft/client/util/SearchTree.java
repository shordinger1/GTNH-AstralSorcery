...
public class SearchTree<T> implements ISearchTree<T> {
    ...
    private void index(T element) {
        (this.idFunc.apply(element)).forEach(
            (p_194039_2_) -> {
                this.byName.add(
                    element,
                    p_194039_2_.toString()
                        .toLowerCase(Locale.ROOT));
            });
        (this.nameFunc.apply(element))
            .forEach((p_194041_2_) -> {
    ...
    public List<T> search(String searchText) {
        ...
        if (searchText.indexOf(58) < 0) {
            ...
            return (List<T>) (list1.isEmpty() ? list
                : Lists.newArrayList(
                new SearchTree.MergingIterator(list.iterator(), list1.iterator(), this.numericContents)));
        }
    }
