...
public class BlockFluidRenderer {
    ...
    public boolean renderFluid(IBlockAccess blockAccess, IBlockState blockStateIn, BlockPos blockPosIn,
        ...
        if (!flag1 && !flag2 && !aboolean[0] && !aboolean[1] && !aboolean[2] && !aboolean[3]) {
            ...
            if (flag1) {
                ...
                float f25 = 1.0F * f1;
                float f26 = 1.0F * f2;
                bufferBuilderIn.pos(d0 + 0.0D, d1 + (double) f7, d2 + 0.0D)
                    .color(f24, f25, f26, 1.0F)
                    .tex((double) f13, (double) f17)
                    .lightmap(l2, i3)
                    .endVertex();
                bufferBuilderIn.pos(d0 + 0.0D, d1 + (double) f8, d2 + 1.0D)
                    .color(f24, f25, f26, 1.0F)
                    .tex((double) f14, (double) f18)
                    .lightmap(l2, i3)
                    .endVertex();
                bufferBuilderIn.pos(d0 + 1.0D, d1 + (double) f9, d2 + 1.0D)
                    .color(f24, f25, f26, 1.0F)
                    .tex((double) f15, (double) f19)
                    .lightmap(l2, i3)
                    .endVertex();
                bufferBuilderIn.pos(d0 + 1.0D, d1 + (double) f10, d2 + 0.0D)
                    .color(f24, f25, f26, 1.0F)
            ...
            if (flag2) {
                ...
                int i2 = l1 >> 16 & 65535;
                int j2 = l1 & 65535;
                bufferBuilderIn.pos(d0, d1, d2 + 1.0D)
                ...
                bufferBuilderIn.pos(d0, d1, d2)
                    ...
                    .lightmap(i2, j2)
                    .endVertex();
                bufferBuilderIn.pos(d0 + 1.0D, d1, d2)
                    .color(0.5F, 0.5F, 0.5F, 1.0F)
                    .tex((double) f36, (double) f37)
                    .lightmap(i2, j2)
                    .endVertex();
                bufferBuilderIn.pos(d0 + 1.0D, d1, d2 + 1.0D)
                    .color(0.5F, 0.5F, 0.5F, 1.0F)
            ...
            for (int i1 = 0; i1 < 4; ++i1) {
                ...
                if (!flag) {
                    ...
                    if (state.getBlockFaceShape(blockAccess, blockpos, EnumFacing.VALUES[i1 + 2].getOpposite())
                ...
                if (aboolean[i1]) {
                    ...
                    bufferBuilderIn.pos(d5, d1 + (double) f40, d6)
                        ...
                        .lightmap(k, l)
                        .endVertex();
                    bufferBuilderIn.pos(d5, d1 + 0.0D, d6)
                        .color(f32, f33, f34, 1.0F)
                        .tex((double) f27, (double) f30)
                        .lightmap(k, l)
                        .endVertex();
                    bufferBuilderIn.pos(d3, d1 + 0.0D, d4)
                        .color(f32, f33, f34, 1.0F)
    ...
    private float getFluidHeight(IBlockAccess blockAccess, BlockPos blockPosIn, Material blockMaterial) {
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            if (blockAccess.getBlockState(blockpos.up())
