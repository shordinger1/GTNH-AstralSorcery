...
import shordinger.wrapper.net.minecraft.client.gui.chat.OverlayChatListener;
...
public class GuiIngame extends Gui {
    ...
    protected static final ResourceLocation PUMPKIN_BLUR_TEX_PATH = new ResourceLocation(
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.overlayMessageTime > 0) {
            ...
            if (l1 > 8) {
                ...
                fontrenderer.drawString(
                    this.overlayMessage,
                    -fontrenderer.getStringWidth(this.overlayMessage) / 2,
                    -4,
                    l + (l1 << 24 & -16777216));
                GlStateManager.disableBlend();
        ...
        if (this.titlesTimer > 0) {
            ...
            if (i2 > 8) {
                ...
                fontrenderer.drawString(
                    ...
                    (float) (-fontrenderer.getStringWidth(this.displayedSubTitle) / 2),
                    5.0F,
                    16777215 | j2,
                    true);
                GlStateManager.popMatrix();
        ...
        if (scoreplayerteam != null) {
            int i1 = scoreplayerteam.getColor()
                .getColorIndex();
        ...
        if (!this.mc.gameSettings.keyBindPlayerList.isKeyDown()
            || this.mc.isIntegratedServerRunning() && this.mc.player.connection.getPlayerInfoMap()
    ...
    protected void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_) {
        ...
        if (gamesettings.thirdPersonView == 0) {
            if (this.mc.playerController.isSpectator() && this.mc.pointedEntity == null) {
                ...
                if (!state.getBlock()
                    .hasTileEntity(state) || !(this.mc.world.getTileEntity(blockpos) instanceof IInventory)) {
                    return;
                }
            }
            ...
            if (gamesettings.showDebugInfo && !gamesettings.hideGUI
                && !this.mc.player.hasReducedDebug()
                ...
                GlStateManager.rotate(
                    ...
                    0.0F);
                GlStateManager.rotate(
                    entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * p_184045_1_,
                    0.0F,
                    1.0F,
                    0.0F);
    ...
    protected void renderPotionEffects(ScaledResolution resolution) {
        ...
        if (!collection.isEmpty()) {
            this.mc.getTextureManager()
                .bindTexture(GuiContainer.INVENTORY_BACKGROUND);
            GlStateManager.enableBlend();
            int i = 0;
            int j = 0;
            ...
            for (PotionEffect potioneffect : Ordering.natural()
                ...
                if (potioneffect.doesShowParticles()) {
                    ...
                    // FORGE - Move status icon check down from above so renderHUDEffect will still be called without a
                    // status icon
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            EnumHandSide enumhandside = entityplayer.getPrimaryHand()
                .opposite();
    ...
    public void renderExpBar(ScaledResolution scaledRes, int x) {
        ...
        if (this.mc.player.experienceLevel > 0) {
            ...
            int i1 = (scaledRes.getScaledWidth() - this.getFontRenderer()
                .getStringWidth(s)) / 2;
            int j1 = scaledRes.getScaledHeight() - 31 - 4;
            this.getFontRenderer()
                .drawString(s, i1 + 1, j1, 0);
            this.getFontRenderer()
                .drawString(s, i1 - 1, j1, 0);
            this.getFontRenderer()
                .drawString(s, i1, j1 + 1, 0);
            this.getFontRenderer()
                .drawString(s, i1, j1 - 1, 0);
            this.getFontRenderer()
                .drawString(s, i1, j1, 8453920);
            this.mc.mcProfiler.endSection();
        }
    }
    ...
    public void renderSelectedItem(ScaledResolution scaledRes) {
        ...
        if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty()) {
            ...
            int i = (scaledRes.getScaledWidth() - this.getFontRenderer()
                .getStringWidth(s)) / 2;
            ...
            if (k > 0) {
                ...
                GlStateManager.tryBlendFuncSeparate(
                    ...
                    GlStateManager.DestFactor.ZERO);
                this.getFontRenderer()
                    .drawStringWithShadow(s, (float) i, (float) j, 16777215 + (k << 24));
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        }
    ...
    public void renderDemo(ScaledResolution scaledRes) {
        ...
        int i = this.getFontRenderer()
            .getStringWidth(s);
        this.getFontRenderer()
            .drawStringWithShadow(s, (float) (scaledRes.getScaledWidth() - i - 10), 5.0F, 16777215);
        this.mc.mcProfiler.endSection();
    }
    ...
    protected void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        List<Score> list = Lists.newArrayList(Iterables.filter(collection, new Predicate<Score>() {
            ...
            public boolean apply(@Nullable Score p_apply_1_) {
                return p_apply_1_.getPlayerName() != null && !p_apply_1_.getPlayerName()
                    .startsWith("#");
            }
        }));
        ...
        int i = this.getFontRenderer()
            .getStringWidth(objective.getDisplayName());
        ...
        for (Score score1 : collection) {
            ...
            this.getFontRenderer()
                .drawString(s1, l1, k, 553648127);
            this.getFontRenderer()
                .drawString(
                    s2,
                    l - this.getFontRenderer()
                        .getStringWidth(s2),
                    k,
                    553648127);
    ...
    protected void renderPlayerStats(ScaledResolution scaledRes) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            for (int j5 = MathHelper.ceil((f + (float) k1) / 2.0F) - 1; j5 >= 0; --j5) {
                ...
                if (entityplayer.world.getWorldInfo()
    ...
    protected void renderPumpkinOverlay(ScaledResolution scaledRes) {
        ...
        GlStateManager.disableAlpha();
        this.mc.getTextureManager()
            .bindTexture(PUMPKIN_BLUR_TEX_PATH);
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
        bufferbuilder.pos(0.0D, (double) scaledRes.getScaledHeight(), -90.0D)
            .tex(0.0D, 1.0D)
            .endVertex();
        bufferbuilder.pos((double) scaledRes.getScaledWidth(), (double) scaledRes.getScaledHeight(), -90.0D)
            .tex(1.0D, 1.0D)
            .endVertex();
        bufferbuilder.pos((double) scaledRes.getScaledWidth(), 0.0D, -90.0D)
            .tex(1.0D, 0.0D)
            .endVertex();
    ...
    protected void renderVignette(float lightLevel, ScaledResolution scaledRes) {
        ...
        double d0 = Math.min(
            worldborder.getResizeSpeed() * (double) worldborder.getWarningTime() * 1000.0D,
        ...
        if (f > 0.0F) {
            ...
            GlStateManager
                .color(this.prevVignetteBrightness, this.prevVignetteBrightness, this.prevVignetteBrightness, 1.0F);
        }

        this.mc.getTextureManager()
            .bindTexture(VIGNETTE_TEX_PATH);
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
        bufferbuilder.pos(0.0D, (double) scaledRes.getScaledHeight(), -90.0D)
    ...
    protected void renderPortal(float timeInPortal, ScaledResolution scaledRes) {
        ...
        TextureAtlasSprite textureatlassprite = this.mc.getBlockRendererDispatcher()
            .getBlockModelShapes()
