...
public class MovementStep implements ITutorialStep {
    ...
    private static final ITextComponent MOVE_TITLE = new TextComponentTranslation(
        "tutorial.move.title",
    ...
    private static final ITextComponent MOVE_DESCRIPTION = new TextComponentTranslation(
        "tutorial.move.description",
        new Object[]{Tutorial.createKeybindComponent("jump")});
    private static final ITextComponent LOOK_TITLE = new TextComponentTranslation("tutorial.look.title", new Object[0]);
    private static final ITextComponent LOOK_DESCRIPTION = new TextComponentTranslation(
    ...
    public void update() {
        ...
        if (this.timeWaiting >= 100) {
            if (this.moveCompleted == -1 && this.moveToast == null) {
                this.moveToast = new TutorialToast(
                ...
                this.tutorial.getMinecraft()
                    .getToastGui()
                    .add(this.moveToast);
            } else if (this.moveCompleted != -1 && this.timeWaiting - this.moveCompleted >= 20
    ...
    public void handleMouse(MouseHelper mouseHelperIn) {
        if ((double) MathHelper.abs(mouseHelperIn.deltaX) > 0.01D
            || (double) MathHelper.abs(mouseHelperIn.deltaY) > 0.01D) {
            this.turned = true;
        }
    }
}
