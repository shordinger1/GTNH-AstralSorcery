...
public class MapItemRenderer {
    ...
    class Instance {
        ...
        private Instance(MapData mapdataIn) {
            ...
            this.location = MapItemRenderer.this.textureManager
                .getDynamicTextureLocation("map/" + mapdataIn.mapName, this.mapTexture);
        ...
        private void render(boolean noOverlayRendering) {
            ...
            bufferbuilder.pos(0.0D, 128.0D, -0.009999999776482582D)
                .tex(0.0D, 1.0D)
                .endVertex();
            bufferbuilder.pos(128.0D, 128.0D, -0.009999999776482582D)
                .tex(1.0D, 1.0D)
                .endVertex();
            bufferbuilder.pos(128.0D, 0.0D, -0.009999999776482582D)
                .tex(1.0D, 0.0D)
                .endVertex();
            bufferbuilder.pos(0.0D, 0.0D, -0.009999999776482582D)
                .tex(0.0D, 0.0D)
                .endVertex();
            tessellator.draw();
            ...
            for (MapDecoration mapdecoration : this.mapData.mapDecorations.values()) {
                if (!noOverlayRendering || mapdecoration.renderOnFrame()) {
                    ...
                    MapItemRenderer.this.textureManager.bindTexture(MapItemRenderer.TEXTURE_MAP_ICONS); // Rebind in
                    // case custom
                    // render
                    // changes it
                    GlStateManager.pushMatrix();
                    GlStateManager.translate(
                        0.0F + (float) mapdecoration.getX() / 2.0F + 64.0F,
                        0.0F + (float) mapdecoration.getY() / 2.0F + 64.0F,
                        -0.02F);
                    ...
                    bufferbuilder.pos(-1.0D, 1.0D, (double) ((float) k * -0.001F))
                        ...
                        .endVertex();
                    bufferbuilder.pos(1.0D, 1.0D, (double) ((float) k * -0.001F))
                        .tex((double) f3, (double) f2)
                        .endVertex();
                    bufferbuilder.pos(1.0D, -1.0D, (double) ((float) k * -0.001F))
                        .tex((double) f3, (double) f4)
                        .endVertex();
                    bufferbuilder.pos(-1.0D, -1.0D, (double) ((float) k * -0.001F))
