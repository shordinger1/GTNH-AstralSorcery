...
public class SPacketPlayerListItem implements Packet<INetHandlerPlayClient> {
    ...
    public SPacketPlayerListItem(SPacketPlayerListItem.Action actionIn, EntityPlayerMP... playersIn) {
        ...
        for (EntityPlayerMP entityplayermp : playersIn) {
            this.players.add(
                new SPacketPlayerListItem.AddPlayerData(
                    ...
                    for (; i1 < l; ++i1) {
                        ...
                        if (buf.readBoolean()) {
                            gameprofile.getProperties()
                                .put(s, new Property(s, s1, buf.readString(32767)));
                        } else {
                            gameprofile.getProperties()
                                .put(s, new Property(s, s1));
                        }
                    }
        ...
        for (SPacketPlayerListItem.AddPlayerData spacketplayerlistitem$addplayerdata : this.players) {
            switch (this.action) {
                case ADD_PLAYER:
                    ...
                    buf.writeVarInt(
                        spacketplayerlistitem$addplayerdata.getGameMode()
                            .getID());
    ...
    public class AddPlayerData {
        ...
        public AddPlayerData(GameProfile profileIn, int latencyIn, GameType gameModeIn,
                             @Nullable ITextComponent displayNameIn) {
        ...
        public String toString() {
            return MoreObjects.toStringHelper(this)
                ...
                .add(
                    "displayName",
                    this.displayName == null ? null : ITextComponent.Serializer.componentToJson(this.displayName))
                .toString();
        }
    }
}
