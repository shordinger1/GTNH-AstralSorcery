...
public class SPacketEntityProperties implements Packet<INetHandlerPlayClient> {
    ...
    public SPacketEntityProperties(int entityIdIn, Collection<IAttributeInstance> instances) {
        ...
        for (IAttributeInstance iattributeinstance : instances) {
            this.snapshots.add(
                new SPacketEntityProperties.Snapshot(
                    iattributeinstance.getAttribute()
                        .getName(),
                    iattributeinstance.getBaseValue(),
                    iattributeinstance.getModifiers()));
        }
    }
    ...
    public void readPacketData(PacketBuffer buf) throws IOException {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            for (int l = 0; l < k; ++l) {
                ...
                list.add(
                    new AttributeModifier(uuid, "Unknown synced attribute modifier", buf.readDouble(), buf.readByte()));
            }
    ...
    public void writePacketData(PacketBuffer buf) throws IOException {
        ...
        for (SPacketEntityProperties.Snapshot spacketentityproperties$snapshot : this.snapshots) {
            ...
            buf.writeVarInt(
                spacketentityproperties$snapshot.getModifiers()
                    .size());
