...
public class RConThreadQuery extends RConThreadBase {
    ...
    public RConThreadQuery(IServer p_i1536_1_) {
        ...
        if (!this.serverHostname.isEmpty() && !this.queryHostname.equals(this.serverHostname)) {
            ...
            try {
                ...
                this.logWarning(
                    "Unable to determine local host IP, please set server-ip in '" + p_i1536_1_.getSettingsFilename()
                        + "' : "
    ...
    private Boolean verifyClientAuth(DatagramPacket requestPacket) {
        ...
        if (!this.queryClients.containsKey(socketaddress)) {
            ...
            byte[] abyte = requestPacket.getData();
            return ((RConThreadQuery.Auth) this.queryClients.get(socketaddress)).getRandomChallenge()
                != RConUtils.getBytesAsBEint(abyte, 7, requestPacket.getLength()) ? false : true;
        }
    }
    ...
    private void cleanQueryClientsMap() {
        if (this.running) {
            ...
            if (i >= this.lastAuthCheckTime + 30000L) {
                ...
                Iterator<Entry<SocketAddress, RConThreadQuery.Auth>> iterator = this.queryClients.entrySet()
                    .iterator();

                while (iterator.hasNext()) {
                    Entry<SocketAddress, RConThreadQuery.Auth> entry = (Entry) iterator.next();

                    if (((RConThreadQuery.Auth) entry.getValue()).hasExpired(i)
                        .booleanValue()) {
                        iterator.remove();
                    }
                }
    ...
    public void startThread() {
        if (!this.running) {
            if (0 < this.queryPort && 65535 >= this.queryPort) {
                ...
                this.logWarning(
                    "Invalid query port " + this.queryPort
                        + " found in '"
                        + this.server.getSettingsFilename()
                        + "' (queries disabled)");
            }
        }
    }
    ...
    private boolean initQuerySystem() {
        try {
            ...
            this.logWarning(
                "Unable to initialise query system on " + this.serverHostname
                    + ":"
                    + this.queryPort
                    + " (Socket): "
    ...
    class Auth {
        ...
        public Auth(DatagramPacket requestPacket) {
            ...
            this.challengeValue = String.format("\t%s%d\u0000", this.requestIdAsString, this.randomChallenge)
