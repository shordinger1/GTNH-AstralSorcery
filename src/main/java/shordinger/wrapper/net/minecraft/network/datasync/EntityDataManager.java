package shordinger.wrapper.net.minecraft.network.datasync;
...
public class EntityDataManager {
    ...
    private static final Map<Class<? extends Entity>, Integer> NEXT_ID_MAP = Maps
    ...
    public static <T> DataParameter<T> createKey(Class<? extends Entity> clazz, DataSerializer<T> serializer) {
        if (true || LOGGER.isDebugEnabled()) // Forge: This is very useful for mods that register keys on classes that
        // are not their own
        {
            try {
                ...
                if (!oclass.equals(clazz)) {
                    // Forge: log at warn, mods should not add to classes that they don't own, and only add stacktrace
                    // when in debug is enabled as it is mostly not needed and consumes time
    ...
    public void setEntryValues(List<EntityDataManager.DataEntry<?>> entriesIn) {
        ...
        for (EntityDataManager.DataEntry<?> dataentry : entriesIn) {
            EntityDataManager.DataEntry<?> dataentry1 = (EntityDataManager.DataEntry) this.entries.get(
                Integer.valueOf(
                    dataentry.getKey()
                        .getId()));
    ...
    public static class DataEntry<T> {
        ...
        public EntityDataManager.DataEntry<T> copy() {
            return new EntityDataManager.DataEntry<T>(
                this.key,
