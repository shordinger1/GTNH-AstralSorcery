...
public class SPacketChunkData implements Packet<INetHandlerPlayClient> {
    ...
    public SPacketChunkData(Chunk chunkIn, int changedSectionFilter) {
        ...
        this.availableSections = this
            .extractChunkData(new PacketBuffer(this.getWriteBuffer()), chunkIn, flag, changedSectionFilter);
        this.tileEntityTags = Lists.<NBTTagCompound>newArrayList();

        for (Entry<BlockPos, TileEntity> entry : chunkIn.getTileEntityMap()
            .entrySet()) {
    ...
    public int extractChunkData(PacketBuffer buf, Chunk chunkIn, boolean writeSkylight, int changedSectionFilter) {
        ...
        for (int k = aextendedblockstorage.length; j < k; ++j) {
            ...
            if (extendedblockstorage != Chunk.NULL_BLOCK_STORAGE
                && (!this.isFullChunk() || !extendedblockstorage.isEmpty())
                && (changedSectionFilter & 1 << j) != 0) {
                i |= 1 << j;
    ...
    protected int calculateChunkSize(Chunk chunkIn, boolean p_189556_2_, int p_189556_3_) {
        ...
        for (int k = aextendedblockstorage.length; j < k; ++j) {
            ...
            if (extendedblockstorage != Chunk.NULL_BLOCK_STORAGE
                && (!this.isFullChunk() || !extendedblockstorage.isEmpty())
                && (p_189556_3_ & 1 << j) != 0) {
                i = i + extendedblockstorage.getData()
                    .getSerializedSize();
                i = i + extendedblockstorage.getBlockLight()
                    .getData().length;

                if (p_189556_2_) {
                    i += extendedblockstorage.getSkyLight()
                        .getData().length;
                }
            }
        }
