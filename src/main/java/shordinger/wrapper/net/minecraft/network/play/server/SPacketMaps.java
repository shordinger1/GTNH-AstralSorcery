...
public class SPacketMaps implements Packet<INetHandlerPlayClient> {
    ...
    public SPacketMaps(int mapIdIn, byte mapScaleIn, boolean trackingPositionIn, Collection<MapDecoration> iconsIn,
                       byte[] p_i46937_5_, int minXIn, int minZIn, int columnsIn, int rowsIn) {
        this.mapId = mapIdIn;
    ...
    public void readPacketData(PacketBuffer buf) throws IOException {
        ...
        for (int i = 0; i < this.icons.length; ++i) {
            ...
            this.icons[i] = new MapDecoration(
                MapDecoration.Type.byIcon((byte) (short1 >> 4 & 15)),
                buf.readByte(),
                buf.readByte(),
                (byte) (short1 & 15));
        }
