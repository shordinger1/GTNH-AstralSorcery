...
public class NettyPacketEncoder extends MessageToByteEncoder<Packet<?>> {
    ...
    private static final Marker RECEIVED_PACKET_MARKER = MarkerManager
        .getMarker("PACKET_SENT", NetworkManager.NETWORK_PACKETS_MARKER);
    ...
    protected void encode(ChannelHandlerContext p_encode_1_, Packet<?> p_encode_2_, ByteBuf p_encode_3_)
        ...
        EnumConnectionState enumconnectionstate = (EnumConnectionState) p_encode_1_.channel()
        ...
        if (enumconnectionstate == null) {
            ...
            if (integer == null) {
                ...
                try {
                    ...
                    throw throwable; // Forge: throw this instead of logging it, to prevent corrupt packets from being
                    // sent to the client where they are much harder to debug.
                }
            }
        }
    }
