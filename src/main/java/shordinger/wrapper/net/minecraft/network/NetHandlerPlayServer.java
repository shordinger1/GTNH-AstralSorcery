...
import shordinger.wrapper.net.minecraft.entity.IJumpingMount;
...
import shordinger.wrapper.net.minecraft.inventory.ContainerBeacon;
...
import shordinger.wrapper.net.minecraft.item.ItemWritableBook;
import shordinger.wrapper.net.minecraft.item.ItemWrittenBook;
...
import shordinger.wrapper.net.minecraft.util.ServerRecipeBookHelper;
...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
    ...
    public void update() {
        ...
        if (this.lowestRiddenEnt != this.player && this.lowestRiddenEnt.getControllingPassenger() == this.player) {
            ...
            if (this.vehicleFloating && this.player.getLowestRidingEntity()
                .getControllingPassenger() == this.player) {
        ...
        if (this.player.getLastActiveTime() > 0L && this.server.getMaxPlayerIdleMinutes() > 0
            && MinecraftServer.getCurrentTimeMillis() - this.player.getLastActiveTime()
            > (long) (this.server.getMaxPlayerIdleMinutes() * 1000 * 60)) {
    ...
    public void disconnect(final ITextComponent textComponent) {
        this.netManager
            .sendPacket(new SPacketDisconnect(textComponent), new GenericFutureListener<Future<? super Void>>() {
    ...
    public void processInput(CPacketInput packetIn) {
        ...
        this.player.setEntityActionState(
    ...
    private static boolean isMovePlayerPacketInvalid(CPacketPlayer packetIn) {
        if (Doubles.isFinite(packetIn.getX(0.0D)) && Doubles.isFinite(packetIn.getY(0.0D))
            && Doubles.isFinite(packetIn.getZ(0.0D))
    ...
    public void processVehicleMove(CPacketVehicleMove packetIn) {
        ...
        if (isMoveVehiclePacketInvalid(packetIn)) {
            ...
            if (entity != this.player && entity.getControllingPassenger() == this.player
                ...
                if (d10 - d9 > 100.0D && (!this.server.isSinglePlayer() || !this.server.getServerOwner()
                ...
                boolean flag = worldserver.getCollisionBoxes(
                        entity,
                ...
                // vehicle
                // moving
                boolean flag2 = worldserver.getCollisionBoxes(
                        entity,
                        entity.getEntityBoundingBox()
                            .shrink(0.0625D))
                    .isEmpty();
                ...
                this.server.getPlayerList()
                    .serverUpdateMovingPlayer(this.player);
                this.player.addMovementStat(this.player.posX - d0, this.player.posY - d1, this.player.posZ - d2);
                this.vehicleFloating = d11 >= -0.03125D && !this.server.isFlightAllowed()
                    && !worldserver.checkBlockCollision(
                    entity.getEntityBoundingBox()
                        .grow(0.0625D)
                        .expand(0.0D, -0.55D, 0.0D));
    ...
    public void handleRecipeBookUpdate(CPacketRecipeInfo p_191984_1_) {
        ...
        if (p_191984_1_.getPurpose() == CPacketRecipeInfo.Purpose.SHOWN) {
            this.player.getRecipeBook()
                .markSeen(p_191984_1_.getRecipe());
        } else if (p_191984_1_.getPurpose() == CPacketRecipeInfo.Purpose.SETTINGS) {
            this.player.getRecipeBook()
                .setGuiOpen(p_191984_1_.isGuiOpen());
            this.player.getRecipeBook()
                .setFilteringCraftable(p_191984_1_.isFilteringCraftable());
        }
    }
    ...
    public void handleSeenAdvancements(CPacketSeenAdvancements p_194027_1_) {
        ...
        if (p_194027_1_.getAction() == CPacketSeenAdvancements.Action.OPENED_TAB) {
            ...
            Advancement advancement = this.server.getAdvancementManager()
                .getAdvancement(resourcelocation);

            if (advancement != null) {
                this.player.getAdvancements()
                    .setSelectedTab(advancement);
            }
        }
    }
                    ...
                    if (this.player.isRiding()) {
                        ...
                        if (this.player.isPlayerSleeping()) {
                            ...
                            if (!this.player.isInvulnerableDimensionChange() && (!this.player.getServerWorld()
                                .getGameRules()
                                .getBoolean("disableElytraMovementCheck") || !this.player.isElytraFlying())) {
                                float f2 = this.player.isElytraFlying() ? 300.0F : 100.0F;

                                if (d11 - d10 > (double) (f2 * (float) i)
                                    && (!this.server.isSinglePlayer() || !this.server.getServerOwner()
                                    .equals(this.player.getName()))) {
                                    LOGGER.warn(
                                        "{} moved too quickly! {},{},{}",
                                        this.player.getName(),
                                        Double.valueOf(d7),
                            ...
                            boolean flag2 = worldserver
                                .getCollisionBoxes(
                                    this.player,
                                    this.player.getEntityBoundingBox()
                                        .shrink(0.0625D))
                                .isEmpty();
    ...
    public void processPlayerDigging(CPacketPlayerDigging packetIn) {
        ...
        switch (packetIn.getAction()) {
            ...
            case STOP_DESTROY_BLOCK:
                ...
                double dist = player.getEntityAttribute(EntityPlayer.REACH_DISTANCE)
                    .getAttributeValue() + 1;
                dist *= dist;
                ...
                if (d3 > dist) {
                    ...
                    if (packetIn.getAction() == CPacketPlayerDigging.Action.START_DESTROY_BLOCK) {
                        if (!this.server.isBlockProtected(worldserver, blockpos, this.player)
                            && worldserver.getWorldBorder()
                            .contains(blockpos)) {
                        ...
                        if (worldserver.getBlockState(blockpos)
    ...
    public void processTryUseItemOnBlock(CPacketPlayerTryUseItemOnBlock packetIn) {
        ...
        if (blockpos.getY() < this.server.getBuildLimit() - 1
            ...
            if (this.targetPos == null
                ...
                && worldserver.getWorldBorder()
                .contains(blockpos)) {
                this.player.interactionManager.processRightClickBlock(
                    this.player,
                    worldserver,
                    itemstack,
                    enumhand,
            ...
            TextComponentTranslation textcomponenttranslation = new TextComponentTranslation(
                "build.tooHigh",
                new Object[]{this.server.getBuildLimit()});
        ...
        if (this.player.isSpectator()) {
            ...
            if (entity != null) {
                ...
                if (entity.world == this.player.world) {
                    ...
                    this.server.getPlayerList()
                        .preparePlayer(this.player, worldserver1);
                    this.player.setPositionAndUpdate(entity.posX, entity.posY, entity.posZ);
                    this.player.interactionManager.setWorld(worldserver2);
                    this.server.getPlayerList()
                        .updateTimeAndWeatherForPlayer(this.player, worldserver2);
                    this.server.getPlayerList()
                        .syncPlayerInventory(this.player);
    ...
    public void onDisconnect(ITextComponent reason) {
        ...
        this.server.getPlayerList()
            .playerLoggedOut(this.player);

        if (this.server.isSinglePlayer() && this.player.getName()
            .equals(this.server.getServerOwner())) {
        ...
        if (this.player.getChatVisibility() == EntityPlayer.EnumChatVisibility.HIDDEN) {
            ...
            if (s.startsWith("/")) {
                ...
                ITextComponent itextcomponent = new TextComponentTranslation(
                    ...
                    net.minecraftforge.common.ForgeHooks.newChatWithLinks(s));
            ...
            if (this.chatSpamThresholdCount > 200 && !this.server.getPlayerList()
                .canSendCommands(this.player.getGameProfile())) {
    ...
    private void handleSlashCommand(String command) {
        this.server.getCommandManager()
            .executeCommand(this.player, command);
    }
            ...
            if (this.player.getDistanceSq(entity) < d0) {
                if (packetIn.getAction() == CPacketUseEntity.Action.INTERACT) {
                    ...
                    if (net.minecraftforge.common.ForgeHooks
                        .onInteractEntityAt(player, entity, packetIn.getHitVec(), enumhand1) != null) return;
            ...
            case PERFORM_RESPAWN:
                ...
                if (this.player.queuedEndExit) {
                    ...
                    this.player = this.server.getPlayerList()
                        .recreatePlayerEntity(this.player, 0, true);
                    CriteriaTriggers.CHANGED_DIMENSION
                        .trigger(this.player, DimensionType.THE_END, DimensionType.OVERWORLD);
                } else {
                    ...
                    this.player = this.server.getPlayerList()
                        .recreatePlayerEntity(this.player, player.dimension, false);

                    if (this.server.isHardcore()) {
                        this.player.setGameType(GameType.SPECTATOR);
                        this.player.getServerWorld()
                            .getGameRules()
                            .setOrCreateGameRule("spectatorsGenerateChunks", "false");
                    }
                }

                break;
            case REQUEST_STATS:
                this.player.getStatFile()
                    .sendStats(this.player);
        }
    }
    ...
    public void processClickWindow(CPacketClickWindow packetIn) {
        ...
        if (this.player.openContainer.windowId == packetIn.getWindowId()
            ...
            if (this.player.isSpectator()) {
                ...
                ItemStack itemstack2 = this.player.openContainer
                ...
                if (ItemStack.areItemStacksEqualUsingNBTShareTag(packetIn.getClickedItem(), itemstack2)) {
                    ...
                    this.pendingTransactions
                        .addKey(this.player.openContainer.windowId, Short.valueOf(packetIn.getActionNumber()));
                    this.player.connection.sendPacket(
    ...
    public void processTabComplete(CPacketTabComplete packetIn) {
        ...
        for (String s : this.server.getTabCompletions(
            this.player,
            packetIn.getMessage(),
            packetIn.getTargetBlock(),
            packetIn.hasTargetBlock())) {
            list.add(s);
        }
        ...
        if ("MC|BEdit".equals(s)) {
            ...
            try {
                ...
                if (itemstack3.getItem() == Items.WRITABLE_BOOK && itemstack4.getItem() == Items.WRITABLE_BOOK) {
                    ...
                    itemstack2.setTagInfo(
                        "title",
                        new NBTTagString(
                            itemstack3.getTagCompound()
                                .getString("title")));
                    NBTTagList nbttaglist = itemstack3.getTagCompound()
                        .getTagList("pages", 8);
            ...
            try {
                ...
                if (commandblockbaselogic1 != null) {
                    ...
                    this.player
                        .sendMessage(new TextComponentTranslation("advMode.setCommand.success", new Object[]{s6}));
                }
            ...
            if (this.player.openContainer instanceof ContainerRepair) {
                ...
                if (packetIn.getBufferData() != null && packetIn.getBufferData()
                    .readableBytes() >= 1) {
                    String s5 = ChatAllowedCharacters.filterAllowedCharacters(
                ...
                if (tileentity1 instanceof TileEntityStructure) {
                    ...
                    if (l1 == 2) {
                        ...
                        if (tileentitystructure.detectSize()) {
                            ...
                            this.player.sendStatusMessage(
                                new TextComponentTranslation("structure_block.size_failure", new Object[0]),
                                false);
                        }
                    }
