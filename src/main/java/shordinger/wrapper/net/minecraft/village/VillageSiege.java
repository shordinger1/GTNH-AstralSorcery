...
public class VillageSiege {
        ...
        while (true) {
            ...
            if (!entityplayer.isSpectator()) {
                this.village = this.world.getVillageCollection()
                    .getNearestVillage(new BlockPos(entityplayer), 1);

                if (this.village != null && this.village.getNumVillageDoors() >= 10
                    ...
                    for (int i = 0; i < 10; ++i) {
                        ...
                        for (Village village : this.world.getVillageCollection()
                            .getVillageList()) {
                            if (village != this.village && village
                                .isBlockPosWithinSqVillageRadius(new BlockPos(this.spawnX, this.spawnY, this.spawnZ))) {
                                flag = true;
                                break;
                            }
                        }
                    ...
                    if (vec3d != null) {
                        if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(
                            new net.minecraftforge.event.village.VillageSiegeEvent(
                                this,
                                world,
                                entityplayer,
                                village,
    ...
    private Vec3d findRandomSpawnPos(BlockPos pos) {
        for (int i = 0; i < 10; ++i) {
            ...
            if (this.village.isBlockPosWithinSqVillageRadius(blockpos) && WorldEntitySpawner
                .canCreatureTypeSpawnAtLocation(EntityLiving.SpawnPlacementType.ON_GROUND, this.world, blockpos)) {
