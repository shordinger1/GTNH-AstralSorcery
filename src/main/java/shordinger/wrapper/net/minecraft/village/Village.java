package shordinger.wrapper.net.minecraft.village;
...
public class Village implements net.minecraftforge.common.capabilities.ICapabilitySerializable<NBTTagCompound> {
    ...
    private Vec3d findRandomSpawnPos(BlockPos pos, int x, int y, int z) {
        for (int i = 0; i < 10; ++i) {
            BlockPos blockpos = pos
                .add(this.world.rand.nextInt(16) - 8, this.world.rand.nextInt(6) - 3, this.world.rand.nextInt(16) - 8);

            if (this.isBlockPosWithinSqVillageRadius(blockpos)
                && this.isAreaClearAround(new BlockPos(x, y, z), blockpos)) {
    ...
    private boolean isAreaClearAround(BlockPos blockSize, BlockPos blockLocation) {
        if (!this.world.getBlockState(blockLocation.down())
            .isTopSolid()) {
            return false;
        } else {
            ...
            for (int k = i; k < i + blockSize.getX(); ++k) {
                for (int l = blockLocation.getY(); l < blockLocation.getY() + blockSize.getY(); ++l) {
                    for (int i1 = j; i1 < j + blockSize.getZ(); ++i1) {
                        if (this.world.getBlockState(new BlockPos(k, l, i1))
                            .isNormalCube()) {
                            return false;
                        }
                    }
    ...
    private void updateNumIronGolems() {
        List<EntityIronGolem> list = this.world.<EntityIronGolem>getEntitiesWithinAABB(
    ...
    private void updateNumVillagers() {
        List<EntityVillager> list = this.world.<EntityVillager>getEntitiesWithinAABB(
    ...
    public VillageDoorInfo getDoorInfo(BlockPos pos) {
        ...
        for (VillageDoorInfo villagedoorinfo1 : this.villageDoorInfoList) {
            ...
            if (j < i) {
                ...
                if (this.world.getBlockState(blockpos.offset(enumfacing, 1))
                    .getBlock()
                    .isPassable(this.world, blockpos.offset(enumfacing, 1))
                    && this.world.getBlockState(blockpos.offset(enumfacing, -1))
                    .getBlock()
                    .isPassable(this.world, blockpos.offset(enumfacing, -1))
    ...
    public VillageDoorInfo getExistedDoor(BlockPos doorBlock) {
        if (this.center.distanceSq(doorBlock) > (double) (this.villageRadius * this.villageRadius)) {
            ...
            for (VillageDoorInfo villagedoorinfo : this.villageDoorInfoList) {
                if (villagedoorinfo.getDoorBlockPos()
                    .getX() == doorBlock.getX()
    ...
    private void removeDeadAndOutOfRangeDoors() {
        ...
        while (iterator.hasNext()) {
            ...
            if (world.isBlockLoaded(villagedoorinfo.getDoorBlockPos())) // Forge: check that the door block is loaded to
                // avoid loading chunks
                if (!this.isWoodDoor(villagedoorinfo.getDoorBlockPos())
                    || Math.abs(this.tickCounter - villagedoorinfo.getLastActivityTimestamp()) > 1200) {
    ...
    private void updateVillageRadiusAndCenter() {
        ...
        if (i == 0) {
            ...
            this.center = new BlockPos(
                this.centerHelper.getX() / i,
                this.centerHelper.getY() / i,
    ...
    private UUID findUUID(String name) {
        if (this.world == null || this.world.getMinecraftServer() == null) return EntityPlayer.getOfflineUUID(name);
        GameProfile profile = this.world.getMinecraftServer()
    ...
    public void readVillageDataFromNBT(NBTTagCompound compound) {
        ...
        this.centerHelper = new BlockPos(
            compound.getInteger("ACX"),
            compound.getInteger("ACY"),
        ...
        for (int j = 0; j < nbttaglist1.tagCount(); ++j) {
            ...
            if (nbttagcompound1.hasKey("UUID")) {
                this.playerReputation.put(
                    UUID.fromString(nbttagcompound1.getString("UUID")),
                ...
                this.playerReputation
                    .put(findUUID(nbttagcompound1.getString("Name")), Integer.valueOf(nbttagcompound1.getInteger("S")));
            }
        }
