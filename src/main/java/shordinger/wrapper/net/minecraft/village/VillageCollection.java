...
public class VillageCollection extends WorldSavedData {
    ...
    public Village getNearestVillage(BlockPos doorBlock, int radius) {
        ...
        for (Village village1 : this.villageList) {
            double d1 = village1.getCenter()
                .distanceSq(doorBlock);
    ...
    private void addDoorsAround(BlockPos central) {
        if (!this.world.isAreaLoaded(central, 16)) return; // Forge: prevent loading unloaded chunks when checking for
        // doors
        int i = 16;
        int j = 4;
        int k = 16;
    ...
    public static String fileNameForProvider(WorldProvider provider) {
        return "villages" + provider.getDimensionType()
            .getSuffix();
    }
}
