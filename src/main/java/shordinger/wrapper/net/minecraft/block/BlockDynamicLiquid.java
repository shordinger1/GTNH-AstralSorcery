...
public class BlockDynamicLiquid extends BlockLiquid {
    ...
    private void placeStaticBlock(World worldIn, BlockPos pos, IBlockState currentState) {
        worldIn.setBlockState(
            pos,
            getStaticBlock(this.blockMaterial).getDefaultState()
                .withProperty(LEVEL, currentState.getValue(LEVEL)),
            2);
    }

    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (!worldIn.isAreaLoaded(pos, this.getSlopeFindDistance(worldIn))) return; // Forge: avoid loading unloaded
        // chunks
        ...
        if (i > 0) {
            ...
            if (this.adjacentSourceBlocks >= 2 && net.minecraftforge.event.ForgeEventFactory
                .canCreateFluidSource(worldIn, pos, state, this.blockMaterial == Material.WATER)) {
                IBlockState iblockstate = worldIn.getBlockState(pos.down());

                if (iblockstate.getMaterial()
                    .isSolid()) {
                    i1 = 0;
                } else if (iblockstate.getMaterial() == this.blockMaterial
        ...
        if (this.canFlowInto(worldIn, pos.down(), iblockstate1)) {
            if (this.blockMaterial == Material.LAVA && worldIn.getBlockState(pos.down())
                .getMaterial() == Material.WATER) {
                worldIn.setBlockState(
                    pos.down(),
                    net.minecraftforge.event.ForgeEventFactory
                        .fireFluidPlaceBlockEvent(worldIn, pos.down(), pos, Blocks.STONE.getDefaultState()));
        ...
        if (this.canFlowInto(worldIn, pos, state)) {
            if (state.getMaterial() != Material.AIR) {
                if (this.blockMaterial == Material.LAVA) {
                    ...
                    if (state.getBlock() != Blocks.SNOW_LAYER) // Forge: Vanilla has a 'bug' where snowballs don't drop
                        // like every other block. So special case because
                        // ewww...
                        state.getBlock()
            ...
            worldIn.setBlockState(
                ...
                this.getDefaultState()
                    .withProperty(LEVEL, Integer.valueOf(level)),
                3);
        }
    }
