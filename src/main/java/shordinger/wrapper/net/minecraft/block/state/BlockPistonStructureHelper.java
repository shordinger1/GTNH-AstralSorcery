...
public class BlockPistonStructureHelper {
        ...
        if (!BlockPistonBase
            ...
            for (int i = 0; i < this.toMove.size(); ++i) {
                ...
                if (this.world.getBlockState(blockpos)
                    .getBlock()
                    .isStickyBlock(this.world.getBlockState(blockpos)) && !this.addBranchingBlocks(blockpos)) {
                    return false;
                }
            }
    ...
    private boolean addBlockLine(BlockPos origin, EnumFacing p_177251_2_) {
        ...
        if (iblockstate.getBlock()
            .isAir(iblockstate, this.world, origin)) {
            return true;
            ...
            if (i + this.toMove.size() > 12) {
                ...
                while (true) {
                    ...
                    if (k > -1) {
                        ...
                        for (int l = 0; l <= k + i1; ++l) {
                            ...
                            if (this.world.getBlockState(blockpos2)
                                .getBlock()
                                .isStickyBlock(this.world.getBlockState(blockpos2))
                                && !this.addBranchingBlocks(blockpos2)) {
                                return false;
                            }
                        }
                    ...
                    if (iblockstate.getBlock()
                        .isAir(iblockstate, this.world, blockpos1)) {
                        return true;
                    }

                    if (!BlockPistonBase
                        .canPush(iblockstate, this.world, blockpos1, this.moveDirection, true, this.moveDirection)
    ...
    private boolean addBranchingBlocks(BlockPos fromPos) {
        for (EnumFacing enumfacing : EnumFacing.values()) {
            if (enumfacing.getAxis() != this.moveDirection.getAxis()
                && !this.addBlockLine(fromPos.offset(enumfacing), enumfacing)) {
                return false;
            }
        }
