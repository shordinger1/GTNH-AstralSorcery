...
public class BlockRedstoneWire extends Block {
        ...
        if (!canConnectTo(worldIn.getBlockState(blockpos), direction, worldIn, blockpos)
            ...
            if (!iblockstate1.isNormalCube()) {
                boolean flag = worldIn.getBlockState(blockpos)
                    .isSideSolid(worldIn, blockpos, EnumFacing.UP)
    ...
    private IBlockState calculateCurrentChanges(World worldIn, BlockPos pos1, BlockPos pos2, IBlockState state) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            if (worldIn.getBlockState(blockpos)
                .isNormalCube()
                && !worldIn.getBlockState(pos1.up())
                .isNormalCube()) {
                if (flag && pos1.getY() >= pos2.getY()) {
                    l = this.getMaxCurrentStrength(worldIn, blockpos.up(), l);
                }
            } else if (!worldIn.getBlockState(blockpos)
                .isNormalCube() && flag && pos1.getY() <= pos2.getY()) {
    ...
    private int getMaxCurrentStrength(World worldIn, BlockPos pos, int strength) {
        if (worldIn.getBlockState(pos)
            ...
            int i = ((Integer) worldIn.getBlockState(pos)
                .getValue(POWER)).intValue();
    ...
    private boolean isPowerSourceAt(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        ...
        boolean flag1 = worldIn.getBlockState(pos.up())
            .isNormalCube();
        ...
        if (!flag1 && flag && canConnectUpwardsTo(worldIn, blockpos.up())) {
            ...
            return true;
        } else if (iblockstate.getBlock() == Blocks.POWERED_REPEATER
    ...
    protected static boolean canConnectTo(IBlockState blockState, @Nullable EnumFacing side, IBlockAccess world,
        ...
        if (block == Blocks.REDSTONE_WIRE) {
            ...
            return blockState.getBlock()
                .canConnectRedstone(blockState, world, pos, side);
        }
    }
