...
public class BlockSkull extends BlockContainer {
    ...
    private static final Predicate<BlockWorldState> IS_WITHER_SKELETON = new Predicate<BlockWorldState>() {
        ...
        public boolean apply(@Nullable BlockWorldState p_apply_1_) {
            return p_apply_1_.getBlockState() != null && p_apply_1_.getBlockState()
                .getBlock() == Blocks.SKULL
                ...
                if (tileentity instanceof TileEntitySkull) {
                    ...
                    if (tileentityskull.getSkullType() == 3 && tileentityskull.getPlayerProfile() != null) {
                        ...
                        itemstack.getTagCompound()
                            .setTag("SkullOwner", nbttagcompound);
                    }
    ...
    public boolean canDispenserPlace(World worldIn, BlockPos pos, ItemStack stack) {
        if (stack.getMetadata() == 1 && pos.getY() >= 2
            && worldIn.getDifficulty() != EnumDifficulty.PEACEFUL
            && !worldIn.isRemote) {
            return this.getWitherBasePattern()
    ...
    public void checkWitherSpawn(World worldIn, BlockPos pos, TileEntitySkull te) {
        if (te.getSkullType() == 1 && pos.getY() >= 2
            && worldIn.getDifficulty() != EnumDifficulty.PEACEFUL
            && !worldIn.isRemote) {
            ...
            if (blockpattern$patternhelper != null) {
                for (int i = 0; i < 3; ++i) {
                    ...
                    worldIn.setBlockState(
                        ...
                        blockworldstate.getBlockState()
                            .withProperty(NODROP, Boolean.valueOf(true)),
                        2);
                }
                ...
                BlockPos blockpos = blockpattern$patternhelper.translateOffset(1, 0, 0)
                    .getPos();
                EntityWither entitywither = new EntityWither(worldIn);
                BlockPos blockpos1 = blockpattern$patternhelper.translateOffset(1, 2, 0)
                    .getPos();
                entitywither.setLocationAndAngles(
                    (double) blockpos1.getX() + 0.5D,
                    (double) blockpos1.getY() + 0.55D,
                    (double) blockpos1.getZ() + 0.5D,
                    blockpattern$patternhelper.getForwards()
                        .getAxis() == EnumFacing.Axis.X ? 0.0F : 90.0F,
                    0.0F);
                ...
                for (EntityPlayerMP entityplayermp : worldIn.getEntitiesWithinAABB(
                    ...
                    entitywither.getEntityBoundingBox()
                        .grow(50.0D))) {
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState()
            ...
            .withProperty(NODROP, Boolean.valueOf((meta & 8) > 0));
    }
