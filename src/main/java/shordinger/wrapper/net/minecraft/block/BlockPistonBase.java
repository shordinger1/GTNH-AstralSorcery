...
import shordinger.wrapper.net.minecraft.block.state.BlockPistonStructureHelper;
...
public class BlockPistonBase extends BlockDirectional {
    ...
    protected static final AxisAlignedBB PISTON_BASE_SOUTH_AABB = new AxisAlignedBB(
        0.0D,
        0.0D,
        0.0D,
        1.0D,
        1.0D,
        0.75D);
    protected static final AxisAlignedBB PISTON_BASE_NORTH_AABB = new AxisAlignedBB(
        0.0D,
        0.0D,
        0.25D,
        1.0D,
    ...
    public boolean eventReceived(IBlockState state, World worldIn, BlockPos pos, int id, int param) {
        ...
        if (id == 0) {
            ...
            worldIn.setBlockState(
                pos,
                Blocks.PISTON_EXTENSION.getDefaultState()
            ...
            if (this.isSticky) {
                ...
                if (!flag1 && !iblockstate.getBlock()
                    .isAir(iblockstate, worldIn, blockpos)
                    && canPush(iblockstate, worldIn, blockpos, enumfacing.getOpposite(), false, enumfacing)
                    && (iblockstate.getMobilityFlag() == EnumPushReaction.NORMAL || block == Blocks.PISTON
     ...
     */
    public static boolean canPush(IBlockState blockStateIn, World worldIn, BlockPos pos, EnumFacing facing,
                                  boolean destroyBlocks, EnumFacing p_185646_5_) {
        Block block = blockStateIn.getBlock();

        if (block == Blocks.OBSIDIAN) {
            return false;
        } else if (!worldIn.getWorldBorder()
            .contains(pos)) {
            return false;
        ...
        if (!blockpistonstructurehelper.canMove()) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                list1.add(
                    worldIn.getBlockState(blockpos)
                        .getActualState(worldIn, blockpos));
            }
            ...
            for (int j = list2.size() - 1; j >= 0; --j) {
                ...
                iblockstate.getBlock()
                    .dropBlockAsItemWithChance(worldIn, blockpos1, iblockstate, chance, 0);
            ...
            if (extending) {
                ...
                worldIn
                    .setTileEntity(blockpos2, BlockPistonMoving.createTilePiston(iblockstate3, direction, true, true));
            }
    ...
    public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing face) {
        ...
        return state.getValue(FACING) != face.getOpposite() && ((Boolean) state.getValue(EXTENDED)).booleanValue()
