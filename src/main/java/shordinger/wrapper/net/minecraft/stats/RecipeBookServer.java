...
public class RecipeBookServer extends RecipeBook {
    ...
    private void sendPacket(SPacketRecipeBook.State state, EntityPlayerMP player, List<IRecipe> recipesIn) {
        net.minecraftforge.common.ForgeHooks.sendRecipeBook(
            player.connection,
            state,
            recipesIn,
    ...
    public NBTTagCompound write() {
        ...
        for (IRecipe irecipe : this.getRecipes()) {
            nbttaglist.appendTag(
                new NBTTagString(((ResourceLocation) CraftingManager.REGISTRY.getNameForObject(irecipe)).toString()));
        }
        ...
        for (IRecipe irecipe1 : this.getDisplayedRecipes()) {
            nbttaglist1.appendTag(
                new NBTTagString(((ResourceLocation) CraftingManager.REGISTRY.getNameForObject(irecipe1)).toString()));
        }
