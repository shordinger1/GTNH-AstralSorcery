...
import shordinger.wrapper.net.minecraft.util.IJsonSerializable;
import shordinger.wrapper.net.minecraft.util.TupleIntJsonSerializable;
            ...
            for (Entry<String, JsonElement> entry : jsonobject.entrySet()) {
                ...
                if (statbase != null) {
                    ...
                    if (((JsonElement) entry.getValue()).isJsonPrimitive()
                        && ((JsonElement) entry.getValue()).getAsJsonPrimitive()
                        .isNumber()) {
                        ...
                        if (jsonobject1.has("value") && jsonobject1.get("value")
                            ...
                            .getAsJsonPrimitive()
                            .isNumber()) {
                            tupleintjsonserializable.setIntegerValue(
                                jsonobject1.getAsJsonPrimitive("value")
                                    .getAsInt());
                        }
        ...
        for (Entry<StatBase, TupleIntJsonSerializable> entry : p_150880_0_.entrySet()) {
            if (((TupleIntJsonSerializable) entry.getValue()).getJsonSerializableValue() != null) {
                ...
                try {
                    jsonobject1.add(
                        "progress",
                        ((TupleIntJsonSerializable) entry.getValue()).getJsonSerializableValue()
                            .getSerializableElement());
                } catch (Throwable throwable) {
                    LOGGER.warn(
                        "Couldn't save statistic {}: error serializing progress",
                        ((StatBase) entry.getKey()).getStatName(),
                        throwable);
                }
