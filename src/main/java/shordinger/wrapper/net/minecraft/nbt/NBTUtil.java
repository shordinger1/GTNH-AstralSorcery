...
public final class NBTUtil {
                    ...
                    for (int i = 0; i < nbttaglist.tagCount(); ++i) {
                        ...
                        if (nbttagcompound1.hasKey("Signature", 8)) {
                            gameprofile.getProperties()
                                .put(s2, new Property(s2, s3, nbttagcompound1.getString("Signature")));
                        } else {
                            gameprofile.getProperties()
                                .put(s2, new Property(s2, s3));
                        }
                    }
                }
            }
    ...
    public static NBTTagCompound writeGameProfile(NBTTagCompound tagCompound, GameProfile profile) {
        ...
        if (!profile.getProperties()
            ...
            for (String s : profile.getProperties()
                .keySet()) {
    ...
    private static <T extends Comparable<T>> IBlockState setValueHelper(IBlockState p_193590_0_,
        ...
        if (optional.isPresent()) {
            ...
            LOGGER.warn(
                ...
                p_193590_2_,
                p_193590_3_.getString(p_193590_2_),
                p_193590_4_.toString());
            return p_193590_0_;
        }
    }
    ...
    public static NBTTagCompound writeBlockState(NBTTagCompound tag, IBlockState state) {
        ...
        if (!state.getProperties()
            .isEmpty()) {
            NBTTagCompound nbttagcompound = new NBTTagCompound();
            UnmodifiableIterator unmodifiableiterator = state.getProperties()
                .entrySet()
                .iterator();
