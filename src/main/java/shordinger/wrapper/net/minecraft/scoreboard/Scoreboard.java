...
public class Scoreboard {
     ...
     */
    private final Map<IScoreCriteria, List<ScoreObjective>> scoreObjectiveCriterias = Maps
        .<IScoreCriteria, List<ScoreObjective>>newHashMap();
    /**
    ...
    public boolean addPlayerToTeam(String player, String newTeam) {
        if (player.length() > 40) {
            ...
            scoreplayerteam.getMembershipCollection()
                .add(player);
            return true;
        }
    }
    ...
    public void removePlayerFromTeam(String username, ScorePlayerTeam playerTeam) {
        if (this.getPlayersTeam(username) != playerTeam) {
            throw new IllegalStateException(
                "Player is either on another team or not on any team. Cannot remove from team '" + playerTeam.getName()
                    + "'.");
        } else {
