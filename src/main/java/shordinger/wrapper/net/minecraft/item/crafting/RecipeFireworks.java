...
public class RecipeFireworks extends net.minecraftforge.registries.IForgeRegistryEntry.Impl<IRecipe>
    implements IRecipe {
        ...
        if (j <= 3 && i <= 1) {
            if (j >= 1 && i == 1 && i1 == 0) {
                ...
                for (int l1 = 0; l1 < inv.getSizeInventory(); ++l1) {
                    ...
                    if (!itemstack2.isEmpty()) {
                        if (net.minecraftforge.oredict.DyeUtils.isDye(itemstack2)) {
                            list.add(
                                Integer.valueOf(
                                    ItemDye.DYE_COLORS[net.minecraftforge.oredict.DyeUtils
                                        .rawDyeDamageFromStack(itemstack2) & 15]));
                    ...
                    if (!itemstack1.isEmpty()) {
                        if (net.minecraftforge.oredict.DyeUtils.isDye(itemstack1)) {
                            list1.add(
                                Integer.valueOf(
                                    ItemDye.DYE_COLORS[net.minecraftforge.oredict.DyeUtils
                                        .rawDyeDamageFromStack(itemstack1) & 15]));
                ...
                if (!this.resultItem.isEmpty() && this.resultItem.hasTagCompound()) {
                    NBTTagCompound nbttagcompound4 = this.resultItem.getTagCompound()
                        .getCompoundTag("Explosion");
