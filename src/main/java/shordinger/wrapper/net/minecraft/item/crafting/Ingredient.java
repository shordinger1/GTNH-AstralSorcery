...
public class Ingredient implements Predicate<ItemStack> {
    ...
    private static final java.util.Set<Ingredient> INSTANCES = java.util.Collections
        .newSetFromMap(new java.util.WeakHashMap<Ingredient, Boolean>());
    ...
    protected Ingredient(ItemStack... p_i47503_1_) {
        ...
        for (ItemStack s : p_i47503_1_) {
            ...
            if (s.getItem()
                .isDamageable()) simple = false;
            if (s.getMetadata() == net.minecraftforge.oredict.OreDictionary.WILDCARD_VALUE) s.getItem()
                .getSubItems(net.minecraft.creativetab.CreativeTabs.SEARCH, lst);
            else lst.add(s);
        }
    ...
    public static void invalidateAll() {
        for (Ingredient ing : INSTANCES) if (ing != null) ing.invalidate();
    }
    ...
    // Merges several vanilla Ingredients together. As a qwerk of how the json is structured, we can't tell if its a
    // single Ingredient type or multiple so we split per item and remerge here.
    // Only public for internal use, so we can access a private field in here.
    public static Ingredient merge(java.util.Collection<Ingredient> parts) {
        net.minecraft.util.NonNullList<ItemStack> lst = net.minecraft.util.NonNullList.create();
        for (Ingredient part : parts) {
            for (ItemStack stack : part.matchingStacks) lst.add(stack);
        }
