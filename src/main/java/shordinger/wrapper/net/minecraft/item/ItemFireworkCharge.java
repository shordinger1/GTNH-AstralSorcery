...
public class ItemFireworkCharge extends Item {
    ...
    public static void addExplosionInfo(NBTTagCompound nbt, List<String> tooltip) {
        ...
        if (b0 >= 0 && b0 <= 4) {
            tooltip.add(
                I18n.translateToLocal("item.fireworksCharge.type." + b0)
                    .trim());
        } else {
            tooltip.add(
                I18n.translateToLocal("item.fireworksCharge.type")
                    .trim());
        }
                ...
                for (int j = 0; j < ItemDye.DYE_COLORS.length; ++j) {
                    if (i == ItemDye.DYE_COLORS[j]) {
                        ...
                        s = s + I18n.translateToLocal(
                            "item.fireworksCharge." + EnumDyeColor.byDyeDamage(j)
                                .getUnlocalizedName());
                        break;
                    }
                }
                ...
                for (int k = 0; k < 16; ++k) {
                    if (l == ItemDye.DYE_COLORS[k]) {
                        ...
                        s1 = s1 + I18n.translateToLocal(
                            "item.fireworksCharge." + EnumDyeColor.byDyeDamage(k)
                                .getUnlocalizedName());
                        break;
                    }
                }
