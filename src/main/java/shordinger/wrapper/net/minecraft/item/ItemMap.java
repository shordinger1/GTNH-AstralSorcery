...
public class ItemMap extends ItemMapBase {
    ...
    public static ItemStack setupNewMap(World worldIn, double worldX, double worldZ, byte scale,
                        ...
                        if (k1 >= 0 && l1 >= -1 && k1 < 128 && l1 < 128) {
                            ...
                            if (!chunk.isEmpty()) {
                                            ...
                                            if (k4 <= 1) {
                                                ...
                                                label175:
                                                {
                                                    while (true) {
                                                        ...
                                                        if (iblockstate.getMapColor(worldIn, blockpos$mutableblockpos)
                                                            != MapColor.AIR || k4 <= 0) {
                                                            break;
                                                        }
                                                    }

                                                    if (k4 > 0 && iblockstate.getMaterial()
                                                        .isLiquid()) {
                                                        int l4 = k4 - 1;

                                                        while (true) {
                                                            IBlockState iblockstate1 = chunk
                                                                .getBlockState(i4 + i3, l4--, j4 + j3);
                                                            ++k3;

                                                            if (l4 <= 0 || !iblockstate1.getMaterial()
                                                                .isLiquid()) {
                                                                break label175;
                                                            }
                                                        }
                                ...
                                MapColor mapcolor = (MapColor) Iterables
                                    .getFirst(Multisets.copyHighestCountFirst(multiset), MapColor.AIR);
            ...
            if (mapdata != null) {
                if (worldIn.provider.getDimension() == mapdata.dimension) {
                    ...
                    Biome[] abiome = worldIn.getBiomeProvider()
                        .getBiomes((Biome[]) null, (j / i - 64) * i, (k / i - 64) * i, 128 * i, 128 * i, false);
    ...
    public Packet<?> createMapDataPacket(ItemStack stack, World worldIn, EntityPlayer player) {
        return this.getMapData(stack, worldIn)
            .getMapPacket(stack, worldIn, player);
    }
