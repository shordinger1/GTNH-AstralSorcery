...
public class RealmsSliderButton extends RealmsButton {
    ...
    public RealmsSliderButton(int buttonId, int x, int y, int width, int p_i1057_5_, int valueIn, float minValueIn,
                              float maxValueIn) {
    ...
    public void renderBg(int p_renderBg_1_, int p_renderBg_2_) {
        if (this.getProxy().visible) {
            if (this.sliding) {
                this.value = (float) (p_renderBg_1_ - (this.getProxy().x + 4)) / (float) (this.getProxy()
                    .getButtonWidth() - 8);
            ...
            this.blit(
                this.getProxy().x + (int) (this.value * (float) (this.getProxy()
                    .getButtonWidth() - 8)),
                this.getProxy().y,
                0,
                66,
    ...
    public void clicked(int p_clicked_1_, int p_clicked_2_) {
        this.value = (float) (p_clicked_1_ - (this.getProxy().x + 4)) / (float) (this.getProxy()
            .getButtonWidth() - 8);
        this.value = MathHelper.clamp(this.value, 0.0F, 1.0F);
