...
public class EntityAIFollowParent extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.childAnimal.getGrowingAge() >= 0) {
            ...
            List<EntityAnimal> list = this.childAnimal.world.<EntityAnimal>getEntitiesWithinAABB(
                this.childAnimal.getClass(),
                this.childAnimal.getEntityBoundingBox()
                    .grow(8.0D, 4.0D, 8.0D));
    ...
    public void updateTask() {
        if (--this.delayCounter <= 0) {
            ...
            this.childAnimal.getNavigator()
                .tryMoveToEntityLiving(this.parentAnimal, this.moveSpeed);
        }
    }
}
