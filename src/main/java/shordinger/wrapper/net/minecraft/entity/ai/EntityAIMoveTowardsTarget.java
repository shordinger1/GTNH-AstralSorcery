...
public class EntityAIMoveTowardsTarget extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (this.targetEntity == null) {
            return false;
        } else if (this.targetEntity.getDistanceSq(this.creature)
            > (double) (this.maxTargetDistance * this.maxTargetDistance)) {
            return false;
        } else {
            Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockTowards(
                ...
                16,
                7,
                new Vec3d(this.targetEntity.posX, this.targetEntity.posY, this.targetEntity.posZ));
    ...
    public boolean shouldContinueExecuting() {
        return !this.creature.getNavigator()
            ...
            && this.targetEntity.getDistanceSq(this.creature)
            < (double) (this.maxTargetDistance * this.maxTargetDistance);
    }
    ...
    public void startExecuting() {
        this.creature.getNavigator()
            .tryMoveToXYZ(this.movePosX, this.movePosY, this.movePosZ, this.speed);
    }
}
