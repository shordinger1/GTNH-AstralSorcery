...
import shordinger.wrapper.net.minecraft.entity.ai.EntityAIBeg;
...
import shordinger.wrapper.net.minecraft.entity.ai.EntityAIOwnerHurtByTarget;
import shordinger.wrapper.net.minecraft.entity.ai.EntityAIOwnerHurtTarget;
...
import shordinger.wrapper.net.minecraft.item.ItemFood;
...
public class EntityWolf extends EntityTameable {
    ...
    private static final DataParameter<Float> DATA_HEALTH_ID = EntityDataManager
        .<Float>createKey(EntityWolf.class, DataSerializers.FLOAT);
    private static final DataParameter<Boolean> BEGGING = EntityDataManager
        .<Boolean>createKey(EntityWolf.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager
        .<Integer>createKey(EntityWolf.class, DataSerializers.VARINT);
    /**
    ...
    protected void initEntityAI() {
        ...
        this.targetTasks
            .addTask(4, new EntityAITargetNonTamed(this, EntityAnimal.class, false, new Predicate<Entity>() {
            ...
            if (!itemstack.isEmpty()) {
                if (itemstack.getItem() instanceof ItemFood) {
                    ...
                    if (itemfood.isWolfsFavoriteMeat()
                        && ((Float) this.dataManager.get(DATA_HEALTH_ID)).floatValue() < 20.0F) {
    ...
    public float getTailRotation() {
        if (this.isAngry()) {
            ...
            return this.isTamed()
                ? (0.55F - (this.getMaxHealth() - ((Float) this.dataManager.get(DATA_HEALTH_ID)).floatValue()) * 0.02F)
                * (float) Math.PI
                : ((float) Math.PI / 5F);
        }
    }
    ...
    class AIAvoidEntity<T extends Entity> extends EntityAIAvoidEntity<T> {
        ...
        public AIAvoidEntity(EntityWolf wolfIn, Class<T> p_i47251_3_, float p_i47251_4_, double p_i47251_5_,
                             double p_i47251_7_) {
