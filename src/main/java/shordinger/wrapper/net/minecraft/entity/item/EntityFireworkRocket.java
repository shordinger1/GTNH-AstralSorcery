...
public class EntityFireworkRocket extends Entity {
    ...
    private static final DataParameter<ItemStack> FIREWORK_ITEM = EntityDataManager
        .<ItemStack>createKey(EntityFireworkRocket.class, DataSerializers.ITEM_STACK);
    private static final DataParameter<Integer> BOOSTED_ENTITY_ID = EntityDataManager
        .<Integer>createKey(EntityFireworkRocket.class, DataSerializers.VARINT);
    /**
        ...
        if (this.isAttachedToEntity()) {
            if (this.boostedEntity == null) {
                Entity entity = this.world
                    .getEntityByID(((Integer) this.dataManager.get(BOOSTED_ENTITY_ID)).intValue());
                    ...
                    for (int i = 0; i < 2; ++i) {
                        RayTraceResult raytraceresult = this.world.rayTraceBlocks(
                            ...
                            new Vec3d(
                                ...
                                entitylivingbase.posY + (double) entitylivingbase.height * 0.5D * (double) i,
    ...
    public static void registerFixesFireworkRocket(DataFixer fixer) {
        fixer.registerWalker(
            FixTypes.ENTITY,
            new ItemStackData(EntityFireworkRocket.class, new String[]{"FireworksItem"}));
    }
