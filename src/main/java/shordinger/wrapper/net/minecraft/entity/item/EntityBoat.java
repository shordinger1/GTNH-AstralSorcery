...
public class EntityBoat extends Entity {
    ...
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager
        .<Integer>createKey(EntityBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager
        .<Integer>createKey(EntityBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager
        .<Float>createKey(EntityBoat.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager
        .<Integer>createKey(EntityBoat.class, DataSerializers.VARINT);
        ...
        if (this.isEntityInvulnerable(source)) {
            ...
            if (source instanceof EntityDamageSourceIndirect && source.getTrueSource() != null
                ...
                if (flag || this.getDamageTaken() > 40.0F) {
                    if (!flag && this.world.getGameRules()
    ...
    public void onUpdate() {
        ...
        List<Entity> list = this.world.getEntitiesInAABBexcluding(
            ...
            this.getEntityBoundingBox()
                .grow(0.20000000298023224D, -0.009999999776482582D, 0.20000000298023224D),
    ...
    public float getRowingTime(int side, float limbSwing) {
        return this.getPaddleState(side) ? (float) MathHelper.clampedLerp(
            (double) this.paddlePositions[side] - 0.39269909262657166D,
            (double) this.paddlePositions[side],
            (double) limbSwing) : 0.0F;
    }
                    ...
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        Boolean result = iblockstate.getBlock()
                            .isAABBInsideMaterial(
                                ...
                                new AxisAlignedBB(blockpos$pooledmutableblockpos),
                                Material.WATER);
                        if (result != null) {
                            if (!result) continue;
                            f = Math.max(
                                ...
                                iblockstate.getBlock()
                                    .getBlockLiquidHeight(
                                        ...
                                        iblockstate,
                                        Material.WATER)
                                    + blockpos$pooledmutableblockpos.getY());
                        }
                    ...
                    if (j2 != 2) {
                        for (int k2 = k; k2 < l; ++k2) {
                            if (j2 <= 0 || k2 != k && k2 != l - 1) {
                                ...
                                if (!list.isEmpty()) {
                                    f += iblockstate.getBlock()
                                        .getSlipperiness(iblockstate, this.world, blockpos$pooledmutableblockpos, this);
                                    ++k1;
                                }
            ...
            for (int k1 = i; k1 < j; ++k1) {
                for (int l1 = k; l1 < l; ++l1) {
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        Boolean result = iblockstate.getBlock()
                            .isAABBInsideMaterial(world, blockpos$pooledmutableblockpos, axisalignedbb, Material.WATER);
                        if (result != null) {
                            if (!result) continue;

                            float f = iblockstate.getBlock()
                                .getBlockLiquidHeight(
                                    world,
                                    blockpos$pooledmutableblockpos,
                                    iblockstate,
            ...
            for (int k1 = i; k1 < j; ++k1) {
                for (int l1 = k; l1 < l; ++l1) {
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double) BlockLiquid
                            .getLiquidHeight(iblockstate, this.world, blockpos$pooledmutableblockpos)) {
    ...
    private void updateMotion() {
        ...
        if (this.previousStatus == EntityBoat.Status.IN_AIR && this.status != EntityBoat.Status.IN_AIR
    ...
    private void controlBoat() {
        if (this.isBeingRidden()) {
            ...
            this.setPaddleState(
                this.rightInputDown && !this.leftInputDown || this.forwardInputDown,
    ...
    public void updatePassenger(Entity passenger) {
        if (this.isPassenger(passenger)) {
            ...
            if (passenger instanceof EntityAnimal && this.getPassengers()
                .size() > 1) {
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos) {
        ...
        if (!this.isRiding()) {
            if (onGroundIn) {
                ...
                this.fallDistance = 0.0F;
            } else if (this.world.getBlockState((new BlockPos(this)).down())
                .getMaterial() != Material.WATER && y < 0.0D) {
    ...
    public boolean getPaddleState(int side) {
        return ((Boolean) this.dataManager.get(DATA_ID_PADDLE[side])).booleanValue()
