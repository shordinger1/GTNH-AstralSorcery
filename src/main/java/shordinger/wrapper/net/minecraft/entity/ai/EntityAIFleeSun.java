...
public class EntityAIFleeSun extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (!this.world.isDaytime()) {
            ...
            return false;
        } else if (!this.world.canSeeSky(
            new BlockPos(this.creature.posX, this.creature.getEntityBoundingBox().minY, this.creature.posZ))) {
            return false;
        } else if (!this.creature.getItemStackFromSlot(EntityEquipmentSlot.HEAD)
            .isEmpty()) {
            return false;
        } else {
    ...
    private Vec3d findPossibleShelter() {
        ...
        BlockPos blockpos = new BlockPos(
            this.creature.posX,
            this.creature.getEntityBoundingBox().minY,
            this.creature.posZ);
