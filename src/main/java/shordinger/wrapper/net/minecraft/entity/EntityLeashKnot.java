...
public class EntityLeashKnot extends EntityHanging {
    ...
    public EntityLeashKnot(World worldIn, BlockPos hangingPositionIn) {
        ...
        this.setPosition(
            (double) hangingPositionIn.getX() + 0.5D,
            (double) hangingPositionIn.getY() + 0.5D,
    ...
    public void setPosition(double x, double y, double z) {
        super.setPosition(
            (double) MathHelper.floor(x) + 0.5D,
            (double) MathHelper.floor(y) + 0.5D,
            (double) MathHelper.floor(z) + 0.5D);
    }
    ...
    public boolean processInitialInteract(EntityPlayer player, EnumHand hand) {
        if (this.world.isRemote) {
            ...
            boolean flag = false;
            double d0 = 7.0D;
            List<EntityLiving> list = this.world.<EntityLiving>getEntitiesWithinAABB(
                EntityLiving.class,
                new AxisAlignedBB(
                    this.posX - 7.0D,
                    this.posY - 7.0D,
    ...
    public boolean onValidSurface() {
        return this.world.getBlockState(this.hangingPosition)
    ...
    public static EntityLeashKnot getKnotForPosition(World worldIn, BlockPos pos) {
        ...
        for (EntityLeashKnot entityleashknot : worldIn.getEntitiesWithinAABB(
            ...
            new AxisAlignedBB(
                ...
                (double) j + 1.0D,
                (double) k + 1.0D))) {
            if (entityleashknot.getHangingPosition()
                .equals(pos)) {
                return entityleashknot;
            }
        }
