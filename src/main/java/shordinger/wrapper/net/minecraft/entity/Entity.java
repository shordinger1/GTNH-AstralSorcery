...
public abstract class Entity extends net.minecraft.entity.Entity
    implements ICommandSender, ICapabilitySerializable<NBTTagCompound> {
    ...
    private static final DataParameter<Integer> AIR = EntityDataManager
        .<Integer>createKey(Entity.class, DataSerializers.VARINT);
    private static final DataParameter<String> CUSTOM_NAME = EntityDataManager
        .<String>createKey(Entity.class, DataSerializers.STRING);
    ...
    private static final DataParameter<Boolean> SILENT = EntityDataManager
        .<Boolean>createKey(Entity.class, DataSerializers.BOOLEAN);
    ...
    protected void preparePlayerToSpawn() {
        if (this.world != null) {
            while (this.posY > 0.0D && this.posY < 256.0D) {
                ...
                if (this.world.getCollisionBoxes(this, this.getEntityBoundingBox())
                    .isEmpty()) {
                    break;
                }
                ...
                if (minecraftserver.getAllowNether()) {
                    if (!this.isRiding()) {
                        ...
                        if (this.portalCounter++ >= i) {
                            ...
                            if (this.world.provider.getDimensionType()
                                .getId() == -1) {
                                j = 0;
                            } else {
                                j = -1;
    ...
    private boolean isLiquidPresentInAABB(AxisAlignedBB bb) {
        return this.world.getCollisionBoxes(this, bb)
            .isEmpty() && !this.world.containsAnyLiquid(bb);
    }
    ...
    public void move(MoverType type, double x, double y, double z) {
        if (this.noClip) {
            ...
            List<AxisAlignedBB> list1 = this.world.getCollisionBoxes(
                this,
                this.getEntityBoundingBox()
                    .expand(x, y, z));
            AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
            ...
            if (this.stepHeight > 0.0F && flag && (d2 != x || d4 != z)) {
                ...
                List<AxisAlignedBB> list = this.world.getCollisionBoxes(
                    this,
                    this.getEntityBoundingBox()
                        .expand(d2, y, d4));
            ...
            if (this.canTriggerWalking() && (!this.onGround || !this.isSneaking() || !(this instanceof EntityPlayer))
                && !this.isRiding()) {
            ...
            if (this.world.isFlammableWithin(
                this.getEntityBoundingBox()
                    .shrink(0.001D))) {
                        ...
                        try {
                            iblockstate.getBlock()
                                .onEntityCollidedWithBlock(
                                    this.world,
                                    blockpos$pooledmutableblockpos2,
                                    iblockstate,
                                    this);
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos) {
        if (onGroundIn) {
            if (this.fallDistance > 0.0F) {
                state.getBlock()
                    .onFallenUpon(this.world, pos, this, this.fallDistance);
            }
    ...
    public boolean isWet() {
        if (this.inWater) {
            ...
            if (!this.world.isRainingAt(blockpos$pooledmutableblockpos) && !this.world.isRainingAt(
    ...
    public boolean handleWaterMovement() {
        if (this.getRidingEntity() instanceof EntityBoat) {
            this.inWater = false;
        } else if (this.world.handleMaterialAcceleration(
            this.getEntityBoundingBox()
    ...
    public boolean isInLava() {
        return this.world.isMaterialInBB(
            this.getEntityBoundingBox()
                .grow(-0.10000000149011612D, -0.4000000059604645D, -0.10000000149011612D),
            Material.LAVA);
    }
    ...
    public RayTraceResult rayTrace(double blockReachDistance, float partialTicks) {
        ...
        Vec3d vec3d2 = vec3d
            .addVector(vec3d1.x * blockReachDistance, vec3d1.y * blockReachDistance, vec3d1.z * blockReachDistance);
    ...
    public boolean isInRangeToRenderDist(double distance) {
        double d0 = this.getEntityBoundingBox()
        ...
        if (this.noClip) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                if (blockpos$pooledmutableblockpos.getX() != k || blockpos$pooledmutableblockpos.getY() != j
                    ...
                    if (this.world.getBlockState(blockpos$pooledmutableblockpos)
                        .causesSuffocation()) {
                        blockpos$pooledmutableblockpos.release();
                        return true;
                    }
    ...
    public void updateRidden() {
        ...
        if (this.isRiding() && entity.isDead) {
            ...
            this.motionY = 0.0D;
            this.motionZ = 0.0D;
            if (!updateBlocked) this.onUpdate();
    ...
    public void setPortal(BlockPos pos) {
        if (this.timeUntilPortal > 0) {
            ...
            if (!this.world.isRemote && !pos.equals(this.lastPortalPos)) {
                ...
                d1 = Math.abs(
                    MathHelper.pct(
                        d1 - (double) (blockpattern$patternhelper.getForwards()
                            .rotateY()
    ...
    public boolean isOnScoreboardTeam(Team teamIn) {
        return this.getTeam() != null ? this.getTeam()
            .isSameTeam(teamIn) : false;
    }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        ...
        if (!this.world.collidesWithAnyBlock(this.getEntityBoundingBox())) {
            ...
            float f1 = (float) enumfacing.getAxisDirection()
                .getOffset();
        ...
        if (!this.world.isRemote && !this.isDead) {
            ...
            if (dimensionIn == 1 && teleporter.isVanilla()) {
                ...
                double d0 = MathHelper.clamp(
                    this.posX * moveFactor,
                    worldserver1.getWorldBorder()
                        .minX() + 16.0D,
                    worldserver1.getWorldBorder()
                        .maxX() - 16.0D);
                double d1 = MathHelper.clamp(
                    this.posZ * moveFactor,
                    worldserver1.getWorldBorder()
                        .minZ() + 16.0D,
                    worldserver1.getWorldBorder()
                        .maxZ() - 16.0D);
    ...
    public void addEntityCrashInfo(CrashReportCategory category) {
        ...
        category.addCrashSection(
            ...
            String.format("%.2f, %.2f, %.2f", this.posX, this.posY, this.posZ));
        category.addCrashSection(
        ...
        category.addCrashSection(
            "Entity's Momentum",
            String.format("%.2f, %.2f, %.2f", this.motionX, this.motionY, this.motionZ));
        category.addDetail("Entity's Passengers", new ICrashReportDetail<String>() {

            public String call() throws Exception {
                return Entity.this.getPassengers()
                    .toString();
            }
        });
