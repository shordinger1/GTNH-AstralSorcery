...
public class EntityAIFollowGolem extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.villager.getGrowingAge() >= 0) {
            ...
            List<EntityIronGolem> list = this.villager.world.<EntityIronGolem>getEntitiesWithinAABB(
    ...
    public void startExecuting() {
        this.takeGolemRoseTick = this.villager.getRNG()
            .nextInt(320);
        this.tookGolemRose = false;
        this.ironGolem.getNavigator()
            .clearPath();
    }
    ...
    public void updateTask() {
        this.villager.getLookHelper()
            .setLookPositionWithEntity(this.ironGolem, 30.0F, 30.0F);

        if (this.ironGolem.getHoldRoseTick() == this.takeGolemRoseTick) {
            this.villager.getNavigator()
                .tryMoveToEntityLiving(this.ironGolem, 0.5D);
