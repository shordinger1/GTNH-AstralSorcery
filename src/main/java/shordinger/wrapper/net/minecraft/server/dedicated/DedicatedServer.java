...
import shordinger.wrapper.net.minecraft.network.rcon.IServer;
import shordinger.wrapper.net.minecraft.network.rcon.RConConsoleSource;
import shordinger.wrapper.net.minecraft.network.rcon.RConThreadMain;
import shordinger.wrapper.net.minecraft.network.rcon.RConThreadQuery;
import shordinger.wrapper.net.minecraft.profiler.Snooper;
import shordinger.wrapper.net.minecraft.server.MinecraftServer;
import shordinger.wrapper.net.minecraft.server.ServerEula;
import shordinger.wrapper.net.minecraft.server.gui.MinecraftServerGui;
...
public class DedicatedServer extends MinecraftServer implements IServer {
    ...
    public final List<PendingCommand> pendingCommandList = Collections
    ...
    public DedicatedServer(File anvilFileIn, DataFixer dataFixerIn, YggdrasilAuthenticationService authServiceIn,
        ...
        if (!this.eula.hasAcceptedEULA()) {
            ...
            LOGGER.info(
                ...
                this.getServerHostname()
                    .isEmpty() ? "*" : this.getServerHostname(),
                Integer.valueOf(this.getServerPort()));

            try {
                this.getNetworkSystem()
                    .addLanEndpoint(inetaddress, this.getServerPort());
            ...
            if (!PreYggdrasilConverter.tryConvert(this.settings)) {
                ...
                if (this.settings.hasProperty("announce-player-achievements")) {
                    this.worlds[0].getGameRules()
                        .setOrCreateGameRule(
                            "announceAdvancements",
                            this.settings.getBooleanProperty("announce-player-achievements", true) ? "true" : "false");
    ...
    public String loadResourcePackSHA() {
        if (this.settings.hasProperty("resource-pack-hash")) {
            if (this.settings.hasProperty("resource-pack-sha1")) {
                ...
                this.settings
                    .getStringProperty("resource-pack-sha1", this.settings.getStringProperty("resource-pack-hash", ""));
        ...
        if (!s.isEmpty() && !RESOURCE_PACK_SHA1_PATTERN.matcher(s)
            .matches()) {
            LOGGER.warn("Invalid sha1 for ressource-pack-sha1");
        }

        if (!this.settings.getStringProperty("resource-pack", "")
            .isEmpty() && s.isEmpty()) {
            LOGGER.warn(
    ...
    public EnumDifficulty getDifficulty() {
        return EnumDifficulty
            .getDifficultyEnum(this.settings.getIntProperty("difficulty", EnumDifficulty.NORMAL.getDifficultyId()));
    }
    ...
    public void addServerStatsToSnooper(Snooper playerSnooper) {
        ...
        playerSnooper.addClientStat(
            ...
            Integer.valueOf(
                this.getPlayerList()
                    .getWhitelistedPlayerNames().length));
    ...
    public void executePendingCommands() {
        while (!this.pendingCommandList.isEmpty()) {
            ...
            this.getCommandManager()
                .executeCommand(pendingcommand.sender, pendingcommand.command);
        }
    }
    ...
    public boolean isBlockProtected(World worldIn, BlockPos pos, EntityPlayer playerIn) {
        if (worldIn.provider.getDimension() != 0) {
            return false;
        } else if (this.getPlayerList()
            .getOppedPlayers()
            .isEmpty()) {
            return false;
        } else if (this.getPlayerList()
            .canSendCommands(playerIn.getGameProfile())) {
            return false;
