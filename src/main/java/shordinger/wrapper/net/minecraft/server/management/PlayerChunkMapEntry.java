...
public class PlayerChunkMapEntry {
    ...
    private Runnable loadedRunnable = new Runnable() {
        ...
        public void run() {
            PlayerChunkMapEntry.this.chunk = PlayerChunkMapEntry.this.playerChunkMap.getWorldServer()
                .getChunkProvider()
    ...
    public PlayerChunkMapEntry(PlayerChunkMap mapIn, int chunkX, int chunkZ) {
        ...
        mapIn.getWorldServer()
            .getChunkProvider()
            .loadChunk(chunkX, chunkZ, this.loadedRunnable);
    }
    ...
    public void addPlayer(EntityPlayerMP player) {
        if (this.players.contains(player)) {
            LOGGER.debug(
                ...
                player,
                Integer.valueOf(this.pos.x),
                Integer.valueOf(this.pos.z));
        } else {
            if (this.players.isEmpty()) {
                this.lastUpdateInhabitedTime = this.playerChunkMap.getWorldServer()
                    .getTotalWorldTime();
            }
    ...
    public void removePlayer(EntityPlayerMP player) {
        if (this.players.contains(player)) {
            ...
            if (this.chunk == null) {
                ...
                if (this.players.isEmpty()) {
                    if (this.loading) net.minecraftforge.common.chunkio.ChunkIOExecutor.dropQueuedChunkLoad(
                        this.playerChunkMap.getWorldServer(),
                        this.pos.x,
                        this.pos.z,
                        this.loadedRunnable);
            ...
            net.minecraftforge.common.MinecraftForge.EVENT_BUS
                .post(new net.minecraftforge.event.world.ChunkWatchEvent.UnWatch(this.chunk, player));
    ...
    public boolean providePlayerChunk(boolean canGenerate) {
        ...
        if (this.chunk != null) {
            ...
            if (canGenerate) {
                this.chunk = this.playerChunkMap.getWorldServer()
                    .getChunkProvider()
                    .provideChunk(this.pos.x, this.pos.z);
            } else {
                this.chunk = this.playerChunkMap.getWorldServer()
                    .getChunkProvider()
                    .loadChunk(this.pos.x, this.pos.z);
            }
    ...
    public boolean sendToPlayers() {
        if (this.sentToPlayers) {
            ...
            for (EntityPlayerMP entityplayermp : this.players) {
                ...
                this.playerChunkMap.getWorldServer()
                    .getEntityTracker()
                    .sendLeashedEntitiesInChunk(entityplayermp, this.chunk);
                // chunk watch event - delayed to here as the chunk wasn't ready in addPlayer
                net.minecraftforge.common.MinecraftForge.EVENT_BUS
                    .post(new net.minecraftforge.event.world.ChunkWatchEvent.Watch(this.chunk, entityplayermp));
            }
    ...
    public void sendToPlayer(EntityPlayerMP player) {
        if (this.sentToPlayers) {
            ...
            this.playerChunkMap.getWorldServer()
                .getEntityTracker()
                .sendLeashedEntitiesInChunk(player, this.chunk);
        }
    }

    public void updateChunkInhabitedTime() {
        long i = this.playerChunkMap.getWorldServer()
            .getTotalWorldTime();
        ...
        if (this.sentToPlayers && this.chunk != null) {
            if (this.changes != 0) {
                if (this.changes == 1) {
                    ...
                    // TODO: FDix Mojang's fuckup to modded by combining all TE data into the chunk data packet...
                    // seriously... packet size explosion!
                } else {
                    this.sendPacket(new SPacketMultiBlockChange(this.changes, this.changedBlocks, this.chunk));
                    // } Keep this in the else until we figure out a fix for mojang's derpitude on the data packet so we
                    // don't double send crap.
    ...
    public boolean hasPlayerMatching(Predicate<EntityPlayerMP> predicate) {
        return Iterables.tryFind(this.players, predicate)
            .isPresent();
    }
