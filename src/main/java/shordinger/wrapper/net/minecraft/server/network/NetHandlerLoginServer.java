...
public class NetHandlerLoginServer implements INetHandlerLoginServer, ITickable {
    ...
    public void update() {
        if (this.currentLoginState == NetHandlerLoginServer.LoginState.READY_TO_ACCEPT) {
            ...
            EntityPlayerMP entityplayermp = this.server.getPlayerList()
            ...
            if (entityplayermp == null) {
                ...
                net.minecraftforge.fml.common.network.internal.FMLNetworkHandler
                    .fmlServerHandshake(this.server.getPlayerList(), this.networkManager, this.player);
                this.player = null;
            }
        }
    ...
    public void tryAcceptPlayer() {
        ...
        String s = this.server.getPlayerList()
            .allowUserToConnect(this.networkManager.getRemoteAddress(), this.loginGameProfile);
        ...
        if (s != null) {
            ...
            if (this.server.getNetworkCompressionThreshold() >= 0 && !this.networkManager.isLocalChannel()) {
                this.networkManager.sendPacket(
                    new SPacketEnableCompression(this.server.getNetworkCompressionThreshold()),
                    new ChannelFutureListener() {

                        public void operationComplete(ChannelFuture p_operationComplete_1_) throws Exception {
                            NetHandlerLoginServer.this.networkManager.setCompressionThreshold(
                                NetHandlerLoginServer.this.server.getNetworkCompressionThreshold());
                        }
                    });
            }
            ...
            if (entityplayermp != null) {
                ...
                this.player = this.server.getPlayerList()
                    .createPlayerForUser(this.loginGameProfile);
            } else {
                net.minecraftforge.fml.common.network.internal.FMLNetworkHandler.fmlServerHandshake(
    ...
    public void processLoginStart(CPacketLoginStart packetIn) {
        Validate
            .validState(this.currentLoginState == NetHandlerLoginServer.LoginState.HELLO, "Unexpected hello packet");
    ...
    public void processEncryptionResponse(CPacketEncryptionResponse packetIn) {
        ...
        PrivateKey privatekey = this.server.getKeyPair()
            .getPrivate();
        ...
        if (!Arrays.equals(this.verifyToken, packetIn.getVerifyToken(privatekey))) {
            ...
            (new Thread(
                ...
                "User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet()) {
                    ...
                    try {
                        String s = (new BigInteger(
                            CryptManager.getServerIdHash(
                                "",
                                NetHandlerLoginServer.this.server.getKeyPair()
                                    .getPublic(),
                ...
                private InetAddress getAddress() {
                    ...
                    return NetHandlerLoginServer.this.server.getPreventProxyConnections()
                        && socketaddress instanceof InetSocketAddress ? ((InetSocketAddress) socketaddress).getAddress()
                        : null;
                }
            }).start();
        }
