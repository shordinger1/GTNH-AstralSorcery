...
public abstract class UserListEntryBan<T> extends UserListEntry<T> {
    ...
    protected UserListEntryBan(T valueIn, JsonObject json) {
        ...
        try {
            date = json.has("created") ? DATE_FORMAT.parse(
                json.get("created")
                    .getAsString())
                : new Date();
        ...
        this.bannedBy = json.has("source") ? json.get("source")
            .getAsString() : "(Unknown)";
        Date date1;

        try {
            date1 = json.has("expires") ? DATE_FORMAT.parse(
                json.get("expires")
                    .getAsString())
                : null;
        ...
        this.reason = json.has("reason") ? json.get("reason")
