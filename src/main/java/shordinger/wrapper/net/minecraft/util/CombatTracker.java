...
public class CombatTracker {
    ...
    public void calculateFallSuffix() {
        ...
        if (this.fighter.isOnLadder()) {
            Block block = this.fighter.world
                .getBlockState(
    ...
    public void trackDamage(DamageSource damageSrc, float healthIn, float damageAmount) {
        ...
        CombatEntry combatentry = new CombatEntry(
            damageSrc,
            this.fighter.ticksExisted,
            healthIn,
            damageAmount,
            this.fallSuffix,
    ...
    public ITextComponent getDeathMessage() {
        if (this.combatEntries.isEmpty()) {
            ...
            Entity entity = combatentry1.getDamageSrc()
                .getTrueSource();
            ...
            if (combatentry != null && combatentry1.getDamageSrc() == DamageSource.FALL) {
                ...
                if (combatentry.getDamageSrc() != DamageSource.FALL
                    && combatentry.getDamageSrc() != DamageSource.OUT_OF_WORLD) {
                    if (itextcomponent2 != null
                        && (itextcomponent1 == null || !itextcomponent2.equals(itextcomponent1))) {
                        Entity entity1 = combatentry.getDamageSrc()
                            .getTrueSource();
                        ItemStack itemstack1 = entity1 instanceof EntityLivingBase
                            ? ((EntityLivingBase) entity1).getHeldItemMainhand()
                            : ItemStack.EMPTY;

                        if (!itemstack1.isEmpty() && itemstack1.hasDisplayName()) {
                            itextcomponent = new TextComponentTranslation(
                                "death.fell.assist.item",
                                new Object[]{this.fighter.getDisplayName(), itextcomponent2,
                        ...
                        if (!itemstack.isEmpty() && itemstack.hasDisplayName()) {
                            itextcomponent = new TextComponentTranslation(
                                ...
                                new Object[]{this.fighter.getDisplayName(), itextcomponent1,
                    ...
                    itextcomponent = new TextComponentTranslation(
                        "death.fell.accident." + this.getFallSuffix(combatentry),
                        new Object[]{this.fighter.getDisplayName()});
                }
            } else {
                itextcomponent = combatentry1.getDamageSrc()
                    .getDeathMessage(this.fighter);
            }
    ...
    public EntityLivingBase getBestAttacker() {
        ...
        for (CombatEntry combatentry : this.combatEntries) {
            if (combatentry.getDamageSrc()
                ...
                entityplayer = (EntityPlayer) combatentry.getDamageSrc()
                    .getTrueSource();
            }

            if (combatentry.getDamageSrc()
                .getTrueSource() instanceof EntityLivingBase
                && (entitylivingbase == null || combatentry.getDamage() > f)) {
                f = combatentry.getDamage();
                entitylivingbase = (EntityLivingBase) combatentry.getDamageSrc()
                    .getTrueSource();
            }
        }
    ...
    private CombatEntry getBestCombatEntry() {
        ...
        for (int i = 0; i < this.combatEntries.size(); ++i) {
            ...
            if ((combatentry2.getDamageSrc() == DamageSource.FALL
                || combatentry2.getDamageSrc() == DamageSource.OUT_OF_WORLD) && combatentry2.getDamageAmount() > 0.0F
    ...
    public int getCombatDuration() {
        return this.inCombat ? this.fighter.ticksExisted - this.combatStartTime
            : this.combatEndTime - this.combatStartTime;
    }
    ...
    public void reset() {
        ...
        if (this.takingDamage
            && (!this.fighter.isEntityAlive() || this.fighter.ticksExisted - this.lastDamageTime > i)) {
