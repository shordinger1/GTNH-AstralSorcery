package shordinger.wrapper.net.minecraft.util.text.translation;
...
public class LanguageMap {
     ...
     */
    private static final Splitter EQUAL_SIGN_SPLITTER = Splitter.on('=')
        .limit(2);
    /**
    ...
    public static Map<String, String> parseLangFile(InputStream inputstream) {
        ...
        try {
            inputstream = net.minecraftforge.fml.common.FMLCommonHandler.instance()
                .loadLanguage(table, inputstream);
            ...
            for (String s : IOUtils.readLines(inputstream, StandardCharsets.UTF_8)) {
                if (!s.isEmpty() && s.charAt(0) != '#') {
                    ...
                    if (astring != null && astring.length == 2) {
                        String s1 = astring[0];
                        String s2 = NUMERIC_VARIABLE_PATTERN.matcher(astring[1])
                            .replaceAll("%$1s");
                        table.put(s1, s2);
                    }
                }
