...
public class TileEntityEndGateway extends TileEntityEndPortal implements ITickable {
    ...
    public void update() {
        ...
        if (flag1) {
            ...
            List<Entity> list = this.world
                .<Entity>getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(this.getPos()));
    ...
    private void findExitPortal() {
        ...
        for (int i = 16; getChunk(this.world, vec3d1).getTopFilledSegment() > 0
            && i-- > 0; vec3d1 = vec3d1.add(vec3d.scale(-16.0D))) {
            LOGGER.debug("Skipping backwards past nonempty chunk at {}", (Object) vec3d1);
        }

        for (int j = 16; getChunk(this.world, vec3d1).getTopFilledSegment() == 0
    ...
    private static BlockPos findSpawnpointInChunk(Chunk chunkIn) {
        ...
        for (BlockPos blockpos3 : BlockPos.getAllInBox(blockpos, blockpos1)) {
            ...
            if (iblockstate.getBlock() == Blocks.END_STONE && !chunkIn.getBlockState(blockpos3.up(1))
                .isBlockNormalCube()
                && !chunkIn.getBlockState(blockpos3.up(2))
                .isBlockNormalCube()) {
    ...
    public boolean shouldRenderFace(EnumFacing p_184313_1_) {
        return this.getBlockType()
            .getDefaultState()
            .shouldSideBeRendered(this.world, this.getPos(), p_184313_1_);
    }
