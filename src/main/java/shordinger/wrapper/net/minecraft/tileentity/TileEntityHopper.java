...
public class TileEntityHopper extends TileEntityLockableLoot implements IHopper, ITickable {
        ...
        if (iinventory == null) {
            ...
            if (this.isInventoryFull(iinventory, enumfacing)) {
                ...
                for (int i = 0; i < this.getSizeInventory(); ++i) {
                    if (!this.getStackInSlot(i)
                        .isEmpty()) {
                        ItemStack itemstack = this.getStackInSlot(i)
                            .copy();
                        ItemStack itemstack1 = putStackInInventoryAllSlots(
                            this,
                            iinventory,
    ...
    private static boolean isInventoryEmpty(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int i : aint) {
                if (!isidedinventory.getStackInSlot(i)
                    .isEmpty()) {
                    return false;
                }
            }
            ...
            for (int k = 0; k < j; ++k) {
                if (!inventoryIn.getStackInSlot(k)
                    .isEmpty()) {
                    return false;
                }
            }
    ...
    private static boolean pullItemFromSlot(IHopper hopper, IInventory inventoryIn, int index, EnumFacing direction) {
        ...
        if (!itemstack.isEmpty() && canExtractItemFromSlot(inventoryIn, itemstack, index, direction)) {
            ...
            ItemStack itemstack2 = putStackInInventoryAllSlots(
                inventoryIn,
                hopper,
                inventoryIn.decrStackSize(index, 1),
                (EnumFacing) null);
    ...
    private IInventory getInventoryForHopperTransfer() {
        ...
        return getInventoryAtPosition(
            this.getWorld(),
            this.getXPos() + (double) enumfacing.getFrontOffsetX(),
