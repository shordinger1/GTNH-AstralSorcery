...
public class TileEntityStructure extends TileEntity {
    ...
    private List<TileEntityStructure> filterRelatedCornerBlocks(List<TileEntityStructure> p_184415_1_) {
        Iterable<TileEntityStructure> iterable = Iterables.filter(p_184415_1_, new Predicate<TileEntityStructure>() {
            ...
            public boolean apply(@Nullable TileEntityStructure p_apply_1_) {
                return p_apply_1_.mode == TileEntityStructure.Mode.CORNER
                    && TileEntityStructure.this.name.equals(p_apply_1_.name);
            }
        });
    ...
    private List<TileEntityStructure> getNearbyCornerBlocks(BlockPos p_184418_1_, BlockPos p_184418_2_) {
        ...
        for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos
            .getAllInBoxMutable(p_184418_1_, p_184418_2_)) {
    ...
    private StructureBoundingBox calculateEnclosingBoundingBox(BlockPos p_184416_1_,
                                                               List<TileEntityStructure> p_184416_2_) {
     ...
     *                    return value will dictate the return value of this method. If false, the template will be
     *                    updated but not written
     *                    to disk.
     * @return true if the template was successfully saved.
     */
    public boolean save(boolean writeToDisk) {
        if (this.mode == TileEntityStructure.Mode.SAVE && !this.world.isRemote
     ...
     *                            block, the structure will not be loaded into the world and false will be returned.
     *                            Regardless of the value of
     *                            this parameter, the size in the structure block will be updated after calling this
     *                            method.
     * @return true if the template was successfully added to the world.
     */
    ...
    public ITextComponent getDisplayName() {
        return new TextComponentTranslation(
            "structure_block.hover." + this.mode.modeName,
