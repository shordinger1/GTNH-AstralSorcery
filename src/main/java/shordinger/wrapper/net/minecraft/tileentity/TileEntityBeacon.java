...
import shordinger.wrapper.net.minecraft.block.BlockStainedGlass;
import shordinger.wrapper.net.minecraft.block.BlockStainedGlassPane;
...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, ISidedInventory {
    ...
    public static final Potion[][] EFFECTS_LIST = new Potion[][]{{MobEffects.SPEED, MobEffects.HASTE},
        {MobEffects.RESISTANCE, MobEffects.JUMP_BOOST}, {MobEffects.STRENGTH}, {MobEffects.REGENERATION}};
    ...
    private void updateSegmentColors() {
        ...
        for (int i1 = j + 1; i1 < 256; ++i1) {
            ...
            if (iblockstate.getBlock() == Blocks.STAINED_GLASS) {
                ...
                if (iblockstate.getBlock() != Blocks.STAINED_GLASS_PANE) {
                    ...
                    float[] customColor = iblockstate.getBlock()
                        .getBeaconColorMultiplier(iblockstate, this.world, blockpos$mutableblockpos, getPos());
                    if (customColor != null) afloat = customColor;
                    else {
                        tileentitybeacon$beamsegment.incrementHeight();
                        continue;
                    }
                } else afloat = ((EnumDyeColor) iblockstate.getValue(BlockStainedGlassPane.COLOR))
            ...
            for (int l1 = 1; l1 <= 4; this.levels = l1++) {
                ...
                for (int j1 = i - l1; j1 <= i + l1 && flag1; ++j1) {
                    for (int k1 = k - l1; k1 <= k + l1; ++k1) {
                        Block block = this.world.getBlockState(new BlockPos(j1, i2, k1))
                            .getBlock();
        ...
        if (!this.world.isRemote && l < this.levels) {
            for (EntityPlayerMP entityplayermp : this.world.getEntitiesWithinAABB(
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        return stack.getItem() != null && stack.getItem()
            .isBeaconPayment(stack);
    }
