...
public class WorldGenIceSpike extends WorldGenerator {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        if (worldIn.getBlockState(position)
                    ...
                    for (int j1 = -l; j1 <= l; ++j1) {
                        ...
                        if ((i1 == 0 && j1 == 0 || f1 * f1 + f2 * f2 <= f * f)
                            ...
                            if (iblockstate.getBlock()
                                .isAir(iblockstate, worldIn, position.add(i1, k, j1)) || block == Blocks.DIRT
                                || block == Blocks.SNOW
                                || block == Blocks.ICE) {
                                this.setBlockAndNotifyAdequately(
                                    worldIn,
                                    position.add(i1, k, j1),
                ...
                for (int i2 = -k1; i2 <= k1; ++i2) {
                    ...
                    while (blockpos.getY() > 50) {
                        ...
                        if (!iblockstate1.getBlock()
                            .isAir(iblockstate1, worldIn, blockpos) && block1 != Blocks.DIRT
                            && block1 != Blocks.SNOW
                            && block1 != Blocks.ICE
