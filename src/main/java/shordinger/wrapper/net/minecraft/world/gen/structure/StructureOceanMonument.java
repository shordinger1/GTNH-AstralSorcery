...
public class StructureOceanMonument extends MapGenStructure {
    ...
    public static final List<Biome> WATER_BIOMES = Arrays
        .<Biome>asList(Biomes.OCEAN, Biomes.DEEP_OCEAN, Biomes.RIVER, Biomes.FROZEN_OCEAN, Biomes.FROZEN_RIVER);
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        if (i == k && j == l) {
            if (!this.world.getBiomeProvider()
                .areBiomesViable(i * 16 + 8, j * 16 + 8, 16, SPAWN_BIOMES)) {
                return false;
            }

            boolean flag = this.world.getBiomeProvider()
                .areBiomesViable(i * 16 + 8, j * 16 + 8, 29, WATER_BIOMES);
