...
public class EntityHasScore implements LootCondition {
    ...
    protected boolean entityScoreMatch(Entity entityIn, Scoreboard scoreboardIn, String objectiveStr,
        ...
        if (scoreobjective == null) {
            ...
            return !scoreboardIn.entityHasObjective(s, scoreobjective) ? false
                : rand.isInRange(
                scoreboardIn.getOrCreateScore(s, scoreobjective)
                    .getScorePoints());
        }
    }
    ...
    public static class Serializer extends LootCondition.Serializer<EntityHasScore> {
        ...
        public EntityHasScore deserialize(JsonObject json, JsonDeserializationContext context) {
            Set<Entry<String, JsonElement>> set = JsonUtils.getJsonObject(json, "scores")
                .entrySet();
            ...
            for (Entry<String, JsonElement> entry : set) {
                map.put(
                    entry.getKey(),
                    JsonUtils.deserializeClass(entry.getValue(), "score", context, RandomValueRange.class));
            }

            return new EntityHasScore(
                map,
