...
public class MapGenStronghold extends MapGenStructure {
    ...
    public MapGenStronghold() {
        ...
        for (Biome biome : Biome.REGISTRY) {
            if (biome != null && biome.getBaseHeight() > 0.0F
                && !net.minecraftforge.common.BiomeManager.strongHoldBiomesBlackList.contains(biome)) {
    ...
    public MapGenStronghold(Map<String, String> p_i2068_1_) {
        ...
        for (Entry<String, String> entry : p_i2068_1_.entrySet()) {
            if (((String) entry.getKey()).equals("distance")) {
                ...
                this.structureCoords = new ChunkPos[MathHelper
                    .getInt(entry.getValue(), this.structureCoords.length, 1)];
    ...
    private void generatePositions() {
        ...
        int i = 0;
        ObjectIterator lvt_2_1_ = this.structureMap.values()
            .iterator();
        ...
        if (l < this.structureCoords.length) {
            for (int i1 = 0; i1 < this.structureCoords.length; ++i1) {
                double d0 = 4.0D * this.distance + this.distance * (double) j * 6.0D
                    + (random.nextDouble() - 0.5D) * this.distance * 2.5D;
                int j1 = (int) Math.round(Math.cos(d1) * d0);
                int k1 = (int) Math.round(Math.sin(d1) * d0);
                BlockPos blockpos = this.world.getBiomeProvider()
                    .findBiomePosition((j1 << 4) + 8, (k1 << 4) + 8, 112, this.allowedBiomes, random);
    ...
    protected StructureStart getStructureStart(int chunkX, int chunkZ) {
        ...
        for (mapgenstronghold$start = new MapGenStronghold.Start(
            this.world,
            this.rand,
            chunkX,
            chunkZ); mapgenstronghold$start.getComponents()
                 .isEmpty()
    ...
    public static class Start extends StructureStart {
        ...
        public Start(World worldIn, Random random, int chunkX, int chunkZ) {
            ...
            structurestrongholdpieces$stairs2
                .buildComponent(structurestrongholdpieces$stairs2, this.components, random);
