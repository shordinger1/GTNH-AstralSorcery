...
public class RegionFile {
    ...
    private static final boolean FORGE_ENABLE_EXTENDED_SAVE = Boolean
        .parseBoolean(System.getProperty("forge.enableExtendedSave", "true"));
    ...
    public RegionFile(File fileNameIn) {
        ...
        try {
            ...
            for (int j1 = 0; j1 < 1024; ++j1) {
                ...
                int length = k & 255;
                if (length == 255) {
                    if ((k >> 8) <= this.sectorFree.size()) { // We're maxed out, so we need to read the proper length
                        // from the section
                ...
                } else if (length > 0) net.minecraftforge.fml.common.FMLLog.log.warn(
                    "Invalid chunk: ({}, {}) Offset: {} Length: {} runs off end file. {}",
                    j1 % 32,
                    (int) (j1 / 32),
                    k >> 8,
                    length,
                    ...
                    if (j + k > this.sectorFree.size()) {
                        ...
                        if (l > 4096 * k) {
                            net.minecraftforge.fml.common.FMLLog.log.warn(
                                "Invalid chunk: ({}, {}) Offset: {} Invalid Size: {}>{} {}",
                                x,
                                z,
                                j,
                                l,
                            ...
                            net.minecraftforge.fml.common.FMLLog.log
                                .warn("Invalid chunk: ({}, {}) Offset: {} Invalid Size: {} {}", x, z, j, l, fileName);
                            return null;
                        } else {
                            ...
                            if (b0 == 1) {
                                ...
                                return new DataInputStream(
                                    new BufferedInputStream(new InflaterInputStream(new ByteArrayInputStream(abyte))));
                            } else {
                                return null;
                            }
                        }
    ...
    public DataOutputStream getChunkDataOutputStream(int x, int z) {
        return this.outOfBounds(x, z) ? null
            ...
            new BufferedOutputStream(new DeflaterOutputStream(new RegionFile.ChunkBuffer(x, z))));
    }
    ...
    protected synchronized void write(int x, int z, byte[] data, int length) {
        try {
            ...
            if (l >= 256) {
                ...
                net.minecraftforge.fml.common.FMLLog.log
                    .warn("Large Chunk Detected: ({}, {}) Size: {} {}", x, z, l, fileName);
            }
