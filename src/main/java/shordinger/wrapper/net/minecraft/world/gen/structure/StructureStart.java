...
public abstract class StructureStart {
    ...
    public NBTTagCompound writeStructureComponentsToNBT(int chunkX, int chunkZ) {
        if (MapGenStructureIO.getStructureStartName(this) == null) // This is just a more friendly error instead of the
        // 'Null String' below
        {
            throw new RuntimeException(
                "StructureStart \"" + this.getClass()
    ...
    public void readStructureComponentsFromNBT(World worldIn, NBTTagCompound tagCompound) {
        ...
        for (int i = 0; i < nbttaglist.tagCount(); ++i) {
            ...
            if (tmp != null) this.components.add(tmp); // Forge: Prevent NPEs further down the line when a componenet
            // can't be loaded.
        }
