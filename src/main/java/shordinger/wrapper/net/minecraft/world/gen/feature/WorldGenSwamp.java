...
public class WorldGenSwamp extends WorldGenAbstractTree {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        for (i = rand.nextInt(4) + 5; worldIn.getBlockState(position.down())
                    ...
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
                        if (j >= 0 && j < 256) {
                            ...
                            if (!iblockstate.getBlock()
                                .isAir(iblockstate, worldIn, blockpos$mutableblockpos.setPos(l, j, i1))
                                && !iblockstate.getBlock()
                                .isLeaves(iblockstate, worldIn, blockpos$mutableblockpos.setPos(l, j, i1))) {
