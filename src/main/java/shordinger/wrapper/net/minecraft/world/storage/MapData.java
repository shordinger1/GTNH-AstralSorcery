...
public class MapData extends WorldSavedData {
    ...
    public void updateVisiblePlayers(EntityPlayer player, ItemStack mapStack) {
        ...
        for (int i = 0; i < this.playersArrayList.size(); ++i) {
            ...
            if (!mapdata$mapinfo1.player.isDead
                ...
                if (!mapStack.isOnItemFrame() && mapdata$mapinfo1.player.dimension == this.dimension
        ...
        if (mapStack.hasTagCompound() && mapStack.getTagCompound()
            .hasKey("Decorations", 9)) {
            NBTTagList nbttaglist = mapStack.getTagCompound()
                .getTagList("Decorations", 10);
    ...
    public static void addTargetDecoration(ItemStack map, BlockPos target, String decorationName,
                                           MapDecoration.Type type) {
        NBTTagList nbttaglist;

        if (map.hasTagCompound() && map.getTagCompound()
            .hasKey("Decorations", 9)) {
            nbttaglist = map.getTagCompound()
                .getTagList("Decorations", 10);
        } else {
    ...
    private void updateDecorations(MapDecoration.Type type, World worldIn, String decorationName, double worldX,
    ...
    public class MapInfo {
        ...
        public Packet<?> getPacket(ItemStack stack) {
            if (this.isDirty) {
                ...
                return this.tick++ % 5 == 0 ? new SPacketMaps(
                    stack.getMetadata(),
                    MapData.this.scale,
                    MapData.this.trackingPosition,
                    MapData.this.mapDecorations.values(),
