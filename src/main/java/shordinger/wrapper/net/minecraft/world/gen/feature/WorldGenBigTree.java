...
public class WorldGenBigTree extends WorldGenAbstractTree {
    ...
    void crosSection(BlockPos pos, float p_181631_2_, IBlockState p_181631_3_) {
        ...
        for (int j = -i; j <= i; ++j) {
            for (int k = -i; k <= i; ++k) {
                if (Math.pow((double) Math.abs(j) + 0.5D, 2.0D) + Math.pow((double) Math.abs(k) + 0.5D, 2.0D)
                    <= (double) (p_181631_2_ * p_181631_2_)) {
                    BlockPos blockpos = pos.add(j, 0, k);
                    IBlockState state = this.world.getBlockState(blockpos);

                    if (state.getBlock()
                        .isAir(state, world, blockpos)
                        || state.getBlock()
    ...
    void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_) {
        ...
        for (int j = 0; j <= i; ++j) {
            BlockPos blockpos1 = p_175937_1_.add(
