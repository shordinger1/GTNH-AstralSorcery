...
import shordinger.wrapper.net.minecraft.advancements.FunctionManager;
...
import shordinger.wrapper.net.minecraft.pathfinding.PathWorldListener;
...
import shordinger.wrapper.net.minecraft.util.IntHashMap;
...
import shordinger.wrapper.net.minecraft.village.VillageCollection;
...
import shordinger.wrapper.net.minecraft.world.storage.MapStorage;
...
public abstract class World extends net.minecraft.world.World implements IBlockAccess, ICapabilityProvider {
    ...
    // public java.util.ArrayList<net.minecraftforge.common.util.BlockSnapshot> capturedBlockSnapshots = new
    // java.util.ArrayList<net.minecraftforge.common.util.BlockSnapshot>();
    ...
    protected World(ISaveHandler saveHandlerIn, WorldInfo info, WorldProvider providerIn, Profiler profilerIn,
                    boolean client) {
        super(saveHandlerIn, info.getWorldName(), profilerIn, profilerIn);
    ...
    public boolean isValid(BlockPos pos) {
        return !this.isOutsideBuildHeight(pos) && pos.getX() >= -30000000
            && pos.getZ() >= -30000000
            && pos.getX() < 30000000
            && pos.getZ() < 30000000;
    }
    ...
    public boolean setBlockState(BlockPos pos, IBlockState newState, int flags) {
        if (this.isOutsideBuildHeight(pos)) {
            ...
            if (iblockstate == null) {
                ...
                if (newState.getLightOpacity(this, pos) != oldOpacity
                    || newState.getLightValue(this, pos) != oldLight) {
    ...
    // Split off from original setBlockState(BlockPos, IBlockState, int) method in order to directly send client and
    // physic updates
    public void markAndNotifyBlock(BlockPos pos, @Nullable Chunk chunk, IBlockState iblockstate, IBlockState newState,
                                   int flags) {
        Block block = newState.getBlock();
        {
            {
                if ((flags & 2) != 0 && (!this.isRemote || (flags & 4) == 0)
                    && (chunk == null || chunk.isPopulated())) {
    ...
    public void notifyNeighborsOfStateChange(BlockPos pos, Block blockType, boolean updateObservers) {
        if (net.minecraftforge.event.ForgeEventFactory
            .onNeighborNotify(
                ...
                java.util.EnumSet.allOf(EnumFacing.class),
                updateObservers)
            .isCanceled()) return;
    ...
    public void notifyNeighborsOfStateExcept(BlockPos pos, Block blockType, EnumFacing skipSide) {
        ...
        if (net.minecraftforge.event.ForgeEventFactory
            .onNeighborNotify(this, pos, this.getBlockState(pos), directions, false)
            .isCanceled()) return;
                    ...
                    public String call() throws Exception {
                        try {
                            return String.format(
                                "ID #%d (%s // %s // %s)",
                                Block.getIdFromBlock(blockIn),
                                blockIn.getUnlocalizedName(),
                                blockIn.getClass()
                                    .getName(),
                                blockIn.getRegistryName());
            ...
            if (true) {
                try {
                    iblockstate.getBlock()
                        .observedNeighborChange(iblockstate, this, pos, p_190529_2_, p_190529_3_);
                } catch (Throwable throwable) {
                    CrashReport crashreport = CrashReport
                        .makeCrashReport(throwable, "Exception while updating neighbours");
                    CrashReportCategory crashreportcategory = crashreport.makeCategory("Block being updated");
                    crashreportcategory.addDetail("Source block type", new ICrashReportDetail<String>() {
                        ...
                        public String call() throws Exception {
                            try {
                                return String.format(
                                    "ID #%d (%s // %s // %s)",
                                    Block.getIdFromBlock(p_190529_2_),
                                    p_190529_2_.getUnlocalizedName(),
                                    p_190529_2_.getClass()
                                        .getName(),
                                    p_190529_2_.getRegistryName());
                            } catch (Throwable var2) {
    ...
    public boolean canSeeSky(BlockPos pos) {
        return this.getChunkFromBlockCoords(pos)
            .canSeeSky(pos);
    }
    ...
    public boolean canBlockSeeSky(BlockPos pos) {
        if (pos.getY() >= this.getSeaLevel()) {
            ...
            if (!this.canSeeSky(blockpos)) {
                ...
                for (BlockPos blockpos1 = blockpos.down(); blockpos1.getY()
                    > pos.getY(); blockpos1 = blockpos1.down()) {
                    IBlockState iblockstate = this.getBlockState(blockpos1);

                    if (iblockstate.getBlock()
                        .getLightOpacity(iblockstate, this, blockpos) > 0
    ...
    public int getLight(BlockPos pos, boolean checkNeighbors) {
        if (pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000) {
            if (checkNeighbors && this.getBlockState(pos)
    ...
    public int getHeight(int x, int z) {
        ...
        if (x >= -30000000 && z >= -30000000 && x < 30000000 && z < 30000000) {
            if (this.isChunkLoaded(x >> 4, z >> 4, true)) {
                i = this.getChunkFromChunkCoords(x >> 4, z >> 4)
    ...
    public int getLightFromNeighborsFor(EnumSkyBlock type, BlockPos pos) {
        if (!this.provider.hasSkyLight() && type == EnumSkyBlock.SKY) {
            ...
            if (!this.isValid(pos)) {
                ...
                return type.defaultLightValue;
            } else if (this.getBlockState(pos)
                .useNeighborBrightness()) {
        ...
        if (!Double.isNaN(vec31.x) && !Double.isNaN(vec31.y) && !Double.isNaN(vec31.z)) {
            if (!Double.isNaN(vec32.x) && !Double.isNaN(vec32.y) && !Double.isNaN(vec32.z)) {
                ...
                if ((!ignoreBlockWithoutBoundingBox
                    || iblockstate.getCollisionBoundingBox(this, blockpos) != Block.NULL_AABB)
                ...
                while (k1-- >= 0) {
                    ...
                    if (!ignoreBlockWithoutBoundingBox || iblockstate1.getMaterial() == Material.PORTAL
                        || iblockstate1.getCollisionBoundingBox(this, blockpos) != Block.NULL_AABB) {
                        if (block1.canCollideCheck(iblockstate1, stopOnLiquid)) {
                            RayTraceResult raytraceresult1 = iblockstate1
                                .collisionRayTrace(this, blockpos, vec31, vec32);
     ...
     */
    public void playSound(@Nullable EntityPlayer player, BlockPos pos, SoundEvent soundIn, SoundCategory category,
    ...
    public void playSound(@Nullable EntityPlayer player, double x, double y, double z, SoundEvent soundIn,
                          SoundCategory category, float volume, float pitch) {
        net.minecraftforge.event.entity.PlaySoundAtEntityEvent event = net.minecraftforge.event.ForgeEventFactory
            .onPlaySoundAtEntity(player, soundIn, category, volume, pitch);
        ...
        for (int i = 0; i < this.eventListeners.size(); ++i) {
            ((IWorldEventListener) this.eventListeners.get(i))
    ...
    public void spawnAlwaysVisibleParticle(int p_190523_1_, double p_190523_2_, double p_190523_4_, double p_190523_6_,
        ...
        for (int i = 0; i < this.eventListeners.size(); ++i) {
            ((IWorldEventListener) this.eventListeners.get(i)).spawnParticle(
                p_190523_1_,
                false,
                true,
                p_190523_2_,
    ...
    private void spawnParticle(int particleID, boolean ignoreRange, double xCood, double yCoord, double zCoord,
                               double xSpeed, double ySpeed, double zSpeed, int... parameters) {
        for (int i = 0; i < this.eventListeners.size(); ++i) {
            ((IWorldEventListener) this.eventListeners.get(i))
    ...
    public boolean addWeatherEffect(Entity entityIn) {
        if (net.minecraftforge.common.MinecraftForge.EVENT_BUS
            .post(new net.minecraftforge.event.entity.EntityJoinWorldEvent(entityIn, this))) return false;
    ...
    public boolean spawnEntity(Entity entityIn) {
        ...
        if (!this.isRemote && (entityIn == null
            || (entityIn instanceof net.minecraft.entity.item.EntityItem && this.restoringBlockSnapshots)))
            return false;
        ...
        if (!flag && !this.isChunkLoaded(i, j, false)) {
            ...
            if (net.minecraftforge.common.MinecraftForge.EVENT_BUS
                .post(new net.minecraftforge.event.entity.EntityJoinWorldEvent(entityIn, this)) && !flag) return false;
     ...
     */
    private boolean getCollisionBoxes(@Nullable Entity entityIn, AxisAlignedBB aabb, boolean p_191504_3_,
        ...
        if (p_191504_3_
            && !net.minecraftforge.event.ForgeEventFactory.gatherCollisionBoxes(this, entityIn, aabb, outList))
            return true;
        try {
                    ...
                    if ((!flag2 || !flag3) && this.isBlockLoaded(blockpos$pooledmutableblockpos.setPos(k1, 64, l1))) {
                        for (int i2 = k; i2 < l; ++i2) {
                            if (!flag2 && !flag3 || i2 != l - 1) {
                                ...
                                iblockstate1.addCollisionBoxToList(
                                    this,
                                ...
                                if (p_191504_3_ && !net.minecraftforge.event.ForgeEventFactory
                                    .gatherCollisionBoxes(this, entityIn, aabb, outList)) {
                                    boolean flag5 = true;
                                    return flag5;
                                }
                            }
    ...
    public List<AxisAlignedBB> getCollisionBoxes(@Nullable Entity entityIn, AxisAlignedBB aabb) {
        ...
        net.minecraftforge.common.MinecraftForge.EVENT_BUS
            .post(new net.minecraftforge.event.world.GetCollisionBoxesEvent(this, entityIn, aabb, list));
        return list;
    }
    ...
    public BlockPos getTopSolidOrLiquidBlock(BlockPos pos) {
        ...
        for (blockpos = new BlockPos(pos.getX(), chunk.getTopFilledSegment() + 16, pos.getZ()); blockpos.getY()
            >= 0; blockpos = blockpos1) {
            ...
            if (state.getMaterial()
                .blocksMovement()
                && !state.getBlock()
                .isLeaves(state, this, blockpos1)
                && state.getBlock()
                .isFoliage(this, blockpos1)) {
                break;
            }
        }
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
        if (!(entityIn instanceof EntityPlayer)) {
            ...
            boolean isForced = !this.isRemote
                && getPersistentChunks().containsKey(new net.minecraft.util.math.ChunkPos(j2 >> 4, k2 >> 4));
            int range = isForced ? 0 : 32;
            boolean canUpdate = !forceUpdate
                || this.isAreaLoaded(j2 - range, 0, k2 - range, j2 + range, 0, k2 + range, true);
        ...
        if (forceUpdate && entityIn.addedToChunk) {
            ...
            if (entityIn.isRiding()) {
                ...
                if (!entityIn.updateBlocked) entityIn.onUpdate();
            }
        }
        ...
        if (!entityIn.addedToChunk || entityIn.chunkCoordX != i3
            || entityIn.chunkCoordY != j3
            ...
            if (!entityIn.setPositionNonDirty() && !this.isChunkLoaded(i3, k3, true)) {
                ...
                this.getChunkFromChunkCoords(i3, k3)
                    .addEntity(entityIn);
            }
        }
    ...
    public boolean checkNoEntityCollision(AxisAlignedBB bb, @Nullable Entity entityIn) {
        ...
        for (int j2 = 0; j2 < list.size(); ++j2) {
            ...
            if (!entity4.isDead && entity4.preventEntitySpawning
                && entity4 != entityIn
                && (entityIn == null || !entity4.isRidingSameEntity(entityIn))) // Forge: fix MC-103516
            {
                return false;
            }
        }
        ...
        for (int l3 = j2; l3 < k2; ++l3) {
            for (int i4 = l2; i4 < i3; ++i4) {
                for (int j4 = j3; j4 < k3; ++j4) {
                    ...
                    Boolean result = iblockstate1.getBlock()
                        .isAABBInsideLiquid(this, blockpos$pooledmutableblockpos, bb);
                    if (result != null) {
                        if (!result) continue;
                        blockpos$pooledmutableblockpos.release();
                        return true;
                    }
                    if (iblockstate1.getMaterial()
                        .isLiquid()) {
                        blockpos$pooledmutableblockpos.release();
                        return true;
                    }
    ...
    public boolean isFlammableWithin(AxisAlignedBB bb) {
        ...
        if (this.isAreaLoaded(j2, l2, j3, k2, i3, k3, true)) {
            ...
            for (int l3 = j2; l3 < k2; ++l3) {
                for (int i4 = l2; i4 < i3; ++i4) {
                    for (int j4 = j3; j4 < k3; ++j4) {
                        Block block = this.getBlockState(blockpos$pooledmutableblockpos.setPos(l3, i4, j4))
                            .getBlock();
        ...
        for (int l3 = j2; l3 < k2; ++l3) {
            for (int i4 = l2; i4 < i3; ++i4) {
                for (int j4 = j3; j4 < k3; ++j4) {
                    IBlockState iblockstate1 = this.getBlockState(blockpos$pooledmutableblockpos.setPos(l3, i4, j4));
                    Boolean result = iblockstate1.getBlock()
                        .isAABBInsideMaterial(this, blockpos$pooledmutableblockpos, bb, materialIn);
                    if (result != null) {
                        if (!result) continue;
                        blockpos$pooledmutableblockpos.release();
                        return true;
     ...
     */
    public Explosion createExplosion(@Nullable Entity entityIn, double x, double y, double z, float strength,
     ...
     */
    public Explosion newExplosion(@Nullable Entity entityIn, double x, double y, double z, float strength,
    ...
    public TileEntity getTileEntity(BlockPos pos) {
        if (this.isOutsideBuildHeight(pos)) {
            ...
            if (tileentity2 == null) {
                tileentity2 = this.getChunkFromBlockCoords(pos)
                    .getTileEntity(pos, Chunk.EnumCreateEntityType.IMMEDIATE);
            }
    ...
    private TileEntity getPendingTileEntityAt(BlockPos pos) {
        for (int j2 = 0; j2 < this.addedTileEntityList.size(); ++j2) {
            ...
            if (!tileentity2.isInvalid() && tileentity2.getPos()
                .equals(pos)) {
                return tileentity2;
            }
        }
        ...
        if (!this.isOutsideBuildHeight(pos)) {
            if (tileEntityIn != null && !tileEntityIn.isInvalid()) {
                if (this.processingLoadedTiles) {
                    ...
                    if (tileEntityIn.getWorld() != this) tileEntityIn.setWorld(this); // Forge - set the world early as
                    // vanilla doesn't set it until
                    // next tick
    ...
    public boolean isBlockFullCube(BlockPos pos) {
        AxisAlignedBB axisalignedbb = this.getBlockState(pos)
    ...
    public void updateWeatherBody() {
        if (this.provider.hasSkyLight()) {
            if (!this.isRemote) {
                boolean flag = this.getGameRules()
    ...
    public void immediateBlockTick(BlockPos pos, IBlockState state, Random random) {
        ...
        state.getBlock()
            .updateTick(this, pos, state, random);
        ...
        if (f >= 0.15F) {
            ...
            if (pos.getY() >= 0 && pos.getY() < 256 && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10) {
                ...
                if ((block == Blocks.WATER || block == Blocks.FLOWING_WATER)
                    ...
                    boolean flag = this.isWater(pos.west()) && this.isWater(pos.east())
        ...
        if (f >= 0.15F) {
            ...
            if (pos.getY() >= 0 && pos.getY() < 256 && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10) {
                ...
                if (iblockstate1.getBlock()
                    .isAir(iblockstate1, this, pos) && Blocks.SNOW_LAYER.canPlaceBlockAt(this, pos)) {
                    return true;
                }
            }
    ...
    private int getRawLight(BlockPos pos, EnumSkyBlock lightType) {
        if (lightType == EnumSkyBlock.SKY && this.canSeeSky(pos)) {
            ...
            IBlockState iblockstate1 = this.getBlockState(pos);
            int j2 = lightType == EnumSkyBlock.SKY ? 0
                : iblockstate1.getBlock()
                .getLightValue(iblockstate1, this, pos);
            int k2 = iblockstate1.getBlock()
                .getLightOpacity(iblockstate1, this, pos);
                    ...
                    if (k8 > j8) {
                        ...
                        if (l8 + i9 + j9 < updateRange && flag) {
                            if (this.getLightFor(lightType, blockpos2.west()) < k8) {
                                this.lightUpdateBlockList[k2++] = k7 - 1
                                    - j3
                                    + 32
                                    + (l7 - k3 + 32 << 6)
                                    + (i8 - l3 + 32 << 12);
                            ...
                            if (this.getLightFor(lightType, blockpos2.down()) < k8) {
                                this.lightUpdateBlockList[k2++] = k7 - j3
                                    + 32
                                    + (l7 - 1 - k3 + 32 << 6)
                                    + (i8 - l3 + 32 << 12);
                            }
        ...
        for (int j3 = j2; j3 <= k2; ++j3) {
            for (int k3 = l2; k3 <= i3; ++k3) {
                if (this.isChunkLoaded(j3, k3, true)) {
                    this.getChunkFromChunkCoords(j3, k3)
                        .getEntitiesWithinAABBForEntity(entityIn, boundingBox, list, predicate);
                }
            }
        }
    ...
    public int countEntities(Class<?> entityType) {
        ...
        for (Entity entity4 : this.loadedEntityList) {
            if ((!(entity4 instanceof EntityLiving) || !((EntityLiving) entity4).isNoDespawnRequired())
                && entityType.isAssignableFrom(entity4.getClass())) {
                ++j2;
            }
        }
    ...
    public void loadEntities(Collection<Entity> entityCollection) {
        for (Entity entity4 : entityCollection) {
            if (!net.minecraftforge.common.MinecraftForge.EVENT_BUS
                .post(new net.minecraftforge.event.entity.EntityJoinWorldEvent(entity4, this))) {
     ...
     */
    public boolean mayPlace(Block blockIn, BlockPos pos, boolean skipCollisionCheck, EnumFacing sidePlacedOn,
                            @Nullable Entity placer) {
        IBlockState iblockstate1 = this.getBlockState(pos);
        AxisAlignedBB axisalignedbb = skipCollisionCheck ? null
            : blockIn.getDefaultState()
            .getCollisionBoundingBox(this, pos);

        if (!((placer instanceof EntityPlayer) || !net.minecraftforge.event.ForgeEventFactory
            .onBlockPlace(
                placer,
                new net.minecraftforge.common.util.BlockSnapshot(this, pos, blockIn.getDefaultState()),
                sidePlacedOn)
            .isCanceled())) return false;
        if (axisalignedbb != Block.NULL_AABB && !this.checkNoEntityCollision(axisalignedbb.offset(pos))) // Forge:
        // Remove
        // second
        // parameter,
        // we patch
        ...
        // the placer
        {
            ...
            return iblockstate1.getBlock()
                .isReplaceable(this, pos) && blockIn.canPlaceBlockOnSide(this, pos, sidePlacedOn);
        }
    }
    ...
    public int getStrongPower(BlockPos pos, EnumFacing direction) {
        return this.getBlockState(pos)
            .getStrongPower(this, pos, direction);
    }
    ...
    public int getRedstonePower(BlockPos pos, EnumFacing facing) {
        IBlockState iblockstate1 = this.getBlockState(pos);
        return iblockstate1.getBlock()
            .shouldCheckWeakPower(iblockstate1, this, pos, facing) ? this.getStrongPower(pos)
            : iblockstate1.getWeakPower(this, pos, facing);
    }
    ...
    public EntityPlayer getNearestAttackablePlayer(double posX, double posY, double posZ, double maxXZDistance,
        ...
        for (int j2 = 0; j2 < this.playerEntities.size(); ++j2) {
            ...
            if (!entityplayer1.capabilities.disableDamage && entityplayer1.isEntityAlive()
                ...
                && (p_184150_12_ == null || p_184150_12_.apply(entityplayer1))) {
                ...
                if (playerToDouble != null) {
                    d2 *= ((Double) MoreObjects.firstNonNull(playerToDouble.apply(entityplayer1), Double.valueOf(1.0D)))
                        .doubleValue();
                }

                d2 = net.minecraftforge.common.ForgeHooks.getPlayerVisibilityDistance(entityplayer1, d2, maxYDistance);

                if ((maxYDistance < 0.0D || Math.abs(entityplayer1.posY - posY) < maxYDistance * maxYDistance)
                    && (maxXZDistance < 0.0D || d1 < d2 * d2)
                    && (d0 == -1.0D || d1 < d0)) {
                    d0 = d1;
                    entityplayer = entityplayer1;
                }
    ...
    public BlockPos getSpawnPoint() {
        ...
        if (!this.getWorldBorder()
            .contains(blockpos1)) {
            blockpos1 = this.getHeight(
                new BlockPos(
                    this.getWorldBorder()
                        .getCenterX(),
                    0.0D,
    ...
    public void joinEntityInSurroundings(Entity entityIn) {
        ...
        if (!this.loadedEntityList.contains(entityIn)) {
            if (!net.minecraftforge.common.MinecraftForge.EVENT_BUS
                .post(new net.minecraftforge.event.entity.EntityJoinWorldEvent(entityIn, this)))
                this.loadedEntityList.add(entityIn);
        }
    }
    ...
    public void addBlockEvent(BlockPos pos, Block blockIn, int eventID, int eventParam) {
        this.getBlockState(pos)
            .onBlockEventReceived(this, pos, eventID, eventParam);
    }
    ...
    public boolean isRainingAt(BlockPos position) {
        if (!this.isRaining()) {
            ...
            return false;
        } else if (this.getPrecipitationHeight(position)
            .getY() > position.getY()) {
            return false;
        } else {
    ...
    public Random setRandomSeed(int p_72843_1_, int p_72843_2_, int p_72843_3_) {
        long j2 = (long) p_72843_1_ * 341873128712L + (long) p_72843_2_ * 132897987541L
            + this.getWorldInfo()
            .getSeed()
            + (long) p_72843_3_;
    ...
    public void updateComparatorOutputLevel(BlockPos pos, Block blockIn) {
        for (EnumFacing enumfacing : EnumFacing.VALUES) {
            ...
            if (this.isBlockLoaded(blockpos1)) {
                ...
                iblockstate1.getBlock()
                    .onNeighborChange(this, blockpos1, pos);
                if (iblockstate1.getBlock()
                    .isNormalCube(iblockstate1, this, blockpos1)) {
                    blockpos1 = blockpos1.offset(enumfacing);
                    iblockstate1 = this.getBlockState(blockpos1);

                    if (iblockstate1.getBlock()
                        .getWeakChanges(this, blockpos1)) {
                        iblockstate1.getBlock()
                            .onNeighborChange(this, blockpos1, pos);
                    }
                }
    ...
    public DifficultyInstance getDifficultyForLocation(BlockPos pos) {
        ...
        if (this.isBlockLoaded(pos)) {
            ...
            j2 = this.getChunkFromBlockCoords(pos)
                .getInhabitedTime();
        }
    ...
    public void markTileEntitiesInChunkForRemoval(Chunk chunk) {
        for (TileEntity tileentity : chunk.getTileEntityMap()
            .values()) {
    ...
    protected void initCapabilities() {
        ...
        net.minecraftforge.common.util.WorldCapabilityData data = (net.minecraftforge.common.util.WorldCapabilityData) perWorldStorage
            .getOrLoadData(
                net.minecraftforge.common.util.WorldCapabilityData.class,
                net.minecraftforge.common.util.WorldCapabilityData.ID);
        if (data == null) {
    ...
    protected MapStorage perWorldStorage; // Moved to a getter to simulate final without being final so we can load in
    // subclasses.
