...
public class LootTable {
    ...
    public void fillInventory(IInventory inventory, Random rand, LootContext context) {
        ...
        for (ItemStack itemstack : list) {
            ...
            if (itemstack.isEmpty()) {
                inventory
                    .setInventorySlotContents(((Integer) list1.remove(list1.size() - 1)).intValue(), ItemStack.EMPTY);
            } else {
    ...
    public void freeze() {
        this.isFrozen = true;
        for (LootPool pool : this.pools) pool.freeze();
    }
    ...
    private void checkFrozen() {
        if (this.isFrozen()) throw new RuntimeException("Attempted to modify LootTable after being finalized!");
    }
    ...
    public static class Serializer implements JsonDeserializer<LootTable>, JsonSerializer<LootTable> {
        ...
        public LootTable deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_,
            ...
            LootPool[] alootpool = (LootPool[]) JsonUtils
                .deserializeClass(jsonobject, "pools", new LootPool[0], p_deserialize_3_, LootPool[].class);
            return new LootTable(alootpool);
        }

        public JsonElement serialize(LootTable p_serialize_1_, Type p_serialize_2_,
