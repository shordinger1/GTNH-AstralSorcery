...
public class MapGenScatteredFeature extends MapGenStructure {
    ...
    public MapGenScatteredFeature(Map<String, String> p_i2061_1_) {
        ...
        for (Entry<String, String> entry : p_i2061_1_.entrySet()) {
            if (((String) entry.getKey()).equals("distance")) {
                this.maxDistanceBetweenScatteredFeatures = MathHelper
                    .getInt(entry.getValue(), this.maxDistanceBetweenScatteredFeatures, 9);
            }
        }
    }
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        if (i == k && j == l) {
            Biome biome = this.world.getBiomeProvider()
                .getBiome(new BlockPos(i * 16 + 8, 0, j * 16 + 8));
