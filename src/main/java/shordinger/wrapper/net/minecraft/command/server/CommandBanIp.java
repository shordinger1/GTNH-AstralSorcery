...
import shordinger.wrapper.net.minecraft.server.management.UserListIPBansEntry;
...
public class CommandBanIp extends CommandBase {
        ...
        if (args.length >= 1 && args[0].length() > 1) {
            ...
            if (matcher.matches()) {
                this.banIp(
                    ...
                    sender,
                    args[0],
                    itextcomponent == null ? null : itextcomponent.getUnformattedText());
            } else {
                EntityPlayerMP entityplayermp = server.getPlayerList()
    ...
    protected void banIp(MinecraftServer server, ICommandSender sender, String ipAddress, @Nullable String banReason) {
        UserListIPBansEntry userlistipbansentry = new UserListIPBansEntry(
            ipAddress,
            (Date) null,
            sender.getName(),
            (Date) null,
        ...
        server.getPlayerList()
            .getBannedIPs()
            .addEntry(userlistipbansentry);
        List<EntityPlayerMP> list = server.getPlayerList()
            .getPlayersMatchingAddress(ipAddress);
        String[] astring = new String[list.size()];
        int i = 0;

        for (EntityPlayerMP entityplayermp : list) {
            entityplayermp.connection
                .disconnect(new TextComponentTranslation("multiplayer.disconnect.ip_banned", new Object[0]));
        ...
        if (list.isEmpty()) {
            ...
            notifyCommandListener(
                ...
                new Object[]{ipAddress, joinNiceString(astring)});
        }
    }
}
