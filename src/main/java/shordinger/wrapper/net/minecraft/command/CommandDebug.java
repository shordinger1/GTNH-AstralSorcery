...
public class CommandDebug extends CommandBase {
        ...
        if (args.length < 1) {
            ...
            if ("start".equals(args[0])) {
                ...
                notifyCommandListener(
                    ...
                    this,
                    "commands.debug.stop",
                    new Object[]{String.format("%.2f", (float) k / 1000.0F), l});
            }
        }
    }
    ...
    private String getProfilerResults(long timeSpan, int tickSpan, MinecraftServer server) {
        ...
        stringbuilder.append("\n\n");
        stringbuilder.append("Time span: ")
            .append(timeSpan)
            .append(" ms\n");
        stringbuilder.append("Tick span: ")
            .append(tickSpan)
            .append(" ticks\n");
        ...
        if (list != null && list.size() >= 3) {
            for (int i = 1; i < list.size(); ++i) {
                ...
                if (!"unspecified".equals(profiler$result.profilerName)) {
                    try {
                        this.appendProfilerResults(
                            depth + 1,
                            sectionName + "." + profiler$result.profilerName,
                            builder,
                            server);
                    } catch (Exception exception) {
                        builder.append("[[ EXCEPTION ")
                            .append((Object) exception)
                            .append(" ]]");
                    }
                }
            }
