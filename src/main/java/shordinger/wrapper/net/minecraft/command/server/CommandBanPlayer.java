...
import shordinger.wrapper.net.minecraft.server.management.UserListBansEntry;
        ...
        if (args.length >= 1 && args[0].length() > 0) {
            ...
            if (gameprofile == null) {
                ...
                server.getPlayerList()
                    .getBannedPlayers()
                    .addEntry(userlistbansentry);
                EntityPlayerMP entityplayermp = server.getPlayerList()
                    .getPlayerByUsername(args[0]);

                if (entityplayermp != null) {
                    entityplayermp.connection
                        .disconnect(new TextComponentTranslation("multiplayer.disconnect.banned", new Object[0]));
                }
