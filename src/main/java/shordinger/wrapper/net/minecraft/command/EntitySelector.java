...
public class EntitySelector {
    ...
    private static final Predicate<String> IS_VALID_ARGUMENT = new Predicate<String>() {
        ...
        public boolean apply(@Nullable String p_apply_1_) {
            return p_apply_1_ != null && (EntitySelector.VALID_ARGUMENTS.contains(p_apply_1_)
                || p_apply_1_.length() > "score_".length() && p_apply_1_.startsWith("score_"));
        }
    };
    /**
                    ...
                    if (world != null) {
                        ...
                        list2.addAll(
                            net.minecraftforge.fml.common.registry.GameRegistry
                                .createEntitySelectors(map, s, sender, vec3d));
        ...
        return (List<Predicate<Entity>>) (map.isEmpty() ? Collections.emptyList()
            ...
            public boolean apply(@Nullable Entity p_apply_1_) {
                if (p_apply_1_ == null) {
                    ...
                    Scoreboard scoreboard = sender.getServer()
                        .getWorld(0)
                        .getScoreboard();
                    ...
                    for (Entry<String, Integer> entry : map.entrySet()) {
                        ...
                        String s1 = p_apply_1_ instanceof EntityPlayerMP ? p_apply_1_.getName()
                            : p_apply_1_.getCachedUniqueIdString();
    ...
    private static List<Predicate<Entity>> getNamePredicates(Map<String, String> params) {
        ...
        if (s != null) {
            ...
            list.add(new Predicate<Entity>() {
                ...
                public boolean apply(@Nullable Entity p_apply_1_) {
                    return p_apply_1_ != null && p_apply_1_.getName()
                        .equals(s_f_) != flag;
                }
            });
        }
    ...
    private static <T extends Entity> List<T> filterResults(Map<String, String> params, Class<? extends T> entityClass,
        ...
        if (!params.containsKey(ARGUMENT_DELTA_X) && !params.containsKey(ARGUMENT_DELTA_Y)
            && !params.containsKey(ARGUMENT_DELTA_Z)) {
            if (l >= 0) {
    ...
    private static <T extends Entity> List<T> getEntitiesFromPredicates(List<T> matchingEntities,
        ...
        if (entity != null && targetClass.isAssignableFrom(entity.getClass())
            && i == 1
            && matchingEntities.contains(entity)
            && !"r".equals(type)) {
    ...
    private static double getCoordinate(Map<String, String> params, String key, double defaultD, boolean offset) {
        return params.containsKey(key)
            ? (double) MathHelper.getInt(params.get(key), MathHelper.floor(defaultD)) + (offset ? 0.5D : 0.0D)
            : defaultD;
    }
    ...
    public static boolean isSelectorDefault(String selectorStr) {
        return TOKEN_PATTERN.matcher(selectorStr)
            .matches();
    }
    ...
    private static Map<String, String> getArgumentMap(@Nullable String argumentString) throws CommandException {
        ...
        if (argumentString == null) {
            ...
            for (String s : COMMA_SPLITTER.split(argumentString)) {
                Iterator<String> iterator = EQUAL_SPLITTER.split(s)
                    .iterator();
