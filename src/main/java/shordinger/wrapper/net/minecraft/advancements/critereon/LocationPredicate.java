...
public class LocationPredicate {
    ...
    public static LocationPredicate ANY = new LocationPredicate(
    ...
    public LocationPredicate(MinMaxBounds x, MinMaxBounds y, MinMaxBounds z, @Nullable Biome biome,
                             @Nullable String feature, @Nullable DimensionType dimension) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.biome = biome;
    ...
    public boolean test(WorldServer world, float x, float y, float z) {
        if (!this.x.test(x)) {
            ...
            if (this.biome != null && this.biome != world.getBiome(blockpos)) {
                ...
                return this.feature == null || world.getChunkProvider()
                    .isInsideStructure(world, this.feature, blockpos);
            }
        }
    }
    ...
    public static LocationPredicate deserialize(@Nullable JsonElement element) {
        if (element != null && !element.isJsonNull()) {
            ...
            DimensionType dimensiontype = jsonobject.has("dimension")
                ? DimensionType.byName(JsonUtils.getString(jsonobject, "dimension"))
                : null;
