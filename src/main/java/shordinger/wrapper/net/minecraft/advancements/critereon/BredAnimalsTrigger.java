...
public class BredAnimalsTrigger implements ICriterionTrigger<BredAnimalsTrigger.Instance> {
    ...
    public void addListener(PlayerAdvancements playerAdvancementsIn,
                            ICriterionTrigger.Listener<BredAnimalsTrigger.Instance> listener) {
    ...
    public static class Instance extends AbstractCriterionInstance {
        ...
        public boolean test(EntityPlayerMP player, EntityAnimal parent1In, EntityAnimal parent2In,
                            EntityAgeable childIn) {
            if (!this.child.test(player, childIn)) {
                return false;
            } else {
                return this.parent.test(player, parent1In) && this.partner.test(player, parent2In)
                    || this.parent.test(player, parent2In) && this.partner.test(player, parent1In);
            }
        }
    }
    ...
    static class Listeners {
        ...
        public void trigger(EntityPlayerMP player, EntityAnimal parent1, EntityAnimal parent2, EntityAgeable child) {
            ...
            for (ICriterionTrigger.Listener<BredAnimalsTrigger.Instance> listener : this.listeners) {
                if (((BredAnimalsTrigger.Instance) listener.getCriterionInstance())
                    .test(player, parent1, parent2, child)) {
                    if (list == null) {
