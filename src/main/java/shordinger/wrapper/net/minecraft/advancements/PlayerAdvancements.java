...
public class PlayerAdvancements {
    ...
    private final Map<Advancement, AdvancementProgress> progress = Maps
        .<Advancement, AdvancementProgress>newLinkedHashMap();
    ...
    private void registerListeners() {
        for (Advancement advancement : this.server.getAdvancementManager()
            .getAdvancements()) {
    ...
    private void checkForAutomaticTriggers() {
        for (Advancement advancement : this.server.getAdvancementManager()
            .getAdvancements()) {
            if (advancement.getCriteria()
                .isEmpty()) {
                this.grantCriterion(advancement, "");
                advancement.getRewards()
                    .apply(this.player);
            }
        }
    }
    ...
    private void load() {
        if (this.progressFile.isFile()) {
            try {
                ...
                Map<ResourceLocation, AdvancementProgress> map = (Map) JsonUtils
                    .gsonDeserialize(GSON, s, MAP_TOKEN.getType());
                ...
                for (Entry<ResourceLocation, AdvancementProgress> entry : stream.collect(Collectors.toList())) {
                    Advancement advancement = this.server.getAdvancementManager()
                        .getAdvancement(entry.getKey());

                    if (advancement == null) {
                        LOGGER.warn(
                            "Ignored advancement '" + entry.getKey()
                                + "' in progress file "
                                + this.progressFile
                ...
                LOGGER.error(
                    "Couldn't parse player advancements in " + this.progressFile,
    ...
    public void save() {
        ...
        if (this.progressFile.getParentFile() != null) {
            this.progressFile.getParentFile()
                .mkdirs();
        }
    ...
    public boolean grantCriterion(Advancement p_192750_1_, String p_192750_2_) {
        ...
        if (advancementprogress.grantCriterion(p_192750_2_)) {
            ...
            if (!flag1 && advancementprogress.isDone()) {
                p_192750_1_.getRewards()
                    .apply(this.player);

                if (p_192750_1_.getDisplay() != null && p_192750_1_.getDisplay()
                    .shouldAnnounceToChat()
                    && this.player.world.getGameRules()
                    .getBoolean("announceAdvancements")) {
                    this.server.getPlayerList()
                        .sendMessage(
                            new TextComponentTranslation(
                                "chat.type.advancement." + p_192750_1_.getDisplay()
                                    .getFrame()
                                    .getName(),
                                new Object[]{this.player.getDisplayName(), p_192750_1_.getDisplayText()}));
                }
    ...
    private void registerListeners(Advancement p_193764_1_) {
        ...
        if (!advancementprogress.isDone()) {
            for (Entry<String, Criterion> entry : p_193764_1_.getCriteria()
                .entrySet()) {
                ...
                if (criterionprogress != null && !criterionprogress.isObtained()) {
                    ...
                    if (icriterioninstance != null) {
                        ICriterionTrigger<ICriterionInstance> icriteriontrigger = CriteriaTriggers
                            .<ICriterionInstance>get(icriterioninstance.getId());

                        if (icriteriontrigger != null) {
                            icriteriontrigger.addListener(
                                this,
                                new ICriterionTrigger.Listener(icriterioninstance, p_193764_1_, entry.getKey()));
                        }
                    }
                }
            }
    ...
    private void unregisterListeners(Advancement p_193765_1_) {
        ...
        for (Entry<String, Criterion> entry : p_193765_1_.getCriteria()
            .entrySet()) {
            CriterionProgress criterionprogress = advancementprogress.getCriterionProgress(entry.getKey());
            ...
            if (criterionprogress != null && (criterionprogress.isObtained() || advancementprogress.isDone())) {
                ...
                if (icriterioninstance != null) {
                    ...
                    if (icriteriontrigger != null) {
                        icriteriontrigger.removeListener(
                            this,
                            new ICriterionTrigger.Listener(icriterioninstance, p_193765_1_, entry.getKey()));
                    }
                }
            }
        }
    ...
    public void setSelectedTab(@Nullable Advancement p_194220_1_) {
        ...
        if (advancement != this.lastSelectedTab) {
            this.player.connection.sendPacket(
                new SPacketSelectAdvancementsTab(this.lastSelectedTab == null ? null : this.lastSelectedTab.getId()));
        }
    }
