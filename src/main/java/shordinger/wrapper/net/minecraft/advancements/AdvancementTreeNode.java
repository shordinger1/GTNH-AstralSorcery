...
public class AdvancementTreeNode {
    ...
    public AdvancementTreeNode(Advancement advancementIn, @Nullable AdvancementTreeNode parentIn,
                               @Nullable AdvancementTreeNode siblingIn, int indexIn, int xIn) {
    ...
    private AdvancementTreeNode apportion(AdvancementTreeNode nodeIn) {
        if (this.sibling == null) {
            ...
            for (f3 = advancementtreenode3.mod; advancementtreenode2.getLastChild() != null
                ...
                if (f4 > 0.0F) {
                    advancementtreenode2.getAncestor(this, nodeIn)
                        .moveSubtree(this, f4);
                    f += f4;
                    f1 += f4;
                }
    ...
    private AdvancementTreeNode getAncestor(AdvancementTreeNode p_192326_1_, AdvancementTreeNode p_192326_2_) {
        return this.ancestor != null && p_192326_1_.parent.children.contains(this.ancestor) ? this.ancestor
            : p_192326_2_;
    }

    private void updatePosition() {
        if (this.advancement.getDisplay() != null) {
            this.advancement.getDisplay()
                .setPosition((float) this.x, this.y);
        }
    ...
    public static void layout(Advancement root) {
        if (root.getDisplay() == null) {
            ...
            AdvancementTreeNode advancementtreenode = new AdvancementTreeNode(
                root,
