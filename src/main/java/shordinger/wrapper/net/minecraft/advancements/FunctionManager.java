...
public class FunctionManager implements ITickable {
    ...
    public int getMaxCommandChainLength() {
        return this.server.worlds[0].getGameRules()
            .getInt("maxCommandChainLength");
    }
     ...
     */
    public void update() {
        String s = this.server.worlds[0].getGameRules()
            .getString("gameLoopFunction");
        ...
        if (this.isExecuting) {
            if (this.commandQueue.size() < i) {
                this.commandQueue.addFirst(
                    new FunctionManager.QueuedCommand(this, sender, new FunctionObject.FunctionEntry(function)));
            }
            ...
            for (File file1 : FileUtils.listFiles(this.functionDir, new String[]{"mcfunction"}, true)) {
                ...
                if (astring.length == 2) {
                    ...
                    try {
                        this.functions.put(
                            resourcelocation,
                            FunctionObject.create(this, Files.readLines(file1, StandardCharsets.UTF_8)));
