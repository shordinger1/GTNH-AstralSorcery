...
import shordinger.wrapper.net.minecraftforge.registries.IForgeRegistry.AddCallback;
import shordinger.wrapper.net.minecraftforge.registries.IForgeRegistry.ClearCallback;
import shordinger.wrapper.net.minecraftforge.registries.IForgeRegistry.CreateCallback;
import shordinger.wrapper.net.minecraftforge.registries.IForgeRegistry.DummyFactory;
import shordinger.wrapper.net.minecraftforge.registries.IForgeRegistry.MissingFactory;
import shordinger.wrapper.net.minecraftforge.registries.IForgeRegistry.ValidateCallback;
...
public class ForgeRegistry<V extends IForgeRegistryEntry<V>>
    ...
    ForgeRegistry(Class<V> superType, ResourceLocation defaultKey, int min, int max, @Nullable CreateCallback<V> create,
        ...
        this.isDelegated = IForgeRegistryEntry.Impl.class.isAssignableFrom(superType); // TODO: Make this
        // IDelegatedRegistryEntry?
    ...
    public void registerAll(@SuppressWarnings("unchecked") V... values) {
        for (V value : values) register(value);
    }

    @Override
    public boolean containsKey(ResourceLocation key) {
        while (key != null) {
            if (this.names.containsKey(key)) return true;
    ...
    public ResourceLocation getKey(V value) {
        ResourceLocation ret = this.names.inverse()
            .get(value);
    ...
    public int getID(V value) {
        Integer ret = this.ids.inverse()
            .get(value);
        if (ret == null && this.defaultValue != null) ret = this.ids.inverse()
    ...
    void validateKey() {
        if (this.defaultKey != null) Validate.notNull(
            this.defaultValue,
    ...
    int add(int id, V value) {
        ModContainer mc = Loader.instance()
            .activeModContainer();
        String owner = mc == null || (mc instanceof InjectedModContainer
            && ((InjectedModContainer) mc).wrappedContainer instanceof FMLContainer) ? null
            : mc.getModId()
            .toLowerCase();
    ...
    int add(int id, V value, String owner) {
        ...
        int idToUse = id;
        if (idToUse < 0 || availabilityMap.get(idToUse)) idToUse = availabilityMap.nextClearBit(min);
        ...
        {
            FMLLog.bigWarning(
                "Registry {}: The object {} has been registered twice for the same name {}.",
                this.superType.getSimpleName(),
                value,
                key);
        ...
        {
            if (!this.allowOverrides) throw new IllegalArgumentException(
                String.format("The name %s has been registered twice, for %s and %s.", key, getRaw(key), value));
            ...
            if (DEBUG) FMLLog.log
                .debug("Registry {} Override: {} {} -> {}", this.superType.getSimpleName(), key, oldEntry, value);
            idToUse = this.getID(oldEntry);
        }

        Integer foundId = this.ids.inverse()
            .get(value); // Is this ever possible to trigger with otherThing being different?
        if (foundId != null) {
        ...
        if (defaultKey != null && defaultKey.equals(key)) {
            if (this.defaultValue != null) throw new IllegalStateException(
                String.format(
        ...
        if (this.add != null) this.add.onAdd(this, this.stage, idToUse, value, oldEntry);

        if (this.dummies.remove(key) && DEBUG)
            FMLLog.log.debug("Registry {} Dummy Remove: {}", this.superType.getSimpleName(), key);

        if (DEBUG) FMLLog.log
            .trace("Registry {} add: {} {} {} (req. id {})", this.superType.getSimpleName(), key, idToUse, value, id);
    ...
    void addAlias(ResourceLocation from, ResourceLocation to) {
        if (this.isLocked()) throw new IllegalStateException(
            String.format("Attempted to register the alias %s -> %s to late", from, to));
        this.aliases.put(from, to);
        if (DEBUG) FMLLog.log.trace("Registry {} alias: {} -> {}", this.superType.getSimpleName(), from, to);
    }
    ...
    void addDummy(ResourceLocation key) {
        ...
        if (DEBUG) FMLLog.log.trace("Registry {} dummy: {}", this.superType.getSimpleName(), key);
    }

    private RegistryDelegate<V> getDelegate(V thing) {
        if (isDelegated) return (RegistryDelegate<V>) ((IForgeRegistryEntry.Impl<V>) thing).delegate;
        else throw new IllegalStateException("Tried to get existing delegate from registry that is not delegated.");
    }

    void resetDelegates() {
        if (!this.isDelegated) return;

        for (V value : this) getDelegate(value).changeReference(value);

        for (V value : this.overrides.values()) getDelegate(value).changeReference(value);
    }
    ...
    void validateContent(ResourceLocation registryName) {
        ...
        for (V obj : this) {
            ...
            if (name == null) throw new IllegalStateException(
                String.format("Registry entry for %s %s, id %d, doesn't yield a name.", registryName, obj, id));

            // id is too high
            if (id > max) throw new IllegalStateException(
                String.format(
                    "Registry entry for %s %s, name %s uses the too large id %d.",
                    registryName,
                    obj,
                    name,
                    id));
            ...
            if (getValue(id) != obj) throw new IllegalStateException(
                String.format(
                    "Registry entry for id %d, name %s, doesn't yield the expected %s %s.",
                    id,
                    name,
                    registryName,
                    obj));
            ...
            if (getValue(name) != obj) throw new IllegalStateException(
                String.format(
                    "Registry entry for name %s, id %d, doesn't yield the expected %s %s.",
                    name,
                    id,
                    registryName,
                    obj));
            ...
            if (getID(name) != id) throw new IllegalStateException(
                String.format("Registry entry for name %s doesn't yield the expected id %d.", name, id));

            /*
             * // entry is blocked, thus should be empty
             * if (blockedIds.contains(id))
             * throw new
             * IllegalStateException(String.format("Registry entry for %s %s, id %d, name %s, marked as dangling.",
             * registryName, obj, id, name));
             */

            // registry-specific validation
            if (this.validate != null) this.validate.onValidate(this, this.stage, id, name, obj);
        }
    }

    void sync(ResourceLocation name, ForgeRegistry<V> from) {
        if (DEBUG) FMLLog.log.debug(
            "Registry {} Sync: {} -> {}",
            this.superType.getSimpleName(),
            this.stage.getName(),
            from.stage.getName());
        if (this == from) throw new IllegalArgumentException("WTF We are the same!?!?!");
        if (from.superType != this.superType) throw new IllegalArgumentException(
            "Attempted to copy to incompatible registry: " + name + " " + from.superType + " -> " + this.superType);

        this.isFrozen = false;

        if (this.clear != null) this.clear.onClear(this, stage);
        ...
        for (Entry<ResourceLocation, V> entry : from.names.entrySet()) {
            ...
            if (overrides.isEmpty()) {
                ...
                for (V value : overrides) {
                    OverrideOwner owner = from.owners.inverse()
                        .get(value);
                    if (owner == null) {
                        FMLLog.log.warn(
                            "Registry {}: Override did not have an associated owner object. Name: {} Value: {}",
                            this.superType.getSimpleName(),
                            entry.getKey(),
                            value);
                        errored = true;
        ...
        if (errored) throw new RuntimeException(
    ...
    public void clear() {
        ...
        if (this.isLocked()) throw new IllegalStateException("Attempted to clear the registry to late.");
    ...
    public V remove(ResourceLocation key) {
        ...
        if (this.isLocked()) throw new IllegalStateException("Attempted to remove from the registry to late.");

        V value = this.names.remove(key);
        if (value != null) {
            Integer id = this.ids.inverse()
                .remove(value);
            if (id == null) throw new IllegalStateException(
                "Removed a entry that did not have an associated id: " + key
                    + " "
                    + value.toString()
                    + " This should never happen unless hackery!");

            if (DEBUG) FMLLog.log.trace("Registry {} remove: {} {}", this.superType.getSimpleName(), key, id);
        }
    ...
    public void loadIds(Map<ResourceLocation, Integer> ids, Map<ResourceLocation, String> overrides,
                        Map<ResourceLocation, Integer> missing, Map<ResourceLocation, Integer[]> remapped, ForgeRegistry<V> old,
        ...
        for (Map.Entry<ResourceLocation, Integer> entry : ids.entrySet()) {
            ...
            if (currId == -1) {
                FMLLog.log.info(
                    "Registry {}: Found a missing id from the world {}",
                    this.superType.getSimpleName(),
                    itemName);
                ...
                FMLLog.log.debug(
                    "Registry {}: Fixed {} id mismatch {}: {} (init) -> {} (map).",
                    this.superType.getSimpleName(),
                    name,
                    itemName,
                    currId,
            ...
            Preconditions
                .checkState(obj != null, "objectKey has an ID but no object. Reflection/ASM hackery? Registry bug?");
            ...
            if (old.overrides.containsKey(itemName)) {
                if (!overrides.containsKey(itemName)) {
                    lst.add(obj);
                    obj = old.overrides.get(itemName)
                        .iterator()
                        .next(); // Get the first one in the list, Which should be the first one registered
                    primaryName = old.owners.inverse()
                        .get(obj).owner;
                } else primaryName = overrides.get(itemName);
            }

            for (V value : lst) {
                OverrideOwner owner = old.owners.inverse()
                    .get(value);
                if (owner == null) {
                    FMLLog.log.warn(
                        "Registry {}: Override did not have an associated owner object. Name: {} Value: {}",
                ...
                if (primaryName.equals(owner.owner)) continue;

                int realId = add(newId, value, owner.owner);
                if (newId != realId) FMLLog.log.warn(
                    "Registry {}: Object did not get ID it asked for. Name: {} Expected: {} Got: {}",
                    this.superType.getSimpleName(),
                    entry.getKey(),
                    newId,
            ...
            if (realId != newId) FMLLog.log.warn(
                "Registry {}: Object did not get ID it asked for. Name: {} Expected: {} Got: {}",
                this.superType.getSimpleName(),
                entry.getKey(),
                newId,
        ...
        for (Map.Entry<ResourceLocation, String> entry : ovs.entrySet()) {
            ...
            String current = this.owners.inverse()
                .get(this.getRaw(itemName)).owner;
            if (!owner.equals(current)) {
                V _new = this.owners.get(new OverrideOwner(owner, itemName));
                if (_new == null) {
                    FMLLog.log.warn(
                        "Registry {}: Skipping override for {}, Unknown owner {}",
                        this.superType.getSimpleName(),
                        itemName,
                        owner);
                    continue;
                ...
                FMLLog.log.info(
                    "Registry {}: Activating override {} for {}",
                    this.superType.getSimpleName(),
                    owner,
                    itemName);
    ...
    boolean markDummy(ResourceLocation key, int id) {
        if (this.dummyFactory == null) return false;

        V dummy = this.dummyFactory.createDummy(key);
        if (DEBUG) FMLLog.log.debug("Registry Dummy Add: {} {} -> {}", key, id, dummy);
        ...
        if (this.containsKey(key)) {
            ...
            if (value == null) throw new IllegalStateException(
                "ContainsKey for " + key
                    + " was true, but removing by name returned no value.. This should never happen unless hackery!");

            Integer oldid = this.ids.inverse()
                .remove(value);
            ...
            if (oldid != id) FMLLog.log
                .debug("Registry {}: Dummy ID mismatch {} {} -> {}", this.superType.getSimpleName(), key, oldid, id);

            if (DEBUG) FMLLog.log.debug("Registry {} remove: {} {}", this.superType.getSimpleName(), key, oldid);
        }

        int realId = this.add(id, dummy);
        if (realId != id) FMLLog.log.warn(
            "Registry {}: Object did not get ID it asked for. Name: {} Expected: {} Got: {}",
            this.superType.getSimpleName(),
            key,
            id,
    ...
    Map<ResourceLocation, String> getOverrideOwners() {
        ...
        for (ResourceLocation key : this.overrides.keySet()) {
            ...
            OverrideOwner owner = this.owners.inverse()
                .get(obj);
            if (owner == null && DEBUG) FMLLog.log.debug(
                "Registry {} {}: Invalid override {} {}",
                this.superType.getSimpleName(),
                this.stage.getName(),
                key,
                obj);
    ...
    public static class Snapshot {
        ...
        public NBTTagCompound write() {
            ...
            int[] blocked = this.blocked.stream()
                .mapToInt(x -> x)
                .sorted()
                .toArray();
            ...
            this.dummied.stream()
                .sorted()
                .forEach(e -> dummied.appendTag(new NBTTagString(e.toString())));
        ...
        public static Snapshot read(NBTTagCompound nbt) {
            ...
            list.forEach(e -> {
                ...
                if (v.indexOf(':') == -1) // Forge Bug: https://github.com/MinecraftForge/MinecraftForge/issues/4894
                // TODO: Remove in 1.13
                {
                    ...
                    if (aliasv.equals(aliask)) {
                        FMLLog.log.warn(
                            "Found unrecoverable 4894 bugged alias/override: {} -> {}, skipping.",
                            aliask,
                            aliasv);
                    } else {
            ...
            list = nbt.getTagList("dummied", 10); // 10 - NBTTagCompound, Old format. New format is String list. For now
            // we will just merge the old and new. TODO: Remove in 1.13
    ...
    void processMissingEvent(ResourceLocation name, ForgeRegistry<V> pool, List<MissingMappings.Mapping<V>> mappings,
                             Map<ResourceLocation, Integer> missing, Map<ResourceLocation, Integer[]> remaps,
                             Collection<ResourceLocation> defaulted, Collection<ResourceLocation> failed, boolean injectNetworkDummies) {
        ...
        for (MissingMappings.Mapping<V> remap : mappings) {
            ...
            if (action == MissingMappings.Action.REMAP) {
                ...
                // I don't think this will work, but I dont think it ever worked.. the item is already in the map with a
                // different id... we want to fix that..
                int realId = this.add(remap.id, remap.getTarget());
                if (realId != remap.id) FMLLog.log.warn(
                ...
                if (action == MissingMappings.Action.DEFAULT) {
                    ...
                    if (m == null) defaulted.add(remap.key);
                    else this.add(remap.id, m, remap.key.getResourceDomain());
    ...
    private static class OverrideOwner {
        ...
        public boolean equals(Object o) {
            ...
            if (!(o instanceof OverrideOwner)) return false;
