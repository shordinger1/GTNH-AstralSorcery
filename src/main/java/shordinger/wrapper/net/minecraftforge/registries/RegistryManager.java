...
import shordinger.wrapper.net.minecraftforge.registries.ForgeRegistry.Snapshot;
...
public class RegistryManager {
    ...
    public <V extends IForgeRegistryEntry<V>> ForgeRegistry<V> getRegistry(ResourceLocation key,
        ...
        if (!this.registries.containsKey(key)) {
            ...
            if (other.persisted.contains(key)) this.persisted.add(key);
        }
        return getRegistry(key);
    }

    <V extends IForgeRegistryEntry<V>> ForgeRegistry<V> createRegistry(ResourceLocation name, Class<V> type,
                                                                       ResourceLocation defaultKey, int min, int max, @Nullable AddCallback<V> add, @Nullable ClearCallback<V> clear,
                                                                       @Nullable CreateCallback<V> create, @Nullable ValidateCallback<V> validate, boolean persisted,
                                                                       boolean allowOverrides, boolean isModifiable, @Nullable DummyFactory<V> dummyFactory,
        ...
        ForgeRegistry<V> reg = new ForgeRegistry<V>(
            type,
            defaultKey,
            min,
            max,
        ...
        if (persisted) this.persisted.add(name);
