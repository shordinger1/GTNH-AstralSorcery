...
import shordinger.wrapper.net.minecraftforge.fml.common.Mod;
...
public enum ObjectHolderRegistry {
    ...
    public void findObjectHolders(ASMDataTable table) {
        ...
        for (ASMData data : table.getAll(Mod.class.getName())) {
            String modid = (String) data.getAnnotationInfo()
                .get("modid");
        ...
        for (ASMData data : allObjectHolders) {
            ...
            String value = (String) data.getAnnotationInfo()
                .get("value");
    ...
    private void scanTarget(Map<String, String> classModIds, Map<String, Class<?>> classCache, String className,
                            @Nullable String annotationTarget, String value, boolean isClass, boolean extractFromValue) {
        Class<?> clazz;
        ...
        if (isClass) {
            ...
            if (value.indexOf(':') == -1) {
                ...
                if (prefix == null) {
                    FMLLog.log.warn(
                        "Found an unqualified ObjectHolder annotation ({}) without a modid context at {}.{}, ignoring",
                        value,
                        className,
                        annotationTarget);
    ...
    private void scanClassForFields(Map<String, String> classModIds, String className, String value, Class<?> clazz,
        ...
        for (Field f : clazz.getFields()) {
            ...
            addHolderReference(
                new ObjectHolderRef(f, new ResourceLocation(value, f.getName()), extractFromExistingValues));
        }
    }
