...
// import shordinger.wrapper.net.minecraft.block.BlockAir;
// import shordinger.wrapper.net.minecraft.block.BlockObserver;
...
// import shordinger.wrapper.net.minecraft.util.registry.RegistryNamespacedDefaultedByKey;
...
// import shordinger.wrapper.net.minecraftforge.event.RegistryEvent.MissingMappings;
// import shordinger.wrapper.net.minecraftforge.fml.common.EnhancedRuntimeException;
...
// import shordinger.wrapper.net.minecraftforge.fml.common.InjectedModContainer;
...
// import shordinger.wrapper.net.minecraftforge.fml.common.ZipperUtil;
// import shordinger.wrapper.net.minecraftforge.fml.common.registry.EntityEntry;
// import shordinger.wrapper.net.minecraftforge.fml.common.registry.EntityEntryBuilder;
...
// private static final int MAX_ENCHANTMENT_ID = Short.MAX_VALUE - 1; // Short - serialized as a short in ItemStack
// // NBTs.
...
// private static final boolean DISABLE_VANILLA_REGISTRIES = Boolean
// .parseBoolean(System.getProperty("forge.disableVanillaGameData", "false")); // Use for unit tests/debugging
// private static final BiConsumer<ResourceLocation, ForgeRegistry<?>> LOCK_VANILLA = (name, reg) -> reg.slaves
// .values()
// .stream()
// .filter(o -> o instanceof ILockableRegistry)
// .forEach(o -> ((ILockableRegistry) o).lock());
//
// static {
// init();
// }
...
// FMLLog.bigWarning(
// "DISABLING VANILLA REGISTRY CREATION AS PER SYSTEM VARIABLE SETTING! forge.disableVanillaGameData");
// return;
// }
// if (hasInit) return;
// hasInit = true;
// makeRegistry(BLOCKS, Block.class, MAX_BLOCK_ID, new ResourceLocation("air"))
// .addCallback(BlockCallbacks.INSTANCE)
// .create();
// makeRegistry(ITEMS, Item.class, MIN_ITEM_ID, MAX_ITEM_ID).addCallback(ItemCallbacks.INSTANCE)
// .create();
...
// makeRegistry(RECIPES, IRecipe.class, MAX_RECIPE_ID).disableSaving()
// .allowModification()
// .addCallback(RecipeCallbacks.INSTANCE)
// .create();
// makeRegistry(PROFESSIONS, VillagerProfession.class, MAX_PROFESSION_ID).create();
// makeRegistry(SERIALIZERS, DataSerializerEntry.class, MIN_SERIALIZER_ID, MAX_SERIALIZER_ID).disableSaving()
// .disableOverrides()
// .addCallback(SerializerCallbacks.INSTANCE)
// .create();
// entityRegistry = (ForgeRegistry<EntityEntry>) makeRegistry(ENTITIES, EntityEntry.class, MAX_ENTITY_ID)
// .addCallback(EntityCallbacks.INSTANCE)
// .create();
// }
//
// private static <T extends IForgeRegistryEntry<T>> RegistryBuilder<T> makeRegistry(ResourceLocation name,
// Class<T> type, int min, int max) {
...
//
// private static <T extends IForgeRegistryEntry<T>> RegistryBuilder<T> makeRegistry(ResourceLocation name,
// Class<T> type, int max, ResourceLocation _default) {
// return new RegistryBuilder<T>().setName(name)
// .setType(type)
// .setMaxID(max)
// .addCallback(new NamespacedDefaultedWrapper.Factory<T>())
// .setDefaultKey(_default);
// }
//
// public static <V extends IForgeRegistryEntry<V>> RegistryNamespacedDefaultedByKey<ResourceLocation, V>
/// getWrapperDefaulted(
// Class<V> cls) {
...
// Validate.notNull(reg, "Attempted to get vanilla wrapper for unknown registry: " + cls.toString());
// @SuppressWarnings("unchecked")
// RegistryNamespaced<ResourceLocation, V> ret = reg
// .getSlaveMap(NamespacedWrapper.Factory.ID, NamespacedWrapper.class);
// Validate.notNull(ret, "Attempted to get vanilla wrapper for registry created incorrectly: " + cls.toString());
// return ret;
// }
//
...
// return GameRegistry.findRegistry(Item.class)
// .getSlaveMap(BLOCK_TO_ITEM, BiMap.class);
// }
//
// @SuppressWarnings("unchecked")
// public static ObjectIntIdentityMap<IBlockState> getBlockStateIDMap() {
// return GameRegistry.findRegistry(Block.class)
// .getSlaveMap(BLOCKSTATE_TO_ID, ObjectIntIdentityMap.class);
// }
//
// @SuppressWarnings("unchecked")
// public static Map<Class<? extends Entity>, EntityEntry> getEntityClassMap() {
// return GameRegistry.findRegistry(EntityEntry.class)
// .getSlaveMap(ENTITY_CLASS_TO_ENTRY, Map.class);
// }
//
// @SuppressWarnings("unchecked")
// public static Map<DataSerializer<?>, DataSerializerEntry> getSerializerMap() {
// return GameRegistry.findRegistry(DataSerializerEntry.class)
// .getSlaveMap(SERIALIZER_TO_ENTRY, Map.class);
// }
//
...
// String.format(
// "Attempt to register object without having set a registry name %s (type %s)",
// value,
// value.getClass()
// .getName()));
...
// Validate.notNull(
// registry,
// "Attempted to registry object without creating registry first: " + value.getRegistryType()
// .getName());
...
// for (Map.Entry<ResourceLocation, ForgeRegistry<? extends IForgeRegistryEntry<?>>> r :
/// RegistryManager.ACTIVE.registries
// .entrySet()) {
...
//
// // the id mapping is fanilized, no ids actually changed but this is a good place to tell everyone to 'bake'
// // their stuff.
// Loader.instance()
...
// }
//
// private static class BlockCallbacks implements IForgeRegistry.AddCallback<Block>,
// IForgeRegistry.ClearCallback<Block>, IForgeRegistry.CreateCallback<Block>, IForgeRegistry.DummyFactory<Block> {
//
...
// @Nullable Block oldBlock) {
// @SuppressWarnings("unchecked")
// ClearableObjectIntIdentityMap<IBlockState> blockstateMap = owner
// .getSlaveMap(BLOCKSTATE_TO_ID, ClearableObjectIntIdentityMap.class);
//
// if (oldBlock != null) {
// for (IBlockState state : oldBlock.getBlockState()
// .getValidStates()) {
// blockstateMap.remove(state);
// }
// }
//
// if ("minecraft:tripwire".equals(
// block.getRegistryName()
// .toString())) // Tripwire is crap so we have to special case whee!
// {
// for (int meta = 0; meta < 15; meta++) blockstateMap.put(block.getStateFromMeta(meta), id << 4 | meta);
// }
//
// // So, due to blocks having more in-world states then metadata allows, we have to turn the map into a
// // semi-milti-bimap.
// // We can do this however because the implementation of the map is last set wins. So we can add all states,
// // then fix the meta bimap.
...
// if (block.getClass() == BlockObserver.class) continue; // Observers are bad and have non-cyclical
// // states. So we HAVE to use the vanilla logic
// // above.
// if (usedMeta[meta]) blockstateMap.put(block.getStateFromMeta(meta), id << 4 | meta); // Put the CORRECT
// // thing!
// }
//
// if (oldBlock != null) {
...
// if (item != null) blockToItem.forcePut(block, item);
// }
// }
//
// @Override
// public void onClear(IForgeRegistryInternal<Block> owner, RegistryManager stage) {
// owner.getSlaveMap(BLOCKSTATE_TO_ID, ClearableObjectIntIdentityMap.class)
// .clear();
// }
//
// @Override
...
// if (integer == null && key != null) integer = this.identityMap.get(
// key.getBlock()
// .getStateFromMeta(
// key.getBlock()
// .getMetaFromState(key)));
...
// }
//
// private static class BlockDummyAir extends BlockAir // A named class so DummyBlockReplacementTest can detect if
// // its a dummy
// {
//
...
// }
//
// private static class ItemCallbacks implements IForgeRegistry.AddCallback<Item>, IForgeRegistry.ClearCallback<Item>,
...
// owner.getSlaveMap(BLOCK_TO_ITEM, BiMap.class)
// .clear();
// }
//
// @Override
...
// BiMap<?, ?> map = stage.getRegistry(BLOCKS)
// .getSlaveMap(BLOCK_TO_ITEM, BiMap.class);
...
// implements IForgeRegistry.ValidateCallback<IRecipe>, IForgeRegistry.MissingFactory<IRecipe> {
//
// static final RecipeCallbacks INSTANCE = new RecipeCallbacks();
//
// @Override
// public void onValidate(IForgeRegistryInternal<IRecipe> owner, RegistryManager stage, int id,
// ResourceLocation key, IRecipe obj) {
// if (stage != RegistryManager.ACTIVE) return;
// // verify the recipe output yields a registered item
// Item item = obj.getRecipeOutput()
// .getItem();
// if (!stage.getRegistry(Item.class)
// .containsValue(item)) {
// throw new IllegalStateException(
// String.format(
// "Recipe %s (%s) produces unregistered item %s (%s)",
// key,
// obj,
...
// IForgeRegistry.ClearCallback<EntityEntry>, IForgeRegistry.CreateCallback<EntityEntry> {
//
...
// owner.getSlaveMap(ENTITY_CLASS_TO_ENTRY, Map.class)
// .clear();
// }
//
// @Override
...
// }
//
// private static class SerializerCallbacks implements IForgeRegistry.AddCallback<DataSerializerEntry>,
// IForgeRegistry.ClearCallback<DataSerializerEntry>, IForgeRegistry.CreateCallback<DataSerializerEntry> {
//
// static final SerializerCallbacks INSTANCE = new SerializerCallbacks();
//
// @Override
// public void onAdd(IForgeRegistryInternal<DataSerializerEntry> owner, RegistryManager stage, int id,
// DataSerializerEntry entry, @Nullable DataSerializerEntry oldEntry) {
// @SuppressWarnings("unchecked")
...
// owner.getSlaveMap(SERIALIZER_TO_ENTRY, Map.class)
// .clear();
// }
//
// @Override
...
// }
//
// private static <T extends IForgeRegistryEntry<T>> void loadRegistry(final ResourceLocation registryName,
// final RegistryManager from, final RegistryManager to, final Class<T> regType, boolean freeze) {
...
// if (freeze) toRegistry.isFrozen = true;
// }
// }
//
// @SuppressWarnings({"unchecked", "rawtypes"})
// public static Multimap<ResourceLocation, ResourceLocation> injectSnapshot(
// Map<ResourceLocation, ForgeRegistry.Snapshot> snapshot, boolean injectFrozenData, boolean isLocalWorld) {
// FMLLog.log.info(
// "Injecting existing registry data into this {} instance",
...
//
// if (isLocalWorld) {
// List<ResourceLocation> missingRegs = snapshot.keySet()
// .stream()
// .filter(name -> !RegistryManager.ACTIVE.registries.containsKey(name))
// .collect(Collectors.toList());
// if (missingRegs.size() > 0) {
// String text = "Forge Mod Loader detected missing/unknown registrie(s).\n\n" + "There are "
// + missingRegs.size()
// + " missing registries in this save.\n"
...
//
// if (!StartupQuery.confirm(text)) StartupQuery.abort();
// }
// }
//
...
// if (reg.markDummy(dummy, m.get(dummy))) m.remove(dummy);
// } else if (isLocalWorld) {
// if (ForgeRegistry.DEBUG) FMLLog.log.debug("Registry {}: Resuscitating dummy entry {}", key, dummy);
// } else {
// // The server believes this is a dummy block identity, but we seem to have one locally. This is
// // likely a conflict
...
//
// List<MissingMappings.Mapping<?>> lst = event.getAllMappings()
// .stream()
// .filter(e -> e.getAction() == MissingMappings.Action.DEFAULT)
// .sorted(
// (a, b) -> a.toString()
...
// .stream()
// .filter(e -> e.getAction() == MissingMappings.Action.FAIL)
...
// });
//
// if (!defaulted.isEmpty() && !isLocalWorld) return defaulted;
//
// if (!defaulted.isEmpty()) {
// StringBuilder buf = new StringBuilder();
// buf.append("Forge Mod Loader detected missing registry entries.\n\n")
// .append("There are ")
// .append(defaulted.size())
// .append(" missing entries in this save.\n")
...
//
// if (!defaulted.isEmpty()) {
// if (isLocalWorld) FMLLog.log
// .error("There are unidentified mappings in this world - we are going to attempt to process anyway");
// }
//
// }
//
// if (injectFrozenData) {
// // If we're loading from disk, we can actually substitute air in the block map for anything that is
// // otherwise "missing". This keeps the reference in the map, in case
...
// });
//
// // If we're loading up the world from disk, we want to add in the new data that might have been provisioned
// // by mods
...
// private static <T extends IForgeRegistryEntry<T>> void loadPersistentDataToStagingRegistry(RegistryManager pool,
// RegistryManager to, Map<ResourceLocation, Integer[]> remaps, Map<ResourceLocation, Integer> missing,
// ResourceLocation name, ForgeRegistry.Snapshot snap, Class<T> regType) {
// ForgeRegistry<T> active = pool.getRegistry(name);
// if (active == null) return; // We've already asked the user if they wish to continue. So if the reg isnt found
// // just assume the user knows and accepted it.
...
// @SuppressWarnings("unchecked")
// private static <T extends IForgeRegistryEntry<T>> void processMissing(Class<T> clazz, ResourceLocation name,
// RegistryManager STAGING, MissingMappings<?> e, Map<ResourceLocation, Integer> missing,
// Map<ResourceLocation, Integer[]> remaps, Collection<ResourceLocation> defaulted,
// Collection<ResourceLocation> failed, boolean injectNetworkDummies) {
...
// }
//
// private static <T extends IForgeRegistryEntry<T>> void loadFrozenDataToStagingRegistry(RegistryManager STAGING,
// ResourceLocation name, Map<ResourceLocation, Integer[]> remaps, Class<T> clazz) {
...
// frozen.getKeys()
// .stream()
// .filter(key -> !newRegistry.containsKey(key))
// .forEach(key -> _new.put(key, frozen.getID(key)));
...
// RegistryManager.ACTIVE.getRegistry(BLOCKS)
// .getRegisterEvent(BLOCKS));
// ObjectHolderRegistry.INSTANCE.applyObjectHolders(); // inject any blocks
// }
// if (filter.test(ITEMS)) {
// MinecraftForge.EVENT_BUS.post(
// RegistryManager.ACTIVE.getRegistry(ITEMS)
// .getRegisterEvent(ITEMS));
...
// MinecraftForge.EVENT_BUS.post(
// RegistryManager.ACTIVE.getRegistry(rl)
// .getRegisterEvent(rl));
// }
...
// FMLLog.log.warn(
// "Potentially Dangerous alternative prefix `{}` for name `{}`, expected `{}`. This could be a intended override, but
