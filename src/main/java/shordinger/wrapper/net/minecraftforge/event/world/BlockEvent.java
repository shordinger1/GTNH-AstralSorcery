...
public class BlockEvent extends Event {
    ...
    /**
     * Fired when a block is about to drop it's harvested items. The {@link #drops} array can be amended, as can the
     * {@link #dropChance}.
     * <strong>Note well:</strong> the {@link #harvester} player field is null in a variety of scenarios. Code expecting
     * null.
     * <p>
     * The {@link #dropChance} is used to determine which items in this array will actually drop, compared to a random
     * number. If you wish, you
     * can pre-filter yourself, and set {@link #dropChance} to 1.0f to always drop the contents of the {@link #drops}
     * array.
     * <p>
     * {@link #isSilkTouching} is set if this is considered a silk touch harvesting operation, vs a normal harvesting
     * operation. Act accordingly.
     *
     * @author cpw
     */
    public static class HarvestDropsEvent extends BlockEvent {
        ...
        private float dropChance; // Change to e.g. 1.0f, if you manipulate the list and want to guarantee it always
        // drops
        ...
        public HarvestDropsEvent(World world, BlockPos pos, IBlockState state, int fortuneLevel, float dropChance,
                                 List<ItemStack> drops, EntityPlayer harvester, boolean isSilkTouching) {
    ...
    public static class BreakEvent extends BlockEvent {
        ...
        public BreakEvent(World world, BlockPos pos, IBlockState state, EntityPlayer player) {
            ...
            if (state == null || !ForgeHooks.canHarvestBlock(state.getBlock(), player, world, pos) || // Handle empty
                // block or player
                // unable to break
                // block scenario
                (state.getBlock()
                    .canSilkHarvest(world, pos, world.getBlockState(pos), player)
                    && EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, player.getHeldItemMainhand())
                    > 0)) // If the block is being silk harvested, the exp dropped is 0
            {
                this.exp = 0;
            } else {
                int bonusLevel = EnchantmentHelper
                    .getEnchantmentLevel(Enchantments.FORTUNE, player.getHeldItemMainhand());
                this.exp = state.getBlock()
                    .getExpDrop(state, world, pos, bonusLevel);
            }
        }
    ...
    public static class EntityPlaceEvent extends BlockEvent {
        ...
        public EntityPlaceEvent(@Nonnull BlockSnapshot blockSnapshot, @Nonnull IBlockState placedAgainst,
                                @Nullable Entity entity) {
            super(
                blockSnapshot.getWorld(),
                blockSnapshot.getPos(),
                !(entity instanceof EntityPlayer) ? blockSnapshot.getReplacedBlock() : blockSnapshot.getCurrentBlock());
            this.entity = entity;
    ...
    public static class PlaceEvent extends EntityPlaceEvent {
        ...
        public PlaceEvent(@Nonnull BlockSnapshot blockSnapshot, @Nonnull IBlockState placedAgainst,
            ...
            if (DEBUG) {
                System.out.printf(
                    "Created PlaceEvent - [PlacedBlock: %s ][PlacedAgainst: %s ][ItemStack: %s ][Player: %s ][Hand: %s]\n",
                    getPlacedBlock(),
                    placedAgainst,
    ...
    public static class EntityMultiPlaceEvent extends EntityPlaceEvent {
        ...
        public EntityMultiPlaceEvent(@Nonnull List<BlockSnapshot> blockSnapshots, @Nonnull IBlockState placedAgainst,
                                     @Nullable Entity entity) {
            super(blockSnapshots.get(0), placedAgainst, entity);
            this.blockSnapshots = ImmutableList.copyOf(blockSnapshots);
            if (DEBUG) {
                System.out.printf(
                    "Created EntityMultiPlaceEvent - [PlacedAgainst: %s ][Entity: %s ]\n",
                    placedAgainst,
                    entity);
            }
        }
    ...
    public static class MultiPlaceEvent extends PlaceEvent {
        ...
        public MultiPlaceEvent(@Nonnull List<BlockSnapshot> blockSnapshots, @Nonnull IBlockState placedAgainst,
                               @Nonnull EntityPlayer player, @Nonnull EnumHand hand) {
            super(blockSnapshots.get(0), placedAgainst, player, hand);
            this.blockSnapshots = ImmutableList.copyOf(blockSnapshots);
            if (DEBUG) {
                System.out.printf(
                    "Created MultiPlaceEvent - [PlacedAgainst: %s ][ItemInHand: %s ][Player: %s ]\n",
                    placedAgainst,
                    player.getHeldItem(hand),
                    player);
            }
