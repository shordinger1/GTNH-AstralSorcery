...
package shordinger.wrapper.net.minecraftforge.event.terraingen;
...
import shordinger.wrapper.net.minecraft.world.gen.MapGenBase;
...
import shordinger.wrapper.net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate;
import shordinger.wrapper.net.minecraftforge.event.terraingen.OreGenEvent.GenerateMinable;
import shordinger.wrapper.net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate;
import shordinger.wrapper.net.minecraftforge.fml.common.eventhandler.Event.Result;

public abstract class TerrainGen {

    public static <T extends InitNoiseGensEvent.Context> T getModdedNoiseGenerators(World world, Random rand,
                                                                                    T original) {
    ...
    public static boolean populate(IChunkGenerator chunkProvider, World world, Random rand, int chunkX, int chunkZ,
                                   boolean hasVillageGenerated, Populate.EventType type) {
        PopulateChunkEvent.Populate event = new PopulateChunkEvent.Populate(
            chunkProvider,
            world,
            rand,
            chunkX,
     ...
     * If only the chunk position is available, use {@link #decorate(World, Random, ChunkPos, Decorate.EventType)}
     * instead.
     *
     ...
     * @param placementPos the specific position used for generating a feature, somewhere in the 2x2 chunks used for
     *                     decoration
     * @param type         the type of decoration
     */
    public static boolean decorate(World world, Random rand, ChunkPos chunkPos, BlockPos placementPos,
     ...
     * If a specific BlockPos for generation is available, use
     * {@link #decorate(World, Random, ChunkPos, BlockPos, Decorate.EventType)} instead.
     *
     * @param world    the world being generated in
     * @param rand     the random generator used for decoration
     * @param chunkPos the original chunk position used for generation, passed to the decorator
    ...
    public static boolean generateOre(World world, Random rand, WorldGenerator generator, BlockPos pos,
