...
import shordinger.wrapper.net.minecraftforge.fml.common.eventhandler.IContextSetter;
    ...
    public static class MissingMappings<T extends IForgeRegistryEntry<T>> extends RegistryEvent<T> {
        ...
        public static class Mapping<T extends IForgeRegistryEntry<T>> {
            ...
            public void remap(T target) {
                ...
                Validate.isTrue(
                    pool.getKey(target) != null,
                    String.format("The specified entry %s hasn't been registered in registry yet.", target));
                action = Action.REMAP;
                this.target = target;
            }
