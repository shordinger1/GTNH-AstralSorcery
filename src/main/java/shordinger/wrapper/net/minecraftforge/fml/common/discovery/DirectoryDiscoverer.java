...
public class DirectoryDiscoverer implements ITypeDiscoverer {
    ...
    private class ClassFilter implements FileFilter {
        ...
        public boolean accept(File file) {
            return (file.isFile() && classFile.matcher(file.getName())
                .matches()) || file.isDirectory();
        }
    }
    ...
    public List<ModContainer> discover(ModCandidate candidate, ASMDataTable table) {
        ...
        FMLLog.log.debug(
            "Examining directory {} for potential mods",
    ...
    public void exploreFileSystem(String path, File modDir, List<ModContainer> harvestedMods, ModCandidate candidate,
                                  @Nullable MetadataCollection mc) {
        if (path.length() == 0) {
        ...
        for (File file : content) {
            ...
            if (match.matches()) {
                ...
                try {
                    ...
                    FMLLog.log.error(
                        "There was a problem reading the file {} - probably this is a corrupt file",
                        file.getPath(),
                        e);
                    throw e;
                } catch (IOException e) {
