 ...
 * It assumes that you have a series of unique message types with each having a unique handler. Generally, context
 * should be
 * derived at message handling time.
 * <p>
 * Usage is simple:
 * <ul>
 * <li>construct, and store, an instance of this class. It will automatically register and configure your underlying
 * netty channel.
 *
 * <li>Then, call {@link #registerMessage(Class, Class, int, Side)} for each message type you want to exchange
 * providing an {@link IMessageHandler} implementation class as well as an {@link IMessage} implementation class. The
 * side parameter
 * to that method indicates which side (server or client) the <em>message processing</em> will occur on. The
 * discriminator byte
 ...
 * <li>To get a packet suitable for presenting to the rest of minecraft, you can call {@link #getPacketFrom(IMessage)}.
 * The return result
 ...
 * {@link IThreadListener#addScheduledTask(Runnable)}.
 * The way to obtain an {@link IThreadListener} is using either the {@link net.minecraft.client.Minecraft} instance
 * (client side) or a {@link net.minecraft.world.WorldServer} instance (server side).
 *
 * @author cpw
 */
public class SimpleNetworkWrapper {
    ...
    private <REQ extends IMessage, REPLY extends IMessage, NH extends INetHandler> void addServerHandlerAfter(
        ...
        channel.pipeline()
            .addAfter(type, generateName(channel.pipeline(), handler), handler);
    }
