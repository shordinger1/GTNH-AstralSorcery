...
import shordinger.wrapper.net.minecraftforge.fml.common.network.internal.FMLMessage.EntityMessage;
...
import shordinger.wrapper.net.minecraftforge.fml.common.registry.IEntityAdditionalSpawnData;
import shordinger.wrapper.net.minecraftforge.fml.common.registry.IThrowableEntity;
...
public class EntitySpawnHandler extends SimpleChannelInboundHandler<FMLMessage.EntityMessage> {
    ...
    private void process(EntityMessage msg) {
        if (msg.getClass()
            .equals(FMLMessage.EntitySpawnMessage.class)) {
    ...
    private void spawnEntity(FMLMessage.EntitySpawnMessage spawnMsg) {
        ...
        EntityRegistration er = EntityRegistry.instance()
            .lookupModSpawn(mc, spawnMsg.modEntityTypeId);
        if (er == null) {
            throw new RuntimeException(
                "Could not spawn mod entity ModID: " + spawnMsg.modId
                    + " EntityID: "
                    ...
                    + ", "
                    + spawnMsg.rawZ
                    + ") Please contact mod author or server admin.");
        }
        WorldClient wc = FMLClientHandler.instance()
            .getWorldClient();
        try {
            Entity entity;
            ...
            EntityPlayerSP clientPlayer = FMLClientHandler.instance()
                .getClientPlayerEntity();
            if (entity instanceof IThrowableEntity) {
                Entity thrower = clientPlayer.getEntityId() == spawnMsg.throwerId ? clientPlayer
            ...
            if (spawnMsg.dataWatcherList != null) {
                entity.getDataManager()
                    .setEntryValues(spawnMsg.dataWatcherList);
            }
            ...
            throw new RuntimeException(
                "A severe problem occurred during the spawning of an entity at (" + spawnMsg.rawX
                    + ", "
                    + spawnMsg.rawY
                    + ", "
                    + spawnMsg.rawZ
