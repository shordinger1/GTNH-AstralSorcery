...
import shordinger.wrapper.net.minecraftforge.fml.common.thread.SidedThreadGroup;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.CoreModManager;
import shordinger.wrapper.net.minecraftforge.fml.server.FMLServerHandler;
...
public class FMLCommonHandler {
    ...
    private List<ICrashCallable> crashCallables = Lists.newArrayList(
        Loader.instance()
            .getCallableCrashInformation());
    private Set<SaveHandler> handlerSet = Collections.newSetFromMap(
        new MapMaker().weakKeys()
            .<SaveHandler, Boolean>makeMap());
    ...
    private FMLCommonHandler() {
        registerCrashCallable(new ICrashCallable() {
            ...
            public String call() throws Exception {
                ...
                for (String coreMod : CoreModManager.getTransformers()
                    .keySet()) {
                    builder.append("\n" + coreMod + "\n  ")
                        .append(
                            joiner.join(
    ...
    public void raiseException(Throwable exception, String message, boolean stopGame) {
        FMLLog.log.error(
            "Something raised an exception. The message was '{}'. 'stopGame' is {}",
            message,
            stopGame,
            exception);
        if (stopGame) {
    ...
    public void computeBranding() {
        if (brandings == null) {
            ...
            if (Loader.instance()
                ...
                .containsKey("fmlbranding")) {
                brd.add(
                    Loader.instance()
                        .getFMLBrandingProperties()
                        .get("fmlbranding"));
            }
            int tModCount = Loader.instance()
                .getModList()
                .size();
            int aModCount = Loader.instance()
                .getActiveModList()
                .size();
            brd.add(
                String.format(
    ...
    public boolean handleServerHandshake(C00Handshake packet, NetworkManager manager) {
        ...
        if (packet.getRequestedState() == EnumConnectionState.LOGIN
            ...
            FMLLog.log.info(
                "Disconnecting Player: This server has mods that require FML/Forge to be installed on the client: {}",
                modNames);
            manager.sendPacket(new SPacketDisconnect(text));
            manager.closeChannel(text);
            return false;
     ...
     * @param hardExit Perform a halt instead of an exit (only use when the world is unsavable) - read the warnings at
     ...
     * @return A new InputStream that vanilla uses to load normal Lang files, Null if this is a 'enhanced' file and
     * loading is done.
     */
    @Nullable
