...
public final class DependencyParser {
    ...
    private static final ImmutableList<String> DEPENDENCY_INSTRUCTIONS = ImmutableList
        .of("client", "server", "required", "before", "after");
    private static final Splitter DEPENDENCY_INSTRUCTIONS_SPLITTER = Splitter.on("-")
        .omitEmptyStrings()
        .trimResults();
    private static final Splitter DEPENDENCY_PART_SPLITTER = Splitter.on(":")
        .omitEmptyStrings()
        .trimResults();
    ...
    private void parseDependency(String dep, List<String> instructions, String target, DependencyInfo info) {
        ...
        if (targetIsAll) {
            if (target.length() > 1) {
                throw new DependencyParserException(
                    modId,
                    dep,
            ...
            if ("client".equals(instruction)) {
                if (depSide != null) {
                    throw new DependencyParserException(
                        modId,
                        dep,
                        "Up to one side (client or server) can be specified.");
                }
                depSide = Side.CLIENT;
                ...
                throw new DependencyParserException(
                    ...
                    dep,
                    String.format(
                        "Found invalid instruction '%s'. Only %s are allowed.",
                        instruction,
    ...
    private static void sanityCheckModId(String modId, String dep, String depModId) {
        if (Strings.isNullOrEmpty(depModId)) {
            ...
            FMLLog.log.error(
                new DependencyParserException(
                    modId,
                    dep,
                    String.format("The modId '%s' is longer than the maximum of 64 characters.", depModId))
                    .getMessage());
            ...
            FMLLog.log.error(
                new DependencyParserException(
                    modId,
                    dep,
                    String.format("The modId '%s' must be all lowercase.", depModId)).getMessage());
        }
    }
    ...
    private static class DependencyParserException extends LoaderException {
        ...
        public static String formatMessage(String modId, String dependencyString, String explanation) {
            return String.format(
                "Unable to parse dependency for mod '%s' with dependency string '%s'. %s",
                modId,
                dependencyString,
                explanation);
        }
