...
import shordinger.wrapper.net.minecraft.entity.passive.EntityVillager.ITradeList;
import shordinger.wrapper.net.minecraft.util.EnumFacing;
import shordinger.wrapper.net.minecraft.util.ResourceLocation;
import shordinger.wrapper.net.minecraft.util.registry.RegistryNamespaced;
import shordinger.wrapper.net.minecraft.world.gen.structure.StructureComponent;
import shordinger.wrapper.net.minecraft.world.gen.structure.StructureVillagePieces;
import shordinger.wrapper.net.minecraft.world.gen.structure.StructureVillagePieces.PieceWeight;
import shordinger.wrapper.net.minecraft.world.gen.structure.StructureVillagePieces.Village;
import shordinger.wrapper.net.minecraftforge.fml.common.registry.GameRegistry.ObjectHolder;
...
public class VillagerRegistry {
    ...
    /**
     * Allow access to the {@link net.minecraft.world.gen.structure.StructureVillagePieces} array controlling new
     * village
    ...
    public static Village getVillageComponent(StructureVillagePieces.PieceWeight villagePiece,
                                              StructureVillagePieces.Start startPiece, List<StructureComponent> pieces, Random random, int p1, int p2, int p3,
                                              EnumFacing facing, int p5) {
        return instance().villageCreationHandlers.get(villagePiece.villagePieceClass)
            .buildComponent(villagePiece, startPiece, pieces, random, p1, p2, p3, facing, p5);
    }
    ...
    public static class VillagerCareer {
        ...
        public List<ITradeList> getTrades(int level) {
            return level >= 0 && level < this.trades.size() ? Collections.unmodifiableList(this.trades.get(level))
                : null;
        }

        private VillagerCareer init(EntityVillager.ITradeList[][] trades) {
            for (int x = 0; x < trades.length; x++) this.trades.add(Lists.newArrayList(trades[x]));
            return this;
        }
    ...
    public static void onSetProfession(EntityVillager entity, int network) {
        ...
        if (prof == null || INSTANCE.REGISTRY.getIDForObject(prof) != network) {
            throw new RuntimeException(
                "Attempted to set villager profession to unregistered profession: " + network + " " + prof);
        }
    ...
    public static void onSetProfession(EntityZombieVillager entity, int network) {
        ...
        if (prof != entity.getForgeProfession()) entity.setForgeProfession(prof);
    }
