...
public class SnapshotJson implements Comparable<SnapshotJson> {
    ...
    public static SnapshotJson create(File target) {
        if (!target.exists()) return new SnapshotJson();

        try {
            String json = Files.asCharSource(target, StandardCharsets.UTF_8)
                .read();
            ...
            FMLLog.log.info(
                FMLLog.log.getMessageFactory()
                    .newMessage("Failed to parse snapshot json file {}.", target),
                jse);
        } catch (IOException ioe) {
            FMLLog.log.info(
                FMLLog.log.getMessageFactory()
                    .newMessage("Failed to read snapshot json file {}.", target),
                ioe);
        }
    ...
    public void merge(SnapshotJson o) {
        if (o.versions != null) {
            if (versions == null) versions = new ArrayList<>(o.versions);
            else o.versions.stream()
                .filter(
                    e -> versions.stream()
                        .anyMatch(e2 -> e.timestamp.equals(e2.timestamp)))
                .forEach(e -> versions.add(e));
            updateLatest();
        }
    }
    ...
    public boolean remove(String timestamp) {
        ...
        if (versions.removeIf(e -> e.timestamp.equals(timestamp))) updateLatest();
        return false;
    }
    ...
    public int compareTo(SnapshotJson o) {
        return o == null ? 1
            : o.latest == null ? latest == null ? 0 : 1 : latest == null ? -1 : o.latest.compareTo(latest);
    }
