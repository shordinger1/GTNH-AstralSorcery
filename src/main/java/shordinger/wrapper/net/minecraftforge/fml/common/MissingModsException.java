...
import shordinger.wrapper.net.minecraftforge.fml.client.GuiModsMissing;
...
public class MissingModsException extends EnhancedRuntimeException implements IDisplayableError {
    ...
    public String getMessage() {
        Set<ArtifactVersion> missingMods = missingModsInfos.stream()
            .map(MissingModInfo::getAcceptedVersion)
            .collect(Collectors.toSet());
        return String.format("Mod %s (%s) requires %s", id, name, missingMods);
    }

    public void addMissingMod(ArtifactVersion acceptedVersion, @Nullable ArtifactVersion currentVersion,
                              boolean required) {
    ...
    protected void printStackTrace(WrappedPrintStream stream) {
        ...
        for (MissingModInfo info : this.missingModsInfos) {
            ...
            stream.println(
                String.format(
                    "\t%s : need %s: have %s",
                    acceptedVersion.getVersionString(),
                    acceptedVersion.getRangeString(),
                    currentString));
        }
        stream.println("");
    }
    ...
    public static class MissingModInfo {
        ...
        private MissingModInfo(ArtifactVersion acceptedVersion, @Nullable ArtifactVersion currentVersion,
                               boolean required) {
