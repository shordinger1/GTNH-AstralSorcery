...
public class Repository {
    ...
    public static Artifact resolveAll(Artifact artifact) {
        ...
        for (Repository repo : cache.values()) {
            ...
            if (!artifact.isSnapshot()) return tmp; // If its a concrete version *assume* any resolved one in any repo
            // will work. As they shouldn't release overriding versions.
    ...
    protected Repository(File root, String name) {
        this.root = root;
        this.name = name;
        if (name == null) throw new IllegalArgumentException("Invalid Repository Name, for " + root);
    }
    ...
    public Artifact resolve(Artifact artifact) {
        if (!artifact.isSnapshot()) return getFile(artifact.getPath()).exists() ? artifact : null;

        File meta = getFile(artifact.getFolder() + File.separatorChar + SnapshotJson.META_JSON_FILE);
        if (!meta.exists()) return null;

        SnapshotJson json = SnapshotJson
            .create(getFile(artifact.getFolder() + File.separatorChar + SnapshotJson.META_JSON_FILE));
        if (json.getLatest() == null) return null;

        Artifact ret = new Artifact(artifact, this, json.getLatest());
        while (json.getLatest() != null && !getFile(ret.getPath()).exists()) {
            if (!json.remove(json.getLatest())) throw new IllegalStateException(
                "Something went wrong, Latest (" + json.getLatest()
                    + ") did not point to an entry in the json list: "
    ...
    public File archive(Artifact artifact, File file, byte[] manifest) {
        ...
        try {
            if (target.exists()) {
                FMLLog.log.debug(
                    "Maven file already exists for {}({}) at {}, deleting duplicate.",
                    file.getName(),
                    artifact.toString(),
                ...
                if (artifact.isSnapshot()) {
                    ...
                    json.add(
                        new SnapshotJson.Entry(
                            artifact.getTimestamp(),
                            Files.hash(target, Hashing.md5())
                                .toString()));
            ...
            FMLLog.log.error(
                FMLLog.log.getMessageFactory()
                    .newMessage("Error moving file {} to {}", file, target.getAbsolutePath()),
                e);
        }
        return file;
    }
