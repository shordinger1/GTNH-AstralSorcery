...
package shordinger.wrapper.net.minecraftforge.fml.relauncher;
...
import shordinger.wrapper.net.minecraft.launchwrapper.ITweaker;
import shordinger.wrapper.net.minecraft.launchwrapper.Launch;
import shordinger.wrapper.net.minecraft.launchwrapper.LaunchClassLoader;
import shordinger.wrapper.net.minecraftforge.fml.common.CertificateHelper;
import shordinger.wrapper.net.minecraftforge.fml.common.FMLLog;
import shordinger.wrapper.net.minecraftforge.fml.common.asm.ASMTransformerWrapper;
import shordinger.wrapper.net.minecraftforge.fml.common.asm.transformers.ModAccessTransformer;
import shordinger.wrapper.net.minecraftforge.fml.common.launcher.FMLInjectionAndSortingTweaker;
import shordinger.wrapper.net.minecraftforge.fml.common.launcher.FMLTweaker;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.IFMLLoadingPlugin.DependsOn;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.IFMLLoadingPlugin.MCVersion;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.IFMLLoadingPlugin.Name;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.IFMLLoadingPlugin.SortingIndex;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.IFMLLoadingPlugin.TransformerExclusions;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.libraries.Artifact;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.libraries.LibraryManager;
import shordinger.wrapper.net.minecraftforge.fml.relauncher.libraries.Repository;
...
public class CoreModManager {
    ...
    private static class FMLPluginWrapper implements ITweaker {
        ...
        public void injectIntoClassLoader(LaunchClassLoader classLoader) {
            FMLLog.log.debug(
                "Injecting coremod {} \\{{}\\} class transformers",
                name,
            ...
            if (coreModInstance.getASMTransformerClass() != null)
                for (String transformer : coreModInstance.getASMTransformerClass()) {
                    ...
                    classLoader.registerTransformer(
                        ASMTransformerWrapper.getTransformerWrapper(classLoader, transformer, name));
                    ts.add(transformer);
                }
            ...
            FMLLog.log.debug(
                "Coremod plugin class {} run successfully",
                coreModInstance.getClass()
                    .getSimpleName());
    ...
    public static void handleLaunch(File mcDir, LaunchClassLoader classLoader, FMLTweaker tweaker) {
        ...
        // TODO: Detect this better? Support install time deobfusication? Decouple deobf from dev? Add dev flag in
        // GradleStart?
        try {
        ...
        if (loadPlugins.isEmpty()) {
            throw new RuntimeException(
                "A fatal error has occurred - no valid fml load plugin was found - this is a completely corrupt FML installation.");
        }
    ...
    private static void findDerpMods(LaunchClassLoader classLoader, File modDir, File modDirVer) {
        ...
        if (derplist != null && derplist.length > 0) {
            ...
            for (File f : derplist) FMLLog.log.fatal("Problem file : {}", f.getName());
        }
        ...
        if (derpdirlist != null && derpdirlist.length > 0) {
            ...
            for (File f : derpdirlist) {
                FMLLog.log
                    .fatal("Directory {} contains {}", f.getName(), Arrays.asList(new File(f, "META-INF").list()));
            }
            ...
            try {
                ...
                File crashreportfile = new File(
                    new File(modDir.getParentFile(), "crash-reports"),
                    String.format("fml-crash-%1$tY-%1$tm-%1$td_%1$tH.%1$tM.%1$tS.txt", Calendar.getInstance()));
    ...
    private static File[] listFiles(FileFilter filter, File... dirs) {
        ...
        for (File dir : dirs) {
            ...
            if (ret == null) ret = dir.listFiles(filter);
            else ret = ObjectArrays.concat(ret, dir.listFiles(filter), File.class);
        }
        ...
        for (File coreMod : file_canidates) {
            ...
            try {
                ...
                if (LibraryManager.DISABLE_EXTERNAL_MANIFEST || !manifest.exists()) {
                    ...
                    mfAttributes = jar.getManifest() == null ? null
                        : jar.getManifest()
                        .getMainAttributes();
                } else {
                ...
                if (modSide != null && !"BOTH".equals(modSide)
                    && !FMLLaunchHandler.side()
                    .name()
                    .equals(modSide)) {
                    FMLLog.log.debug(
                        "Mod {} has ModSide meta-inf value {}, and we're {} It will be ignored",
                        coreMod.getName(),
                        modSide,
                ...
                if (!modTypes.contains("FML")) {
                    FMLLog.log.debug(
                        "Adding {} to the list of things to skip. It is not an FML mod, it has types {}",
                        coreMod.getName(),
                        modTypes);
                    ignoredModFiles.add(coreMod.getName());
                    continue;
            ...
            try {
                ...
                if (!mfAttributes.containsKey(COREMODCONTAINSFMLMOD)) {
                    FMLLog.log.trace(
                        "Adding {} to the list of known coremods, it will not be examined again",
                        coreMod.getName());
                    ignoredModFiles.add(coreMod.getName());
                } else {
                    FMLLog.log.warn(
                        "Found FMLCorePluginContainsFMLMod marker in {}. This is not recommended, @Mods should be in a separate jar from the coremod.",
                        coreMod.getName());
    ...
    private static FMLPluginWrapper loadCoreMod(LaunchClassLoader classLoader, String coreModClass, File location) {
        ...
        try {
            ...
            if (!Arrays.asList(rootPlugins)
                ...
                FMLLog.log.warn(
                    "The coremod {} does not have a MCVersion annotation, it may cause issues with this version of Minecraft",
                    coreModClass);
            } else if (requiredMCVersion != null && !FMLInjectionData.mccversion.equals(requiredMCVersion.value())) {
                FMLLog.log.error(
                    "The coremod {} is requesting minecraft version {} and minecraft is {}. It will be ignored.",
                    coreModClass,
            ...
            Certificate[] certificates = coreModClazz.getProtectionDomain()
                .getCodeSource()
                .getCertificates();
            ImmutableList<String> certList = CertificateHelper.getFingerprints(certificates);
            if (certList.isEmpty()) {
                if (deobfuscatedEnvironment && Arrays.asList(rootPlugins)
                    .contains(coreModClass)) // This is probably a forge/mod dev environment - ignore missing forge
                // certificates
                {
                    FMLLog.log.info(
                        "Ignoring missing certificate for coremod {} ({}), we are in deobf and it's a forge core plugin",
                        coreModName,
                        coreModClass);
                } else if (deobfuscatedEnvironment && location == null) // This is probably a mod dev workspace
                {
                    FMLLog.log.info(
                        "Ignoring missing certificate for coremod {} ({}), as this is probably a dev workspace",
                        coreModName,
                        coreModClass);
            ...
            if (accessTransformerClass != null) {
                FMLLog.log
                    .debug("Added access transformer class {} to enqueued access transformers", accessTransformerClass);
            ...
            else FMLLog.log.debug("Skipping root plugin {}", coreModClass);
        } catch (ClassCastException cce) {
            FMLLog.log.error(
                "Coremod {}: The plugin {} is not an implementor of IFMLLoadingPlugin",
                coreModName,
                coreModClass,
                cce);
    ...
    public static void sortTweakList() {
        ...
        ToIntFunction<ITweaker> getOrder = o -> o instanceof FMLInjectionAndSortingTweaker ? Integer.MIN_VALUE
            : o instanceof FMLPluginWrapper ? ((FMLPluginWrapper) o).sortIndex
            : tweakSorting.getOrDefault(
            o.getClass()
                .getName(),
            0);
        Arrays.sort(
            toSort,
            (o1, o2) -> Ints.saturatedCast((long) getOrder.applyAsInt(o1) - (long) getOrder.applyAsInt(o2)));
