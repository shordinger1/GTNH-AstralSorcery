...
import shordinger.wrapper.net.minecraftforge.fml.client.config.GuiConfigEntries.ArrayEntry;
import shordinger.wrapper.net.minecraftforge.fml.common.FMLLog;

/**
 * This class implements the scrolling list functionality of the GuiEditList screen. It also provides all the default
 * controls
...
public class GuiEditArrayEntries extends GuiListExtended {
    ...
    public GuiEditArrayEntries(GuiEditArray parent, Minecraft mc, IConfigElement configElement, Object[] beforeValues,
        ...
        super(
            ...
            parent.width,
            parent.height,
            parent.titleLine2 != null ? (parent.titleLine3 != null ? 43 : 33) : 23,
            parent.height - 32,
            20);
        ...
        } else if (configElement.isList() && configElement.getType()
            .equals(ConfigGuiType.BOOLEAN))
            for (Object value : currentValues)
                listEntries
                    .add(new BooleanEntry(this.owningGui, this, configElement, Boolean.valueOf(value.toString())));
        else if (configElement.isList() && configElement.getType()
            .equals(ConfigGuiType.INTEGER))
            for (Object value : currentValues)
                listEntries
                    .add(new IntegerEntry(this.owningGui, this, configElement, Integer.parseInt(value.toString())));
        else if (configElement.isList() && configElement.getType()
            .equals(ConfigGuiType.DOUBLE))
            for (Object value : currentValues)
                listEntries
                    .add(new DoubleEntry(this.owningGui, this, configElement, Double.parseDouble(value.toString())));
        else if (configElement.isList()) for (Object value : currentValues)
    ...
    public void addNewEntry(int index) {
        ...
        this.canAddMoreEntries = !configElement.isListLengthFixed() && (configElement.getMaxListLength() == -1
            || this.listEntries.size() - 1 < configElement.getMaxListLength());
    ...
    public void recalculateState() {
        ...
        if (isDefault) for (int i = 0; i < listLength; i++)
            if (!configElement.getDefaults()[i].equals(
                listEntries.get(i)
                    .getValue()))
                isDefault = false;

        if (!isChanged) for (int i = 0; i < listLength; i++)
            if (!beforeValues[i].equals(
                listEntries.get(i)
                    .getValue()))
                isChanged = true;
    }
    ...
    protected boolean isListSavable() {
        for (IArrayEntry entry : this.listEntries) if (!entry.isValueSavable()) return false;
    ...
    public static class DoubleEntry extends StringEntry {
        ...
        public void keyTyped(char eventChar, int eventKey) {
            if (owningScreen.enabled || eventKey == Keyboard.KEY_LEFT
                ...
                if (validChars.contains(String.valueOf(eventChar))
                    ...
                    || eventKey == Keyboard.KEY_END)
                    this.textFieldValue
                        .textboxKeyTyped((owningScreen.enabled ? eventChar : Keyboard.CHAR_NONE), eventKey);
    ...
    public static class StringEntry extends BaseEntry {
        ...
        public StringEntry(GuiEditArray owningScreen, GuiEditArrayEntries owningEntryList, IConfigElement configElement,
            ...
            this.textFieldValue = new GuiTextField(
                0,
                owningEntryList.getMC().fontRenderer,
                owningEntryList.width / 4 + 1,
                0,
                owningEntryList.controlWidth - 3,
                16);
    ...
    public static class BooleanEntry extends BaseEntry {
        ...
        public BooleanEntry(GuiEditArray owningScreen, GuiEditArrayEntries owningEntryList,
            ...
            super(owningScreen, owningEntryList, configElement);
            this.value = value;
            this.btnValue = new GuiButtonExt(
                0,
                0,
                0,
        ...
        public boolean mousePressed(int index, int x, int y, int mouseEvent, int relativeX, int relativeY) {
            if (this.btnValue.mousePressed(owningEntryList.getMC(), x, y)) {
                btnValue.playPressSound(
                    owningEntryList.getMC()
                        .getSoundHandler());
                value = !value;
    ...
    public static class BaseEntry implements IArrayEntry {
        ...
        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY,
                              boolean isSelected, float partial) {
            if (this.getValue() != null && this.isValidated) owningEntryList.getMC().fontRenderer.drawString(
        ...
        public void drawToolTip(int mouseX, int mouseY) {
            ...
            if (this.btnAddNewEntryAbove.visible
                && this.addNewEntryAboveHoverChecker.checkHover(mouseX, mouseY, canHover))
        ...
        public boolean mousePressed(int index, int x, int y, int mouseEvent, int relativeX, int relativeY) {
            if (this.btnAddNewEntryAbove.mousePressed(owningEntryList.getMC(), x, y)) {
                btnAddNewEntryAbove.playPressSound(
                    owningEntryList.getMC()
                        .getSoundHandler());
