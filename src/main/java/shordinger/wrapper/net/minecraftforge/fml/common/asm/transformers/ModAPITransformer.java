...
public class ModAPITransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        for (ASMData optional : optionals.get(lookupName)) {
            String modId = (String) optional.getAnnotationInfo()
                .get("modid");
            ...
            if (optional.getAnnotationInfo()
                .containsKey("iface")) {
                Boolean stripRefs = (Boolean) optional.getAnnotationInfo()
                    .get("striprefs");
                if (stripRefs == null) stripRefs = Boolean.FALSE;
                stripInterface(
                    classNode,
                    (String) optional.getAnnotationInfo()
                        .get("iface"),
                    stripRefs);
            } else {
    ...
    private void stripMethod(ClassNode classNode, String methodDescriptor) {
        if (classNode.name.endsWith("$class")) {
            ...
            methodDescriptor = methodDescriptor.substring(0, pos) + 'L'
                + subName
                + ';'
                + methodDescriptor.substring(pos);
        }
    ...
    private void stripInterface(ClassNode classNode, String interfaceName, boolean stripRefs) {
        ...
        if (found && stripRefs) {
            if (logDebugInfo) FMLLog.log
                .debug("Optional removal - interface {} - stripping method signature references", interfaceName);
            for (Iterator<MethodNode> iterator = classNode.methods.iterator(); iterator.hasNext(); ) {
                ...
                if (node.desc.contains(ifaceName)) {
                    if (logDebugInfo) FMLLog.log.debug(
                        "Optional removal - interface {} - stripping method containing reference {}",
                        interfaceName,
                        node.name);
                    iterator.remove();
                }
            ...
            if (logDebugInfo) FMLLog.log
                .debug("Optional removal - interface {} - all method signature references stripped", interfaceName);
        } else if (found) {
            if (logDebugInfo) FMLLog.log
                .debug("Optional removal - interface {} - NOT stripping method signature references", interfaceName);
        }
    }
    ...
    private Set<ASMData> unpackInterfaces(Set<ASMData> packedInterfaces) {
        ...
        for (ASMData data : packedInterfaces) {
            ...
            List<Map<String, Object>> packedList = (List<Map<String, Object>>) data.getAnnotationInfo()
                .get("value");
