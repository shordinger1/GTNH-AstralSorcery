...
import shordinger.wrapper.net.minecraft.entity.EntityList.EntityEggInfo;
...
import shordinger.wrapper.net.minecraft.world.biome.Biome.SpawnListEntry;
...
import shordinger.wrapper.net.minecraftforge.fml.common.network.internal.FMLMessage.EntitySpawnMessage;
...
public class EntityRegistry {
    ...
    public class EntityRegistration {
        ...
        @Deprecated // 1.13
        public EntityRegistration(ModContainer mc, ResourceLocation registryName, Class<? extends Entity> entityClass,
                                  String entityName, int id, int trackingRange, int updateFrequency, boolean sendsVelocityUpdates) {
            this(
                mc,
                registryName,
                entityClass,
    ...
    public static void registerModEntity(ResourceLocation registryName, Class<? extends Entity> entityClass,
                                         String entityName, int id, Object mod, int trackingRange, int updateFrequency, boolean sendsVelocityUpdates) {
        instance().doModEntityRegistration(
            registryName,
            entityClass,
            entityName,
            id,
    ...
    private void doModEntityRegistration(ResourceLocation registryName, Class<? extends Entity> entityClass,
                                         String entityName, int id, Object mod, int trackingRange, int updateFrequency, boolean sendsVelocityUpdates) {
        ...
        try {
            ...
            if (!ForgeRegistries.ENTITIES.containsKey(registryName)) {
                ...
                FMLLog.log.trace(
                    "Automatically registered mod {} entity {} as {}",
                    mc.getModId(),
                    entityName,
                    entry.getRegistryName());
            } else {
                FMLLog.log.debug(
                    "Skipping automatic mod {} entity registration for already registered entry {} class {}",
                    mc.getModId(),
                    registryName,
                    entityClass.getName());
            }
            ...
            FMLLog.log.warn(
                "The mod {} tried to register the entity (registry,name,class) ({},{},{}) one or both of which are already registered",
                mc.getModId(),
                registryName,
                entityName,
                entityClass.getName(),
    ...
    public static void registerEgg(ResourceLocation name, int primary, int secondary) {
        ...
        if (entry == null) {
            FMLLog.bigWarning(
                "Attempted to registry a entity egg for entity ({}) that is not in the Entity Registry",
                name);
            return;
        }
    ...
    public static void addSpawn(String entityName, int weightedProb, int min, int max, EnumCreatureType typeOfCreature,
                                Biome... biomes) {
     ...
     */
    public static void removeSpawn(Class<? extends EntityLiving> entityClass, EnumCreatureType typeOfCreature,
                                   Biome... biomes) {
        for (Biome biome : biomes) {
            biome.getSpawnableList(typeOfCreature)
                .removeIf(entry -> entry.entityClass == entityClass);
        }
    }
