...
import shordinger.wrapper.net.minecraftforge.fml.common.versioning.InvalidVersionSpecificationException;
...
public class NetworkModHolder {
    ...
    private class DefaultNetworkChecker extends NetworkChecker {
        ...
        @Override
        public String toString() {
            return acceptableRange != null ? String.format("Accepting range %s", acceptableRange.toStringFriendly())
    ...
    private class MethodNetworkChecker extends NetworkChecker {
        ...
        public String checkCompatible(Map<String, String> remoteVersions, Side side) {
            try {
                ...
                FMLLog.log.error(
                    "Error occurred invoking NetworkCheckHandler {} at {}",
                    checkHandler.getName(),
                    container,
                    e);
                return String
                    .format("Error occurred invoking NetworkCheckHandler %s at %s", checkHandler.getName(), container);
            }
        }
    ...
    public NetworkModHolder(ModContainer container, NetworkChecker checker) {
        ...
        FMLLog.log.debug(
            "The mod {} is using a custom checker {}",
            container.getModId(),
            checker.getClass()
                .getName());
    }

    public NetworkModHolder(ModContainer container, Class<?> modClass, @Nullable String acceptableVersionRange,
                            ASMDataTable table) {
        this(container);
        ...
        for (ASMData vch : versionCheckHandlers) {
            if (vch.getClassName()
                .equals(modClass.getName())) {
                networkCheckHandlerMethod = vch.getObjectName();
                networkCheckHandlerMethod = networkCheckHandlerMethod
                    .substring(0, networkCheckHandlerMethod.indexOf('('));
                break;
            }
        }
        ...
        if (networkCheckHandlerMethod != null) {
            try {
                Method checkHandlerMethod = modClass
                    .getDeclaredMethod(networkCheckHandlerMethod, Map.class, Side.class);
                if (checkHandlerMethod.isAnnotationPresent(NetworkCheckHandler.class)) {
                    this.checkHandler = checkHandlerMethod;
                }
            } catch (Exception e) {
                FMLLog.log.warn(
                    "The declared version check handler method {} on network mod id {} is not accessible",
        ...
        FMLLog.log
            .trace("Testing mod {} to verify it accepts its own version in a remote connection", container.getModId());
        boolean acceptsSelf = acceptVersion(container.getVersion());
        if (!acceptsSelf) {
            FMLLog.log.fatal(
                "The mod {} appears to reject its own version number ({}) in its version handling. This is likely a severe bug in the mod!",
                container.getModId(),
