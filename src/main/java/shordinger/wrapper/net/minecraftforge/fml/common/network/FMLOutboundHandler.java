...
import shordinger.wrapper.net.minecraftforge.fml.common.network.NetworkRegistry.TargetPoint;
...
public class FMLOutboundHandler extends ChannelOutboundHandlerAdapter {
    ...
    public enum OutboundTarget {
        ...
        PLAYER(Sets.immutableEnumSet(Side.SERVER)) {
            ...
            public List<NetworkDispatcher> selectNetworks(Object args, ChannelHandlerContext context,
                ...
                NetworkDispatcher dispatcher = (player == null || player instanceof FakePlayer) ? null
                    : player.connection.netManager.channel()
        ...
        ALL(Sets.immutableEnumSet(Side.SERVER)) {
            ...
            public List<NetworkDispatcher> selectNetworks(Object args, ChannelHandlerContext context,
                ...
                for (EntityPlayerMP player : FMLCommonHandler.instance()
                    ...
                    .getPlayerList()
                    .getPlayers()) {
                    NetworkDispatcher dispatcher = player.connection.netManager.channel()
                        .attr(NetworkDispatcher.FML_DISPATCHER)
                        .get();
        ...
        TRACKING_POINT(Sets.immutableEnumSet(Side.SERVER)) {
            ...
            public List<NetworkDispatcher> selectNetworks(Object args, ChannelHandlerContext context,
                ...
                PlayerChunkMapEntry entry = world.getPlayerChunkMap()
                    .getEntry(MathHelper.floor(tp.x) >> 4, MathHelper.floor(tp.z) >> 4);
                if (entry == null) {
                    return ImmutableList.of();
                }
        ...
        /**
         * The packet is sent to all players tracking the supplied {@link Entity}. This is different from
         * {@link #TRACKING_POINT} because Entities
        ...
        TRACKING_ENTITY(Sets.immutableEnumSet(Side.SERVER)) {
            ...
            public List<NetworkDispatcher> selectNetworks(Object args, ChannelHandlerContext context,
                ...
                Set<? extends EntityPlayer> players = FMLCommonHandler.instance()
                    .getMinecraftServerInstance()
                    .getWorld(e.dimension)
                    .getEntityTracker()
                    .getTrackingPlayers(e);

                ImmutableList.Builder<NetworkDispatcher> builder = ImmutableList.builder();
                for (EntityPlayer player : players) {
                    NetworkDispatcher dispatcher = ((EntityPlayerMP) player).connection.netManager.channel()
                        .attr(NetworkDispatcher.FML_DISPATCHER)
                        .get();
                    if (dispatcher != null) builder.add(dispatcher);
                }
        ...
        TOSERVER(Sets.immutableEnumSet(Side.CLIENT)) {
            ...
            public List<NetworkDispatcher> selectNetworks(Object args, ChannelHandlerContext context,
                                                          FMLProxyPacket packet) {
                NetworkManager clientConnection = FMLCommonHandler.instance()
                    .getClientToServerNetworkManager();
                return clientConnection == null || clientConnection.channel()
                    .attr(NetworkDispatcher.FML_DISPATCHER)
                    .get() == null ? ImmutableList.<NetworkDispatcher>of()
                    : ImmutableList.of(
        ...
        @Nullable
        public abstract List<NetworkDispatcher> selectNetworks(Object args, ChannelHandlerContext context,
                                                               FMLProxyPacket packet);
    }
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        ...
        outboundTarget = ctx.channel()
            .attr(FML_MESSAGETARGET)
            .get();
        Side channelSide = ctx.channel()
            .attr(NetworkRegistry.CHANNEL_SOURCE)
            .get();
        if (outboundTarget != null && outboundTarget.allowed.contains(channelSide)) {
            args = ctx.channel()
                .attr(FML_MESSAGETARGETARGS)
                .get();
