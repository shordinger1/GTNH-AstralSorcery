...
import shordinger.wrapper.net.minecraftforge.fml.common.network.PacketLoggingHandler;
...
public class NetworkDispatcher extends SimpleChannelInboundHandler<Packet<?>> implements ChannelOutboundHandler {
    ...
    private static boolean DEBUG_HANDSHAKE = Boolean
        .parseBoolean(System.getProperty("fml.debugNetworkHandshake", "false"));
    ...
    public static final AttributeKey<Map<ResourceLocation, ForgeRegistry.Snapshot>> FML_GAMEDATA_SNAPSHOT = AttributeKey
        .valueOf("fml:gameDataSnapshot");
    ...
    public NetworkDispatcher(NetworkManager manager) {
        ...
        this.handshakeChannel = new EmbeddedChannel(
        ...
        this.handshakeChannel.attr(FML_DISPATCHER)
            .set(this);
        this.handshakeChannel.attr(NetworkRegistry.CHANNEL_SOURCE)
            .set(Side.SERVER);
        this.handshakeChannel.attr(NetworkRegistry.FML_CHANNEL)
            .set("FML|HS");
        this.handshakeChannel.attr(IS_LOCAL)
    ...
    public void serverToClientHandshake(EntityPlayerMP player) {
        this.player = player;
        Boolean fml = this.manager.channel()
            .attr(NetworkRegistry.FML_MARKER)
            .get();
        if (fml != null && fml) {
    ...
    private void insertIntoChannel() {
        ...
        this.manager.channel()
            .pipeline()
            .addBefore("packet_handler", "fml:packet_handler", this);
        if (this.state != null) {
            FMLLog.log.info(
        ...
        this.handshakeChannel.pipeline()
            .fireUserEventTriggered(this);
        this.manager.channel()
            .config()
            .setAutoRead(true);
    }
    ...
    int serverInitiateHandshake() {
        ...
        serverHandler = new NetHandlerPlayServer(scm.getServerInstance(), manager, player) {
            ...
            public void update() {
                ...
                // FORGE: sometimes the netqueue will tick while login is occurring, causing an NPE. We shouldn't tick
    ...
    void clientListenForServerHandshake() {
        ...
        this.netHandler = FMLCommonHandler.instance()
    ...
    private boolean handleVanilla(Packet<?> msg) {
        if (state == ConnectionState.AWAITING_HANDSHAKE && msg instanceof SPacketJoinGame) {
            handshakeChannel.pipeline()
                .fireUserEventTriggered(msg);
        } else {
            FMLLog.log.info(
                "Unexpected packet during modded negotiation - assuming vanilla or keepalives : {}",
                msg.getClass()
                    .getName());
        }
        return false;
    ...
    private void kickWithMessage(String message) {
        ...
        if (side == Side.CLIENT) {
            ...
            manager.sendPacket(
                new SPacketDisconnect(TextComponentString),
    ...
    private boolean handleClientSideCustomPacket(SPacketCustomPayload msg, ChannelHandlerContext context) {
        ...
        if ("FML|HS".equals(channelName) || "REGISTER".equals(channelName) || "UNREGISTER".equals(channelName)) {
            ...
            handshakeChannel.inboundMessages()
                .clear();
            return true;
    ...
    public void sendProxy(FMLProxyPacket msg) {
        if (!manager.isChannelOpen()) msg = msg.copy();
    ...
    private class MultiPartCustomPayload extends SPacketCustomPayload {
        ...
        public void processPart(PacketBuffer input) throws IOException {
            ...
            if (part != part_expected) {
                throw new IOException(
                    "Received FML MultiPart packet out of order, Expected " + part_expected + " Got " + part);
            }
