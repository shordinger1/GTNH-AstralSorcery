...
public abstract class FMLHandshakeMessage {
    ...
    public static FMLProxyPacket makeCustomChannelRegistration(Set<String> channels) {
        String salutation = Joiner.on('\0')
            .join(Iterables.concat(Arrays.asList("FML|HS", "FML", "FML|MP"), channels));
        FMLProxyPacket proxy = new FMLProxyPacket(
            new PacketBuffer(Unpooled.wrappedBuffer(salutation.getBytes(StandardCharsets.UTF_8))),
            "REGISTER");
        return proxy;
    }
    ...
    public static class ServerHello extends FMLHandshakeMessage {
        ...
        public void fromBytes(ByteBuf buffer) {
            ...
            if (serverProtocolVersion > 1) {
                ...
                FMLLog.log.debug(
                    "Server FML protocol version {}, 4 byte dimension received {}",
    ...
    public static class RegistryData extends FMLHandshakeMessage {
        ...
        public void fromBytes(ByteBuf buffer) {
            ...
            for (int i = 0; i < length; i++) {
                overrides.put(
                    new ResourceLocation(ByteBufUtils.readUTF8String(buffer)),
