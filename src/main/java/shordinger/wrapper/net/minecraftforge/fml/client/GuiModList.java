...
import shordinger.wrapper.net.minecraft.client.gui.GuiUtilRenderComponents;
...
import shordinger.wrapper.net.minecraftforge.common.ForgeVersion.CheckResult;
...
import shordinger.wrapper.net.minecraftforge.fml.common.versioning.ComparableVersion;
...
public class GuiModList extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList
            .add(new GuiButton(6, ((modList.right + this.width) / 2) - 100, this.height - 38, I18n.format("gui.done")));
        ...
        int x = 10, y = 10;
        GuiButton normalSort = new GuiButton(
            SortType.NORMAL.buttonID,
            x,
            y,
            width - buttonMargin,
            20,
    ...
    private void reloadMods() {
        ...
        mods.clear();
        for (ModContainer m : Loader.instance()
            .getActiveModList()) {
    ...
    private void updateCache() {
        ...
        if (!logoFile.isEmpty()) {
            ...
            IResourcePack pack = FMLClientHandler.instance()
                .getResourcePackFor(selectedMod.getModId());
            try {
                ...
                if (pack != null) {
                    ...
                    if (logoResource != null) logo = TextureUtil.readBufferedImage(logoResource);
                }
                if (logo != null) {
        ...
        if (!selectedMod.getMetadata().autogenerated) {
            ...
            lines.add(
                String.format(
                    "Mod ID: '%s' Mod State: %s",
                    selectedMod.getModId(),
                    Loader.instance()
                        .getModState(selectedMod)));
            ...
            lines.add(
                "Authors: " + selectedMod.getMetadata()
                    .getAuthorList());
            ...
            if (selectedMod.getMetadata().childMods.isEmpty()) lines.add("No child mods for this mod");
                ...
                "Child mods: " + selectedMod.getMetadata()
                    .getChildModList());
    ...
    private class Info extends GuiScrollingList {
        ...
        public Info(int width, List<String> lines, @Nullable ResourceLocation logoPath, Dimension logoDims) {
            super(
                GuiModList.this.getMinecraftInstance(),
                width,
                GuiModList.this.height,
                32,
                GuiModList.this.height - 88 + 4,
                GuiModList.this.listWidth + 20,
                60,
                ...
                if (maxTextLength >= 0) {
                    ret.addAll(
                        GuiUtilRenderComponents
                            .splitText(chat, maxTextLength, GuiModList.this.fontRenderer, false, true));
                }
            }
            return ret;
        }
        ...
        protected void drawHeader(int entryRight, int relativeY, Tessellator tess) {
            ...
            if (logoPath != null) {
                ...
                wr.pos(offset, top + logoDims.height, zLevel)
                    .tex(0, 1)
                    .endVertex();
                wr.pos(offset + logoDims.width, top + logoDims.height, zLevel)
                    .tex(1, 1)
                    .endVertex();
                wr.pos(offset + logoDims.width, top, zLevel)
                    .tex(1, 0)
                    .endVertex();
                wr.pos(offset, top, zLevel)
                    .tex(0, 0)
                    .endVertex();
                tess.draw();
