...
import shordinger.wrapper.net.minecraftforge.fml.common.network.internal.NetworkModHolder;
...
public enum NetworkRegistry {
     ...
     * <ul>
     * <li>{@link #newSimpleChannel(String)} provides {@link SimpleNetworkWrapper}, a simple implementation of a netty
     * handler, suitable for those who don't
     * wish to dive too deeply into netty.
     * <li>{@link #newEventDrivenChannel(String)} (String)} provides {@link FMLEventChannel} an event driven
     * implementation, with lower level
     * access to the network data stream, for those with advanced bitbanging needs that don't wish to poke netty too
     * hard.
     ...
     * {@link FMLOutboundHandler#FML_MESSAGETARGET} and {@link FMLOutboundHandler#FML_MESSAGETARGETARGS} set on the
     * channel.
     * For the client to server channel (source side : CLIENT) this is fixed as "TOSERVER". For SERVER to CLIENT
     * packets,
    ...
    public void registerGuiHandler(Object mod, IGuiHandler handler) {
        ...
        if (mc == null) {
            FMLLog.log.error(
                "Mod of type {} attempted to register a gui network handler during a construction phase",
                mod.getClass()
                    .getName());
     ...
     */
    @Nullable
    public Object getLocalGuiContainer(ModContainer mc, EntityPlayer player, int modGuiId, World world, int x, int y,
                                       int z) {
     ...
     */
    public void register(ModContainer fmlModContainer, Class<?> clazz, @Nullable String remoteVersionRange,
                         ASMDataTable asmHarvestedData) {
        NetworkModHolder networkModHolder = new NetworkModHolder(
            fmlModContainer,
            clazz,
            remoteVersionRange,
            asmHarvestedData);
    ...
    public void fireNetworkHandshake(NetworkDispatcher networkDispatcher, Side origin) {
        NetworkHandshakeEstablished handshake = new NetworkHandshakeEstablished(
            networkDispatcher,
            networkDispatcher.getNetHandler(),
            origin);
        for (FMLEmbeddedChannel channel : channels.get(origin)
            .values()) {
            channel.attr(FMLOutboundHandler.FML_MESSAGETARGET)
                .set(OutboundTarget.DISPATCHER);
            channel.attr(FMLOutboundHandler.FML_MESSAGETARGETARGS)
                .set(networkDispatcher);
            channel.pipeline()
                .fireUserEventTriggered(handshake);
        }
    }
    ...
    public void cleanAttributes() {
        channels.values()
            .forEach(
                map -> map.values()
                    .forEach(FMLEmbeddedChannel::cleanAttributes));
    }
}
