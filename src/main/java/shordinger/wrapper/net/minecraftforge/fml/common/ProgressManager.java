...
public class ProgressManager {
    ...
    public static void pop(ProgressBar bar) {
        ...
        if (bar.getSteps() != 0) {
            ...
            else FMLLog.log.debug("Bar Finished: {} took {}s", bar.getTitle(), timeString);
        }
        FMLCommonHandler.instance()
            .processWindowMessages();
    }
    ...
    public static class ProgressBar {
        ...
        public void step(Class<?> classToName, String... extra) {
            step(
                ClassNameUtils.shortName(classToName) + Joiner.on(' ')
                    .join(extra));
        }
            ...
            if (timeEachStep && step != 0) {
                ...
                FMLLog.log.debug(
                    String.format(
                        ...
                        getTitle(),
                        getMessage(),
                        ((float) (newTime - lastTime) / 1000000 / 1000)));
                lastTime = newTime;
            }
            step++;
            this.message = FMLCommonHandler.instance()
