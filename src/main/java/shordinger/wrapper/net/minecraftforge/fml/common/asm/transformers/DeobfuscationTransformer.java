...
import shordinger.wrapper.net.minecraft.launchwrapper.IClassNameTransformer;
...
import shordinger.wrapper.net.minecraftforge.fml.common.asm.transformers.deobf.FMLRemappingAdapter;

public class DeobfuscationTransformer implements IClassTransformer, IClassNameTransformer {

    private static final String[] EXEMPT_LIBS = new String[]{"com.google.", "com.mojang.", "joptsimple.", "io.netty.",
        "it.unimi.dsi.fastutil.", "oshi.", "com.sun.", "com.ibm.", "paulscode.", "com.jcraft"};
    private static final String[] EXEMPT_DEV = new String[]{"net.minecraft.", "net.minecraftforge."};

    private static final boolean RECALC_FRAMES = Boolean
        .parseBoolean(System.getProperty("FORGE_FORCE_FRAME_RECALC", "false"));
    ...
    private boolean shouldTransform(String name) {
        boolean transformLib = Arrays.stream(EXEMPT_LIBS)
            .noneMatch(name::startsWith);

        if (deobfuscatedEnvironment) {
            return transformLib && Arrays.stream(EXEMPT_DEV)
                .noneMatch(name::startsWith);
        } else {
            return transformLib;
        }
