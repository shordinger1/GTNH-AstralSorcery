...
import shordinger.wrapper.net.minecraftforge.fml.common.Mod.Instance;
import shordinger.wrapper.net.minecraftforge.fml.common.Mod.Metadata;
import shordinger.wrapper.net.minecraftforge.fml.common.ModContainer.Disableable;
...
import shordinger.wrapper.net.minecraftforge.fml.common.discovery.ModCandidate;
...
import shordinger.wrapper.net.minecraftforge.fml.common.versioning.DependencyParser;
import shordinger.wrapper.net.minecraftforge.fml.common.versioning.VersionParser;
...
public class FMLModContainer implements ModContainer {
    ...
    public FMLModContainer(String className, ModCandidate container, Map<String, Object> modDescriptor) {
        ...
        if (Strings.isNullOrEmpty(languageAdapterType)) {
            this.languageAdapter = "scala".equals(modLanguage) ? new ILanguageAdapter.ScalaAdapter()
            ...
            FMLLog.log.trace(
                "Using custom language adapter {} for {} (modid: {})",
                languageAdapterType,
                this.className,
                getModId());
        }
    ...
    private void sanityCheckModId() {
        ...
        if (Strings.isNullOrEmpty(modid)) {
            ...
            if (Strings.isNullOrEmpty(modid)) {
                ...
                // This is a 1.13 mod
                FMLLog.bigWarning(
                    "The mod {} has been identified as a probable 1.13 or greater mod. Things will not work well.",
                    modid);
        ...
        if (modid.length() > 64) {
            throw new IllegalArgumentException(
                String.format("The modId %s is longer than the maximum of 64 characters.", modid));
        }
    ...
    private ILanguageAdapter getLanguageAdapter() {
        if (languageAdapter == null) {
            try {
                ...
                FMLLog.log.error(
                    "Error constructing custom mod language adapter referenced by {} (modid: {})",
                    this.className,
                    getModId(),
                    ex);
    ...
    public void bindMetadata(MetadataCollection mc) {
        ...
        if (overridesMetadata || !modMetadata.useDependencyInformation) {
            ...
            info.dependants.addAll(
                Loader.instance()
                    .getInjectedBefore(getModId()));
            info.dependencies.addAll(
                Loader.instance()
                    .getInjectedAfter(getModId()));
            ...
            FMLLog.log.trace(
                "Parsed dependency info for {}: Requirements: {} After:{} Before:{}",
                getModId(),
                info.requirements,
                info.dependencies,
                info.dependants);
            ...
            FMLLog.log.trace(
                "Using mcmod dependency info for {}: {} {} {}",
                getModId(),
        ...
        if (Strings.isNullOrEmpty(internalVersion)) {
            ...
            if (versionProps != null) {
                ...
                FMLLog.log
                    .debug("Found version {} for mod {} in version.properties, using", internalVersion, getModId());
            }

        }
        if (Strings.isNullOrEmpty(internalVersion) && !Strings.isNullOrEmpty(modMetadata.version)) {
            FMLLog.log.warn(
                "Mod {} is missing the required element 'version' and a version.properties file could not be found. Falling back to metadata version {}",
                getModId(),
                modMetadata.version);
        ...
        if (Strings.isNullOrEmpty(internalVersion)) {
            FMLLog.log.warn(
                "Mod {} is missing the required element 'version' and no fallback can be found. Substituting '1.0'.",
                getModId());
            modMetadata.version = internalVersion = "1.0";
        }

        String mcVersionString = (String) descriptor.get("acceptedMinecraftVersions");
        if ("[1.12]".equals(mcVersionString)) mcVersionString = "[1.12,1.12.2]";
        ...
        if (!Strings.isNullOrEmpty(mcVersionString)) {
            ...
            minecraftAccepted = Loader.instance()
    ...
    public Properties searchForVersionProperties() {
        try {
            FMLLog.log.debug(
                "Attempting to load the file version.properties from {} to locate a version number for mod {}",
                getSource().getName(),
                getModId());
            ...
            for (Annotation a : m.getAnnotations()) {
                if (a.annotationType()
                    ...
                    if (m.getParameterTypes().length == 1
                        ...
                        FMLLog.log.error(
                            "The mod {} appears to have an invalid event annotation {}. This annotation can only apply to methods with recognized event arguments - it will not be called",
                            getModId(),
                            a.annotationType()
                                .getSimpleName());
                    }
                } else if (a.annotationType()
                    .equals(Mod.InstanceFactory.class)) {
                    if (Modifier.isStatic(m.getModifiers()) && m.getParameterTypes().length == 0
                        ...
                        FMLLog.log.error(
                            "The InstanceFactory annotation can only apply to a static method, taking zero arguments - it will be ignored on {}({}) for mod {}",
                            m.getName(),
                            Arrays.asList(m.getParameterTypes()),
                            getModId());
                    } else if (factoryMethod != null) {
                        FMLLog.log.error(
                            "The InstanceFactory annotation can only be used once, the application to {}({}) will be ignored for mod {}",
                            m.getName(),
                            Arrays.asList(m.getParameterTypes()),
                            getModId());
                    }
    ...
    private void parseSimpleFieldAnnotation(SetMultimap<String, ASMData> annotations, String annotationClassName,
        ...
        for (ASMData targets : annotations.get(annotationClassName)) {
            String targetMod = (String) targets.getAnnotationInfo()
                .get("value");
            String owner = (String) targets.getAnnotationInfo()
                .get("owner");
            if (Strings.isNullOrEmpty(owner)) {
                owner = ASMDataTable.getOwnerModID(mods, targets);
                if (Strings.isNullOrEmpty(owner)) {
                    FMLLog.bigWarning(
                        "Could not determine owning mod for @{} on {} for mod {}",
                        annotationClassName,
            ...
            if (!this.getModId()
                .equals(owner)) {
                FMLLog.log.debug(
                    "Skipping @{} injection for {}.{} since it is not for mod {}",
                    annotationClassName,
                    targets.getClassName(),
            ...
            if (mc != null) {
                try {
                    ...
                    FMLLog.log.warn(
                        "Attempting to load @{} in class {} for {} and failing",
                        annotationName,
                        targets.getClassName(),
                        mc.getModId(),
                        e);
            ...
            if (f != null) {
                ...
                if (!isStatic) {
                    target = modInstance;
                    if (!modInstance.getClass()
                        .equals(clz)) {
                        FMLLog.log.warn(
                            "Unable to inject @{} in non-static field {}.{} for {} as it is NOT the primary mod instance",
                            annotationName,
                            targets.getClassName(),
                            targets.getObjectName(),
                            mc.getModId());
    ...
    public void constructMod(FMLConstructionEvent event) {
        ...
        try {
            ...
            FormattedMessage message = new FormattedMessage(
                "{} Failed to add file to classloader: {}",
                getModId(),
                source);
        ...
        Certificate[] certificates = clazz.getProtectionDomain()
            .getCodeSource()
            .getCertificates();
        ...
        if (expectedFingerprint != null && !expectedFingerprint.isEmpty()) {
            if (!sourceFingerprints.contains(expectedFingerprint)) {
                ...
                FMLLog.log.log(
                    warnLevel,
                    "The mod {} is expecting signature {} for source {}, however there is no signature matching that description",
                    getModId(),
                    expectedFingerprint,
                    source.getName());
            } else {
        ...
        boolean hasReverseDepends = !event.getReverseDependencies()
            .get(getModId())
            .isEmpty();
        ...
        NetworkRegistry.INSTANCE.register(
            this,
            clazz,
            (String) (descriptor.getOrDefault("acceptableRemoteVersions", null)),
    ...
    public Class<?> getCustomResourcePackClass() {
        try {
            return getSource().isDirectory()
                ? Class
                .forName("net.minecraftforge.fml.client.FMLFolderResourcePack", true, getClass().getClassLoader())
