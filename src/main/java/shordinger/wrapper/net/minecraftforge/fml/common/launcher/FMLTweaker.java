...
package shordinger.wrapper.net.minecraftforge.fml.common.launcher;
...
import shordinger.wrapper.net.minecraftforge.fml.relauncher.FMLSecurityManager;
...
public class FMLTweaker implements ITweaker {
    ...
    public void acceptOptions(List<String> args, File gameDir, File assetsDir, String profile) {
        ...
        for (String arg : args) {
            if (arg.startsWith("-")) {
                if (classifier != null) {
                    ...
                    classifier = launchArgs
                        .put(arg.substring(0, arg.indexOf('=')), arg.substring(arg.indexOf('=') + 1));
                } else {
                    classifier = arg;
                }
            } else {
        ...
        try {
            jarLocation = getClass().getProtectionDomain()
                .getCodeSource()
                .getLocation()
                .toURI();
        } catch (URISyntaxException e) {
            LogManager.getLogger("FML.TWEAK")
                .error("Missing URI information for FML tweak");
