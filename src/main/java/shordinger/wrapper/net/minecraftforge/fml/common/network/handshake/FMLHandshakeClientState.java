...
import shordinger.wrapper.net.minecraftforge.fml.common.network.handshake.FMLHandshakeMessage.ServerHello;
import shordinger.wrapper.net.minecraftforge.fml.common.network.internal.FMLMessage;
 ...
 * 2. Receive Server Hello. Send customchannel registration. Send Client Hello. Send our modlist. Move to
 * WAITINGFORSERVERDATA state.
 * 3. Receive server modlist. Send ack if acceptable, else send nack and exit error. Receive server IDs. Move to
 * COMPLETE state. Send ack.
 *
 * @author cpw
 */
enum FMLHandshakeClientState implements IHandshakeState<FMLHandshakeClientState> {
    START {
        @Override
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg,
                           Consumer<? super FMLHandshakeClientState> cons) {
            cons.accept(HELLO);
            NetworkDispatcher dispatcher = ctx.channel()
    ...
    HELLO {
        ...
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg,
            ...
            ctx.writeAndFlush(
                FMLHandshakeMessage
                    .makeCustomChannelRegistration(NetworkRegistry.INSTANCE.channelNamesFor(Side.CLIENT)));
            if (isVanilla) {
                NetworkDispatcher dispatcher = ctx.channel()
                    .attr(NetworkDispatcher.FML_DISPATCHER)
                    .get();
    ...
    WAITINGSERVERDATA {
        ...
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg,
            ...
            if (!ctx.channel()
                .attr(NetworkDispatcher.IS_LOCAL)
                .get()) {
    ...
    WAITINGSERVERCOMPLETE {
        ...
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg,
            ...
            Map<ResourceLocation, ForgeRegistry.Snapshot> snap = ctx.channel()
                .attr(NetworkDispatcher.FML_GAMEDATA_SNAPSHOT)
                .get();
            if (snap == null) {
            ...
            if (pkt.hasMore()) {
                cons.accept(WAITINGSERVERCOMPLETE);
                FMLLog.log.debug(
                    "Received Mod Registry mapping for {}: {} IDs {} overrides {} dummied",
                    pkt.getName(),
                    entry.ids.size(),
                    entry.overrides.size(),
                    entry.dummied.size());
                return;
            }
            ...
            // Do the remapping on the Client's thread in case things are reset while the client is running. We stall
            // the network thread until this is finished which can cause the IO thread to time out... Not sure if we can
            // do anything about that.
            final Map<ResourceLocation, ForgeRegistry.Snapshot> snap_f = snap;
            Multimap<ResourceLocation, ResourceLocation> locallyMissing = Futures.getUnchecked(
                Minecraft.getMinecraft()
                    .addScheduledTask(() -> GameData.injectSnapshot(snap_f, false, false)));
            if (!locallyMissing.isEmpty()) {
                ...
                FMLLog.log
                    .fatal("Failed to connect to server: there are {} missing registry items", locallyMissing.size());
                locallyMissing.asMap()
                    .forEach((key, value) -> FMLLog.log.debug("Missing {} Entries: {}", key, value));
                return;
            }
            cons.accept(PENDINGCOMPLETE);
            ctx.writeAndFlush(new FMLHandshakeMessage.HandshakeAck(ordinal()))
    ...
    DONE {
        ...
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg,
            ...
            if (msg instanceof FMLHandshakeMessage.HandshakeReset) {
                cons.accept(HELLO);
                // Run the revert on the client thread in case things are currently running to prevent race conditions
                // while rebuilding the registries.
                Minecraft.getMinecraft()
                    .addScheduledTask(GameData::revertToFrozen);
            }
        }
    },
    ERROR {
