...
public class SideTransformer implements IClassTransformer {
    ...
    private static String SIDE = FMLLaunchHandler.side()
        .name();
    ...
    public byte[] transform(String name, String transformedName, byte[] bytes) {
        ...
        if (remove(classNode.visibleAnnotations, SIDE)) {
            if (DEBUG) {
                System.out
                    .println(String.format("Attempted to load class %s for invalid side %s", classNode.name, SIDE));
            }
        ...
        while (methods.hasNext()) {
            ...
            if (remove(method.visibleAnnotations, SIDE)) {
                if (DEBUG) {
                    System.out
                        .println(String.format("Removing Method: %s.%s%s", classNode.name, method.name, method.desc));
                }
                methods.remove();
