...
public class FieldRedirectTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        if (!this.clsName.equals(transformedName)) return basicClass;
        ...
        if (fieldRef == null) {
            throw new RuntimeException(
                "Error processing " + clsName + " - no holder field declared (is the code somehow obfuscated?)");
        }
            ...
            for (ListIterator<AbstractInsnNode> it = m.instructions.iterator(); it.hasNext(); ) {
                ...
                if (insnNode.getType() == AbstractInsnNode.FIELD_INSN) {
                    ...
                    if (fieldRef.name.equals(fi.name) && fi.getOpcode() == Opcodes.GETFIELD) {
                        it.remove();
                        MethodInsnNode replace = new MethodInsnNode(
