...
public class ModAccessTransformer extends AccessTransformer {
    ...
    private static Map<String, String> embedded = Maps.newHashMap(); // Needs to be primitive so that both classloaders
    // get the same class.
    ...
    public ModAccessTransformer() throws Exception {
        ...
        ClassLoader classLoader = this.getClass()
            .getClassLoader()
            .getClass()
            .getClassLoader(); // Bit odd but it gets the class loader that loaded our current class loader yay java!
        Class<?> otherClazz = Class.forName(
            this.getClass()
                .getName(),
            true,
            classLoader);
        ...
        for (String at : atList.split(" ")) {
            ...
            if (jarEntry != null) {
                embedded.put(
                    ...
                    new JarByteSource(jar, jarEntry).asCharSource(StandardCharsets.UTF_8)
                        .read());
            }
        }
    }
