...
public class FMLProxyPacket implements Packet<INetHandler> {
    ...
    public void processPacket(INetHandler inethandler) {
        ...
        if (internalChannel != null) {
            internalChannel.attr(NetworkRegistry.NET_HANDLER)
                .set(this.netHandler);
            try {
                if (internalChannel.writeInbound(this)) {
                    ...
                    if (badPackets.size() % packetCountWarning == 0) {
                        FMLLog.log.fatal(
                            "Detected ongoing potential memory leak. {} packets have leaked. Top offenders",
                            badPackets.size());
                        int i = 0;
                ...
                internalChannel.inboundMessages()
                    .clear();
