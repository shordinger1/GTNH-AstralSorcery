...
public class LinkRepository extends Repository {
    ...
    public File archive(Artifact artifact, File file, byte[] manifest) {
        ...
        if (artifact_to_file.containsKey(key)) {
            FMLLog.log.debug(
                "Maven file already exists for {}({}) at {}, ignoring duplicate.",
                file.getName(),
                artifact.toString(),
                artifact_to_file.get(key)
                    .getAbsolutePath());
            ...
            if (artifact.isSnapshot()) {
                ...
                if (old == null || old.compareVersion(artifact) < 0) {
                    FMLLog.log.debug(
                        "Overriding Snapshot {} -> {}",
                        old == null ? "null" : old.getTimestamp(),
            ...
            if (artifact.isSnapshot()) snapshots.put(key, artifact);
    ...
    public Artifact resolve(Artifact artifact) {
        ...
        if (file == null || !file.exists()) return super.resolve(artifact);
