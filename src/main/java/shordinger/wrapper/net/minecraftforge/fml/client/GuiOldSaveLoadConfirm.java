...
public class GuiOldSaveLoadConfirm extends GuiYesNo implements GuiYesNoCallback {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawCenteredString(
            this.fontRenderer,
            String.format("The world %s contains pre-update modding data", saveName),
            this.width / 2,
            50,
            16777215);
        this.drawCenteredString(
            this.fontRenderer,
            String.format("There may be problems updating it to this version"),
            this.width / 2,
            70,
            16777215);
        this.drawCenteredString(
        ...
        this.drawCenteredString(
            this.fontRenderer,
            String.format("Do you wish to continue loading?"),
            this.width / 2,
            110,
            16777215);
        int k;
        ...
        for (k = 0; k < this.buttonList.size(); ++k) {
            this.buttonList.get(k)
                .drawButton(this.mc, mouseX, mouseY, partialTicks);
        }

        for (k = 0; k < this.labelList.size(); ++k) {
            this.labelList.get(k)
                .drawLabel(this.mc, mouseX, mouseY);
        }
    }
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 1) {
            ...
            try {
                ...
                FMLClientHandler.instance()
                    .showGuiScreen(new GuiBackupFailed(parent, zip));
                return;
            }
            FMLClientHandler.instance()
                .showGuiScreen(null);
