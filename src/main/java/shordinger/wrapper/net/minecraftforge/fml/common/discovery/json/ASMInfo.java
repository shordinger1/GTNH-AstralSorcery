...
package shordinger.wrapper.net.minecraftforge.fml.common.discovery.json;
...
class ASMInfo {
    ...
    public enum ValueType {
        ...
        private static EnumHolder getEnumHolder(String value) {
            ...
            if (idx <= 1) throw new IllegalArgumentException("Can not create a EnumHolder for value: " + value);
    ...
    static class Annotation {
        ...
        public Map<String, Object> getValues(ASMInfo pool) {
            if (_values == null) {
                ...
                if (values != null) values.forEach((k, v) -> _values.put(k, v.get(pool)));
                else _values.put("value", value == null ? null : value.get(pool));
            }
                ...
                if (values != null) {
                    if (type == ValueType.ANNOTATION) {
                        ...
                        for (String s : values) {
                            ...
                            if (sub == null) FMLLog.log.error("Invalid Sub-Annotation in Annotation JSON: " + s);
                            else list.add(sub.getValues(pool));
                        }
                    } else _value = getType().array.apply(values);
                } else {
                    if (type == ValueType.ANNOTATION) {
                        Annotation sub = pool.getSubAnnotation(Integer.parseInt(value));
                        if (sub == null) FMLLog.log.error("Invalid Sub-Annotation in Annotation JSON: " + value);
                        else _value = sub.getValues(pool);
                    } else _value = getType().single.apply(value);
                }
            }
            return _value;
        }
