...
import shordinger.wrapper.net.minecraft.client.gui.GuiWorldSelection;
import shordinger.wrapper.net.minecraft.client.gui.ScaledResolution;
import shordinger.wrapper.net.minecraft.client.gui.ServerListEntryNormal;
import shordinger.wrapper.net.minecraft.client.multiplayer.GuiConnecting;
...
import shordinger.wrapper.net.minecraft.client.network.ServerPinger;
import shordinger.wrapper.net.minecraft.client.renderer.GlStateManager;
import shordinger.wrapper.net.minecraft.client.resources.AbstractResourcePack;
import shordinger.wrapper.net.minecraft.client.resources.FallbackResourceManager;
...
import shordinger.wrapper.net.minecraft.client.resources.LegacyV2Adapter;
...
import shordinger.wrapper.net.minecraft.network.ServerStatusResponse;
...
import shordinger.wrapper.net.minecraft.world.storage.SaveFormatOld;
import shordinger.wrapper.net.minecraft.world.storage.WorldSummary;
import shordinger.wrapper.net.minecraftforge.client.CloudRenderer;
...
import shordinger.wrapper.net.minecraftforge.client.resource.ReloadRequirements;
...
import shordinger.wrapper.net.minecraftforge.common.config.ConfigManager;
...
import shordinger.wrapper.net.minecraftforge.fml.common.DuplicateModsFoundException;
...
import shordinger.wrapper.net.minecraftforge.fml.common.IFMLSidedHandler;
import shordinger.wrapper.net.minecraftforge.fml.common.Loader;
import shordinger.wrapper.net.minecraftforge.fml.common.LoaderException;
import shordinger.wrapper.net.minecraftforge.fml.common.MetadataCollection;
...
import shordinger.wrapper.net.minecraftforge.fml.common.ModMetadata;
import shordinger.wrapper.net.minecraftforge.fml.common.MultipleModsErrored;
...
import shordinger.wrapper.net.minecraftforge.fml.common.WrongMinecraftVersionException;
...
import shordinger.wrapper.net.minecraftforge.fml.common.network.internal.FMLNetworkHandler;
import shordinger.wrapper.net.minecraftforge.fml.common.toposort.ModSortingException;
 ...
 * {@link #beginMinecraftLoading(Minecraft, List, IReloadableResourceManager, MetadataSerializer)} called from
 * {@link Minecraft}
 * <p>
...
public class FMLClientHandler implements IFMLSidedHandler {
    ...
    @Deprecated // TODO remove in 1.13. mods are referencing this to get around client-only dependencies in old Forge
    // versions
     ...
     */
    public void beginMinecraftLoading(Minecraft minecraft, List<IResourcePack> resourcePackList,
                                      IReloadableResourceManager resourceManager, MetadataSerializer metaSerializer) {
        detectOptifine();
        ...
        List<String> injectedModContainers = FMLCommonHandler.instance()
            .beginLoading(this);
        try {
            Loader.instance()
                .loadMods(injectedModContainers);
        ...
        Map<String, Map<String, String>> sharedModList = (Map<String, Map<String, String>>) Launch.blackboard
            .get("modList");
    ...
    private void detectOptifine() {
        try {
            Class<?> optifineConfig = Class.forName(
                "Config",
                false,
                Loader.instance()
                    .getModClassLoader());
            String optifineVersion = (String) optifineConfig.getField("VERSION")
                .get(null);
            Map<String, Object> dummyOptifineMeta = ImmutableMap.<String, Object>builder()
                .put("name", "Optifine")
            ...
            try (InputStream optifineModInfoInputStream = getClass().getResourceAsStream("optifinemod.info")) {
                ...
                FMLLog.log.info(
                    "Forge Mod Loader has detected optifine {}, enabling compatibility features",
    ...
    public void finishMinecraftLoading() {
        ...
        RenderingRegistry.loadEntityRenderers(
            Minecraft.getMinecraft()
                .getRenderManager().entityRenderMap);
        ...
        for (ModContainer mc : Loader.instance()
            ...
            try {
                ...
                FMLLog.log
                    .error("A critical error occurred instantiating the gui factory for mod {}", mc.getModId(), e);
            }
        }
        loading = false;
        client.gameSettings.loadOptions(); // Reload options to load any mod added keybindings.
        if (!hasError()) Loader.instance()
            .loadingComplete();
        SplashProgress.finish();
    }
    ...
    public void extendModList() {
        ...
        if (modList != null) {
            for (Entry<String, Map<String, String>> modEntry : modList.entrySet()) {
                 ...
                 * String modDescription = mod.get("description"); // a (potentially) multiline description (can be
                 * empty string)
                 */
            }
        }
    ...
    public boolean isGUIOpen(Class<? extends GuiScreen> gui) {
        return client.currentScreen != null && client.currentScreen.getClass()
            .equals(gui);
    }
    ...
    public void addModAsResource(ModContainer container) {
        ...
        if (resourcePackType != null) {
            try {
                IResourcePack pack = (IResourcePack) resourcePackType.getConstructor(ModContainer.class)
                ...
                FMLLog.log.error(
                    "The container {} (type {}) returned an invalid class for its resource pack.",
                    container.getName(),
                    container.getClass()
                        .getName());
            } catch (Exception e) {
                FormattedMessage message = new FormattedMessage(
                    "An unexpected exception occurred constructing the custom resource pack for {} ({})",
                    container.getName(),
                    container.getModId());
    ...
    public NetworkManager getClientToServerNetworkManager() {
        return this.client.getConnection() != null ? this.client.getConnection()
    ...
    public void tryLoadExistingWorld(GuiWorldSelection selectWorldGUI, WorldSummary comparator) {
        ...
        try {
            ...
            try {
                ...
                FMLLog.log.warn(
                    "There appears to be a problem loading the save {}, both level files are unreadable.",
                    comparator.getFileName());
                return;
            }
        }
        ...
        if (fmlData.hasKey("ModItemData")) {
            showGuiScreen(
                new GuiOldSaveLoadConfirm(comparator.getFileName(), comparator.getDisplayName(), selectWorldGUI));
        } else {
            try {
    ...
    public void captureAdditionalData(ServerStatusResponse serverstatusresponse, JsonObject jsonobject) {
        if (jsonobject.has("modinfo")) {
            JsonObject fmlData = jsonobject.get("modinfo")
                .getAsJsonObject();
    ...
    public void bindServerListData(ServerData data, ServerStatusResponse originalResponse) {
        if (extraServerListData.containsKey(originalResponse)) {
            ...
            String type = jsonData.get("type")
                .getAsString();
            JsonArray modDataArray = jsonData.get("modList")
                .getAsJsonArray();
            ...
            serverDataTag
                .put(data, new ExtendedServerListData(type, modRejections == null, modListMap, !moddedClientAllowed));
        } else {
            ...
            serverDataTag
                .put(data, new ExtendedServerListData("VANILLA", false, ImmutableMap.of(), !moddedClientAllowed));
        }
    ...
    public String fixDescription(String description) {
        return description.endsWith(":NOFML??r") ? description.substring(0, description.length() - 8) + "??r"
            : description;
    }
    ...
    @Override
    public void fireNetRegistrationEvent(EventBus bus, NetworkManager manager, Set<String> channelSet, String channel,
                                         Side side) {
        if (side == Side.CLIENT) {
            bus.post(
                new FMLNetworkEvent.CustomPacketRegistrationEvent<NetHandlerPlayClient>(
                    manager,
                    channelSet,
                    channel,
                    side,
            ...
            bus.post(
                new FMLNetworkEvent.CustomPacketRegistrationEvent<NetHandlerPlayServer>(
                    manager,
                    channelSet,
                    channel,
                    side,
    ...
    public IThreadListener getWorldThread(INetHandler net) {
        if (net instanceof INetHandlerPlayClient || net instanceof INetHandlerLoginClient
            || net instanceof INetHandlerStatusClient) return getClient();
        if (net instanceof INetHandlerHandshakeServer || net instanceof INetHandlerLoginServer
    ...
    public void trackBrokenTexture(ResourceLocation resourceLocation, String error) {
        ...
        if (badType == null) {
            ...
            brokenTextures
                .put(resourceLocation.getResourceDomain(), MoreObjects.firstNonNull(error, "Unknown error"), badType);
        }
    ...
    public void logMissingTextureErrors() {
        ...
        Map<String, FallbackResourceManager> resManagers = ObfuscationReflectionHelper.getPrivateValue(
        ...
        for (String resourceDomain : badTextureDomains) {
            ...
            if (fallbackResourceManager == null) {
                ...
                List<IResourcePack> resPacks = ObfuscationReflectionHelper
                    .getPrivateValue(FallbackResourceManager.class, fallbackResourceManager, "field_110540" + "_a");
                logger.error(
                    "    domain {} has {} location{}:",
                    resourceDomain,
                    resPacks.size(),
                ...
                for (IResourcePack resPack : resPacks) {
                    if (resPack instanceof FMLContainerHolder) {
                        ...
                        logger.error(
                            "      mod {} resources at {}",
                            fmlContainer.getModId(),
                            fmlContainer.getSource()
                                .getPath());
                    } else if (resPack instanceof AbstractResourcePack) {
                        AbstractResourcePack resourcePack = (AbstractResourcePack) resPack;
                        File resPath = ObfuscationReflectionHelper
                            .getPrivateValue(AbstractResourcePack.class, resourcePack, "field_110597" + "_b");
    ...
    private static final CharMatcher DISALLOWED_CHAR_MATCHER = CharMatcher.anyOf(ALLOWED_CHARS)
        .negate();
    ...
    public void reloadSearchTrees() {
        ...
        this.client.getSearchTreeManager()
            .onResourceManagerReload(this.client.getResourceManager());
    }
    ...
    private CloudRenderer getCloudRenderer() {
        if (cloudRenderer == null) cloudRenderer = new CloudRenderer();
