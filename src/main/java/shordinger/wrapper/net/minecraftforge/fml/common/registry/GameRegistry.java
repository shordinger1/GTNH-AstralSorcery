...
import shordinger.wrapper.net.minecraftforge.fml.common.IEntitySelectorFactory;
import shordinger.wrapper.net.minecraftforge.fml.common.IFuelHandler;
...
public class GameRegistry {
    ...
    /**
     * Registers a entity selector factory which is used to create predicates whenever a command containing selectors is
     * executed
     * Any non vanilla arguments that you expect has to be registered. Otherwise Minecraft will throw an
     * CommandException on usage.
     * <p>
     * If you want to react to a command like "/kill @e[xyz=5]", you would have to register the argument "xyz" here and
     * check for that argument in the factory.
     ...
     */
    public static List<Predicate<Entity>> createEntitySelectors(Map<String, String> arguments, String mainSelector,
                                                                ICommandSender sender, Vec3d position) {
        ...
        for (IEntitySelectorFactory factory : entitySelectorFactories) {
            try {
                ...
                FMLLog.log.error(
                    "Exception caught during entity selector creation with {} for argument map {} of {} for {} at {}",
                    factory,
                    arguments,
                    mainSelector,
                    sender,
     ...
     */
    public static void generateWorld(int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator,
    ...
    public static void addShapedRecipe(ResourceLocation name, ResourceLocation group, @Nonnull ItemStack output,
        ...
        register(
            new ShapedRecipes(group == null ? "" : group.toString(), primer.width, primer.height, primer.input, output)
                .setRegistryName(name));
    }

    public static void addShapelessRecipe(ResourceLocation name, ResourceLocation group, @Nonnull ItemStack output,
                                          Ingredient... params) {
        NonNullList<Ingredient> lst = NonNullList.create();
        for (Ingredient i : params) lst.add(i);
    ...
    public static void addSmelting(Block input, @Nonnull ItemStack output, float xp) {
        FurnaceRecipes.instance()
            .addSmeltingRecipeForBlock(input, output, xp);
    }
    ...
    public static void addSmelting(@Nonnull ItemStack input, @Nonnull ItemStack output, float xp) {
        FurnaceRecipes.instance()
            .addSmeltingRecipe(input, output, xp);
    }
    ...
    /**
     * @deprecated set your item's {@link Item#getItemBurnTime(ItemStack)} or subscribe to
     * {@link FurnaceFuelBurnTimeEvent} instead.
     */
    @Deprecated
