...
public class ModAPIManager {
    ...
    private static class APIContainer extends DummyModContainer {
        ...
        public void validate(String providedAPI, String apiOwner, String apiVersion) {
            if (Loader.instance()
                .getModClassLoader()
                .containsSource(this.getSource())) {
                FMLLog.bigWarning(
                    "The API {} from source {} is loaded from an incompatible classloader. THIS WILL NOT WORK!",
                    providedAPI,
                    this.getSource()
                        .getAbsolutePath());
            }
    ...
    public void registerDataTableAndParseAPI(ASMDataTable dataTable) {
        ...
        for (ASMData data : apiList) {
            ...
            String apiPackage = data.getClassName()
                .substring(
                    0,
                    data.getClassName()
                        .indexOf(".package-info"));
            ...
            for (ModContainer mc : data.getCandidate()
                .getContainedMods()) {
                ...
                FMLLog.log.debug(
                    "Found API {} (owned by {} providing {}) embedded in {}",
                    apiPackage,
                    apiOwner,
                    providedAPI,
                    embeddedIn);
        ...
        for (APIContainer container : apiContainers.values()) {
            for (String pkg : container.packages) {
                ...
                for (ModCandidate candidate : candidates) {
                    List<String> candidateIds = candidate.getContainedMods()
                        .stream()
                    ...
                    if (!candidateIds.contains(container.ownerMod.getLabel())
                        ...
                        FMLLog.log.info(
                            "Found mod(s) {} containing declared API package {} (owned by {}) without associated API reference",
                            candidateIds,
                            pkg,
                            container.ownerMod);
            ...
            FMLLog.log.debug(
                "Creating API container dummy for API {}: owner: {}, dependents: {}",
                container.providedAPI,
                container.ownerMod,
                container.referredMods);
        }
    }
