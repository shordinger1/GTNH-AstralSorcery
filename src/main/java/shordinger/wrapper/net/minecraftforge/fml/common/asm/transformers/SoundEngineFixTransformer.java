...
package shordinger.wrapper.net.minecraftforge.fml.common.asm.transformers;
...
public class SoundEngineFixTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        if (transformedName.equals("paulscode.sound.Source")) {
            ...
            classNode.fields.add(new FieldNode(Opcodes.ACC_PUBLIC, "removed", "Z", null, null)); // adding field 'public
            // boolean removed;'
            ...
            for (MethodNode m : classNode.methods) {
                if (m.name.equals("removeSource") && m.desc.equals("(Ljava/lang/String;)V")) // trying to find
                // paulscode.sound.Library.removeSource(String)
                {
                    method = m;
                    break;
                }
            ...
            for (Iterator<AbstractInsnNode> iterator = method.instructions.iterator(); iterator.hasNext(); ) {
                ...
                if (insn instanceof MethodInsnNode && ((MethodInsnNode) insn).owner.equals("paulscode/sound/Source") // searching
                    // for
                    // mySource.cleanup()
                    // node
                    // (line
            ...
            if (referenceNode != null) {
                ...
                method.instructions.insertBefore(beginning, new VarInsnNode(Opcodes.ALOAD, varIndex)); // adding extra
                // if
                // (mySource.toStream)
                ...
                method.instructions.insertBefore(beginning, new JumpInsnNode(Opcodes.IFEQ, elseNode)); // if fails
                // (else) -> go
                // to
                // mySource.cleanup();

                method.instructions.insertBefore(beginning, new VarInsnNode(Opcodes.ALOAD, varIndex)); // if
                // (mySource.toStream)
                // {
                // mySource.removed
                // = true; }
                ...
                method.instructions.insertBefore(beginning, new JumpInsnNode(Opcodes.GOTO, after)); // still inside if
                // -> jump to
                // sourceMap.remove(
                // sourcename );
            ...
            for (Iterator<AbstractInsnNode> iterator = method.instructions.iterator(); iterator.hasNext(); ) {
                AbstractInsnNode insn = iterator.next();
                if (insn instanceof MethodInsnNode && ((MethodInsnNode) insn).owner.equals("java/util/ListIterator") // searching
                    // for
                    // 'src
                    // =
                    // iter.next();'
            ...
            if (referenceNode != null) {
                ...
                method.instructions
                    .insertBefore(after, new FieldInsnNode(Opcodes.GETFIELD, "paulscode/sound/Source", "removed", "Z"));
                method.instructions.insertBefore(after, new JumpInsnNode(Opcodes.IFEQ, after));

                // if the source has been marked as removed, clean it up and set the variable to null so it will be
                // removed from the list
                method.instructions.insertBefore(after, new VarInsnNode(Opcodes.ALOAD, varIndex)); // src.cleanup();
                method.instructions.insertBefore(
                    after,
                    new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "paulscode/sound/Source", "cleanup", "()V", false));
