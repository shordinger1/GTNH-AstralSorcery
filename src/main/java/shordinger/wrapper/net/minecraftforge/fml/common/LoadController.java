...
import shordinger.wrapper.net.minecraftforge.common.util.TextTable;
...
import shordinger.wrapper.net.minecraftforge.fml.common.event.FMLModDisabledEvent;
...
import shordinger.wrapper.net.minecraftforge.fml.common.eventhandler.FMLThrowingEventBus;
...
public class LoadController {
    ...
    private Multimap<String, ModState> modStates = MultimapBuilder.hashKeys()
        .enumSetValues(ModState.class)
        .build();
    ...
    public LoadController(Loader loader) {
        ...
        this.masterChannel = new FMLThrowingEventBus((exception, context) -> {
            ...
            String parameterNames = Stream.of(method.getParameterTypes())
                .map(Class::getName)
        ...
        if (state != desiredState) {
            if (!forceState) {
                FormattedMessage message = new FormattedMessage(
                    "A fatal error occurred during the state transition from {} to {}. State became {} instead. Loading cannot continue.",
                    oldState,
                    desiredState,
                    state);
                ...
                FMLLog.log.info(
                    "The state engine was in incorrect state {} and forced into state {}. Errors may have been discarded.",
                    state,
                    desiredState);
    ...
    private void sendEventToModContainer(FMLEvent stateEvent, ModContainer mc) {
        ...
        Collection<String> requirements = mc.getRequirements()
            .stream()
        ...
        for (ArtifactVersion av : mc.getDependencies()) {
            if (av.getLabel() != null && requirements.contains(av.getLabel())
                ...
                FMLLog.log.error(
                    "Skipping event {} and marking errored mod {} since required dependency {} has errored",
    ...
    public ImmutableBiMap<ModContainer, Object> buildModObjectList() {
        ...
        for (ModContainer mc : activeModList) {
            ...
            if (mc.getMod() == null && !mc.isImmutable() && state != LoaderState.CONSTRUCTING) {
                FormattedMessage message = new FormattedMessage(
                    "There is a severe problem with {} ({}) - it appears not to have constructed correctly",
                    mc.getName(),
                    mc.getModId());
    ...
    public void printModStates(StringBuilder ret) {
        ...
        for (ModContainer mc : loader.getModList()) {
            table.add(
                modStates.get(mc.getModId())
                    .stream()
                    .map(ModState::getMarker)
                    .reduce("", (a, b) -> a + b),
                mc.getModId(),
                mc.getVersion(),
                mc.getSource()
                    .getName(),
                mc.getSigningCertificate() != null ? CertificateHelper.getFingerprint(mc.getSigningCertificate())
                    : "None");
        }
    ...
    public BiMap<ModContainer, Object> getModObjectList() {
        if (modObjectList == null) {
            FMLLog.log.fatal(
                "Detected an attempt by a mod {} to perform game activity during mod construction. This is a serious programming error.",
                activeContainer);
