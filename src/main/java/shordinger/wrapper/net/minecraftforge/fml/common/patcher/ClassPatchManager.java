...
package shordinger.wrapper.net.minecraftforge.fml.common.patcher;
...
import shordinger.wrapper.net.minecraftforge.fml.repackage.com.nothome.delta.GDiffPatcher;
...
public class ClassPatchManager {
    ...
    public byte[] applyPatch(String name, String mappedName, byte[] inputData) {
        ...
        if (DEBUG) FMLLog.log.debug(
            "Runtime patching class {} (input size {}), found {} patch{}",
            mappedName,
        ...
        for (ClassPatch patch : list) {
            ...
            if (!patch.existsAtTarget && (inputData == null || inputData.length == 0)) {
                ...
                FMLLog.log.warn(
                    "Patcher expecting empty class data file for {}, but received non-empty",
                    patch.targetClassName);
            } else if (patch.existsAtTarget && (inputData == null || inputData.length == 0)) {
                FMLLog.log.fatal(
                    "Patcher expecting non-empty class data file for {}, but received empty.",
                    patch.targetClassName);
                throw new RuntimeException(
                    String.format(
                        "Patcher expecting non-empty class data file for %s, but received empty, your vanilla jar may be corrupt.",
                        patch.targetClassName));
            } else {
                ...
                if (patch.inputChecksum != inputChecksum) {
                    FMLLog.log.fatal(
                        "There is a binary discrepancy between the expected input class {} ({}) and the actual class. Checksum on disk is {}, in patch {}. Things are probably about to go very wrong. Did you put something into the jar file?",
                        mappedName,
                        name,
                        Integer.toHexString(inputChecksum),
                        Integer.toHexString(patch.inputChecksum));
            ...
            try {
                ...
                FMLLog.log.error(
                    FMLLog.log.getMessageFactory()
                        .newMessage("Failed to write {} to {}", mappedName, tempDir.getAbsolutePath()),
                    e);
            }
        }
            ...
            try {
                ...
                try (LzmaInputStream binpatchesDecompressed = new LzmaInputStream(binpatchesCompressed)) {
                    ...
                    try (JarOutputStream jos = new JarOutputStream(jarBytes)) {
                        Pack200.newUnpacker()
                            .unpack(binpatchesDecompressed, jos);
    ...
    private ClassPatch readPatch(JarEntry patchEntry, JarInputStream jis) {
        if (DEBUG) FMLLog.log.trace("Reading patch data from {}", patchEntry.getName());
        ...
        try {
            ...
            FMLLog.log.warn(
                FMLLog.log.getMessageFactory()
                    .newMessage("Unable to read binpatch file {} - ignoring", patchEntry.getName()),
                e);
            return null;
        }
