...
public class ByteBufUtils {
    ...
    /**
     * Read a registry entry from the stream. The same format as in
     * {@link #writeRegistryEntry(ByteBuf, IForgeRegistryEntry)} is used.
     *
     ...
     */
    @Nonnull
    public static <T extends IForgeRegistryEntry<T>> T readRegistryEntry(@Nonnull ByteBuf in,
        ...
        if (!expectedRegistryName.toString()
            .equals(registryName))
        ...
        if (thing == null) throw new IllegalArgumentException(
            "Unknown ID " + id + " for registry " + expectedRegistryName + " received.");

        return thing;
    }

    /**
     * Write multiple registry entries from the same registry to the stream. The serialized format may be more compact
     * than using
     ...
     */
    public static <T extends IForgeRegistryEntry<T>> void writeRegistryEntries(@Nonnull ByteBuf out,
                                                                               @Nonnull Collection<T> entries) {
        ...
        if (it.hasNext()) {
            ...
            while (it.hasNext()) {
                ...
                if (id == -1) throw new IllegalArgumentException(
                    "Unregistered IForgeRegistryEntry in collection " + entries + ".");
    ...
    /**
     * Read multiple registry entries from the same registries from the stream. The list of entries must have been
     * written by
     ...
     */
    @Nonnull
    public static <T extends IForgeRegistryEntry<T>> List<T> readRegistryEntries(@Nonnull ByteBuf in,
                                                                                 @Nonnull IForgeRegistry<T> registry) {
        ...
        if (size == 0) {
            ...
            if (!expectedRegistryName.toString()
                ...
                throw new IllegalArgumentException(
                    "Registry mismatch: " + registryName + " != " + expectedRegistryName);
            }
    ...
    public static String getContentDump(ByteBuf buffer) {
        ...
        for (i = 0; i < currentLength; i++) {
            ...
            returnString.append(
                Integer.toString((buffer.getByte(i) & 0xF0) >> 4, 16)
                    + Integer.toString((buffer.getByte(i) & 0x0F) >> 0, 16));
            returnString.append(' ');
        }
