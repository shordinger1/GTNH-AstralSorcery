...
public class ASMDataTable {
    ...
    public final static class ASMData implements Cloneable {
        ...
        public ASMData(ModCandidate candidate, String annotationName, String className, @Nullable String objectName,
    ...
    private static class ModContainerPredicate implements Predicate<ASMData> {
        ...
        public boolean apply(ASMData data) {
            return container.getSource()
                .equals(data.candidate.getModContainer());
        }
    }
                ...
                .map(
                    cont -> Pair.of(
                        ...
                        ImmutableSetMultimap
                            .copyOf(Multimaps.filterValues(globalAnnotationData, new ModContainerPredicate(cont)))))
    ...
    public void addASMData(ModCandidate candidate, String annotation, String className, @Nullable String objectName,
                           @Nullable Map<String, Object> annotationInfo) {
    ...
    public static String getOwnerModID(Set<ASMData> mods, ASMData targ) {
        if (mods.size() == 1) {
            return (String) mods.iterator()
                .next()
                .getAnnotationInfo()
                .get("modid");
        } else {
            for (ASMData m : mods) {
                if (targ.getClassName()
                    .startsWith(m.getClassName())) {
                    return (String) m.getAnnotationInfo()
                        .get("modid");
                }
            }
        }
