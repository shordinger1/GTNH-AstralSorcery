...
import shordinger.wrapper.net.minecraftforge.fml.common.discovery.json.ASMInfo.Annotation;
import shordinger.wrapper.net.minecraftforge.fml.common.discovery.json.ASMInfo.TargetType;
...
public class JsonAnnotationLoader {
    ...
    public static Multimap<String, ASMData> loadJson(InputStream data, ModCandidate candidate, ASMDataTable table) {
        ...
        for (Entry<String, ASMInfo> entry : map.entrySet()) {
            // TODO: Java9 Multi-Release Jars, picking the correct class for the current platform. For now we just
            // ignore them.
            if (entry.getKey()
                .startsWith("META-INF/")) continue;
            // TODO: Remove in 1.13, some older mods have these in the entries due to FG issue. Basically filter out
            // scala synthetic class.
            if (entry.getKey()
                .endsWith("$")) continue;
            ...
            if (asm_info.annotations != null) {
                for (Annotation anno : asm_info.annotations) {
                    String name = anno.name.indexOf(';') > 0 ? Type.getType(anno.name)
                        .getClassName() : anno.name;
