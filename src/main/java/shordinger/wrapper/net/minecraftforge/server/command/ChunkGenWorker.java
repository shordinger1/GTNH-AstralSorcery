...
import shordinger.wrapper.net.minecraft.server.management.PlayerChunkMapEntry;
...
import shordinger.wrapper.net.minecraftforge.common.WorldWorkerManager.IWorker;
...
public class ChunkGenWorker implements IWorker {
    ...
    public ChunkGenWorker(ICommandSender listener, BlockPos start, int total, int dim, int interval) {
        ...
        this.notificationFrequency = interval != -1 ? interval : Math.max(total / 20, 100); // Every 5% or every 100,
        // whichever is more.
        this.lastNotifcationTime = System.currentTimeMillis(); // We also notify at least once every 60 seconds, to show
        // we haven't froze.
    }
    ...
    protected Queue<BlockPos> buildQueue() {
        ...
        while (ret.size() < total) {
            for (int q = -radius + 1; q <= radius && ret.size() < total; q++) ret.add(start.add(radius, 0, q));

            for (int q = radius - 1; q >= -radius && ret.size() < total; q--) ret.add(start.add(q, 0, radius));
    ...
    public TextComponentBase getStartMessage(ICommandSender sender) {
        return TextComponentHelper
            .createComponentTranslation(sender, "commands.forge.gen.start", total, start.getX(), start.getZ(), dim);
    }
    ...
    public boolean doWork() {
        ...
        if (world == null) {
            ...
            if (world == null) {
                listener.sendMessage(
                    TextComponentHelper.createComponentTranslation(listener, "commands.forge.gen.dim_fail", dim));
                queue.clear();
                return false;
            }
        }

        AnvilChunkLoader loader = world.getChunkProvider().chunkLoader instanceof AnvilChunkLoader
            ? (AnvilChunkLoader) world.getChunkProvider().chunkLoader
            : null;
        ...
        if (next != null) {
            ...
            if (++lastNotification >= notificationFrequency
                || lastNotifcationTime < System.currentTimeMillis() - 60 * 1000) {
                listener.sendMessage(
                    TextComponentHelper.createComponentTranslation(
                        listener,
                        "commands.forge.gen.progress",
            ...
            if (!target.isTerrainPopulated()) {
                ...
                chunks = new Chunk[]{target, world.getChunkFromChunkCoords(x + 1, z),
                    world.getChunkFromChunkCoords(x + 1, z + 1), world.getChunkFromChunkCoords(x, z + 1),};
                try {
                    world.getChunkProvider().chunkLoader.saveChunk(world, target);
                } catch (IOException | MinecraftException e) {
                    listener.sendMessage(
                        TextComponentHelper
                            .createComponentTranslation(listener, "commands.forge.gen.saveerror", e.getMessage()));
                }
                genned++;
            }

            for (Chunk chunk : chunks) // Now lets unload them. Note: Saving is done off thread so there may be cache
            // hits, but this should still unload everything.
            {
                PlayerChunkMapEntry watchers = world.getPlayerChunkMap()
        ...
        if (queue.size() == 0) {
            listener.sendMessage(
                TextComponentHelper
                    .createComponentTranslation(listener, "commands.forge.gen.complete", genned, total, dim));
