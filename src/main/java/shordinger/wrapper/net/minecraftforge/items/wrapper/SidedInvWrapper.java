...
package shordinger.wrapper.net.minecraftforge.items.wrapper;
...
public class SidedInvWrapper implements IItemHandlerModifiable {
    ...
    public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
        ...
        if (!stackInSlot.isEmpty()) {
            ...
            if (!inv.canInsertItem(slot1, stack, side) || !inv.isItemValidForSlot(slot1, stack)) return stack;
    ...
    private void setInventorySlotContents(int slot, ItemStack stack) {
        inv.markDirty(); // Notify vanilla of updates, We change the handler to be responsible for this instead of the
        // caller. So mimic vanilla behavior
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (!inv.canExtractItem(slot1, stackInSlot, side)) return ItemStack.EMPTY;
