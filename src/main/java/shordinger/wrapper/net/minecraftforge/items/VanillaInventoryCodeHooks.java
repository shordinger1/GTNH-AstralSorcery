...
public class VanillaInventoryCodeHooks {
    ...
    public static Boolean extractHook(IHopper dest) {
        ...
        if (itemHandlerResult == null) return null;
            ...
            if (!extractItem.isEmpty()) {
                for (int j = 0; j < dest.getSizeInventory(); j++) {
                    ...
                    if (dest.isItemValidForSlot(j, extractItem)
                        ...
                        if (destStack.isEmpty()) dest.setInventorySlotContents(j, extractItem);
                        else {
                            destStack.grow(1);
    ...
    public static boolean dropperInsertHook(World world, BlockPos pos, TileEntityDispenser dropper, int slot,
        ...
        EnumFacing enumfacing = world.getBlockState(pos)
            .getValue(BlockDropper.FACING);
        BlockPos blockpos = pos.offset(enumfacing);
        Pair<IItemHandler, Object> destinationResult = getItemHandler(
            world,
        ...
        if (destinationResult == null) {
            ...
            if (isFull(itemHandler)) {
                ...
                for (int i = 0; i < hopper.getSizeInventory(); ++i) {
                    if (!hopper.getStackInSlot(i)
                        .isEmpty()) {
                        ItemStack originalSlotContents = hopper.getStackInSlot(i)
                            .copy();
                        ItemStack insertStack = hopper.decrStackSize(i, 1);
                        ItemStack remainder = putStackInInventoryAllSlots(
                            hopper,
                            destination,
                            itemHandler,
                            insertStack);
    ...
    private static ItemStack insertStack(TileEntity source, Object destination, IItemHandler destInventory,
        ...
        if (destInventory.insertItem(slot, stack, true)
            .isEmpty()) {
                ...
                if (inventoryWasEmpty && destination instanceof TileEntityHopper) {
                    ...
                    if (!destinationHopper.mayTransfer()) {
                        ...
                        if (source instanceof TileEntityHopper) {
                            if (destinationHopper.getLastUpdateTime()
                                >= ((TileEntityHopper) source).getLastUpdateTime()) {
                                k = 1;
                            }
                        }
