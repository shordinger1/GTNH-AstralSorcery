...
public class BlockFluidClassic extends BlockFluidBase {
    ...
    protected static final List<EnumFacing> SIDES = Collections
        .unmodifiableList(Arrays.asList(EnumFacing.WEST, EnumFacing.EAST, EnumFacing.NORTH, EnumFacing.SOUTH));
    ...
    public void updateTick(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
        ...
        if (quantaRemaining < quantaPerBlock) {
            ...
            // new source block
            if (adjacentSourceBlocks >= 2 && (world.getBlockState(pos.up(densityDir))
                .getMaterial()
                .isSolid() || isSourceBlock(world, pos.up(densityDir)))) {
            ...
            if (expQuanta != quantaRemaining) {
                ...
                if (expQuanta <= 0) {
                    ...
                    world.setBlockState(
                        pos,
                        state.withProperty(LEVEL, quantaPerBlock - expQuanta),
    ...
    protected final boolean hasDownhillFlow(IBlockAccess world, BlockPos pos, EnumFacing direction) {
        return world.getBlockState(
                pos.offset(direction)
                    .down(densityDir))
            .getBlock() == this
            && (canFlowInto(world, pos.offset(direction)) || canFlowInto(world, pos.down(densityDir)));
    }

    public boolean isFlowingVertically(IBlockAccess world, BlockPos pos) {
        return world.getBlockState(pos.up(densityDir))
            .getBlock() == this
            || (world.getBlockState(pos)
            .getBlock() == this && canFlowInto(world, pos.up(densityDir)));
    }
    ...
    protected int calculateFlowCost(World world, BlockPos pos, int recurseDepth, int side) {
        ...
        for (int adjSide = 0; adjSide < 4; adjSide++) {
            if (SIDES.get(adjSide) == SIDES.get(side)
                .getOpposite()) {
                continue;
            }
    ...
    protected boolean canFlowInto(IBlockAccess world, BlockPos pos) {
        return world.getBlockState(pos)
            .getBlock() == this || canDisplace(world, pos);
    }
