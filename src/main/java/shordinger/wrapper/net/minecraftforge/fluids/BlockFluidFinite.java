...
public class BlockFluidFinite extends BlockFluidBase {
    ...
    public void updateTick(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
        ...
        if (quantaRemaining < 1) {
            ...
            if (quantaRemaining == 1) {
                world.setBlockState(
                    pos,
                    state.withProperty(LEVEL, quantaRemaining - 1),
            ...
            if (quanta >= 0) {
                ...
                if (newQuanta != quanta) {
                    if (newQuanta == 0) {
                        ...
                        world.setBlockState(
                            off,
                            getDefaultState().withProperty(LEVEL, newQuanta - 1),
                            Constants.BlockFlags.SEND_TO_CLIENTS);
                    }
    ...
    public int place(World world, BlockPos pos, @Nonnull FluidStack fluidStack, boolean doPlace) {
        ...
        if (doPlace) {
            ...
            world.setBlockState(
                pos,
                getDefaultState().withProperty(LEVEL, quanta - 1),
    ...
    public FluidStack drain(World world, BlockPos pos, boolean doDrain) {
        final FluidStack fluidStack = new FluidStack(
            getFluid(),
            MathHelper.floor(getQuantaPercentage(world, pos) * Fluid.BUCKET_VOLUME));
