...
import shordinger.wrapper.net.minecraft.client.renderer.ActiveRenderInfo;
...
import shordinger.wrapper.net.minecraftforge.common.property.PropertyFloat;
...
public abstract class BlockFluidBase extends Block implements IFluidBlock {
    ...
    public BlockFluidBase(Fluid fluid, Material material, MapColor mapColor) {
        ...
        this.setDefaultState(
            blockState.getBaseState()
                .withProperty(LEVEL, getMaxRenderHeightMeta()));
    }
    ...
    public boolean displaceIfPossible(World world, BlockPos pos) {
        ...
        if (canDisplace) {
            ...
            if (!block.isAir(state, world, pos) && !isFluid(state)) {
                // Forge: Vanilla has a 'bug' where snowballs don't drop like every other block. So special case because
                // ewww...
    ...
    @Override
    @Nonnull
    public Vec3d modifyAcceleration(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull Entity entity,
                                    @Nonnull Vec3d vec) {
    ...
    private static boolean isFluid(@Nonnull IBlockState blockstate) {
        return blockstate.getMaterial()
            .isLiquid() || blockstate.getBlock() instanceof IFluidBlock;
    }

    @Override
    @Nonnull
    public IBlockState getExtendedState(@Nonnull IBlockState oldState, @Nonnull IBlockAccess world,
        ...
        if (height[1][1] == 1) {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (i != 1 || j != 1) {
                        upBlockState[i][j] = world.getBlockState(
                            pos.add(i - 1, 0, j - 1)
                                .down(densityDir));
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    corner[i][j] = getFluidHeightAverage(
                        height[i][j],
                        height[i][j + 1],
                        height[i + 1][j],
                        height[i + 1][j + 1]);
    ...
    public float getFluidHeightForRender(IBlockAccess world, BlockPos pos, @Nonnull IBlockState up) {
        ...
        return !here.getMaterial()
            .isSolid() && up.getBlock() == this ? 1 : this.getQuantaPercentage(world, pos) * quantaFraction;
    }
    ...
    public Vec3d getFlowVector(IBlockAccess world, BlockPos pos) {
        ...
        if (hasVerticalFlow(world, pos)) {
            for (EnumFacing side : EnumFacing.Plane.HORIZONTAL) {
                BlockPos offset = pos.offset(side);
                if (causesDownwardCurrent(world, offset, side)
                    || causesDownwardCurrent(world, offset.down(densityDir), side)) {
                    vec = vec.normalize()
                        .addVector(0.0, 6.0 * densityDir, 0.0);
                    break;
                }
            }
        }
    ...
    final int getEffectiveQuanta(IBlockAccess world, BlockPos pos) {
        ...
        return quantaValue > 0 && quantaValue < quantaPerBlock && hasVerticalFlow(world, pos) ? quantaPerBlock
            : quantaValue;
    }
