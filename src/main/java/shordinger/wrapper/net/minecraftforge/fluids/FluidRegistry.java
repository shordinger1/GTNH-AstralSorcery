...
public abstract class FluidRegistry {
    ...
    public static final Fluid WATER = new Fluid(
        ...
        .setUnlocalizedName(Blocks.WATER.getUnlocalizedName());

    public static final Fluid LAVA = new Fluid(
        "lava",
        ...
        for (String defaultName : defaultNames) {
            ...
            if (fluid == null) {
                ...
                if (localDefault == null) {
                    FMLLog.log.error(
                        "The fluid {} (specified as {}) is missing from this instance - it will be removed",
                        derivedName,
                        defaultName);
                    continue;
                }
                ...
                FMLLog.log.error(
                    "The fluid {} specified as default is not present - it will be reverted to default {}",
                    defaultName,
                    localDefault);
            }
            FMLLog.log
                .debug("The fluid {} has been selected as the default fluid for {}", defaultName, fluid.getName());
     ...
     * @return True if the fluid was registered as the current default fluid, false if it was only registered as an
     * alternative
     */
    ...
    public static void enableUniversalBucket() {
        if (Loader.instance()
            ...
            FMLLog.log.error(
                "Trying to activate the universal filled bucket too late. Call it statically in your Mods class. Mod: {}",
                modContainerName);
        } else {
     ...
     * @return True if the fluid was added successfully, false if it already was registered or couldn't be registered
     * with the bucket.
     */
    ...
    public static String getDefaultFluidName(Fluid key) {
        String name = masterFluidReference.inverse()
            .get(key);
        if (Strings.isNullOrEmpty(name)) {
            FMLLog.log.error(
                "The fluid registry is corrupted. A fluid {} {} is not properly registered. The mod that registered this is broken",
                key.getClass()
                    .getName(),
                key.getName());
