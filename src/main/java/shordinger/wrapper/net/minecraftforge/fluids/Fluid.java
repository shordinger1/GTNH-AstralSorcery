...
public class Fluid {
    ...
    public Fluid setBlock(Block block) {
        if (this.block == null || this.block == block) {
            ...
            FMLLog.log.warn(
                "A mod has attempted to assign Block {} to the Fluid '{}' but this Fluid has already been linked to the Block {}. "
                    + "You may have duplicate Fluid Blocks as a result. It *may* be possible to configure your mods to avoid this.",
                block,
                fluidName,
                this.block);
        }
    ...
    /**
     * Called instead of placing the fluid block if {@link WorldProvider#doesWaterVaporize()} and
     * {@link #doesVaporize(FluidStack)} are true.
    ...
    public SoundEvent getFillSound() {
        if (fillSound == null) {
            if (getBlock() != null && getBlock().getDefaultState()
