...
public interface ISpecialArmor {
    ...
    public static class ArmorProperties implements Comparable<ArmorProperties> {
        ...
        public int Slot = 0;
        private static final boolean DEBUG = false; // Only enable this if you wish to be spammed with debugging
        // information.
        // Left it in because I figured it'd be useful for modders
         ...
         */
        public static float applyArmor(EntityLivingBase entity, NonNullList<ItemStack> inventory, DamageSource source,
                                       double damage) {
            if (DEBUG) {
            ...
            double totalToughness = entity.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS)
                .getAttributeValue();
            ...
            for (int slot = 0; slot < inventory.size(); slot++) {
                ...
                if (stack.getItem() instanceof ISpecialArmor) {
                    if (!source.isUnblockable() || ((ISpecialArmor) stack.getItem())
                        .handleUnblockableDamage(entity, stack, source, damage, slot)) {
                        ISpecialArmor armor = (ISpecialArmor) stack.getItem();
                        prop = armor.getProperties(entity, stack, source, damage, slot)
                            .copy();
                ...
                for (ArmorProperties prop : props) {
                    ...
                    if (absorb > 0) {
                        ...
                        if (stack.getItem() instanceof ISpecialArmor) {
                            ...
                            if (DEBUG) {
                                System.out.println(
                                    "Item: " + stack.toString() + " Absorbed: " + absorb + " Damaged: " + itemDamage);
                            }
            ...
            if (damage > 0 && (totalArmor > 0 || totalToughness > 0)) {
                ...
                for (int i = 0; i < inventory.size(); i++) {
                    if (inventory.get(i)
                        ...
                        inventory.get(i)
                            .damageItem((int) armorDamage, entity);
        ...
        public String toString() {
            return String.format(
                ...
                AbsorbMax,
                AbsorbRatio,
                (AbsorbRatio == 0 ? 0 : (int) (AbsorbMax * 100.0D / AbsorbRatio)));
        }
