...
public class ChunkIOExecutor {
    ...
    private static final ThreadPoolExecutor pool = new ChunkIOThreadPoolExecutor(
        BASE_THREADS,
        Integer.MAX_VALUE,
        60L,
        TimeUnit.SECONDS,
    ...
    public static Chunk syncChunkLoad(World world, AnvilChunkLoader loader, ChunkProviderServer provider, int x,
                                      int z) {
        ...
        if (task != null) {
            if (!pool.remove(task)) // If it wasn't in the pool, and run hasn't finished, then wait for the async
            // thread.
            {
                synchronized (task) {
    ...
    public static void queueChunkLoad(World world, AnvilChunkLoader loader, ChunkProviderServer provider, int x, int z,
                                      Runnable runnable) {
        QueuedChunk key = new QueuedChunk(x, z, world);
    ...
    public static void tick() {
        Iterator<ChunkIOProvider> itr = tasks.values()
            .iterator();
        while (itr.hasNext()) {
