...
public class ConfigElement implements IConfigElement {
    ...
    public List<IConfigElement> getChildElements() {
        if (!isProperty) {
            ...
            Iterator<ConfigCategory> ccI = category.getChildren()
                .iterator();
            Iterator<Property> pI = category.getOrderedValues()
                .iterator();
    ...
    public static ConfigGuiType getType(Property prop) {
        return prop.getType() == Property.Type.BOOLEAN ? ConfigGuiType.BOOLEAN
            ...
            : prop.getType() == Property.Type.INTEGER ? ConfigGuiType.INTEGER
            : prop.getType() == Property.Type.COLOR ? ConfigGuiType.COLOR
    ...
    public static IConfigElement from(Class<?> configClass) {
        ...
        if (annotation == null) throw new RuntimeException(
            String.format("The class '%s' has no @Config annotation!", configClass.getName()));

        Configuration config = ConfigManager.getConfiguration(annotation.modid(), annotation.name());
        if (config == null) {
            String error = String.format(
                "The configuration '%s' of mod '%s' isn't loaded with the ConfigManager!",
                annotation.name(),
                annotation.modid());
        ...
        if (annotation.category()
            ...
            for (String catName : catNames) {
                ...
                DummyCategoryElement element = new DummyCategoryElement(
                    category.getName(),
