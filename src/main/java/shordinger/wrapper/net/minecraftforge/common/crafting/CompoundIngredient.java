...
public class CompoundIngredient extends Ingredient {
    ...
    protected CompoundIngredient(Collection<Ingredient> children) {
        ...
        boolean simple = true;
        for (Ingredient child : children) simple &= child.isSimple();
    ...
    public ItemStack[] getMatchingStacks() {
        if (stacks == null) {
            ...
            for (Ingredient child : children) Collections.addAll(tmp, child.getMatchingStacks());
    ...
    public IntList getValidItemStacksPacked() {
        ...
        if (this.itemIds == null) {
            ...
            for (Ingredient child : children) this.itemIds.addAll(child.getValidItemStacksPacked());
    ...
    public boolean apply(@Nullable ItemStack target) {
        ...
        for (Ingredient child : children) if (child.apply(target)) return true;
