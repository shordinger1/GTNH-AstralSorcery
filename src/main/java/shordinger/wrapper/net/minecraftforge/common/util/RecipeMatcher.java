...
public class RecipeMatcher {
    ...
    public static <T> int[] findMatches(List<T> inputs, List<? extends Predicate<T>> tests) {
        ...
        if (elements != tests.size()) return null; // There will not be a 1:1 mapping of inputs -> tests

        int[] ret = new int[elements];
        for (int x = 0; x < elements; x++) ret[x] = -1;
        ...
        for (int x = 0; x < elements; x++) {
            ...
            if (matched == 0) return null; // We have an test that matched non of the inputs

            if (matched == 1) {
                if (!claim(ret, data, x, elements)) return null; // We failed to claim this index, which means it caused
                // something else to go to 0 matches, which makes the
                // whole thing fail
            }
        }
        ...
        // We should be in a state where multiple tests are satified by multiple inputs. So we need to try a branching
        // recursive test.
    ...
    private static boolean claim(int[] ret, BitSet data, int claimed, int elements) {
        ...
        while (pending.peek() != null) {
            ...
            if (used >= elements || used < 0) throw new IllegalStateException(
                "What? We matched something, but it wasn't set in the range of this test! Test: " + test
                    + " Used: "
                    + used);
            ...
            for (int x = 0; x < elements; x++) {
                ...
                if (data.get(offset + used) && !data.get(elements + x)) {
                    ...
                    int count = 0;
                    for (int y = offset; y < offset + elements; y++) if (data.get(y)) count++;

                    if (count == 0) return false; // Claiming this caused another test to lose its last match..
    ...
    private static boolean backtrack(BitSet data, int[] ret, int start, int elements) {
        ...
        if (test >= elements) return true; // Could not find the next unused test.

        if (test < 0) throw new IllegalStateException("This should never happen, negative test in backtrack!");
