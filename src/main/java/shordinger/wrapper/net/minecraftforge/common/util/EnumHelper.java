...
import shordinger.wrapper.net.minecraft.block.BlockPressurePlate.Sensitivity;
...
import shordinger.wrapper.net.minecraft.entity.item.EntityPainting.EnumArt;
...
import shordinger.wrapper.net.minecraft.item.Item.ToolMaterial;
import shordinger.wrapper.net.minecraft.item.ItemArmor.ArmorMaterial;
...
import shordinger.wrapper.net.minecraft.world.gen.structure.StructureStrongholdPieces.Stronghold.Door;
...
public class EnumHelper {
    ...
    private static Class<?>[][] commonTypes = {{EnumAction.class},
        ...
        {Door.class}, {EnumEnchantmentType.class, Predicate.class}, {Sensitivity.class},
        {RayTraceResult.Type.class}, {EnumSkyBlock.class, int.class}, {SleepResult.class},
        {ToolMaterial.class, int.class, int.class, float.class, float.class, int.class},
        {EnumRarity.class, TextFormatting.class, String.class}, {HorseArmorType.class, String.class, int.class},
    ...
    @Nullable
    public static EnumCreatureType addCreatureType(String name, Class<? extends IAnimals> typeClass, int maxNumber,
    ...
    public static EntityLiving.SpawnPlacementType addSpawnPlacementType(String name,
                                                                        BiPredicate<IBlockAccess, BlockPos> predicate) {
        return addEnum(EntityLiving.SpawnPlacementType.class, name, predicate);
    }

    /**
     * @param name            the name of the new {@code HorseArmorType}
     * @param textureLocation the path to the texture for this armor type. It must follow the format domain:path and be
    ...
    @Nullable
    protected static <T extends Enum<?>> T addEnum(Class<?>[][] map, Class<T> enumType, String enumName,
                                                   Object... paramValues) {
    ...
    private static <T extends Enum<?>> T addEnum(boolean test, final Class<T> enumType, @Nullable String enumName,
        ...
        int flags = (FMLForgePlugin.RUNTIME_DEOBF ? Modifier.PUBLIC : Modifier.PRIVATE) | Modifier.STATIC
            ...
            | 0x1000 /* SYNTHETIC */;
        if (valuesField == null) {
            String valueType = String.format(
                "[L%s;",
                enumType.getName()
                    .replace('.', '/'));
        ...
        if (valuesField == null) {
            ...
            lines.add(
                String.format(
                    "Flags: %s",
                    String.format("%16s", Integer.toBinaryString(flags))
                        .replace(' ', '0')));
            lines.add("Fields:");
            for (Field field : fields) {
                String mods = String.format("%16s", Integer.toBinaryString(field.getModifiers()))
                    .replace(' ', '0');
                lines.add(
                    String.format(
                        "       %s %s: %s",
            ...
            for (String line : lines) FMLLog.log.fatal(line);
        ...
        if (test) {
            ...
            if (ctr == null || ex != null) {
                throw new EnhancedRuntimeException(
                    String.format("Could not find constructor for Enum %s", enumType.getName()),
                    ex) {
