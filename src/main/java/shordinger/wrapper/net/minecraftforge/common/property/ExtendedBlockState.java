...
package shordinger.wrapper.net.minecraftforge.common.property;
...
import shordinger.wrapper.net.minecraft.block.state.BlockStateContainer.StateImplementation;
...
public class ExtendedBlockState extends BlockStateContainer {
    ...
    private static ImmutableMap<IUnlistedProperty<?>, Optional<?>> buildUnlistedMap(
    ...
    @Override
    @Nonnull
    protected StateImplementation createState(@Nonnull Block block,
    ...
    protected static class ExtendedStateImplementation extends StateImplementation implements IExtendedBlockState {
        ...
        protected ExtendedStateImplementation(Block block, ImmutableMap<IProperty<?>, Comparable<?>> properties,
                                              ImmutableMap<IUnlistedProperty<?>, Optional<?>> unlistedProperties,
                                              @Nullable ImmutableTable<IProperty<?>, Comparable<?>, IBlockState> table, IBlockState clean) {
        ...
        @Override
        @Nonnull
        public <T extends Comparable<T>, V extends T> IBlockState withProperty(@Nonnull IProperty<T> property,
                                                                               @Nonnull V value) {
            ...
            return new ExtendedStateImplementation(
                ...
                clean.getProperties(),
                unlistedProperties,
                ((StateImplementation) clean).getPropertyValueTable(),
                this.cleanState);
        }
        ...
        public <V> IExtendedBlockState withProperty(IUnlistedProperty<V> property, @Nullable V value) {
            ...
            if (oldValue == null) {
                throw new IllegalArgumentException(
                    "Cannot set unlisted property " + property
                        + " as it does not exist in "
                        + getBlock().getBlockState());
            }
            ...
            if (!property.isValid(value)) {
                throw new IllegalArgumentException(
                    "Cannot set unlisted property " + property
                        ...
                        + value
                        + " on block "
                        + Block.REGISTRY.getNameForObject(getBlock())
        ...
        public <V> V getValue(IUnlistedProperty<V> property) {
            ...
            if (value == null) {
                throw new IllegalArgumentException(
                    "Cannot get unlisted property " + property
                        + " as it does not exist in "
                        + getBlock().getBlockState());
            }
            return property.getType()
                .cast(value.orElse(null));
        }
