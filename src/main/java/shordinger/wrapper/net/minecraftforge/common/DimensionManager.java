...
public class DimensionManager {
    ...
    private static final Int2ObjectMap<WorldServer> worlds = Int2ObjectMaps
        .synchronize(new Int2ObjectLinkedOpenHashMap<>());
    private static final Int2ObjectMap<Dimension> dimensions = Int2ObjectMaps
        .synchronize(new Int2ObjectLinkedOpenHashMap<>());
    ...
    private static final ConcurrentMap<World, World> weakWorldMap = new MapMaker().weakKeys()
        .weakValues()
        .makeMap();
    ...
    public static Map<DimensionType, IntSortedSet> getRegisteredDimensions() {
        ...
        for (Int2ObjectMap.Entry<Dimension> entry : dimensions.int2ObjectEntrySet()) {
            map.computeIfAbsent(entry.getValue().type, k -> new IntRBTreeSet())
                .add(entry.getIntKey());
        }
        return map;
    }
    ...
    public static void registerDimension(int id, DimensionType type) {
        DimensionType.getById(type.getId()); // Check if type is invalid {will throw an error} No clue how it would be
        // invalid tho...
        if (dimensions.containsKey(id)) {
            throw new IllegalArgumentException(
                String.format("Failed to register dimension for id %d, One is already registered", id));
        }
    ...
    public static void unregisterDimension(int id) {
        if (!dimensions.containsKey(id)) {
            throw new IllegalArgumentException(
                String.format("Failed to unregister dimension for id %d; No provider registered", id));
        }
    ...
    public static DimensionType getProviderType(int dim) {
        if (!dimensions.containsKey(dim)) {
            throw new IllegalArgumentException(
                String.format("Could not get provider type for dimension %d, does not exist", dim));
        }
    ...
    public static Integer[] getIDs(boolean check) {
        if (check) {
            ...
            for (World w : allWorlds) {
                ...
                if (leakCount == 5) {
                    FMLLog.log.debug(
                        "The world {} ({}) may have leaked: first encounter (5 occurrences).\n",
                        Integer.toHexString(System.identityHashCode(w)),
                        w.getWorldInfo()
                            .getWorldName());
                } else if (leakCount % 5 == 0) {
                    FMLLog.log.debug(
                        "The world {} ({}) may have leaked: seen {} times.\n",
                        Integer.toHexString(System.identityHashCode(w)),
                        w.getWorldInfo()
                            .getWorldName(),
                        leakCount);
    ...
    public static void initDimension(int dim) {
        ...
        WorldServer world = (dim == 0 ? overworld
            : (WorldServer) (new WorldServerMulti(mcServer, savehandler, dim, overworld, mcServer.profiler).init()));
        world.addEventListener(new ServerWorldEventHandler(mcServer, world));
        MinecraftForge.EVENT_BUS.post(new WorldEvent.Load(world));
        if (!mcServer.isSinglePlayer()) {
            world.getWorldInfo()
                .setGameType(mcServer.getGameType());
        }
    ...
    public static WorldProvider createProviderFor(int dim) {
        try {
            if (dimensions.containsKey(dim)) {
                ...
                throw new RuntimeException(String.format("No WorldProvider bound for dimension %d", dim)); // It's going
                // to crash
                // anyway at
                // this
                // point.
            ...
            FMLLog.log.error(
                "An error occurred trying to create an instance of WorldProvider {} ({})",
                dim,
                getProviderType(dim),
                e);
    ...
    public static void loadDimensionDataMap(@Nullable NBTTagCompound compoundTag) {
        ...
        if (compoundTag == null) {
            IntIterator iterator = dimensions.keySet()
                .iterator();
    ...
    public static File getCurrentSaveRootDirectory() {
         ...
         * SaveHandler saveHandler = (SaveHandler) srv.getActiveAnvilConverter().getSaveLoader(srv.getFolderName(),
         * false);
