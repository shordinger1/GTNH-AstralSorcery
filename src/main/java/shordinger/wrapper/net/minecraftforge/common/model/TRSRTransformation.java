...
package shordinger.wrapper.net.minecraftforge.common.model;
...
import shordinger.wrapper.net.minecraftforge.client.ForgeHooksClient;
...
public final class TRSRTransformation implements IModelState, ITransformation {
    ...
    public TRSRTransformation(@Nullable Vector3f translation, @Nullable Quat4f leftRot, @Nullable Vector3f scale,
                              @Nullable Quat4f rightRot) {
    ...
    public TRSRTransformation(ItemTransformVec3f transform) {
        this(
            toVecmath(transform.translation),
            quatFromXYZDegrees(toVecmath(transform.rotation)),
            toVecmath(transform.scale),
            null);
    }
    ...
    public static Vector3f toYXZDegrees(Quat4f q) {
        ...
        return new Vector3f(
            (float) Math.toDegrees(yxz.x),
            (float) Math.toDegrees(yxz.y),
            (float) Math.toDegrees(yxz.z));
    }
    ...
    public static Vector3f toXYZDegrees(Quat4f q) {
        ...
        return new Vector3f(
            (float) Math.toDegrees(xyz.x),
            (float) Math.toDegrees(xyz.y),
            (float) Math.toDegrees(xyz.z));
    }
    ...
    public static Matrix4f mul(@Nullable Vector3f translation, @Nullable Quat4f leftRot, @Nullable Vector3f scale,
                               @Nullable Quat4f rightRot) {
    ...
    public static Triple<Quat4f, Vector3f, Quat4f> svdDecompose(Matrix3f m) {
        ...
        // FIXME: this doesn't work correctly for some reason; not crucial, so disabling for now; investigate in the
        // future.
    ...
    public ItemTransformVec3f toItemTransform() {
        return new ItemTransformVec3f(
            toLwjgl(toXYZDegrees(getLeftRot())),
            toLwjgl(getTranslation()),
            toLwjgl(getScale()));
    }
    ...
    private static final EnumMap<EnumFacing, TRSRTransformation> vanillaUvTransformLocalToGlobal = Maps
        .newEnumMap(EnumFacing.class);
    private static final EnumMap<EnumFacing, TRSRTransformation> vanillaUvTransformGlobalToLocal = Maps
        .newEnumMap(EnumFacing.class);
    ...
    static {
        ...
        vanillaUvTransformLocalToGlobal
            .put(EnumFacing.NORTH, new TRSRTransformation(null, new Quat4f(tmp), null, null));
        ...
        for (EnumFacing side : EnumFacing.values()) {
            vanillaUvTransformGlobalToLocal.put(
                side,
                vanillaUvTransformLocalToGlobal.get(side)
                    .inverse());
        }
    }
    ...
    public TRSRTransformation getUVLockTransform(EnumFacing originalSide) {
        ...
        try {
            return blockCenterToCorner(
                vanillaUvTransformGlobalToLocal.get(originalSide)
                    .compose(blockCornerToCenter(this.inverse()))
