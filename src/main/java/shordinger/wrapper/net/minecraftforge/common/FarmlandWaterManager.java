...
import shordinger.wrapper.net.minecraftforge.common.ticket.AABBTicket;
import shordinger.wrapper.net.minecraftforge.common.ticket.ChunkTicketManager;
import shordinger.wrapper.net.minecraftforge.common.ticket.SimpleTicket;
...
public class FarmlandWaterManager {
    ...
    public static <T extends SimpleTicket<Vec3d>> T addCustomTicket(World world, T ticket, ChunkPos masterChunk,
    ...
    public static AABBTicket addAABBTicket(World world, AxisAlignedBB aabb) {
        if (DEBUG) FMLLog.log.info("FarmlandWaterManager: New AABBTicket, aabb={}", aabb);
        ...
        for (ChunkPos pos : posSet) // Find the chunkPos with the lowest distance to the center and choose it as the
        // master pos
        {
            ...
            if (distToCenter < masterDistance) {
                if (DEBUG) FMLLog.log.info(
                    "FarmlandWaterManager: New better pos then {}: {}, prev dist {}, new dist {}",
                    masterPos,
                    pos,
                    masterDistance,
                    distToCenter);
        ...
        if (DEBUG) FMLLog.log.info(
            "FarmlandWaterManager: {} center pos, {} dummy posses. Dist to center {}",
            masterPos,
            posSet.toArray(new ChunkPos[0]),
            masterDistance);
    ...
    public static boolean hasBlockWaterTicket(World world, BlockPos pos) {
        ChunkTicketManager<Vec3d> ticketManager = getTicketManager(
        ...
        if (ticketManager != null) {
            ...
            for (SimpleTicket<Vec3d> ticket : ticketManager.getTickets()) {
                if (ticket.matches(posAsVec3d)) return true;
            }
        }
        return false;
    }
    ...
    static void removeTickets(Chunk chunk) {
        ...
        if (ticketManager != null) {
            if (DEBUG) FMLLog.log.info(
                "FarmlandWaterManager: got tickets {} at {} before",
                ticketManager.getTickets()
                    .size(),
                ticketManager.pos);
            ticketManager.getTickets()
                .removeIf(next -> next.unload(ticketManager)); // remove if this is the master manager of the ticket
            if (DEBUG) FMLLog.log.info(
                "FarmlandWaterManager: got tickets {} at {} after",
                ticketManager.getTickets()
                    .size(),
                ticketManager.pos);
        }
