...
package shordinger.wrapper.net.minecraftforge.common.command;
 ...
 * {@link EntitySelector#matchesMultiplePlayers matchesMultiplePlayers} and {@link EntitySelector#isSelector
 * isSelector}.<br>
 * The calls are delegated to the handler with the longest matching prefix.<br>
 * <br>
 * <b>Note:</b> If you register a {@link SelectorHandler} to a broader domain (not just a single selector), you should
 * take care of possible shadowing conflicts yourself.
...
public class SelectorHandlerManager {
    ...
    public static final NavigableMap<String, SelectorHandler> selectorHandlers = new TreeMap<String, SelectorHandler>(
        Collections.<String>reverseOrder());
    public static final NavigableMap<String, String> registeringMods = new TreeMap<String, String>(
        Collections.<String>reverseOrder());

    private static final SelectorHandler vanillaHandler = new SelectorHandler() {

        @Override
        public <T extends Entity> List<T> matchEntities(final ICommandSender sender, final String token,
                                                        final Class<? extends T> targetClass) throws CommandException {
    ...
    public static void register(final String prefix, final SelectorHandler handler) {
        ...
        final String modId = Loader.instance()
            .activeModContainer()
            .getModId();
    ...
    public static SelectorHandler getHandler(final String selectorStr) {
        if (!selectorStr.isEmpty()) {
            for (final Entry<String, SelectorHandler> handler : selectorHandlers
                .subMap(selectorStr, true, selectorStr.substring(0, 1), true)
                .entrySet()) {
    ...
    public static <T extends Entity> List<T> matchEntities(final ICommandSender sender, final String token,
                                                           final Class<? extends T> targetClass) throws CommandException {
