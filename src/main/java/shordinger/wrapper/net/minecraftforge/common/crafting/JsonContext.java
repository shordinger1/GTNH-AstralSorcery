...
public class JsonContext {
    ...
    public String appendModId(String data) {
        if (data.indexOf(':') == -1) return modId + ":" + data;
        return data;
    }
    ...
    void loadConstants(JsonObject... jsons) {
        for (JsonObject json : jsons) {
            if (!CraftingHelper.processConditions(json, "conditions", this)) continue;
            if (!json.has("ingredient"))
                throw new JsonSyntaxException("Constant entry must contain 'ingredient' value");
            constants
                .put(JsonUtils.getString(json, "name"), CraftingHelper.getIngredient(json.get("ingredient"), this));
        }
