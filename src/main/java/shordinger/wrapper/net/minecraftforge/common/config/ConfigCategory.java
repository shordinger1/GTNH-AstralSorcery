...
public class ConfigCategory implements Map<String, Property> {
    ...
    /**
     * Returns whether or not this category is able to be edited while a world is running using the in-game Mod Options
     * screen
    ...
    public ConfigCategory setPropertyOrder(List<String> propertyOrder) {
        ...
        for (String s : properties.keySet()) if (!propertyOrder.contains(s)) propertyOrder.add(s);
        return this;
    }
    ...
    public void write(BufferedWriter out, int indent) throws IOException {
        ...
        for (int x = 0; x < props.length; x++) {
            ...
            if (prop.getComment() != null && !prop.getComment()
                .isEmpty()) {
                if (x != 0) {
                    out.newLine();
                }
    ...
    public void putAll(Map<? extends String, ? extends Property> m) {
        changed = true;
        if (this.propertyOrder != null)
            for (String key : m.keySet()) if (!this.propertyOrder.contains(key)) this.propertyOrder.add(key);
        properties.putAll(m);
    }
