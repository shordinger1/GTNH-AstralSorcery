...
import shordinger.wrapper.net.minecraftforge.common.animation.TimeValues;
...
public final class AnimationStateMachine implements IAnimationStateMachine {
    ...
    public Pair<IModelState, Iterable<Event>> apply(float time) {
        ...
        if (shouldHandleSpecialEvents) {
            for (Event event : ImmutableList.copyOf(pair.getRight())
                .reverse()) {
                if (event.event()
                    .startsWith("!")) {
                    shouldFilter = true;
                    if (event.event()
                        ...
                        String newState = event.event()
                            .substring("!transition:".length());
                        transition(newState);
                    } else {
    ...
    public void transition(String newState) {
        ...
        if (!transitions.containsEntry(currentStateName, newState)) {
            throw new IllegalArgumentException(
                "no transition from current clip \"" + currentStateName + "\" to the clip \"" + newState + "\" found.");
        }
    ...
    public static IAnimationStateMachine load(IResourceManager manager, ResourceLocation location,
        ...
        try (IResource resource = manager.getResource(location)) {
            ...
            AnimationStateMachine asm = asmGson.fromJson(
