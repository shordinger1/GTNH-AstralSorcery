...
import shordinger.wrapper.net.minecraft.entity.item.EntityMinecartContainer;
...
import shordinger.wrapper.net.minecraft.item.ItemPickaxe;
...
import shordinger.wrapper.net.minecraft.network.play.server.SPacketRecipeBook.State;
...
import shordinger.wrapper.net.minecraft.util.IntIdentityHashBiMap;
...
import shordinger.wrapper.net.minecraft.world.storage.loot.LootEntry;
...
import shordinger.wrapper.net.minecraftforge.event.AnvilUpdateEvent;
import shordinger.wrapper.net.minecraftforge.event.DifficultyChangeEvent;
import shordinger.wrapper.net.minecraftforge.event.ForgeEventFactory;
import shordinger.wrapper.net.minecraftforge.event.ServerChatEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.EntityTravelToDimensionEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.item.ItemTossEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.living.LivingAttackEvent;
...
import shordinger.wrapper.net.minecraftforge.event.entity.living.LivingDropsEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.living.LivingEvent.LivingJumpEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.living.LivingEvent.LivingUpdateEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.living.LivingFallEvent;
...
import shordinger.wrapper.net.minecraftforge.event.entity.living.LivingSetAttackTargetEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.living.LootingLevelEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.player.AdvancementEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.player.AnvilRepairEvent;
import shordinger.wrapper.net.minecraftforge.event.entity.player.AttackEntityEvent;
...
import shordinger.wrapper.net.minecraftforge.event.world.NoteBlockEvent;
import shordinger.wrapper.net.minecraftforge.fluids.IFluidBlock;
...
import shordinger.wrapper.net.minecraftforge.fml.common.network.handshake.NetworkDispatcher.ConnectionType;
...
public class ForgeHooks {
    ...
    public static boolean canHarvestBlock(@Nonnull Block block, @Nonnull EntityPlayer player,
        ...
        int toolLevel = stack.getItem()
            .getHarvestLevel(stack, tool, player, state);
        if (toolLevel < 0) {
            return player.canHarvestBlock(state);
        }
    ...
    public static boolean canToolHarvestBlock(IBlockAccess world, BlockPos pos, @Nonnull ItemStack stack) {
        ...
        return stack.getItem()
            .getHarvestLevel(stack, tool, null, null)
            >= state.getBlock()
            .getHarvestLevel(state);
    }

    public static float blockStrength(@Nonnull IBlockState state, @Nonnull EntityPlayer player, @Nonnull World world,
    ...
    public static boolean isToolEffective(IBlockAccess world, BlockPos pos, @Nonnull ItemStack stack) {
        ...
        for (String type : stack.getItem()
            ...
            if (state.getBlock()
                .isToolEffective(type, state)) return true;
        }
        return false;
    }
    ...
    static void initTools() {
        ...
        Block[] oreBlocks = new Block[]{Blocks.EMERALD_ORE, Blocks.EMERALD_BLOCK, Blocks.DIAMOND_ORE,
            Blocks.DIAMOND_BLOCK, Blocks.GOLD_ORE, Blocks.GOLD_BLOCK, Blocks.REDSTONE_ORE, Blocks.LIT_REDSTONE_ORE};
    ...
    public static boolean onPickBlock(RayTraceResult target, EntityPlayer player, World world) {
         ...
         * {
         * s =
         * ((ResourceLocation)Block.REGISTRY.getNameForObject(this.world.getBlockState(this.objectMouseOver.getBlockPos(
         * )).getBlock())).toString();
         * }
         ...
         * this.playerController.sendSlotPacket(this.player.getHeldItem(EnumHand.MAIN_HAND), 36 +
        ...
        if (target.typeOfHit == RayTraceResult.Type.BLOCK) {
            ...
            if (state.getBlock()
                .isAir(state, world, target.getBlockPos())) {
                return false;
            }
            ...
            result = state.getBlock()
                .getPickBlock(state, target, world, target.getBlockPos(), player);
        } else {
    ...
    public static LivingKnockBackEvent onLivingKnockBack(EntityLivingBase target, Entity attacker, float strength,
                                                         double ratioX, double ratioZ) {
    ...
    public static boolean onLivingDrops(EntityLivingBase entity, DamageSource source, ArrayList<EntityItem> drops,
    ...
    public static float[] onLivingFall(EntityLivingBase entity, float distance, float damageMultiplier) {
        ...
        return (MinecraftForge.EVENT_BUS.post(event) ? null
    ...
    public static boolean isLivingOnLadder(@Nonnull IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        if (!ForgeModContainer.fullBoundingBoxLadders) {
            return state.getBlock()
                .isLadder(state, world, pos, entity);
        } else {
            ...
            for (int y2 = mY; y2 < bb.maxY; y2++) {
                for (int x2 = mX; x2 < bb.maxX; x2++) {
                    for (int z2 = mZ; z2 < bb.maxZ; z2++) {
                        ...
                        if (state.getBlock()
                            .isLadder(state, world, tmp, entity)) {
                            return true;
                        }
                    }
                }
    ...
    @Nullable
    public static EntityItem onPlayerTossEvent(@Nonnull EntityPlayer player, @Nonnull ItemStack item,
                                               boolean includeName) {
        ...
        if (!player.world.isRemote) {
            player.getEntityWorld()
                .spawnEntity(event.getEntityItem());
        }
    ...
    public static ITextComponent newChatWithLinks(String string, boolean allowMissingHeader) {
        ...
        while (matcher.find()) {
            ...
            if (ichat == null) ichat = new TextComponentString("");
    ...
    public static int onBlockBreakEvent(World world, GameType gameType, EntityPlayerMP entityPlayer, BlockPos pos) {
        ...
        if (gameType.isCreative() && !itemstack.isEmpty()
            && !itemstack.getItem()
            .canDestroyBlockInCreative(world, pos, itemstack, entityPlayer))
            preCancelEvent = true;
        ...
        if (gameType.hasLimitedInteractions()) {
            ...
            if (!entityPlayer.isAllowEdit()) {
                if (itemstack.isEmpty() || !itemstack.canDestroy(
    ...
    public static EnumActionResult onPlaceItemIntoWorld(@Nonnull ItemStack itemstack, @Nonnull EntityPlayer player,
        ...
        EnumActionResult ret = itemstack.getItem()
            .onItemUse(player, world, pos, hand, side, hitX, hitY, hitZ);
        ...
        if (ret == EnumActionResult.SUCCESS) {
            ...
            if (itemstack.getTagCompound() != null) {
                newNBT = itemstack.getTagCompound()
                    .copy();
            }
            ...
            if (placeEvent != null && placeEvent.isCanceled()) {
                ...
                for (BlockSnapshot snap : blockSnapshots) {
                    ...
                    if (!newBlock.getBlock()
                        .hasTileEntity(newBlock)) // Containers get placed automatically
                    {
                        newBlock.getBlock()
                            .onBlockAdded(world, snap.getPos(), newBlock);
                    }
    ...
    public static boolean onAnvilChange(ContainerRepair container, @Nonnull ItemStack left, @Nonnull ItemStack right,
    ...
    public static float onAnvilRepair(EntityPlayer player, @Nonnull ItemStack output, @Nonnull ItemStack left,
    ...
    public static boolean onNoteChange(TileEntityNote te, byte old) {
        NoteBlockEvent.Change e = new NoteBlockEvent.Change(
            te.getWorld(),
            te.getPos(),
            te.getWorld()
    ...
    public static boolean onPlayerAttackTarget(EntityPlayer player, Entity target) {
        ...
        return stack.isEmpty() || !stack.getItem()
            .onLeftClickEntity(stack, player, target);
    }
    ...
    public static RayTraceResult rayTraceEyes(EntityLivingBase entity, double length) {
        ...
        Vec3d endPos = startPos.add(
            new Vec3d(entity.getLookVec().x * length, entity.getLookVec().y * length, entity.getLookVec().z * length));
    ...
    public static EnumActionResult onInteractEntityAt(EntityPlayer player, Entity entity, RayTraceResult ray,
                                                      EnumHand hand) {
    ...
    public static EnumActionResult onInteractEntityAt(EntityPlayer player, Entity entity, Vec3d vec3d, EnumHand hand) {
        PlayerInteractEvent.EntityInteractSpecific evt = new PlayerInteractEvent.EntityInteractSpecific(
            player,
            hand,
            entity,
            vec3d);
    ...
    public static PlayerInteractEvent.LeftClickBlock onLeftClickBlock(EntityPlayer player, BlockPos pos,
                                                                      EnumFacing face, Vec3d hitVec) {
        PlayerInteractEvent.LeftClickBlock evt = new PlayerInteractEvent.LeftClickBlock(player, pos, face, hitVec);
        MinecraftForge.EVENT_BUS.post(evt);
        return evt;
    }

    public static PlayerInteractEvent.RightClickBlock onRightClickBlock(EntityPlayer player, EnumHand hand,
                                                                        BlockPos pos, EnumFacing face, Vec3d hitVec) {
        PlayerInteractEvent.RightClickBlock evt = new PlayerInteractEvent.RightClickBlock(
            player,
            hand,
            pos,
            face,
    ...
    private static LootTableContext getLootTableContext() {
        LootTableContext ctx = lootContext.get()
            .peek();

        if (ctx == null) throw new JsonParseException("Invalid call stack, could not grab json context!"); // Should I
        // throw
        // this? Do
        // we care
        // about
    ...
    public static LootTable loadLootTable(Gson gson, ResourceLocation name, String data, boolean custom,
        ...
        if (!custom) ret = ForgeEventFactory.loadLootTable(name, ret, lootTableManager);
    ...
    private static class LootTableContext {
        ...
        public String validateEntryName(@Nullable String name) {
            ...
            if (!this.vanilla) throw new JsonParseException(
                "Loot Table \"" + this.name.toString()
    ...
    public static String readPoolName(JsonObject json) {
        ...
        if (ctx.custom) return "custom#" + json.hashCode(); // We don't care about custom ones modders shouldn't be
        // editing them!

        ctx.poolCount++;

        if (!ctx.vanilla) throw new JsonParseException(
            "Loot Table \"" + ctx.name.toString() + "\" Missing `name` entry for pool #" + (ctx.poolCount - 1));
    ...
    public static LootEntry deserializeJsonLootEntry(String type, JsonObject json, int weight, int quality,
    ...
    public static void onCropsGrowPost(World worldIn, BlockPos pos, IBlockState state, IBlockState blockState) {
        MinecraftForge.EVENT_BUS
            .post(new BlockEvent.CropGrowEvent.Post(worldIn, pos, state, worldIn.getBlockState(pos)));
    }
    ...
    public static boolean loadAdvancements(Map<ResourceLocation, Advancement.Builder> map) {
        ...
        Loader.instance()
            .getActiveModList()
            .forEach(ForgeHooks::loadFactories);
    ...
    @Nullable
    public static CriticalHitEvent getCriticalHit(EntityPlayer player, Entity target, boolean vanillaCritical,
        ...
        if (hitResult.getResult() == Event.Result.ALLOW
            || (vanillaCritical && hitResult.getResult() == Event.Result.DEFAULT)) {
            return hitResult;
        }
        return null;
    }
    ...
    private static void setActiveModContainer(ModContainer mod) {
        if (Loader.instance()
            .getLoaderState() != LoaderState.NOINIT) // Unit Tests..
            Loader.instance()
                .setActiveModContainer(mod);
    }
        ...
        return CraftingHelper.findFiles(mod, "assets/" + mod.getModId() + "/advancements", null, (root, file) -> {
            ...
            if (!map.containsKey(key)) {
                ...
                try {
                    ...
                    Advancement.Builder builder = JsonUtils
                        .gsonDeserialize(AdvancementManager.GSON, contents, Advancement.Builder.class);
                    map.put(key, builder);
                } catch (JsonParseException jsonparseexception) {
                    FMLLog.log
                        .error("Parsing error loading built-in advancement " + key, (Throwable) jsonparseexception);
                    return false;
    ...
    public static void sendRecipeBook(NetHandlerPlayServer connection, State state, List<IRecipe> recipes,
                                      List<IRecipe> display, boolean isGuiOpen, boolean isFilteringCraftable) {
        NetworkDispatcher disp = NetworkDispatcher.get(connection.getNetworkManager());
        // Not sure how it could ever be null, but screw it lets protect against it. Could Error the client but we dont
        // care if they are asking for this stuff in the wrong state!
        ConnectionType type = disp == null || disp.getConnectionType() == null ? ConnectionType.MODDED
            : disp.getConnectionType();
        if (type == ConnectionType.VANILLA) {
            IForgeRegistry<IRecipe> vanilla = RegistryManager.VANILLA.getRegistry(IRecipe.class);
            if (recipes.size() > 0) recipes = recipes.stream()
                .filter(e -> vanilla.containsValue(e))
                .collect(Collectors.toList());
            if (display.size() > 0) display = display.stream()
                .filter(e -> vanilla.containsValue(e))
                .collect(Collectors.toList());
        }
    ...
    public static boolean onFarmlandTrample(World world, BlockPos pos, IBlockState state, float fallDistance,
                                            Entity entity) {
