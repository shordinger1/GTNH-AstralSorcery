...
public class CompoundDataFixer extends DataFixer {
                ...
                for (Entry<String, ModFixs> e : modFixers.entrySet()) {
                    ...
                    if (ver < fixer.version) {
                        for (IFixableData fix : fixer.getFixes(type)) {
                            if (fix.getFixVersion() > ver) nbt = fix.fixTagCompound(nbt);
                        }

                        for (IDataWalker walker : getWalkers(type)) nbt = walker.process(this, nbt, version); // We pass
                        // in the
                        // holder,
                        // in case
                        // a
    ...
    public NBTTagCompound process(IFixType type, NBTTagCompound nbt, int mcversion) {
        ...
        for (IFixableData fix : vanilla.getFixes(type)) {
            if (fix.getFixVersion() > mcversion) nbt = fix.fixTagCompound(nbt);
        }
    ...
    private void validateModId(String mod) {
        // String current = Loader.instance().activeModContainer() == null ? "minecraft" :
        ...
        if (mod.length() > 64) throw new IllegalArgumentException("Mod ID is to long: " + mod);
    }
    ...
    public void writeVersionData(NBTTagCompound nbt) {
        ...
        for (ModFixs mod : modFixers.values()) sub.setInteger(mod.mod, mod.version);
    }
}
