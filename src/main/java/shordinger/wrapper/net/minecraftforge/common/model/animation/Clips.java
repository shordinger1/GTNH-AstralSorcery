...
public final class Clips {
    ...
    public static final class SlerpClip implements IClip {
        ...
        public Iterable<Event> pastEvents(float lastPollTime, float time) {
            ...
            return Iterables.mergeSorted(
                ImmutableSet.of(from.pastEvents(clipLastPollTime, clipTime), to.pastEvents(clipLastPollTime, clipTime)),
    ...
    private static IJointClip blendClips(final IJoint joint, final IJointClip fromClip, final IJointClip toClip,
                                         final ITimeValue input, final ITimeValue progress) {
        return new IJointClip() {

            @Override
            public TRSRTransformation apply(float time) {
                float clipTime = input.apply(time);
                return fromClip.apply(clipTime)
                    .slerp(toClip.apply(clipTime), MathHelper.clamp(progress.apply(time), 0, 1));
            }
        };
    }
     ...
     */
    public static Pair<IModelState, Iterable<Event>> apply(final IClip clip, final float lastPollTime,
                                                           final float time) {
        return Pair.<IModelState, Iterable<Event>>of(new IModelState() {
            ...
            public Optional<TRSRTransformation> apply(Optional<? extends IModelPart> part) {
                ...
                TRSRTransformation jointTransform = clip.apply(joint)
                    .apply(time)
                    .compose(joint.getInvBindPose());
                Optional<? extends IJoint> parent = joint.getParent();
                while (parent.isPresent()) {
                    TRSRTransformation parentTransform = clip.apply(parent.get())
                        .apply(time);
    ...
    public static final class TriggerClip implements IClip {
        ...
        public Iterable<Event> pastEvents(float lastPollTime, float time) {
            if (parameter.apply(lastPollTime) < 0 && parameter.apply(time) >= 0) {
                return Iterables.mergeSorted(
                    ImmutableSet.of(clip.pastEvents(lastPollTime, time), ImmutableSet.of(new Event(event, 0))),
                    Ordering.<Event>natural());
            }
