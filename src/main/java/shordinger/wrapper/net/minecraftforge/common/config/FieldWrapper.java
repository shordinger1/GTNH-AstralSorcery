...
import shordinger.wrapper.net.minecraftforge.common.config.Config.RangeDouble;
import shordinger.wrapper.net.minecraftforge.common.config.Config.RangeInt;
...
public abstract class FieldWrapper implements IFieldWrapper {
    ...
    public FieldWrapper(String category, Field field, Object instance) {
        ...
        if (field.isAnnotationPresent(Config.Name.class)) this.name = field.getAnnotation(Config.Name.class)
            .value();
    ...
    public static IFieldWrapper get(Object instance, Field field, String category) {
        if (ADAPTERS.get(field.getType()) != null) return new PrimitiveWrapper(category, field, instance);
        else if (Enum.class.isAssignableFrom(field.getType())) return new EnumWrapper(category, field, instance);
        else if (Map.class.isAssignableFrom(field.getType())) return new MapWrapper(category, field, instance);
        else if (field.getType()
            .getSuperclass()
            .equals(Object.class)) throw new RuntimeException("Objects should not be handled by field wrappers");
        else throw new IllegalArgumentException(
                String.format(
                    "Fields of type '%s' are not supported!",
                    field.getType()
                        .getCanonicalName()));
    }

    public static boolean hasWrapperFor(Field field) {
        if (ADAPTERS.get(field.getType()) != null) return true;
        else if (Enum.class.isAssignableFrom(field.getType())) return true;
        else if (Map.class.isAssignableFrom(field.getType())) return true;
        return false;
    }
    ...
    private static class MapWrapper extends FieldWrapper {
        ...
        private MapWrapper(String category, Field field, Object instance) {
            ...
            this.baseName = (this.category == null) ? ""
                : (this.category + ".") + this.name.toLowerCase(Locale.ENGLISH) + ".";
            ...
            try {
                ...
                throw new IllegalArgumentException(
                    String.format(
                        "The map '%s' of class '%s' must have the key type String!",
                        field.getName(),
            ...
            if (this.adapter == null && mType instanceof GenericArrayType) {
                this.adapter = ADAPTERS.get(ARRAY_REMAP.get(((GenericArrayType) mType).getGenericComponentType())); // J6
                // seems
                // to
                // have
                // issues,
            ...
            if (this.adapter == null) throw new IllegalArgumentException(
                String.format(
                    "The map '%s' of class '%s' has target values which are neither primitive nor an enum!",
                    field.getName(),
                    field.getDeclaringClass()
                        .getCanonicalName()));
        }
        ...
        @Override
        public void setupConfiguration(Configuration cfg, String desc, String langKey, boolean reqMCRestart,
        ...
        @Override
        public void setupConfiguration(Configuration cfg, String desc, String langKey, boolean reqMCRestart,
                                       boolean reqWorldRestart, boolean hasSlidingControl) {
    ...
    private static class EnumWrapper extends SingleValueFieldWrapper {
        ...
        @Override
        public Object getValue(String key) {
            if (!hasKey(key)) throw new IllegalArgumentException("Unsupported Key!");
        ...
        public void setupConfiguration(Configuration cfg, String desc, String langKey, boolean reqMCRestart,
            ...
            Property prop = cfg.getCategory(this.category)
                .get(this.name); // Will be setup in general by ConfigManager
            ...
            else prop.setComment("Valid values:" + "\n" + validValues);
        }
    }
    ...
    private static class PrimitiveWrapper extends SingleValueFieldWrapper {
        ...
        @Override
        public Object getValue(String key) {
            if (!hasKey(key)) throw new IllegalArgumentException("Unknown key!");
            try {
