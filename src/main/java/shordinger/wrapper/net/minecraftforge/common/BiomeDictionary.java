...
public class BiomeDictionary {
    ...
    public static final class Type {
        ...
        /*
         * Tags specifying the amount of vegetation a biome has. Specifying neither implies a biome to have moderate
         * amounts
         */
        ...
        /*
         * Tree-based tags, SAVANNA refers to dry, desert-like trees (Such as Acacia), CONIFEROUS refers to snowy trees
         * (Such as Spruce) and JUNGLE refers to jungle trees.
    ...
    public static void addTypes(Biome biome, Type... types) {
        Preconditions.checkArgument(
            ForgeRegistries.BIOMES.containsValue(biome),
            "Cannot add types to unregistered biome %s",
            biome);
    ...
    public static void makeBestGuess(Biome biome) {
        ...
        if (biome.isHighHumidity() && biome.getBaseHeight() < 0.0F
            && (biome.getHeightVariation() <= 0.3F && biome.getHeightVariation() >= 0.0F)) {
        ...
        if (biome.topBlock.getBlock() != Blocks.SAND && biome.getDefaultTemperature() >= 1.0f
            && biome.getRainfall() < 0.2f) {
    ...
    static void ensureHasTypes(Biome biome) {
        if (!hasAnyType(biome)) {
            makeBestGuess(biome);
            FMLLog.log.warn(
                "No types have been added to Biome {}, types have been assigned on a best-effort guess: {}",
    ...
    private static Collection<Type> listSupertypes(Type... types) {
        ...
        while (!next.isEmpty()) {
            ...
            for (Type sType : Type.byName.values()) {
                if (sType.subTypes.contains(type) && supertypes.add(sType)) next.add(sType);
            }
        }
