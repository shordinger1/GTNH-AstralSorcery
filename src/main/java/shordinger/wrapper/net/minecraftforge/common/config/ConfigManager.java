...
package shordinger.wrapper.net.minecraftforge.common.config;
...
import shordinger.wrapper.net.minecraftforge.common.config.Config.Comment;
import shordinger.wrapper.net.minecraftforge.common.config.Config.LangKey;
import shordinger.wrapper.net.minecraftforge.common.config.Config.Name;
...
import shordinger.wrapper.net.minecraftforge.fml.common.discovery.asm.ModAnnotation.EnumHolder;
...
public class ConfigManager {
    ...
    public static void loadData(ASMDataTable data) {
        ...
        for (ASMData target : data.getAll(Config.class.getName())) {
            String modid = (String) target.getAnnotationInfo()
                .get("modid");
            Multimap<Config.Type, ASMData> map = asm_data.computeIfAbsent(modid, k -> ArrayListMultimap.create());

            EnumHolder tholder = (EnumHolder) target.getAnnotationInfo()
                .get("type");
     ...
     * does not exist, it will be created with default values derived from the mods config classes variable default
     * values
     * and comments and ranges, as well as configuration names based on the appropriate annotations found in
     * {@code @Config}.
     * <p>
     * Note, that this method is being called by the {@link net.minecraftforge.fml.common.FMLModContainer}, so the mod
     * needn't call it in init().
     * <p>
     * If this method is called after the initial load, it will check whether the values in the Configuration object
     * differ
     * from the values in the corresponding variables. If they differ, it will either overwrite the variables if the
     * Configuration
     * object is marked as changed (e.g. if it was changed with the ConfigGui) or otherwise overwrite the Configuration
     * object's values.
    ...
    public static void sync(String modid, Config.Type type) {
        ...
        ClassLoader mcl = Loader.instance()
            .getModClassLoader();
        ...
        for (ASMData targ : map.get(type)) {
            try {
                ...
                Set<Class<?>> modConfigClasses = MOD_CONFIG_CLASSES
                    .computeIfAbsent(modid, k -> Sets.<Class<?>>newHashSet());
                modConfigClasses.add(cls);

                String name = (String) targ.getAnnotationInfo()
                    .get("name");
                if (name == null) name = modid;
                String category = (String) targ.getAnnotationInfo()
                    .get("category");
                ...
                sync(
                    ...
                    category,
                    !Loader.instance()
                        .hasReachedState(LoaderState.AVAILABLE),
                    null);
                ...
                FMLLog.log
                    .error("An error occurred trying to load a config for {} into {}", modid, targ.getClassName(), e);
    ...
    public static Class<?>[] getModConfigClasses(String modid) {
        return (MOD_CONFIG_CLASSES.containsKey(modid) ? MOD_CONFIG_CLASSES.get(modid)
            .toArray(new Class<?>[0]) : new Class<?>[0]);
    }
    ...
    static Configuration getConfiguration(String modid, String name) {
        if (Strings.isNullOrEmpty(name)) name = modid;
        File configDir = Loader.instance()
            .getConfigDir();
    ...
    private static void sync(Configuration cfg, Class<?> cls, String modid, String category, boolean loading,
        ...
        for (Field f : cls.getDeclaredFields()) {
            ...
            // Only the root class may have static fields. Otherwise category tree nodes of the same type would share
            // the
            // contained value messing up the sync
            if (Modifier.isStatic(f.getModifiers()) != (instance == null)) continue;

            if (f.isAnnotationPresent(Config.Ignore.class)) continue;

            String comment = null;
            Comment ca = f.getAnnotation(Comment.class);
            if (ca != null) comment = NEW_LINE.join(ca.value());

            String langKey = modid + "."
                + (category.isEmpty() ? "" : category + Configuration.CATEGORY_SPLITTER)
                + f.getName()
            ...
            {
                if (Strings.isNullOrEmpty(category)) throw new RuntimeException(
                ...
                try {
                    ...
                    for (String key : wrapper.getKeys()) // Iterate the fully qualified property names the field
                    // provides
                    {
                        String suffix = StringUtils
                            .replaceOnce(key, wrapper.getCategory() + Configuration.CATEGORY_SPLITTER, "");

                        boolean existed = exists(cfg, wrapper.getCategory(), suffix);
                        if (!existed || loading) // Creates keys in category specified by the wrapper if new ones are
                        // programaticaly added
                        {
                            ...
                            if (!existed) adapt.setValue(property, wrapper.getValue(key));
                            else wrapper.setValue(key, adapt.getValue(property));
                        } else // If the key is not new, sync according to shouldReadFromVar()
                        {
                            ...
                            if (shouldReadFromVar(property, propVal, mapVal)) adapt.setValue(property, mapVal);
                            else wrapper.setValue(key, propVal);
                        }
                    }

                    ConfigCategory confCat = cfg.getCategory(wrapper.getCategory());

                    for (Property property : confCat.getOrderedValues()) // Iterate the properties to check for new data
                    // from the config side
                    {
                        String key = confCat.getQualifiedName() + Configuration.CATEGORY_SPLITTER + property.getName();
                        if (!wrapper.handlesKey(key)) continue;

                        if (loading || !wrapper.hasKey(key)) {
                            Object value = wrapper.getTypeAdapter()
                                .getValue(property);
                            wrapper.setValue(key, value);
                        }
                    }

                    if (loading) wrapper.setupConfiguration(
                        cfg,
                        comment,
                        langKey,
                        requiresMcRestart,
                ...
                .getSuperclass() != null && f.getType()
                .getSuperclass()
                .equals(Object.class)) { // If the field extends Object directly, descend the object tree and access
                // the objects members
    ...
    private static Property property(Configuration cfg, String category, String property, Property.Type type,
                                     boolean isList) {
        Property prop = cfg.getCategory(category)
            .get(property);
        if (prop == null) {
            if (isList) prop = new Property(property, new String[0], type);
            else prop = new Property(property, (String) null, type);
    ...
    private static boolean exists(Configuration cfg, String category, String property) {
        return cfg.hasCategory(category) && cfg.getCategory(category)
    ...
    private static boolean shouldReadFromVar(Property property, Object propValue, Object fieldValue) {
        if (!propValue.equals(fieldValue)) {
            if (property.hasChanged()) return false;
            else return true;
        }
        return false;
    }
    ...
    private static String getName(Field f) {
        if (f.isAnnotationPresent(Name.class)) return f.getAnnotation(Name.class)
            .value();
        return f.getName();
    }
