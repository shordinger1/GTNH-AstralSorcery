...
import shordinger.wrapper.net.minecraftforge.client.model.ModelStateComposition;
import shordinger.wrapper.net.minecraftforge.client.model.PerspectiveMapWrapper;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DModel.Animation;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DModel.Face;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DModel.Key;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DModel.Mesh;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DModel.Node;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DModel.Texture;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DModel.Vertex;
 ...
 * If you need more control over accepted resources - extend the class, and register a new instance with
 * ModelLoaderRegistry.
 */
public enum B3DLoader implements ICustomModelLoader {
    ...
    public boolean accepts(ResourceLocation modelLocation) {
        return enabledDomains.contains(modelLocation.getResourceDomain()) && modelLocation.getResourcePath()
            .endsWith(".b3d");
    }
        ...
        try {
            try {
                ...
                if (modelLocation.getResourcePath()
                    .startsWith("models/block/"))
                    resource = manager.getResource(
                        new ResourceLocation(
                            file.getResourceDomain(),
                            "models/item/" + file.getResourcePath()
                                .substring("models/block/".length())));
                else if (modelLocation.getResourcePath()
                    .startsWith("models/item/"))
                    resource = manager.getResource(
                        new ResourceLocation(
                            file.getResourceDomain(),
                            "models/block/" + file.getResourcePath()
                                .substring("models/item/".length())));
                else throw e;
            }
    ...
    public static final class B3DState implements IModelState {
        ...
        private static LoadingCache<Triple<Animation, Node<?>, Integer>, TRSRTransformation> cache = CacheBuilder
            .newBuilder()
            .maximumSize(16384)
        ...
        public static TRSRTransformation getNodeMatrix(@Nullable Animation animation, Node<?> node, int frame) {
            ...
            Key key = null;
            if (animation != null) key = animation.getKeys()
                .get(frame, node);
            else if (node.getAnimation() != null) key = node.getAnimation()
                .getKeys()
                .get(frame, node);
            if (key != null) {
                ...
                if (parent != null) {
                    ...
                    ret = ret
                        .compose(new TRSRTransformation(parent.getPos(), parent.getRot(), parent.getScale(), null));
                }
                ...
                /*
                 * Matrix4f rm = new TRSRTransformation(node.getPos(), node.getRot(), node.getScale(),
                 * null).getMatrix();
                 * rm.invert();
    ...
    private static final class ModelWrapper implements IModel {
        ...
        public ModelWrapper(ResourceLocation modelLocation, B3DModel model, ImmutableSet<String> meshes, boolean smooth,
                            boolean gui3d, int defaultKey) {
            this(modelLocation, model, meshes, smooth, gui3d, defaultKey, buildTextures(model.getTextures()));
        }

        public ModelWrapper(ResourceLocation modelLocation, B3DModel model, ImmutableSet<String> meshes, boolean smooth,
                            boolean gui3d, int defaultKey, ImmutableMap<String, ResourceLocation> textures) {
        ...
        public IBakedModel bake(IModelState state, VertexFormat format,
            ...
            for (Map.Entry<String, ResourceLocation> e : textures.entrySet()) {
                if (e.getValue()
                    ...
                    .startsWith("#")) {
                    FMLLog.log.fatal(
                        "unresolved texture '{}' for b3d model '{}'",
                        e.getValue()
                            .getResourcePath(),
                        modelLocation);
        ...
        public ModelWrapper process(ImmutableMap<String, String> data) {
            ...
            if (data.containsKey("mesh")) {
                ...
                if (e.isJsonPrimitive() && e.getAsJsonPrimitive()
                    .isString()) {
                    return new ModelWrapper(
                        modelLocation,
                        model,
                        ImmutableSet.of(e.getAsString()),
                        smooth,
                        gui3d,
                        defaultKey,
                        textures);
                    ...
                    for (JsonElement s : e.getAsJsonArray()) {
                        if (s.isJsonPrimitive() && s.getAsJsonPrimitive()
                            .isString()) {
                            builder.add(s.getAsString());
                        } else {
                            FMLLog.log.fatal(
                                "unknown mesh definition '{}' in array for b3d model '{}'",
                                s.toString(),
                                modelLocation);
                            return this;
                        }
            ...
            if (data.containsKey("key")) {
                ...
                if (e.isJsonPrimitive() && e.getAsJsonPrimitive()
                    .isNumber()) {
                    newDefaultKey = e.getAsNumber()
                        .intValue();
                    hasChanged = true;
                } else {
                    FMLLog.log
                        .fatal("unknown keyframe definition '{}' for b3d model '{}'", e.toString(), modelLocation);
                    return this;
                }
            }
            return hasChanged
                ? new ModelWrapper(modelLocation, model, newMeshes, smooth, gui3d, newDefaultKey, textures)
                : this;
        }
    ...
    private static final class BakedWrapper implements IBakedModel {
        ...
        public BakedWrapper(final Node<?> node, final IModelState state, final boolean smooth, final boolean gui3d,
                            final VertexFormat format, final ImmutableSet<String> meshes,
        ...
        public BakedWrapper(Node<?> node, IModelState state, boolean smooth, boolean gui3d, VertexFormat format,
                            ...
                            LoadingCache<Integer, B3DState> cache) {
            this.node = node;
            this.state = state;
            this.smooth = smooth;
            this.gui3d = gui3d;
        ...
        private void generateQuads(ImmutableList.Builder<BakedQuad> builder, Node<?> node, final IModelState state,
            ...
            if (node.getKind() instanceof Mesh && meshes.contains(node.getName())
                && !state.apply(Optional.of(Models.getHiddenModelPart(newPath)))
                .isPresent()) {
                Mesh mesh = (Mesh) node.getKind();
                Collection<Face> faces = mesh.bake(new Function<Node<?>, Matrix4f>() {

                    private final TRSRTransformation global = state.apply(Optional.empty())
                        .orElse(TRSRTransformation.identity());
                    private final LoadingCache<Node<?>, TRSRTransformation> localCache = CacheBuilder.newBuilder()
                        ...
                        .build(new CacheLoader<Node<?>, TRSRTransformation>() {
                            ...
                            public TRSRTransformation load(Node<?> node) throws Exception {
                                return state.apply(Optional.of(new NodeJoint(node)))
                                    .orElse(TRSRTransformation.identity());
                            }
                        });
                    ...
                    public Matrix4f apply(Node<?> node) {
                        return global.compose(localCache.getUnchecked(node))
                            .getMatrix();
                    }
                });
                for (Face f : faces) {
        ...
        private final void putVertexData(UnpackedBakedQuad.Builder builder, Vertex v, Vector3f faceNormal,
                                         TextureAtlasSprite sprite) {
                ...
                switch (format.getElement(e)
                    ...
                    case UV:
                        ...
                        if (format.getElement(e)
                            .getIndex() < v.getTexCoords().length) {
                            builder.put(
                                e,
