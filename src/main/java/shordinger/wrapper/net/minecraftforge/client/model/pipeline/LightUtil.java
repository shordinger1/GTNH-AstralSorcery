...
public class LightUtil {
    ...
    public static int[] mapFormats(VertexFormat from, VertexFormat to) {
        ...
        if (from.equals(DEFAULT_FROM) && to.equals(DEFAULT_TO)) return DEFAULT_MAPPING;
    ...
    public static void renderQuadColor(BufferBuilder buffer, BakedQuad quad, int auxColor) {
        if (quad.getFormat()
            .equals(buffer.getVertexFormat())) {
            buffer.addVertexData(quad.getVertexData());
            if (buffer.getVertexFormat()
                .hasColor()) {
    ...
    public static class ItemConsumer extends VertexTransformer {
        ...
        public void put(int element, float... data) {
            if (getVertexFormat().getElement(element)
                .getUsage() == EnumUsage.COLOR) {
