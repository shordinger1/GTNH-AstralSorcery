...
import shordinger.wrapper.net.minecraftforge.client.model.ModelLoader.VanillaLoader;
import shordinger.wrapper.net.minecraftforge.client.model.ModelLoader.VariantLoader;
import shordinger.wrapper.net.minecraftforge.client.model.b3d.B3DLoader;
...
import shordinger.wrapper.net.minecraftforge.common.model.animation.AnimationStateMachine;
...
public class ModelLoaderRegistry {
    ...
    public static void registerLoader(ICustomModelLoader loader) {
        ...
        ((IReloadableResourceManager) Minecraft.getMinecraft()
            .getResourceManager()).registerReloadListener(loader);
    }
    ...
    public static ResourceLocation getActualLocation(ResourceLocation location) {
        ...
        if (location.getResourcePath()
            .startsWith("builtin/")) return location;
    ...
    public static IModel getModel(ResourceLocation location) throws Exception {
        ...
        for (ResourceLocation loading : loadingModels) {
            if (location.getClass() == loading.getClass() && location.equals(loading)) {
                throw new LoaderException(
                    "circular model dependencies, stack: [" + Joiner.on(", ")
                        .join(loadingModels) + "]");
            }
        }
        ...
        try {
            ...
            try {
                ...
                throw new LoaderException(
                    String.format("Exception loading model %s with loader %s, skipping", location, accepted),
                    e);
            }
            if (model == getMissingModel()) {
                throw new LoaderException(
                    String.format("Loader %s returned missing model while loading model %s", accepted, location));
            }
            if (model == null) {
                throw new LoaderException(
                    String.format("Loader %s returned null while loading model %s", accepted, location));
            }
    ...
    public static IAnimationStateMachine loadASM(ResourceLocation location,
