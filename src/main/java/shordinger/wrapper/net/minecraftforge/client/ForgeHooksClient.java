...
import shordinger.wrapper.net.minecraft.client.gui.BossInfoClient;
...
import shordinger.wrapper.net.minecraft.client.renderer.block.model.BlockFaceUV;
...
import shordinger.wrapper.net.minecraft.client.renderer.block.model.ItemTransformVec3f;
...
import shordinger.wrapper.net.minecraft.client.renderer.vertex.VertexFormatElement.EnumUsage;
...
import shordinger.wrapper.net.minecraftforge.client.event.ColorHandlerEvent;
import shordinger.wrapper.net.minecraftforge.client.event.DrawBlockHighlightEvent;
import shordinger.wrapper.net.minecraftforge.client.event.EntityViewRenderEvent;
import shordinger.wrapper.net.minecraftforge.client.event.FOVUpdateEvent;
import shordinger.wrapper.net.minecraftforge.client.event.GuiScreenEvent;
import shordinger.wrapper.net.minecraftforge.client.event.InputUpdateEvent;
import shordinger.wrapper.net.minecraftforge.client.event.ModelBakeEvent;
import shordinger.wrapper.net.minecraftforge.client.event.MouseEvent;
import shordinger.wrapper.net.minecraftforge.client.event.RenderGameOverlayEvent;
import shordinger.wrapper.net.minecraftforge.client.event.RenderHandEvent;
import shordinger.wrapper.net.minecraftforge.client.event.RenderSpecificHandEvent;
import shordinger.wrapper.net.minecraftforge.client.event.RenderWorldLastEvent;
import shordinger.wrapper.net.minecraftforge.client.event.ScreenshotEvent;
import shordinger.wrapper.net.minecraftforge.client.event.TextureStitchEvent;
import shordinger.wrapper.net.minecraftforge.client.event.sound.PlaySoundEvent;
import shordinger.wrapper.net.minecraftforge.client.model.ModelDynBucket;
import shordinger.wrapper.net.minecraftforge.client.model.ModelLoader;
import shordinger.wrapper.net.minecraftforge.client.model.animation.Animation;
import shordinger.wrapper.net.minecraftforge.client.model.pipeline.QuadGatheringTransformer;
import shordinger.wrapper.net.minecraftforge.client.resource.IResourceType;
import shordinger.wrapper.net.minecraftforge.client.resource.SelectiveReloadStateHandler;
...
import shordinger.wrapper.net.minecraftforge.common.model.IModelPart;
import shordinger.wrapper.net.minecraftforge.common.model.ITransformation;
...
public class ForgeHooksClient {
    // private static final ResourceLocation ITEM_GLINT = new
    ...
    public static String getArmorTexture(Entity entity, ItemStack armor, String _default, EntityEquipmentSlot slot,
                                         String type) {
        String result = armor.getItem()
            .getArmorTexture(armor, entity, slot, type);
    ...
    public static void orientBedCamera(IBlockAccess world, BlockPos pos, IBlockState state, Entity entity) {
        ...
        if (block != null && block.isBed(state, world, pos, entity)) {
            GL11.glRotatef(
                (float) (block.getBedDirection(state, world, pos)
    ...
    public static boolean onDrawBlockHighlight(RenderGlobal context, EntityPlayer player, RayTraceResult target,
    ...
    public static boolean renderSpecificFirstPersonHand(EnumHand hand, float partialTicks, float interpPitch,
                                                        float swingProgress, float equipProgress, ItemStack stack) {
        return MinecraftForge.EVENT_BUS
            .post(new RenderSpecificHandEvent(hand, partialTicks, interpPitch, swingProgress, equipProgress, stack));
    }
    ...
    public static ModelBiped getArmorModel(EntityLivingBase entityLiving, ItemStack itemStack, EntityEquipmentSlot slot,
                                           ModelBiped _default) {
        ModelBiped model = itemStack.getItem()
            .getArmorModel(entityLiving, itemStack, slot, _default);
    ...
    public static float getFOVModifier(EntityRenderer renderer, Entity entity, IBlockState state,
                                       double renderPartialTicks, float fov) {
        EntityViewRenderEvent.FOVModifier event = new EntityViewRenderEvent.FOVModifier(
            renderer,
            entity,
            state,
            renderPartialTicks,
    ...
    public static String renderMainMenu(GuiMainMenu gui, FontRenderer font, int width, int height, String splashText) {
        ...
        if (line != null) {
            ...
            gui.drawString(
                font,
                line,
                width - font.getStringWidth(line) - 2,
                height - (2 * (font.FONT_HEIGHT + 1)),
                -1);
        }
    ...
    public static void drawScreen(GuiScreen screen, int mouseX, int mouseY, float partialTicks) {
        if (!MinecraftForge.EVENT_BUS
            .post(new GuiScreenEvent.DrawScreenEvent.Pre(screen, mouseX, mouseY, partialTicks)))
            screen.drawScreen(mouseX, mouseY, partialTicks);
        MinecraftForge.EVENT_BUS.post(new GuiScreenEvent.DrawScreenEvent.Post(screen, mouseX, mouseY, partialTicks));
    }

    public static float getFogDensity(EntityRenderer renderer, Entity entity, IBlockState state, float partial,
                                      float density) {
        EntityViewRenderEvent.FogDensity event = new EntityViewRenderEvent.FogDensity(
            renderer,
            entity,
            state,
            partial,
    ...
    public static void onFogRender(EntityRenderer renderer, Entity entity, IBlockState state, float partial, int mode,
                                   float distance) {
        MinecraftForge.EVENT_BUS
            .post(new EntityViewRenderEvent.RenderFogEvent(renderer, entity, state, partial, mode, distance));
    }

    public static void onModelBake(ModelManager modelManager,
                                   IRegistry<ModelResourceLocation, IBakedModel> modelRegistry, ModelLoader modelLoader) {
    ...
    public static IBakedModel handleCameraTransforms(IBakedModel model,
                                                     ItemCameraTransforms.TransformType cameraTransformType, boolean leftHandHackery) {
    ...
    private static class LightGatheringTransformer extends QuadGatheringTransformer {
        ...
        private static final VertexFormat FORMAT = new VertexFormat().addElement(DefaultVertexFormats.TEX_2F)
    ...
    public static void renderLitItem(RenderItem ri, IBakedModel model, int color, ItemStack stack) {
        ...
        for (int i = 0; i < allquads.size(); i++) {
            ...
            if (q.getFormat() != DefaultVertexFormats.ITEM && q.getFormat()
                .hasUvOffset(1)) {
        ...
        drawSegment(
            ...
            segmentSkyLight,
            segmentShading,
            segmentLightingDirty && (hasLighting || segment.size() < allquads.size()),
            segmentShadingDirty);
    ...
    private static void drawSegment(RenderItem ri, int baseColor, ItemStack stack, List<BakedQuad> segment, int bl,
                                    int sl, boolean shade, boolean updateLighting, boolean updateShading) {
        ...
        if (updateLighting) {
            ...
            OpenGlHelper
                .setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, Math.max(bl, lastBl), Math.max(sl, lastSl));
        }
    ...
    public static Optional<TRSRTransformation> applyTransform(ItemTransformVec3f transform,
    ...
    public static Optional<TRSRTransformation> applyTransform(ModelRotation rotation,
                                                              Optional<? extends IModelPart> part) {
        if (part.isPresent()) return Optional.empty();
    ...
    public static IBakedModel getDamageModel(IBakedModel ibakedmodel, TextureAtlasSprite texture, IBlockState state,
    ...
    public static BlockFaceUV applyUVLock(BlockFaceUV blockFaceUV, EnumFacing originalSide, ITransformation rotation) {
        ...
        Matrix4f uv = global.getUVLockTransform(originalSide)
            .getMatrix();
    ...
    public static RenderGameOverlayEvent.BossInfo bossBarRenderPre(ScaledResolution res, BossInfoClient bossInfo, int x,
                                                                   int y, int increment) {
        RenderGameOverlayEvent.BossInfo evt = new RenderGameOverlayEvent.BossInfo(
            new RenderGameOverlayEvent(Animation.getPartialTickTime(), res),
            BOSSINFO,
            bossInfo,
            x,
            y,
    ...
    public static void bossBarRenderPost(ScaledResolution res) {
        MinecraftForge.EVENT_BUS.post(
            new RenderGameOverlayEvent.Post(new RenderGameOverlayEvent(Animation.getPartialTickTime(), res), BOSSINFO));
    }
    ...
    public static Pair<? extends IBakedModel, Matrix4f> handlePerspective(IBakedModel model,
        ...
        TRSRTransformation tr = TRSRTransformation.from(
            model.getItemCameraTransforms()
                .getTransform(type));
        Matrix4f mat = null;
    ...
    public static String getHorseArmorTexture(EntityHorse horse, ItemStack armorStack) {
        String texture = armorStack.getItem()
            .getHorseArmorTexture(horse, armorStack);
        if (texture == null) texture = horse.getHorseArmorType()
            .getTextureName();
        return texture;
    }
    ...
    public static boolean shouldUseVanillaReloadableListener(IResourceManagerReloadListener listener) {
        ...
        if (listener instanceof ModelManager || listener instanceof RenderItem)
            ...
            return predicate.test(VanillaResourceType.TEXTURES);
        else if (listener instanceof SoundHandler) return predicate.test(VanillaResourceType.SOUNDS);
        else if (listener instanceof EntityRenderer) return predicate.test(VanillaResourceType.SHADERS);
    ...
    // Resets cached thread fields in ThreadNameCachingStrategy and ReusableLogEventFactory to be repopulated during
    // their next access.
