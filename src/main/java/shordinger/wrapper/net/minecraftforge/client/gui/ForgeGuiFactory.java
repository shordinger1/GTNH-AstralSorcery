...
import shordinger.wrapper.net.minecraftforge.fml.client.config.ConfigGuiType;
import shordinger.wrapper.net.minecraftforge.fml.client.config.DummyConfigElement;
import shordinger.wrapper.net.minecraftforge.fml.client.config.DummyConfigElement.DummyCategoryElement;
import shordinger.wrapper.net.minecraftforge.fml.client.config.GuiConfig;
import shordinger.wrapper.net.minecraftforge.fml.client.config.GuiConfigEntries;
import shordinger.wrapper.net.minecraftforge.fml.client.config.GuiConfigEntries.BooleanEntry;
import shordinger.wrapper.net.minecraftforge.fml.client.config.GuiConfigEntries.CategoryEntry;
import shordinger.wrapper.net.minecraftforge.fml.client.config.GuiConfigEntries.IConfigEntry;
 ...
 * - using custom IConfigEntry objects to define a dummy property that can be used to generate new ConfigCategory
 * objects
 * - defining the configID string for a GuiConfig object so that the config changed events will be posted when that
 * GuiConfig screen is closed
 * (the configID string is optional; if it is not defined the config changed events will be posted when the top-most
 * GuiConfig screen
 * is closed, eg when the parent is null or is not an instance of GuiConfig)
 * - overriding the IConfigEntry.enabled() method to control the enabled state of one list entry based on the value of
 * another entry
 * - overriding the IConfigEntry.onGuiClosed() method to perform custom actions when the screen that owns the entry is
 * closed (in this
...
public class ForgeGuiFactory implements IModGuiFactory {
    ...
    public static class ForgeConfigGui extends GuiConfig {
        ...
        private static List<IConfigElement> getConfigElements() {
            ...
            list.add(
                new DummyCategoryElement("forgeCfg", "forge.configgui.ctgy.forgeGeneralConfig", GeneralEntry.class));
            list.add(
                new DummyCategoryElement(
                    "forgeClientCfg",
                    "forge.configgui.ctgy.forgeClientConfig",
                    ClientEntry.class));
            list.add(
                new DummyCategoryElement(
                    "forgeChunkLoadingCfg",
                    "forge.configgui.ctgy.forgeChunkLoadingConfig",
                    ChunkLoaderEntry.class));
            list.add(
                new DummyCategoryElement(
                    "forgeVersionCheckCfg",
                    "forge.configgui.ctgy.VersionCheckConfig",
                    VersionCheckEntry.class));
            return list;
        }

        /**
         * This custom list entry provides the General Settings entry on the Minecraft Forge Configuration screen.
         * It extends the base Category entry class and defines the IConfigElement objects that will be used to build
         * the child screen.
         */
        public static class GeneralEntry extends CategoryEntry {
            ...
            protected GuiScreen buildChildScreen() {
                // This GuiConfig object specifies the configID of the object and as such will force-save when it is
                // closed. The parent
        ...
        public static class ClientEntry extends CategoryEntry {
            ...
            protected GuiScreen buildChildScreen() {
                ...
                return new GuiConfig(
                    ...
                    (new ConfigElement(
                        ForgeModContainer.getConfig()
                            .getCategory(Configuration.CATEGORY_CLIENT))).getChildElements(),
                    this.owningScreen.modID,
        ...
        /**
         * This custom list entry provides the Forge Chunk Manager Config entry on the Minecraft Forge Configuration
         * screen.
         * It extends the base Category entry class and defines the IConfigElement objects that will be used to build
         * the child screen.
         */
        ...
        /**
         * This custom list entry provides the Forge Version Checking Config entry on the Minecraft Forge Configuration
         * screen.
         * It extends the base Category entry class and defines the IConfigElement objects that will be used to build
         * the child screen.
         */
        public static class VersionCheckEntry extends CategoryEntry {
            ...
            @Override
            protected GuiScreen buildChildScreen() {
                ConfigCategory cfg = ForgeModContainer.getConfig()
                    .getCategory(VERSION_CHECK_CAT);
                Map<String, Property> values = new HashMap<String, Property>(cfg.getValues());
                values.remove("Global");

                Property global = ForgeModContainer.getConfig()
                    .get(VERSION_CHECK_CAT, "Global", true);

                List<Property> props = new ArrayList<Property>();

                for (ModContainer mod : ForgeVersion.gatherMods()
                    .keySet()) {
                    values.remove(mod.getModId());
                    props.add(
                        ForgeModContainer.getConfig()
                            .get(VERSION_CHECK_CAT, mod.getModId(), true)); // Get or make the value in the config
                }
        ...
        public static class ModOverridesEntry extends CategoryEntry {
            ...
            protected GuiScreen buildChildScreen() {
                ...
                list.add(
                    new DummyCategoryElement(
                        "addForgeChunkLoadingModCfg",
                        "forge.configgui.ctgy.forgeChunkLoadingAddModConfig",
            ...
            public boolean enabled() {
                for (IConfigEntry entry : this.owningEntryList.listEntries) {
                    if (entry.getName()
                        .equals("enabled") && entry instanceof BooleanEntry) {
            ...
            public void undoChanges() {
                if (childScreen instanceof GuiConfig) {
                    ...
                    for (IConfigEntry ice : child.entryList.listEntries)
                        if (!child.initEntries.contains(ice) && ForgeChunkManager.getConfig()
                            .hasCategory(ice.getName()))
                            ForgeChunkManager.getConfig()
                                .removeCategory(
                                    ForgeChunkManager.getConfig()
                                        .getCategory(ice.getName()));
        ...
        /**
         * This custom list entry provides a button that will open to a screen that will allow a user to define a new
         * mod override.
         */
        public static class AddModOverrideEntry extends CategoryEntry {
            ...
            protected GuiScreen buildChildScreen() {
                ...
                list.add(
                    new DummyConfigElement("modID", "", ConfigGuiType.STRING, "forge.configgui.modID")
        ...
        /**
         * This custom list entry provides a Mod ID selector. The control is a button that opens a list of values to
         * select from.
         * This entry also overrides onGuiClosed() to run code to save the data to a new ConfigCategory when the user is
         * done.
         */
        public static class ModIDEntry extends SelectValueEntry {
            ...
            /**
             * By overriding onGuiClosed() for this entry we can perform additional actions when the user is done such
             * as saving
             * a new ConfigCategory object to the Configuration object.
             */
            @Override
            public void onGuiClosed() {
                Object modObject = Loader.instance()
                    .getModObjectList()
                    .get(
                        Loader.instance()
                            .getIndexedModList()
                ...
                if (modObject != null) {
                    ...
                    for (IConfigElement ice : this.owningScreen.configElements)
                        if ("maximumTicketCount".equals(ice.getName())) maxTickets = Integer.valueOf(
                            ice.get()
                                .toString());
                        else if ("maximumChunksPerTicket".equals(ice.getName())) maxChunks = Integer.valueOf(
                            ice.get()
                                .toString());

                    ForgeChunkManager.addConfigProperty(
                        modObject,
                        "maximumTicketCount",
                        String.valueOf(maxTickets),
                        Property.Type.INTEGER);
                    ForgeChunkManager.addConfigProperty(
                        modObject,
                        "maximumChunksPerTicket",
                    ...
                    if (this.owningScreen.parentScreen instanceof GuiConfig) {
                        ...
                        for (IConfigElement ice : superParent.configElements)
                            if (ice.getName()
                                .equals(currentValue)) found = true;
