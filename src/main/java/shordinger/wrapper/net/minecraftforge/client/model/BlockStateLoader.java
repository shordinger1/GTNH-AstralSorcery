...
public class BlockStateLoader {
    ...
    public static ModelBlockDefinition load(Reader reader, ResourceLocation location, final Gson vanillaGSON) {
        try {
            ...
            Marker marker = GSON.fromJson(new String(data, StandardCharsets.UTF_8), Marker.class); // Read
            // "forge_marker" to
            // determine what to
            // load.
            ...
            switch (marker.forge_marker) {
                case 1: // Version 1
                    ...
                    for (Entry<String, Collection<ForgeBlockStateV1.Variant>> entry : v1.variants.asMap()
                        .entrySet()) { // Convert Version1 variants into vanilla variants for the ModelBlockDefinition.
                        List<Variant> mcVars = Lists.newArrayList();
                        for (ForgeBlockStateV1.Variant var : entry.getValue()) {
                            boolean uvLock = var.getUvLock()
                                .orElse(false);
                            int weight = var.getWeight()
                                .orElse(1);

                            if (var.isVanillaCompatible()) mcVars.add(
                                new Variant(
                                    var.getModel(),
                                    (ModelRotation) var.getState()
                                        .orElse(ModelRotation.X0_Y0),
                                    uvLock,
                                    weight));
                            else mcVars.add(
                                new ForgeVariant(
    ...
    public static class SubModel {
        ...
        public SubModel(IModelState state, boolean uvLock, boolean smooth, boolean gui3d,
                        ImmutableMap<String, String> textures, @Nullable ResourceLocation model,
    ...
    private static class ForgeVariant extends Variant implements ISmartVariant {
        ...
        ForgeVariant(ResourceLocation blockstateLocation, @Nullable ResourceLocation model, IModelState state,
                     boolean uvLock, Optional<Boolean> smooth, Optional<Boolean> gui3d, int weight,
                     ImmutableMap<String, String> textures, ImmutableMap<String, SubModel> parts,
                     ImmutableMap<String, String> customData) {
            super(
                model == null ? new ResourceLocation("builtin/missing") : model,
        ...
        private IModel runModelHooks(IModel base, Optional<Boolean> smooth, Optional<Boolean> gui3d, boolean uvlock,
                                     ImmutableMap<String, String> textureMap, ImmutableMap<String, String> customData) {
            base = base.process(customData);
            base = base.retexture(textureMap);
            base = smooth.map(base::smoothLighting)
                .orElse(base);
            base = gui3d.map(base::gui3d)
                .orElse(base);
        ...
        public IModel process(IModel base) {
            ...
            for (Entry<String, SubModel> entry : parts.entrySet()) {
                ...
                if (modelLocation == null) {
                    FMLLog.log
                        .error("model not found for variant {} for blockstate {}", entry.getKey(), blockstateLocation);
                    model = ModelLoaderRegistry.getMissingModel(blockstateLocation, new Throwable());
                } else {
                    model = ModelLoaderRegistry
                        .getModelOrLogError(modelLocation, "Unable to load block sub-model: \'" + modelLocation);
                }
                ...
                models.put(
                    ...
                    Pair.of(
                        runModelHooks(
                            model,
                            Optional.of(part.smooth),
                            Optional.of(part.gui3d),
                            part.uvLock,
                            part.getTextures(),
                            part.getCustomData()),
                        part.getState()));
