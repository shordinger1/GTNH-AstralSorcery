...
import shordinger.wrapper.net.minecraft.client.renderer.vertex.VertexFormatElement.EnumType;
...
public class Attributes {
    ...
    public static boolean moreSpecific(VertexFormat first, VertexFormat second) {
        ...
        for (VertexFormatElement firstAttr : first.getElements()) {
            while (j < second.getElementCount() && second.getElement(j)
                .getUsage() == EnumUsage.PADDING) {
                padding += second.getElement(j++)
                    .getSize();
            }
