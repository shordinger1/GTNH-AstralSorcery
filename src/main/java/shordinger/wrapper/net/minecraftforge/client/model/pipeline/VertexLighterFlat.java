...
public class VertexLighterFlat extends QuadGatheringTransformer {
    ...
    protected static final VertexFormatElement NORMAL_4F = new VertexFormatElement(
        0,
    ...
    private void updateIndices() {
        for (int i = 0; i < getVertexFormat().getElementCount(); i++) {
            switch (getVertexFormat().getElement(i)
                .getUsage()) {
                case POSITION:
                    posIndex = i;
                    break;
                ...
                case COLOR:
                    ...
                    break;
                case UV:
                    if (getVertexFormat().getElement(i)
                        .getIndex() == 1) {
                        lightmapIndex = i;
                    }
                    break;
    ...
    static VertexFormat withNormal(VertexFormat format) {
        // This is the case in 99.99%. Cache the value, so we don't have to redo it every time, and the speed up the
        // equals check in LightUtil
        ...
        for (int v = 0; v < 4; v++) {
            ...
            for (int e = 0; e < count; e++) {
                ...
                switch (element.getUsage()) {
                    case POSITION:
                        // position adding moved to VertexBufferConsumer due to x and z not fitting completely into a
                        // float
                        /*
