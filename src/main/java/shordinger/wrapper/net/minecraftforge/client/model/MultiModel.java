...
public final class MultiModel implements IModel {
    ...
    private static final class Baked implements IBakedModel {
        ...
        private final ItemOverrideList overrides = new ItemOverrideList(Lists.newArrayList()) {
            ...
            public IBakedModel handleItemState(IBakedModel originalModel, ItemStack stack, @Nullable World world,
                ...
                if (base != null) {
                    newBase = base.getOverrides()
                        .handleItemState(base, stack, world, entity);
                    if (base != newBase) {
                        dirty = true;
                    }
                }
                ...
                for (Map.Entry<String, IBakedModel> entry : parts.entrySet()) {
                    IBakedModel newPart = entry.getValue()
                        .getOverrides()
                        .handleItemState(entry.getValue(), stack, world, entity);
        ...
        public Baked(ResourceLocation location, boolean perspective, @Nullable IBakedModel base,
                     ImmutableMap<String, IBakedModel> parts) {
            ...
            if (base != null) internalBase = base;
                ...
                Iterator<IBakedModel> iter = parts.values()
                    .iterator();
            ...
            if (base != null && perspective) {
                ...
                for (TransformType type : TransformType.values()) {
                    ...
                    if (newBase != base || matrix != null) {
                        map.put(
                            type,
                            Pair.of(new Baked(location, false, newBase, parts), new TRSRTransformation(matrix)));
                    }
                }
    ...
    @Deprecated
    public MultiModel(ResourceLocation location, @Nullable IModel base, IModelState baseState,
                      ImmutableMap<String, Pair<IModel, IModelState>> parts) {
        this(location, base, parts);
    }

    public MultiModel(ResourceLocation location, @Nullable IModel base,
                      ImmutableMap<String, Pair<IModel, IModelState>> parts) {
        this.location = location;
        this.base = base;
        this.parts = parts;
    ...
    // TODO 1.13 remove, kept for binary compatibility
    @Deprecated
    public MultiModel(ResourceLocation location, IModel base, IModelState baseState,
    ...
    public IBakedModel bake(IModelState state, VertexFormat format,
        ...
        for (Entry<String, Pair<IModel, IModelState>> entry : parts.entrySet()) {
            ...
            mapBuilder.put(
                entry.getKey(),
                pair.getLeft()
                    .bake(new ModelStateComposition(state, pair.getRight()), format, bakedTextureGetter));
        }
