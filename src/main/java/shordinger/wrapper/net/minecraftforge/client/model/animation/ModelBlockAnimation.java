...
import shordinger.wrapper.net.minecraftforge.client.model.animation.ModelBlockAnimation.Parameter.Interpolation;
import shordinger.wrapper.net.minecraftforge.client.model.animation.ModelBlockAnimation.Parameter.Type;
import shordinger.wrapper.net.minecraftforge.client.model.animation.ModelBlockAnimation.Parameter.Variable;
...
import shordinger.wrapper.net.minecraftforge.common.util.JsonUtils;
...
public class ModelBlockAnimation {
    ...
    public ModelBlockAnimation(ImmutableMap<String, ImmutableMap<String, float[]>> joints,
                               ImmutableMap<String, MBClip> clips) {
        this.joints = joints;
        this.clips = clips;
    }
    ...
    public ImmutableCollection<MBJointWeight> getJoint(int i) {
        if (jointIndexMap == null) {
            ...
            for (Map.Entry<String, ImmutableMap<String, float[]>> info : joints.entrySet()) {
                ...
                for (Map.Entry<String, float[]> e : info.getValue()
                    .entrySet()) {
    ...
    protected static class MBClip implements IClip {
        ...
        public MBClip(boolean loop, ImmutableMap<String, ImmutableList<MBVariableClip>> clips,
                      ImmutableMap<String, String> events) {
            this.loop = loop;
        ...
        private void initialize() {
            if (jointClips == null) {
                ...
                if (!eventsRaw.isEmpty()) {
                    ...
                    for (Map.Entry<Float, String> entry : times.descendingMap()
                        .entrySet()) {
                ...
                public Iterator<Event> iterator() {
                    return new UnmodifiableIterator<Event>() {
                        ...
                        public Event next() {
                            ...
                            if (curKey != null) {
                                float checkStopTime = stopTime;
                                while (curKey + events.get(curKey)
                                    .offset() < checkStopTime) checkStopTime--;
                                while (curKey + events.get(curKey)
                                    .offset() >= checkStopTime + 1) checkStopTime++;
        ...
        protected static class MBJointClip implements IJointClip {
            ...
            public TRSRTransformation apply(float time) {
                ...
                base = new TRSRTransformation(origin, null, null, null).compose(base)
                    .compose(new TRSRTransformation(negOrigin, null, null, null));
        ...
        if (!infos.isEmpty()) {
            ...
            for (MBJointWeight info : infos) {
                if (info.getWeights()
                    ...
                    if (trOp.isPresent() && !trOp.get()
                        .isIdentity()) {
                        float w = info.getWeights()
                            .get(i)[0];
                        tmp = trOp.get()
                            .getMatrix();
                        tmp.mul(w);
     ...
     */
    public static ModelBlockAnimation loadVanillaAnimation(IResourceManager manager,
                                                           ResourceLocation armatureLocation) {
        try {
            try (IResource resource = manager.getResource(armatureLocation)) {
                ModelBlockAnimation mba = mbaGson.fromJson(
    ...
    private static final ModelBlockAnimation defaultModelBlockAnimation = new ModelBlockAnimation(
        ImmutableMap.<String, ImmutableMap<String, float[]>>of(),
        ImmutableMap.<String, ModelBlockAnimation.MBClip>of());
}
