...
public final class ItemLayerModel implements IModel {
    ...
    public IBakedModel bake(IModelState state, final VertexFormat format,
        ...
        boolean identity = !transform.isPresent() || transform.get()
            .isIdentity();
    ...
    public static ImmutableList<BakedQuad> getQuadsForSprite(int tint, TextureAtlasSprite sprite, VertexFormat format,
                ...
                for (int u = 0; u < uMax; u++) {
                    ...
                    if (!translucent) {
                        ...
                        if (building && !face) // finish current quad
                        {
                            ...
                            builder.add(
                                buildSideQuad(format, transform, facing, tint, sprite, uStart, v + off, u - uStart));
                            building = false;
                ...
                for (int v = 0; v < vMax; v++) {
                    ...
                    if (!translucent) {
                        ...
                        if (building && !face) // finish current quad
                        {
                            ...
                            builder.add(
                                buildSideQuad(format, transform, facing, tint, sprite, u + off, vStart, v - vStart));
                            building = false;
                        } else if (!building && face) // start new quad
                        {
                            building = true;
    ...
    private static BakedQuad buildSideQuad(VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side,
                                           int tint, TextureAtlasSprite sprite, int u, int v, int size) {
        ...
        float dx = side.getDirectionVec()
            .getX() * eps
            / width;
        float dy = side.getDirectionVec()
            .getY() * eps
            / height;
        ...
        return buildQuad(
            ...
            y0,
            z0,
            sprite.getInterpolatedU(u0),
            sprite.getInterpolatedV(v0),
            x1,
            y1,
            z0,
            sprite.getInterpolatedU(u1),
            sprite.getInterpolatedV(v1),
            x1,
            y1,
            z1,
            sprite.getInterpolatedU(u1),
    ...
    private static BakedQuad buildQuad(VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side,
                                       TextureAtlasSprite sprite, int tint, float x0, float y0, float z0, float u0, float v0, float x1, float y1,
                                       float z1, float u1, float v1, float x2, float y2, float z2, float u2, float v2, float x3, float y3, float z3,
                                       float u3, float v3) {
        ...
        boolean hasTransform = transform.isPresent() && !transform.get()
            .isIdentity();
    ...
    public static enum Loader implements ICustomModelLoader {
        ...
        public boolean accepts(ResourceLocation modelLocation) {
            return modelLocation.getResourceDomain()
                ...
                .equals("item-layer")
                || modelLocation.getResourcePath()
                .equals("models/block/item-layer")
                || modelLocation.getResourcePath()
                .equals("models/item/item-layer"));
        }
