...
public class B3DModel {
    ...
    public B3DModel(List<Texture> textures, List<Brush> brushes, Node<?> root,
                    ImmutableMap<String, Node<Mesh>> meshes) {
    ...
    public static class Parser {
        ...
        public Parser(InputStream in) throws IOException {
            if (in instanceof FileInputStream) {
                ...
                int l = ByteBuffer.wrap(tmp)
                    .order(ByteOrder.LITTLE_ENDIAN)
                    .getInt();
                if (l < 0 || l + 8 < 0) throw new IOException("File is too large");
                buf = ByteBuffer.allocate(l + 8)
                    .order(ByteOrder.LITTLE_ENDIAN);
                buf.clear();
                buf.put(tag);
                buf.put(tmp);
    ...
    public static class Texture {
        ...
        public String toString() {
            return String.format(
                "Texture [path=%s, flags=%s, blend=%s, pos=%s, scale=%s, rot=%s]",
                path,
                flags,
                blend,
                pos,
    ...
    public static class Brush {
        ...
        public String toString() {
            return String.format(
                "Brush [name=%s, color=%s, shininess=%s, blend=%s, fx=%s, textures=%s]",
                name,
                color,
                shininess,
                blend,
    ...
    public static class Vertex {
        ...
        public Vertex bake(Mesh mesh, Function<Node<?>, Matrix4f> animator) {
            ...
            if (mesh.getWeightMap()
                .get(this)
                .isEmpty()) {
                t.setIdentity();
            } else {
                for (Pair<Float, Node<Bone>> bone : mesh.getWeightMap()
                    .get(this)) {
        ...
        public String toString() {
            return String.format(
                "Vertex [pos=%s, normal=%s, color=%s, texCoords=%s]",
                pos,
                normal,
                color,
                java.util.Arrays.toString(texCoords));
        }
    }
    ...
    public static class Node<K extends IKind<K>> {
        ...
        public static <K extends IKind<K>> Node<K> create(String name, Vector3f pos, Vector3f scale, Quat4f rot,
                                                          List<Node<?>> nodes, K kind) {
        ...
        public void setAnimation(Triple<Integer, Integer, Float> animData,
                                 Table<Integer, Optional<Node<?>>, Key> keyData) {
        ...
        public String toString() {
            return String.format(
                "Node [name=%s, kind=%s, pos=%s, scale=%s, rot=%s, keys=..., nodes=..., animation=%s]",
                name,
                kind,
                pos,
                scale,
    ...
    public static class Mesh implements IKind<Mesh> {
        ...
        public void setParent(Node<Mesh> parent) {
            ...
            for (Node<Bone> bone : getBones()) {
                for (Pair<Vertex, Float> b : bone.getKind()
                    .getData()) {
